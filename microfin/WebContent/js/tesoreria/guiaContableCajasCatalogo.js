$(document).ready(function() {
	...
	$('#conceptoCajaID').change(function() {	
		validaConcepto('conceptoCajaID');		
	});	
	...
	function consultaContableCaj() {	
  		dwr.util.removeAllOptions('conceptoCajaID'); 
  		dwr.util.addOptions('conceptoCajaID', {0:'SELECCIONAR'}); 
  		guiaContableCajasServicio.listaCombo(1, function(conceptosCaj){
			dwr.util.addOptions('conceptoCajaID', conceptosCaj, 'conceptoCajaID', 'descripcion');
		});
	}
	
	$('#formaGenerica').validate({
		rules: {
			conceptoCajaID: 'required',
			cuenta		: {
				maxlength	: 	4
			}
		},
		...
		messages: {
			conceptoCajaID: 'Especifique Concepto',
			cuenta		: {
...
function validaConcepto(idControl) {
	...
	var CuentaMayorBeanCon = {
		'conceptoCajaID'	:numConcepto
	};
	setTimeout("$('#cajaLista').hide();", 200);		
	if(numConcepto != '' && !isNaN(numConcepto) && esTab){		
		if($('#conceptoCajaID').val()==0){
			$('#cuenta').focus();
			$('#cuenta').val('');
			$('#nomenclatura').val('');
			$('#nomenclaturaCR').val('');								
			...
			deshabilitaBoton('modificaCM', 'submit');
			deshabilitaBoton('eliminaCM', 'submit');
			inicializaForma('formaGenerica','conceptoCajaID');
			
		} else {
			guiaContableCajasServicio.consulta(tipConPrincipal, CuentaMayorBeanCon,function(cuentaM) {
				if(cuentaM!=null){
