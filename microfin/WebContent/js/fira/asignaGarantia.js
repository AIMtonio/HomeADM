...
// fin  Bloque de funcionalidad extra
	...
	$('#solicitudCreditoID').bind('keyup',function(e){
		lista('solicitudCreditoID', '2', '14', 'clienteID', $('#solicitudCreditoID').val(), 'listaSolicitudCredito.htm');
	});
...
// FIN VALIDACION CREDITOS
	...
	function pegaHtml(solCreID,creditoID){
		...
		$.post("asignacionGarantiasAgroGrid.htm", params, function(data){
			if(data.length >0 ){
...
function limpiarParametrosPantalla(){
	...
	tablaNueva+='<td class="label"> <label for="lblgarantia">Sustituye GL: </label> </td>';
	tablaNueva+='</tr>	</table>';
...
function eliminaDetalle(control){
	...
	var jqSustituyeGL = eval("'#sustituyeGL" + numeroID + "'");
	...
	//Reordenamiento de Controles
	contador = 1;
	$('select[name=lsustituyeGL]').each(function() {
		var jqCicSup = eval("'#" + this.id + "'");
		$(jqCicSup).unbind();
		$(jqCicSup).attr("id", "sustituyeGL" + contador);
		contador = contador + 1;
	});
...
function agregaNuevoDetalle(){
	...
	tds += '<td><script type="text/javascript">';
	tds += '</script>';
	tds += '<select id="sustituyeGL'+nuevaFila+'" name="lsustituyeGL" path="lsustituyeGL" >';
...
function consultaSiPerteneceACredito(idControl) {
	...
	var jqSustituyeGL=eval("'#sustituyeGL" + idControl + "'");
