...
$(document).ready(function() {
	...
	function consultaCliente(idControl) {
		...
		if(numCliente != '0' && numCliente != '' && !isNaN(numCliente) && esTab){
			clienteServicio.consulta(catTipoConsultaDatosSocioEco.principal,numCliente,{
			async : false,
	...
	function consultaProspectoCliente(numCliente) {
		...
		if(numCliente != '' && !isNaN(numCliente)){
			clienteServicio.consulta(tipConProspec,numCliente,{
			async : false,
			callback : function(cliente) {
				if(cliente!=null){
	...
	function consultaTipoIdent() {
		...
		setTimeout("$('#cajaLista').hide();", 200);
			tiposIdentiServicio.consulta(tipConP,numTipoIden,{
			async : false,
			callback : function(identificacion) {
	...
	function llenaCombolineasNegocio(){
		...
		lineaNegocioServicio.listaCombo(catTipoConsultaDatosSocioEco.infoDatosSocioe, {
			async : false,
			callback : function(lineasNegocio){
	...
	function consultaInformacionsocioeconmomica(){
		...
		clidatsocioeServicio.listaCombo(datosSocioEcoBean,catTipoConsultaDatosSocioEco.infoDatosSocioe, {
			async : false,
			callback : function(datosSocioeco){
	...
	function consultaProspecto(idControl) {
		...
		if(numProspecto != '0' && numProspecto != '' && !isNaN(numProspecto) && esTab){
			...
			prospectosServicio.consulta(catTipoConsultaDatosSocioEco.principal,prospectoBeanCon,{
			async : false,
	...
	function creaGridDependientesEconom(clienteID, prospectoID){
		...
		datSocDemogServicio.consulta(tipolistaPrincipal.principal,datSocDemoBean ,{
			async : false,
		...
		datSocDemogServicio.listaDependEconom(tipolistaPrincipal.principal,datSocDemoBean ,{
			async : false,
			callback : function(lisDepends){
			if(lisDepends!=null){
	...
	function consultaDatosConyugue(){
		...
		socDemoConyugServicio.consulta(tipolistaPrincipal.principal,datSocConyugueBean ,{
			async : false,
			callback : function(conyugue){
			if(conyugue!=null){
	...
	function llenaTipoComboMatViviendaCli(idControl){
		...
		socDemoViviendaServicio.comboMaterialVivienda(tipoListaPrincipal,datSocDemogBean ,{
			async : false,
			callback : function(lisMaterialViv){
			dwr.util.removeAllOptions('tipoMaterialID');
