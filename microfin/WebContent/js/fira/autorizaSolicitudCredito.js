...
$('#autorizar').click(function(event) {
	...
	var saldoDisp= $('#saldoDisponible').asNumber();
	if($('#clienteID').asNumber()==0 && var_permiteProspecto != SiPermiteProspecto){
		...
		if( $('#montoAutorizado').val()==''){
			...
			event.preventDefault();
		}else if (monAut > saldoDisp && $('#lineaCreditoID').asNumber()>0){
			agregaFormatoMoneda('formaGenerica');

			mensajeSis("El Monto Autorizado es Mayor al Saldo Disponible de la Linea de Credito Agropecuaria.");
			$('#montoAutorizado').val( $('#montoA').val());
			 $('#montoAutorizado').focus();
			 $('#montoAutorizado').select();
...
// ==================================================== FUNCIONES Y PROCESOS =====================================================
				...
				solicitudCredServicio.consulta(catTipoConsultaSolicitud.principal, SolCredBeanCon,function(solicitud) {
						if(solicitud!=null && solicitud.solicitudCreditoID !=0){
							...
							if((estatus == Constantes.ESTATUSLIBERADO && sucursalSolicitud == sucursalPromotor) || (estatus == Constantes.ESTATUSLIBERADO && atiendeSucursal == Constantes.NO)){
								...
								if(sucursalSolicitud != sucursalPromotor && atiendeSucursal == 'S' && estatus == Constantes.ESTATUSLIBERADO){
	...
	setTimeout("$('#cajaLista').hide();", 200);
		...
		if(ProdCred != '' && !isNaN(ProdCred)){
			productosCreditoServicio.consulta(Enum_Tipo_Consulta.agropecuario,ProdCredBeanCon,
				{
					async : false,
	...
	function consultaDestinoCredito(idControl) {
		...
		if(DestCred != '' && !isNaN(DestCred)){
				destinosCredServicio.consulta(Enum_Tipo_Consulta.principalDestino,DestCredBeanCon,function(destinos) {
					if(destinos!=null){
	...
	lineasCreditoServicio.consulta(tipConPrincipal, lineaCreditoBeanCon, {
		async: false, callback: function(lineaCreditoBean){
			if(lineaCreditoBean != null){
				...
				$('#fechaVencimientoLinea').val(lineaCreditoBean.fechaVencimiento);
				...
				var fechaVencimientoLinea = lineaCreditoBean.fechaVencimiento;
				...
				if( $('#estatus').val() == 'L' ){
					...
					if( saldoDisponible <= 0 ){
						...
						mensajeSis("Saldo de Linea de Credito Insuficiente.");
						$('#lineaCreditoID').focus();
						$('#lineaCreditoID').select();
					}

					if( montoAutorizado > saldoDisponible ){
						banderaBoton = banderaBoton + 1;
						mensajeSis("El Monto Autorizado es mayor que el saldo disponible actual de la línea de crédito.");
						$('#lineaCreditoID').focus();
						$('#lineaCreditoID').select();
					}
