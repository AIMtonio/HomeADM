$(document).ready(function() {
	...
	$('#montoComisiones').blur(function(){
		if ($('#montoComisiones').asNumber()=='0'){
			...
			if($('#montoComisiones').asNumber()>$('#totalComisi').asNumber()){
				...
				if($('#totalComisi').asNumber()>0){
					porcentaje = ($('#montoComisiones').asNumber()/$('#totalComisi').asNumber())*100;
   				...
   				if(credito!=null){
   	   				if(credito.esAgropecuario == 'S'){
	   					if(credito.estatusGarantiaFIRA == 'P'){
	   						mensajeSis("El Crédito " + credito.creditoID + " tiene Garantías Aplicadas.<br>Si desea condonarlo,consulte la pantalla de " + "<b><a onclick=\"$('#Contenedor').load('creQuitasAgroContVista.htm',function(response, status, xhr){" + "if(status == 'error') {$('#Contenedor').html(response);}});consultaSesion();\"> Quitas y Condonaciones de Contingentes Agro. <img src=\"images/external.png\"></a></b> \n Si no,  cierre la Ventana");
	   					}
	   	   					esTab=true;	
