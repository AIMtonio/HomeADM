...
$(document).ready(function() {
   ...
   $('#formaGenerica').validate({
	   ...
	   if ($('#excel').attr('checked')) {
	   	...
	   	var fecha = LastDayOfMonth(anio, mes);
	   	regulatorioInsServicio.consulta(numConsulta,regulatorio,function(valida){
		   if(valida.aplica == 'S'){
		   		url = 'reporteCatMinFap.htm?tipoReporte=' + tipoReporte + '&tipoEntidad='+tipoInstitucion+'&fecha='+fecha+'&mes='+mes+'&anio='+anio+'&version='+version;
				window.open(url); 
		   }else{
   	...
   	function LastDayOfMonth(Year, Month) { 
   		if(Month <= 9){
   			var mes = ("0"+Month).toString();
   		} else{ 
   			var mes = Month;
   		}
   		var dia = new Date(new Date(Year, Month,1)-1).getDate(); 
