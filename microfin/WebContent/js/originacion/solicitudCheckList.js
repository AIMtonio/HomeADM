	...
	$('#solicitudCreditoID').bind('keyup',function(e){
		if(esTab){
			if(this.value.length >= 0){
				...
				lista('solicitudCreditoID', '1', '8', camposLista, parametrosLista, 'listaSolicitudCredito.htm');
			}
		}
	});
		...
		function consultaNumeroDocumentosPorCliente() {
			...
			if( $('#clienteID').val() != '' || !isNaN( $('#clienteID').val())){
				solicitudArchivoServicio.consulta(ConsulCantDoc,solicitudBeanCon,function(solicitud) {
					if(solicitud!=null){
						numeroDocumentos = 	solicitud.numeroDocumentos;
	...
	function consultaDestinoCredito(idControl) {
		...
		if(DestCred != '' && !isNaN(DestCred)){
				destinosCredServicio.consulta(catTipoConsultaSolicitud.principal,DestCredBeanCon,function(destinos) {
					if(destinos!=null){
						$('#nombreDestinoCre').val(destinos.descripcion.toUpperCase());
					}
					else {
