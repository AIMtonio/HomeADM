$(document).ready(function() {
	...
	$('#sucursalRegistroID').bind('keyup',function(e) {
		lista('sucursalRegistroID', '1', '1','nombreSucurs', $('#sucursalRegistroID').val(), 'listaSucursales.htm');
	});

	$('#sucursalRegistroID').blur(function() {
	...
	function generaReporte() {					
			...
			var sucursal = $("#sucursalRegistroID").val();
			...
			$('#ligaGenerar').attr('href','reporteApoyoEscolar.htm?fechaInicio='+fechaInicio+'&fechaFin='+
					fechaFin+'&estatus='+estatus+'&sucursalRegistroID='+sucursal+'&fechaSistema='+fechaSis
					+'&nombreUsuario='+nombreUsuario+'&tipoReporte='+tipoReporte+'&tipoLista='+tipoLista
					+'&nombreInstitucion='+nombreInstitucion+'&nombreSucurs='+nombreSucursal);
		...
		if (numSucursal != '' && !isNaN(numSucursal) && numSucursal != 0) {
			sucursalesServicio.consultaSucursal(conSucursal,
					numSucursal, function(sucursal) {
						if (sucursal != null) {
							$('#sucursalRegistroDes').val(sucursal.nombreSucurs);
						} else {
							alert("No Existe la Sucursal");
							$('#sucursalRegistroID').val('0');
							$('#sucursalRegistroDes').val('TODAS');
						}
					});
		}
		else{
