...
var cobraAccesoriosGen = 'N'; // Indica si se cobra accesorios PARAMGENERALES
var porcentajeMaximo	= 0; // Indica el porcentaje maximo a utilizar por creditos automaticos: PARAMGENERALES
...
$(document).ready(function() {
	...
	deshabilitaControl('cobraSeguroCuota');
	deshabilitaControl('cobraIVASeguroCuota');

	//------------ llena el combo para la Formula de Calculo de Interés -----------------------------------------
	consultaComboCalInteres();
	mostrarTipoGeneraInteres(1);
	...
	$('#productoNomina2').attr("checked",true);
	$('#cobraSeguroCuota2').attr("checked",true);
	$('#cobraAccesorios2').attr("checked", true);
	...
	$.validator.setDefaults({
		submitHandler: function(event) {
			...
			if($('#esGrupal').val()=='S'){ //se agrego por que no se podia encotrar el por que mandaba por defecto el valor S para cobro de seguro.
	...
	$('#participaSpei').blur(function() {
		var valor_no = 'N';
		var participaSpei = $('#participaSpei').val();

		if( participaSpei == valor_no ){
			deshabilitaControl('productoClabe');
			 $('#productoClabe').val('');
		} else{
			habilitaControl('productoClabe');
		}
	});
	...
	$('#obligadosSolidarios').change(function() {
		validaObligados();
	});
	...
	$('#permitePrepago').change(function(){
		...
		var tipoCalInt = $('#tipoCalInteres').val();
		$("#tipoPrepago option[value='P']").remove();
		if(permitePrepago == "S"){
			$('#prepago').show();
			$('#tipoPrepago').val('U');
			if(tipoCalInt == 2){
				$("#tipoPrepago").append('<option value="P">Pago Cuotas Completas Proyectadas</option>');
			}
		}else{
			$('#prepago').hide();
			$('#modificarPrepago2').attr("checked",true);
	...
	$('#manejaLinea').change(function() {
		...
		$("#cobraComAnual2").click();
	});

	$("#cobraComAnual2").click(function(){
		$('#tipoComAnual').val('');
		$('#valorComAnual').val('');
		deshabilitaControl('tipoComAnual');
		deshabilitaControl('valorComAnual');
	});

	$("#cobraComAnual1").click(function(){
		$('#tipoComAnual').val('');
		$('#valorComAnual').val('');
		habilitaControl('tipoComAnual');
		habilitaControl('valorComAnual');
	});
	...
	$('#cobraSeguroCuota2').click(function() {
		$('#cobraIVASeguroCuota').val('N').select();
		deshabilitaControl('cobraIVASeguroCuota');
	});
	$('#cobraSeguroCuota').click(function() {
		$('#cobraIVASeguroCuota').val('S').select();
		habilitaControl('cobraIVASeguroCuota');
	});

	$('#esAutomatico').blur(function() {
		if($('#esAutomatico').val() == 'S'){
			muestraSeccionAutomatico();


		}else{
			ocultaSeccionAutomatico();
	...
	$('#porcMaximo').blur(function() {
		var valorPorcentaje = $('#porcMaximo').val();
		consultaPorcentaje();
		if(valorPorcentaje > porcentajeMaximo  ){

			mensajeSis("El porcentaje excede al valor parametrizado.");
			$('#porcMaximo').val("0.00");
		$('#porcMaximo').focus();
		}
	});

	$('#tipoCalInteres').change(function() {
		var tipoCalInteres = $('#tipoCalInteres').val();
		if(tipoCalInteres == ''){
			tipoCalInteres = 1;
		}
		mostrarTipoGeneraInteres(tipoCalInteres);
	});

	$('#tipoCalInteres').blur(function() {
		var tipoCalInteres = $('#tipoCalInteres').val();
		if(tipoCalInteres == ''){
			tipoCalInteres = 1;
		}
		mostrarTipoGeneraInteres(tipoCalInteres);
		habilitaCampoPrepago();
	});
	...
	$('#formaGenerica').validate({
		...
		rules: {
			...
			tipoAutomatico: {
				required: function() {return $('#esAutomatico').val() == 'S'; }
			},
			...
			productoClabe:{
				required: function() {return $('#participaSpei').val() == 'S';},
				number: true,
			},diasAtrasoMin:{
				required: true,
				number: true
			...
			cobraComAnual:{
				required: function(){return ($('#manejaLinea').val()=='S')?true:false;}
			},
			tipoComAnual: {
				required : function(){return ($('#manejaLinea').val()=='S')?true:false;}
			},
		...
		messages: {
			...
			caracteristicas: {
				required: 'Especificar Características'
			},
			//
			cantidadAvales: {
			...
			requiereAnalisiCre:{
				required: 'Especificar opcion requiere analisis de credito'
			},
			margenPagIgual: {
			...
			tipoGeneraInteres :{
				required : 'Especificar el Tipo de Generación de Interés.'
			},
			requiereCheckList: {
				required : 'Especificar Requiere CheckList.'
			},
			tipoAutomatico : {
				required : 'Especificar Tipo Automático.'
			},
			porcMaximo : {
				required : 'Especificar Porcentaje Máximo.',
				numeroPositivo :'Cantidad Incorrecta',
				min: 'Valor Mínimo 0%'
			},

			participaSpei : {
				required : 'Especificar Si participa en SPEI.'
			},
			productoClabe:{
				required:'Especificar la Clabe SPEI.',
				number : 'Sólo Números.'
			},
			diasAtrasoMin:{
				required:'Especificar los Días de Castigo.',
				number : 'Sólo Números.'
			},
			cobraComAnual:{
				required : 'Especificar Si Cobra o No Comisión Anual.'
			},
			tipoComAnual:{
				required: 'Especificar el Tipo de Comisión Anual.'
			},
			valorComAnual: {
				required: 'Especificar el Valor para la Comisión Anual.'
			},
			permiteConsolidacion : {
				required : 'Especificar Permite Consolidación'
			},
			instruDispersion : {
				required : 'Especificar Instrucciones de dispersión'
			},
		}
	});
					...
					if(prodCred!=null){
						if(prodCred.esAgropecuario==No){
							...
							if(prodCred.tipoCalInteres == "2"){
								mostrarTipoGeneraInteres(2);
								$('#tipoGeneraInteres').val(prodCred.tipoGeneraInteres);
							} else {
								mostrarTipoGeneraInteres(1);
							}

							if(prodCred.manejaLinea == 'N'){
								$('#esRevolvente').attr('disabled', true);
								$('#afectacionContable').attr('disabled', true);
							}else{
								$('#esRevolvente').attr('disabled', false);
							...
							if(prodCred.productoNomina == 'S')
								$('#productoNomina').attr("checked",true);
							else
								$('#productoNomina2').attr("checked",true);

							if(prodCred.requiereAnalisiCre == 'S'){
								$('#requiereAnalisiCre').attr("checked",true);
								}
							else{
								$('#requiereAnalisiCre2').attr("checked",true);
								}

							if(prodCred.cobraSeguroCuota == 'S'){
								$('#cobraSeguroCuota').attr("checked",true);
								habilitaControl('cobraIVASeguroCuota');
							}
							else {
								$('#cobraSeguroCuota2').attr("checked",true);
								$('#cobraIVASeguroCuota').val('N');
								deshabilitaControl('cobraIVASeguroCuota');
							}

							if (prodCred.cobraAccesorios == 'S') {
								$('#cobraAccesorios').attr("checked",true);
							} else {
								$('#cobraAccesorios2').attr("checked",true);
							}
							...
							validaCalcInteres(prodCred.calcInteres,prodCred.proyInteresPagAde);
							consultaTipoContrato('tipoContratoBC');
							consultaTipoContratoCirculoCredito('tipoContratoCCID');
							//Solo se consulta cuando existe la institucion
							...
							limpiaFormaSeguro();
							limpiarDatos();
							mensajeSis('El Producto de Crédito es Agropecuario.<br>Consultar en el Módulo <i><u>Créditos Agro</u></i>.');
	...
	function mostrarTipoGeneraInteres(tipoOperacion){
		var oculta = 1;
		var muestra = 2;

		if(tipoOperacion == oculta){
			$('#trTipoGenInteres').hide();
			$('#tipoGeneraInteres').val('');
		}

		if(tipoOperacion == muestra){
			$('#trTipoGenInteres').show();
		}
	}
...
function deshabilitagrupales(){//deshabilita los campos de grupales
	...
	$('#gHombres').hide();
	$('#esGrupal').val('N');
	habilitaControl('cobraSeguroCuota');
	habilitaControl('cobraSeguroCuota2');
...
function habilitagrupales(){//habilita los campos de grupales
	...
	$('#cobraIVASeguroCuota').val('N').select();
	deshabilitaControl('cobraSeguroCuota');
	deshabilitaControl('cobraSeguroCuota2');
	deshabilitaControl('cobraIVASeguroCuota');
...
function limpiarDatos(){
	...
	$('#manejaLinea').val('S');
	$('#esRevolvente').val('S');
	habilitaControl('esRevolvente');
	$('#afectacionContable').val('S');
	habilitaControl('afectacionContable');
...
function validaObligados() {
	requiereObligadosNo = 'N';
	requiereObligadosSi = 'S';
	requiereObligadosIn = 'I';

	if ($('#obligadosSolidarios').val() == requiereObligadosSi || $('#obligadosSolidarios').val() == requiereObligadosIn) {

		$('#obligadosCruzados').val("");
		$('#obligadosCruzados').removeAttr('disabled');

		//HABILITAR INTERCAMBIO AVALES
		$('#noConsultaSIC').removeAttr('disabled');
		$('#siConsultaSIC').removeAttr('disabled');

	} else {
		$('#obligadosCruzados').val("N");
		$('#obligadosCruzados').attr('disabled','disabled');

		$('#siConsultaSIC').attr("checked",true);
		$('#noConsultaSIC').attr('disabled','disabled');
	}
}
...
function validaCalcInteres(calcInt,proyIntPagAde){
	var calcInteres = Number(calcInt);
	if(calcInteres==1){
		mostrarElementoPorClase('seccionProyInt',"S");
		$('#proyInteresPagAde').val(proyIntPagAde);
	}else{
		mostrarElementoPorClase('seccionProyInt',"N");
		$('#proyInteresPagAde').val('N');
	}
}
...
function muestraCobraAccesorios() {
	if(cobraAccesoriosGen == 'S'){
		mostrarElementoPorClase('ocultarAccesorios', "S");
		$('#cobraAccesorios2').attr("checked", true);
	}else{
		mostrarElementoPorClase('ocultarAccesorios', "N");
	}
}
...
function consultaTipoInstitucion() {
	...
	parametrosSisServicio.consulta(15, parametrosSisCon, {
		async : false,
		callback :function(institucion) {
		if (institucion != null) {
...
function muestraSeccionAutomatico(){
		$('#tipoAutomatico').show();
		$('#porcMaximo').show();
		$('#lblTipoAutomatico').show();
		$('#lblPorcMaximo').show();

}


function ocultaSeccionAutomatico(){
		$('#tipoAutomatico').hide();
		$('#porcMaximo').hide();
		$('#lblTipoAutomatico').hide();
		$('#lblPorcMaximo').hide();
}
...
function consultaPorcentaje(){
	...
	paramGeneralesServicio.consulta(tipoConsulta, bean, { async: false, callback:function(parametro) {
			if (parametro != null){
				porcentajeMaximo = parametro.valorParametro;

			}else{
				mensajeSis("No existe parametrizado un valor para el porcentaje");
			}
...
function habilitaCampoPrepago (){
	var calcInteres = $('#tipoCalInteres').val();
	$("#tipoPrepago option[value='P']").remove();
	if(calcInteres==2){
		$("#tipoPrepago").append('<option value="P">Pago Cuotas Completas Proyectadas</option>');
	}
...
function ayudaValConyuge(){	
	var data;
		
		data ='<fieldset class="ui-widget ui-widget-content ui-corner-all">'+ 
				'<legend class="ui-widget ui-widget-header ui-corner-all">Datos de C&oacute;nyuge Obligatorio:</legend>'+
				'<div id="ContenedorAyuda">'+ 			
				'<table id="tablaLista">'+
					'<tr align="left">'+
						'<td id="contenidoAyuda" align="left">'+
						'<b>Si el par&aacute;metro est&aacute; en NO la informaci&oacute;n del c&oacute;nyuge no ser&aacute; obligatoria, misma que no se evaluar&aacute; en el proceso de riesgo com&uacute;n ya que no se tendr&iacute;a registrada dicha informaci&oacute;n</b>'+
						'</td>'+
					'</tr>'+
				'</table>'+
				'</fieldset>'; 
