	...
	$(document).ready(function() {
			...
			$.validator.setDefaults({		
			    submitHandler: function(event) { 
						grabaFormaTransaccionRetrollamada(event, 'formaGenerica', 'contenedorForma', 'mensaje','true','tipoCedeID',
			...
			$('#grabar').click(function() {	
				...
				$('#instrumentoID').val($('#tipoCedeID').val());
				...
				if(tipCede != '' && !isNaN(tipCede) && esTab){
						tiposCedesServicio.consulta(1,TipoCedeBean, function(tipoCede){
							if(tipoCede!=null){
								...
								$('#tipoCuentaIDDes').val(tipoCede.descripcion);	
								listarSucursales(tipoLista.principalSucursalesGrid, tipoCede.estatus,tipoCede.descripcion);
							}else{
								$('#tipoCuentaIDDes').val('');
								$('#tipoCedeID').val('');
			...
			function listarSucursales(numeroLista, estatusTipoCede, descripTipoCede){  
					...
					if (tipoCuenta != ''  && !isNaN(tipoCuenta)){ 
						...
						$.post("gridTipoCuentaSucursalVista.htm", params, function(sucursalesGrid){	 
							if(sucursalesGrid.length >0) { 
								...
								if(parseInt(consultaFilas()) > 0){
									if(estatusTipoCede == 'I'){
										deshabilitaBoton('grabar', 'submit');
										deshabilitaBoton('imprimir', 'submit');	
										mensajeSis("El Producto "+descripTipoCede+" se encuentra Inactivo, por favor comunicarse con el Administrador para más información.");
										$('#tipoCedeID').focus();
									}else{
	...
	function agregarSucursales(numeroLista){ 
		...
		var params = {};
		params['tipoLista'] = numeroLista;
		params['tipoCuentaID'] = $("#tipoCedeID").val();
		params['sucursalID'] = $("#sucursalID").val();
		params['estadoID'] = $("#estadoID").val();
	...
	function generaReporte() {
		var tipoCuentaID = $("#tipoCedeID").val();	
