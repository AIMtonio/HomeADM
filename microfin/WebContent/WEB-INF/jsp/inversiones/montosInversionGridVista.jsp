...
<form id="gridMontosInv" name="gridMontosInv">
...
<tbody>
	...
	<c:forEach items="${montosInversion}" var="montoInversion" varStatus="status">
		<tr id="renglon${status.count}" name="renglon">
			<td>
				<c:choose>
				<c:when test="${status.count == 1}">
					<input type="text" size="18" name="plazoInferior" id="inferior${status.count}"
							 value="${montoInversion.plazoInferior}" esMoneda="true" maxlength="16" 
							 style="text-align: right" readOnly="true" disabled="true" />
				</c:when>				
				<c:otherwise>					
					<input type="text" size="18" name="plazoInferior" id="inferior${status.count}"
							 value="${montoInversion.plazoInferior}"  esMoneda="true"  
							  style="text-align: right" readOnly="true" disabled="true" />
				</c:otherwise>
				</c:choose>
			</td>
			<td>
				...
				<input type="text" size="18" name="plazoSuperior" id="superior${status.count}" esMoneda="true"
						 value="${montoInversion.plazoSuperior}" onblur="cambiaMontoSuperior(this)"
						 onkeypress="return validaDigitos(event, this.id)" style="text-align: right"/>
				<input type="button" name="elimina" id="${status.count}" class="btnElimina"
...
<script type="text/javascript">
	...
	agregaFormatoControles('gridMontosInv');
	
	if($('#estatusTipoInver').val() == 'I'){
		deshabilitaBoton('agregaMonto', 'submit');
	}else{
		habilitaBoton('agregaMonto', 'submit');
	}
	...
	function cambiaMontoSuperior(control){
		...
		if ($(jqMontoSup).asNumber() <= $(jqMontoInfActual).asNumber() ){
			alert("El Plazo Superior debe ser Mayor al Plazo Inferior");			
			control.focus();
			return false;
		}							
	...
	function eliminaPlazo(control){
		...
		if ($(jqMontoInf).attr("id") == $("input[name=plazoInferior]:first-child").attr("id")){
			$(jqMontoInfSig).val("1");				
	...
	$("#agregaMonto").click(function() {
      ...
      var tds = '<tr id="renglon' + nuevaFila + '" name="renglon">';
   	...
   	if(numeroFila == 0){	    		
    		tds += '<td ><input type="text" size="18"   name="plazoInferior" value="1.00" id="inferior'+nuevaFila+'" esMoneda="true" style="text-align: right" cambiaMontoSuperior(this)/></td>';
    		tds += '<td><input type="text" size="18" name="plazoSuperior" id="superior'+nuevaFila+'" value="" autocomplete="off" onblur="cambiaMontoSuperior(this)" onkeypress="return validaDigitos(event,this.id)" style="text-align: right"/>';
    	} else{    		    	
			//var valor = parseFloat(document.getElementById("superior"+numeroFila+"").value.replace(",","")) + .01;
			var valor = parseFloat(document.getElementById("superior"+numeroFila+"").value.replace(/\,/g,"")) + .01;
			
    		tds += '<td ><input type="text" size="18"  name="plazoInferior" id="inferior'+nuevaFila+'" value="'+valor+'" esMoneda="true" readOnly disabled style="text-align: right"/></td>';
    		tds += '<td><input type="text" size="18"  name="plazoSuperior" id="superior'+nuevaFila+'" value="" autocomplete="off" onblur="cambiaMontoSuperior(this)" esMoneda="true" onkeypress="return validaDigitos(event,this.id)" style="text-align: right"/>';
    	}
