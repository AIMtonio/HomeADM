				...
				<table id="miTabla" class="tablaRegula"  border="0" cellpadding="0" cellspacing="0" style="width: 3200px;">
					<tbody>	
						...
						<tr>
							...
							<td class="label"> 
						   		<label for="lblTipoValor">Tipo de Valor</label> 
							</td>
							<td class="label"> 
							...
							<td class="label"> 
						   		<label for="lblTipoInversion">Tipo de Inversión</label> 
							</td>
						...
						<c:forEach items="${regulatorioI0391}" var="registro" varStatus="status">
							<tr id="renglon${status.count}" name="renglon">
								...
								<td nowrap>
									<input type="text" id="tipoValor${status.count}" maxlength="100"   name="lTipoValorID" size="4" onkeyup="buscaTipoValor(this.id)"
																				onblur="consultaTipoValor(this.id)" value="${registro.tipoValorID}" class="display: inline-block" />	
									<input type="text" id="nombretipoValor${status.count}" readonly="" disabled="" name="tipoValor" size="50" value="${registro.valorDescrip}" class="display: inline-block" />
								</td>
								<td nowrap>
								...
								<td nowrap>
								<input type="hidden" id="stipoInversion${status.count}"  name="stipoInversion" size="3" value="${registro.tipoInversion}"/>
									<select name="lTipoInversion" id="tipoInversion${status.count}" >
									</select>
								</td>
								<td nowrap>
								...
								<td nowrap>
								<input type="text" id="tasaInteres${status.count}" maxlength="16" path="lTasaInteres" name="lTasaInteres" style="text-align:right;" onBlur="evaluaTasa(this)" size="12" value="${registro.tasaInteres}"/><label class="label">%</label>
								</td>
								<td nowrap>
