...
<tbody>
	...
	<c:forEach items="${plazosLista}" var="diasCedes" varStatus="status">
			...
			<td>
				...
				<c:when test="${status.count == 1}">
					<input type="text" size="5" name="plazoInferior"
							 ...
							 value="${diasCedes.plazoInferior}"/>
				</c:when>
				<c:otherwise>
					<input type="text" size="5" name="plazoInferior"
							 id="inferior${status.count}"
							 value="${diasCedes.plazoInferior}" readOnly="true"  />
				</c:otherwise>
				</c:choose>
				<input type="text" size="5" name="plazoSuperior"
						 id="superior${status.count}"
						 value="${diasCedes.plazoSuperior}"
...
<script type="text/javascript">
	...
	function cambiaDiaSuperior(control){
		...
		if (parseInt(control.value) <= parseInt($(jqDiaInfActual).val()) ){
			alert("El Día Superior debe ser Mayor al Día Inferior");			
			control.focus();
			return false;
		}					
