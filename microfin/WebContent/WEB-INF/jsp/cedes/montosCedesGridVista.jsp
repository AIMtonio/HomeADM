...
<form id="gridMontosCedes" name="gridMontosCedes">
...
<tbody>
	...
	<c:forEach items="${montosCedes}" var="montoCede" varStatus="status">
		<tr id="renglon${status.count}" name="renglon">
			<td>
				<c:choose>
				<c:when test="${status.count == 1}">
					<input type="text" size="18" name="montoInferior" id="inferior${status.count}"
							 value="${montoCede.montoInferior}" esMoneda="true" maxlength="16" 
							 style="text-align: right" readOnly="true"  />
				</c:when>				
				<c:otherwise>					
					<input type="text" size="18" name="montoInferior" id="inferior${status.count}"
							 value="${montoCede.montoInferior}"  esMoneda="true"  
			...
			<td>
				...
				<input type="text" size="18" name="montoSuperior" id="superior${status.count}" esMoneda="true"
						 value="${montoCede.montoSuperior}" onblur="cambiaMontoSuperior(this)"
...
<script type="text/javascript">
	...
	function cambiaMontoSuperior(control){
		...
		if ($(jqMontoSup).asNumber() <= $(jqMontoInfActual).asNumber() ){
			alert("El Monto Superior debe ser Mayor al Monto Inferior");			
			control.focus();
			return false;
		}							
	...
	$("#agregaMonto").click(function() {
	 	if($('#tipoCedeID').val()!=""){	
	    	...
	    	return false;
	 		...
	 		alert("Especifique Tipo de CEDES.");
			$('#tipoCedeID').focus();
	 	}
 	});
