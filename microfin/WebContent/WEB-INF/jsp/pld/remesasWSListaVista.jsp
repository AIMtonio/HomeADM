	...
	<c:choose>
		<c:when test="${tipoLista == '1'}"> 
			...
			<c:forEach items="${beanLista}" var="bean" >
				<tr onclick="cargaValorLista('${campoLista}', '${bean.remesaFolioID}');">
					<td nowrap="nowrap">${bean.remesaFolioID}</td>
					<td nowrap="nowrap">${bean.nombreCompleto}</td>
		...
		<c:when test="${tipoLista == '3'}"> 
			<tr id="encabezadoLista">
				<td  nowrap="nowrap">Clave Cobro</td>				
				<td  nowrap="nowrap">Nombre</td>
				<td  nowrap="nowrap">Estatus</td>				
			</tr>
			<c:forEach items="${beanLista}" var="bean" >
