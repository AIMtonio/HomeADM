  ...
  <table border="0" cellpadding="0" cellspacing="0" width="100%">
		...
		<tr>
		   ...
		   <td align="center"  nowrap="nowrap">
              	<input type="checkbox" id="seleccionaTodos2" name="seleccionaTodos2"  onclick="seleccionarTodosDos()"/>
            	<input type="hidden" id="seleccionado2${estatus.count}" name="listaSeleccionados2" value="N"/>
			</td>
		</tr>
		...
		<tr>
		    ...
		    <td width="10px"></td>
		    <td  nowrap="nowrap">
		        <input type="text" id="montoPagos2${estatus.count}" name="listaMontoPagos2" esMoneda="true"
		   ...
		   <td width="10px"></td>
		   <td align="center"  nowrap="nowrap">
              	<input type="checkbox" id="seleccionaCheck2${estatus.count}" name="seleccionaCheck2"  
              		onclick="sumaCheck2(this.id,${pagos.consecutivoID})"
              		${pagos.esSeleccionado == 'S' ? "checked" : ""} />
              	 <input type="hidden" id="seleccionado2${estatus.count}" name="listaSeleccionados2" value="N"/>
              	 <input type="hidden" id="esSeleccionado2${estatus.count}" name="listaEsSeleccionado2" tabindex="101" size="30" value="${pagos.esSeleccionado}" />
		      	<input type="hidden" id="consecutivoID2${estatus.count}" name="listaConsecutivoID2" tabindex="101" size="30" value="${pagos.consecutivoID}" />
			</td>
		</tr>
		</c:forEach>
	</table>
	<c:if test="${!listaPaginada.firstPage}">
		 <input onclick="consultaNoAplicadosAntes('previous')" type="button" value="" class="btnAnterior" />
	</c:if>
	<c:if test="${!listaPaginada.lastPage}">
		 <input onclick="consultaNoAplicadosDespues('next')" type="button" value="" class="btnSiguiente" />
	</c:if>
</fieldset>
<script type="text/javascript">

	function consultaNoAplicadosAntes(pageValor) {
		var lista = ${tipoLista};
		...
		$.post("crediNoAplicadosGrid.htm", params, function(data) {
			...
			if (data.length > 0) {
				...
				$('#totalAcumulado').val(totalAcumulado);
				validaMontos('', 0, '', '');
	...
	function consultaNoAplicadosDespues(pageValor) {
		var lista = ${tipoLista};
		var totalAcumulado = $('#totalPagos').asNumber();
        var EstFolio = $('#estatusPagoInst').val();
