...
public class ReporteSITIControlador extends AbstractCommandController {
	
	private ReporteReelevantesServicio reporteReelevantesServicio = null;
	private OpeInusualesServicio opeInusualesServicio = null;
	private OpIntPreocupantesServicio opIntPreocupantesServicio = null;
	private ParametrosSisServicio parametrosSisServicio = null;
	String nombreReporte = null;
	String successView = null;
	
	public ReporteSITIControlador() {
		setCommandClass(ReportesSITIBean.class);
		setCommandName("reportesSITIBean");
	}

	public static interface Enum_Con_TipRepor {
		  int  ReporteRelevantes		= 1;
		  int  ReporteInusuales			= 2;
		  int  ReportePreocupantes		= 3;
	}
	
	public static interface Enum_Lis_TipRepor {
		  int  ReporteRelevantes		= 1;
		  int  ReporteInusuales			= 2;
		  int  ReportePreocupantes		= 8;
	}
	
	public static interface Enum_NombreReporte {
		  String ReporteRelevantes		= "ReporteOpRelevantesSITI";
		  String ReporteInusuales		= "ReporteOpInusualesSITI";
		  String ReportePreocupantes	= "ReporteOpInternasPreocupantesSITI";
	}
	...
	protected ModelAndView handle(HttpServletRequest request, HttpServletResponse response, Object command, BindException errors) throws Exception {
		...
		switch (tipoReporte) {
		case Enum_Con_TipRepor.ReporteRelevantes:
		case Enum_Con_TipRepor.ReporteInusuales:
		case Enum_Con_TipRepor.ReportePreocupantes:
			List<ReportesSITIBean>listaReportes=reporteSITIExcel(reporteSITIBean, request, response,tipoReporte);
			break;
		}
		return null;
	}
	...
	private List<ReportesSITIBean> reporteSITIExcel(ReportesSITIBean reporteSITIBean, HttpServletRequest request, HttpServletResponse response, int tipoReporte) {
		...
		String leyendaSocap = "Las columnas marcadas con asterisco (*) (14, 34 a la 41) no aplican para Operaciones Relevantes." +
								"\nLas columnas marcadas con asterisco (**) (29 a la 33) aplican únicamente para Instituciones y" +
								" Sociedades Mutualistas de Seguros, y para Instituciones de Fianzas.";
		String leyendaSofom = "Las columnas marcadas con asterisco (*) (14, 29 a la 36) no aplican para Operaciones Relevantes.";
		switch (tipoReporte) {
		case Enum_Con_TipRepor.ReporteRelevantes:
			listaReporte = reporteReelevantesServicio.lista(tipoReporte, reporteSITIBean);
			nombreReporteXLS = Enum_NombreReporte.ReporteRelevantes;
			break;
		case Enum_Con_TipRepor.ReporteInusuales:
			String  datosOperInusuales = request.getParameter("datosOperInusuales");
			int tipoActualizacion = (request.getParameter("tipoActualizacion")!=null)?Utileria.convierteEntero(request.getParameter("tipoActualizacion")):0;
			listaReporte = opeInusualesServicio.listaExcel(tipoActualizacion, 0, reporteSITIBean, datosOperInusuales);
			nombreReporteXLS = Enum_NombreReporte.ReporteInusuales;
			break;
		case Enum_Con_TipRepor.ReportePreocupantes:
			listaReporte = opIntPreocupantesServicio.listaExcel(Enum_Lis_TipRepor.ReportePreocupantes, reporteSITIBean);
			nombreReporteXLS = Enum_NombreReporte.ReportePreocupantes;
			break;
		}
		...
		if(listaReporte != null){
			try{
				...
				celdaUsu.setCellValue(((!reporteSITIBean.getNombreUsuario().isEmpty())?reporteSITIBean.getNombreUsuario(): "TODOS").toUpperCase());
				...
				if (parametrosSisBean.getNombreCortoInst().equals("socap")
						|| parametrosSisBean.getNombreCortoInst().equals("scap")
						|| parametrosSisBean.getNombreCortoInst().equals("sofipo")) {
					celda = fila.createCell(numCelda++);
					celda.setCellValue("NOMBRE**");
					hoja.addMergedRegion(new CellRangeAddress(4,5,numCelda,numCelda));
					celda.setCellStyle(estiloCentrado);
				...
				for(int iter=0; iter<tamanioLista; iter++){
					...
					celda=fila.createCell(numCelda++);
					celda.setCellValue(tipoReporte == Enum_Con_TipRepor.ReporteRelevantes ? valorVacio : sitiBean.getFechaDeteccion());
					celda.setCellStyle(estilo8);
				...
				//Creo la cabecera
				response.addHeader("Content-Disposition","inline; filename="+nombreReporteXLS+".xlsx");
