...
public class MonitorRiesgoComunDAO extends BaseDAO{

	public MonitorRiesgoComunDAO(){
		super();
	}
	...
	public MensajeTransaccionBean grabaListaRiesgoComun( final List listaParametros,  final int tipoActualizacion ) {
		...
		mensaje = (MensajeTransaccionBean) ((TransactionTemplate)conexionOrigenDatosBean.getManejadorTransaccionesMapa().get(parametrosAuditoriaBean.getOrigenDatos())).execute(new TransactionCallback<Object>() {
			public Object doInTransaction(TransactionStatus transaction) {
				...
				try {
					MonitorRiesgoComunBean riesgoComunBean;

						if(listaParametros.size()>0){
							for(int i=0; i < listaParametros.size(); i++){
								riesgoComunBean = new MonitorRiesgoComunBean();
								riesgoComunBean = (MonitorRiesgoComunBean) listaParametros.get(i);
	...
	public MensajeTransaccionBean bajaParametros(final MonitorRiesgoComunBean lisRiesgoComun) {
		MensajeTransaccionBean mensaje = new MensajeTransaccionBean();
		transaccionDAO.generaNumeroTransaccion();
	...
	public MensajeTransaccionBean altaRiesgoComun(final MonitorRiesgoComunBean riesgoComunBean, final int tipoActualizacion) {
		MensajeTransaccionBean mensaje = new MensajeTransaccionBean();
					...
					mensajeBean = (MensajeTransaccionBean) ((JdbcTemplate) conexionOrigenDatosBean.getOrigenDatosMapa().get(parametrosAuditoriaBean.getOrigenDatos())).execute(
						new CallableStatementCreator() {
							public CallableStatement createCallableStatement(Connection arg0) throws SQLException {
									...
									sentenciaStore.setString("Par_Procesado",riesgoComunBean.getProcesado());
									sentenciaStore.setString("Par_ComentarioMonitor",riesgoComunBean.getComentariosMonitor());
									sentenciaStore.setString("Par_ConsecutivoID",riesgoComunBean.getConsecutivoID());
									sentenciaStore.setString("Par_Estatus",riesgoComunBean.getEstatus());
									sentenciaStore.setInt("Par_Clave",Utileria.convierteEntero(riesgoComunBean.getClave()));
	...
	public List listaRiesgoComun(int tipoLista, MonitorRiesgoComunBean riesgoComunBean){
		...
		List matches= ((JdbcTemplate)conexionOrigenDatosBean.getOrigenDatosMapa().get(parametrosAuditoriaBean.getOrigenDatos())).query(query,parametros  ,new RowMapper() {
			public Object mapRow(ResultSet resultSet, int rowNum) throws SQLException {
				...
				MonitorRiesgoComunBean bean = new MonitorRiesgoComunBean();

				bean.setSolicitudCreditoID(resultSet.getString("SolicitudCreditoID"));
				bean.setClienteID(resultSet.getString("ClienteIDRel"));
				bean.setNombreCliente(resultSet.getString("NombreCompletoRel"));
				bean.setClienteIDRel(resultSet.getString("ClienteID"));
				bean.setNombreClienteRel(resultSet.getString("NombreCompleto"));
				bean.setCreditoID(resultSet.getString("CreditoID"));
				bean.setProcesado(resultSet.getString("Procesado"));
				...
				bean.setDescParen(resultSet.getString("Descripcion"));
				bean.setEstatus(resultSet.getString("Estatus"));
				bean.setClave(resultSet.getString("Clave"));
				bean.setMontoAcumulado(resultSet.getString("MontoAcumulado"));
				bean.setClienteIDDesc(resultSet.getString("ClienteIDDesc"));
