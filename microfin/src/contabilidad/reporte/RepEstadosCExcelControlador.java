...
import contabilidad.reporte.RepSATExcelControlador.Enum_Con_TipRepor;
import contabilidad.servicio.CuentasContablesServicio;

public class RepEstadosCExcelControlador extends AbstractCommandController{
	...
	public static interface Enum_Con_TipRepor {
 	...
 	public RepEstadosCExcelControlador(){
 	...
 	protected ModelAndView handle(HttpServletRequest request,
		...
		case Enum_Con_TipRepor.ReporExcel:
			List<CuentasContablesBean>listaReportes=reporteEstadosCuentaExcel(cuentasContables, response);
	...
	// Reporte de Maestr Contable contable en Excel
		public List <CuentasContablesBean>reporteEstadosCuentaExcel(CuentasContablesBean cuentasContables,HttpServletResponse response){
			List <CuentasContablesBean> listaPeriodos= null;
			listaPeriodos=cuentasContablesServicio.listaReporteEstadosCExcel(cuentasContables,response);
			...
			if(listaPeriodos!=null){
				try {
					...
					// Creacion de hoja
					XSSFSheet hoja = libro.createSheet("ReporteEstadosCuenta");
					XSSFRow fila= hoja.createRow(0);

					// Nombre Usuario
					XSSFCell celdaini = fila.createCell((short)1);
					celdaini = fila.createCell((short)7);
					...
					XSSFCell celdafin = fila.createCell((short)7);
					celdafin.setCellValue("Fecha:");
					celdafin.setCellStyle(estiloNeg8);	
					celdafin = fila.createCell((short)8);
					...
					celdaR.setCellValue("REPORTE DE ESTADOS DE CUENTA (ESTATUS TIMBRE) DEL PERIODO " + cuentasContables.getPeriodo() );
					hoja.addMergedRegion(new CellRangeAddress(
				            2, //first row (0-based)
				            2, //last row  (0-based)
					...
					celda.setCellValue("Estatus Timbre");
					celda.setCellStyle(estiloNeg8);
					...
					disposicion = (CuentasContablesBean) listaPeriodos.get(iter);
						...
						celda=fila.createCell((short)4);
						celda.setCellValue(disposicion.getEstatus());
					...
					//Creo la cabecera
					response.addHeader("Content-Disposition","inline; filename=ReporteEstadosCuenta.xls");
