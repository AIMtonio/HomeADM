...
import regulatorios.dao.RegulatorioA1012DAO;
import regulatorios.servicio.RegulatorioInsServicio.Enum_Lis_TipoReporte;
import general.servicio.BaseServicio;
import herramientas.Constantes;

public class RegulatorioA1012Servicio extends BaseServicio{

	RegulatorioA1012DAO regulatorioA1012DAO = null;
	public static Double DOUBLE_VACIO = 0.00;
	
	public RegulatorioA1012Servicio() {
		super();
	}
	
	/* ============ case para listas de reportes regulatorios ===============*/
	public List <RegulatorioA1012Bean>listaReporteRegulatorioA1012(int tipoLista, int tipoEntidad, RegulatorioA1012Bean reporteBean, HttpServletResponse response) throws IOException{
		List<RegulatorioA1012Bean> listaReportes=null;
		
		/*SOFIPO*/
		if(tipoEntidad == RegulatorioInsServicio.Enum_Lis_TiposInstitucion.sofipo){
			switch(tipoLista){
				case Enum_Lis_TipoReporte.excel:
					listaReportes = reporteRegulatorioA1012Excel(Enum_Lis_TipoReporte.excel, reporteBean, response); 
					break;
				case Enum_Lis_TipoReporte.csv:
					listaReportes = reporteRegulatorioA1012Csv(reporteBean, Enum_Lis_TipoReporte.csv,  response); 
					break;		
			}
		}
	...
	private List reporteRegulatorioA1012Csv(RegulatorioA1012Bean reporteBean,int tipoReporte,HttpServletResponse response){
		String nombreArchivo="";
		List listaBeans = regulatorioA1012DAO.reporteRegulatorioA1012Csv(reporteBean, tipoReporte);
		nombreArchivo="R10_A1012.csv"; 
		// se inicia seccion para pintar el archivo csv
		try{
			RegulatorioA1012Bean bean;
			BufferedWriter writer = new BufferedWriter(new FileWriter(nombreArchivo));
			if (!listaBeans.isEmpty()){
				for(int i=0; i < listaBeans.size(); i++){
					bean = (RegulatorioA1012Bean) listaBeans.get(i);
	 ...
	 * @return
	 */
	public List<RegulatorioA1012Bean> reporteRegulatorioA1012Excel(int tipoLista,RegulatorioA1012Bean reporteBean,  HttpServletResponse response) throws IOException
	{
		List<RegulatorioA1012Bean> listaRegulatorioA1012Bean = null;
		String mesEnLetras			= "";
		String anio					= "";
		String nombreArchivo 		= "";
		...
		nombreArchivo 	= "R10_A1012_"+mesEnLetras+"_"+anio; 
		
		listaRegulatorioA1012Bean = regulatorioA1012DAO.reporteRegulatorioA1012Excel(reporteBean,tipoLista);
		int contador = 1;
		
		if(listaRegulatorioA1012Bean != null){
			try {
				...
				hoja = (SXSSFSheet) libro.createSheet("REGULATORIO A-1012");
				...
				/* AGREGAR GRUPOS DE COLUMNAS */
					...
					celda = fila.createCell((short)1);
					celda.setCellValue("Reporte A-1012 Reclasificaciones en el estado de resultados.");
					celda.setCellStyle(estiloDerecha);
					//funcion para unir celdas
					hoja.addMergedRegion(new CellRangeAddress(
					...
					int rowExcel=9;
					contador=9;
					RegulatorioA1012Bean bean = null;
					CellStyle estiloDinamico = null;
					for(int x = 0; x< listaRegulatorioA1012Bean.size() ; x++ ){
						
						if (x == 0){
							estiloDinamico = estiloUpper;
						}else						
						if(x == (listaRegulatorioA1012Bean.size()-1)){
							estiloDinamico = estiloBottom;
						}else{
							estiloDinamico = estilo8;
						}
						bean = (RegulatorioA1012Bean)listaRegulatorioA1012Bean.get(x);
		...
		return listaRegulatorioA1012Bean;
	}

	public RegulatorioA1012DAO getRegulatorioA1012DAO() {
		return regulatorioA1012DAO;
	}

	public void setRegulatorioA1012DAO(RegulatorioA1012DAO regulatorioA1012DAO) {
		this.regulatorioA1012DAO = regulatorioA1012DAO;
	}
