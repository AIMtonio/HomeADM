...
import regulatorios.dao.RegulatorioA1220DAO;
import regulatorios.servicio.RegulatorioInsServicio.Enum_Lis_TipoReporte;

public class RegulatorioA1220Servicio extends BaseServicio {
	
	RegulatorioA1220DAO regulatorioA1220DAO = null;	
	public static Double DOUBLE_VACIO = 0.00;
	
	public RegulatorioA1220Servicio() {
		super();
	}
	
	/* ============ case para listas de reportes regulatorios ===============*/
	public List <RegulatorioA1220Bean>listaReporteRegulatorioA1220(int tipoLista, int tipoEntidad, RegulatorioA1220Bean reporteBean, HttpServletResponse response) throws IOException{
		List<RegulatorioA1220Bean> listaReportes=null;
		
		/*SOFIPO*/
		if(tipoEntidad == RegulatorioInsServicio.Enum_Lis_TiposInstitucion.sofipo){
			switch(tipoLista){
				case Enum_Lis_TipoReporte.excel:
					listaReportes = reporteRegulatorioA1220XLSX_SOFIPO(Enum_Lis_TipoReporte.excel, reporteBean, response); 
					break;
				case Enum_Lis_TipoReporte.csv:
					listaReportes = generarReporteRegulatorioA1220(reporteBean, Enum_Lis_TipoReporte.csv,  response); 
					break;		
			}
		}
...
/* ======================================  FUNCIONES PARA GENERAR REPORTES CSV  ========================================*/	
	...
	private List generarReporteRegulatorioA1220(RegulatorioA1220Bean reporteBean,int tipoReporte,HttpServletResponse response){
		String nombreArchivo="";
		List listaBeans = regulatorioA1220DAO.reporteRegulatorioA1220Csv(reporteBean, tipoReporte);
		nombreArchivo="R12_A1220.csv"; 
		// se inicia seccion para pintar el archivo csv
		try{
			RegulatorioA1220Bean bean;
			BufferedWriter writer = new BufferedWriter(new FileWriter(nombreArchivo));
			if (!listaBeans.isEmpty()){
				for(int i=0; i < listaBeans.size(); i++){
					bean = (RegulatorioA1220Bean) listaBeans.get(i);
	 ...
	 * @return
	 */
	public List<RegulatorioA1220Bean> reporteRegulatorioA1220XLSX_SOFIPO(int tipoLista,RegulatorioA1220Bean reporteBean,  HttpServletResponse response) throws IOException
	{
		List<RegulatorioA1220Bean> listaRegulatorioA1220Bean = null;
		String mesEnLetras			= "";
		String anio					= "";
		String nombreArchivo 		= "";
		...
		nombreArchivo 	= "R12_A1220_"+mesEnLetras+"_"+anio; 
		
		listaRegulatorioA1220Bean = regulatorioA1220DAO.reporteRegulatorioA1220Sofipo(reporteBean,tipoLista);
		int contador = 1;
		
		if(listaRegulatorioA1220Bean != null){
			try {
				...
				hoja = (SXSSFSheet) libro.createSheet("REGULATORIO A-1220");
				...
				/* AGREGAR GRUPOS DE COLUMNAS */
					...
					celda = fila.createCell((short)1);
					celda.setCellValue("Reporte A-1220 Consolidación del estado de resultados de la Sociedad Financiera Popular con sus subsidiarias");
					celda.setCellStyle(estiloDerecha);
					//funcion para unir celdas
					hoja.addMergedRegion(new CellRangeAddress(
					...
					int rowExcel=9;
					contador=9;
					RegulatorioA1220Bean bean = null;
					CellStyle estiloDinamico = null;
					for(int x = 0; x< listaRegulatorioA1220Bean.size() ; x++ ){
						
						if (x == 0){
							estiloDinamico = estiloUpper;
						}else						
						if(x == (listaRegulatorioA1220Bean.size()-1)){
							estiloDinamico = estiloBottom;
						}else{
							estiloDinamico = estilo8;
						}
						bean = (RegulatorioA1220Bean)listaRegulatorioA1220Bean.get(x);
		...
		return listaRegulatorioA1220Bean;
	}


	public RegulatorioA1220DAO getRegulatorioA1220DAO() {
		return regulatorioA1220DAO;
	}


	public void setRegulatorioA1220DAO(RegulatorioA1220DAO regulatorioA1220DAO) {
		this.regulatorioA1220DAO = regulatorioA1220DAO;
	}
