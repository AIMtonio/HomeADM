...
public class TipoCuentaSucursalDAO extends BaseDAO{
	public TipoCuentaSucursalDAO() {
		super();
	}
		...
		mensaje = (MensajeTransaccionBean) ((TransactionTemplate)conexionOrigenDatosBean.getManejadorTransaccionesMapa().get(parametrosAuditoriaBean.getOrigenDatos())).execute(new TransactionCallback<Object>() {
			public Object doInTransaction(TransactionStatus transaction) {
				...
				try {
					...
					TipoCuentaSucursalBean bean;

					if(listaSucursales!=null){
						for(int i=0; i<listaSucursales.size(); i++){
							/* obtenemos un bean de la lista */
							bean = (TipoCuentaSucursalBean)listaSucursales.get(i);
					...
					e.printStackTrace();
					loggerSAFI.error(parametrosAuditoriaBean.getOrigenDatos()+"-"+"Error en procesa alta de tipos de cuenta por sucursal", e);
					if(mensajeBean.getNumero()==0){
	...
	public MensajeTransaccionBean alta(final TipoCuentaSucursalBean bean, final String esPrimero) {
	MensajeTransaccionBean mensaje = new MensajeTransaccionBean();
		...
		public Object doInTransaction(TransactionStatus transaction) {
			...
			mensajeBean = (MensajeTransaccionBean) ((JdbcTemplate) conexionOrigenDatosBean.getOrigenDatosMapa().get(parametrosAuditoriaBean.getOrigenDatos())).execute(
				...
				String query = "call TIPOPRODSUCURSALESALT(?,?,?,?,?, ?,  ?,?,?,  ?,?,?,?,?,?,?);";
				...
				CallableStatement sentenciaStore = arg0.prepareCall(query);
					...
					sentenciaStore.setLong("Aud_NumTransaccion",parametrosAuditoriaBean.getNumeroTransaccion());
							...
							loggerSAFI.info(parametrosAuditoriaBean.getOrigenDatos()+"-"+"call TIPOPRODSUCURSALESALT(" + sentenciaStore.toString() + ")");
	...
	public List lista(TipoCuentaSucursalBean bean, int tipoLista) {
		String query = "call TIPOPRODSUCURSALESLIS(?,?,?,?,?, ?,?,?,?,?,?,?);";
		...
		loggerSAFI.info(parametrosAuditoriaBean.getOrigenDatos()+"-"+"call TIPOPRODSUCURSALESLIS(" + Arrays.toString(parametros) +")");

		List matches= ((JdbcTemplate)conexionOrigenDatosBean.getOrigenDatosMapa().get(parametrosAuditoriaBean.getOrigenDatos())).query(query,parametros  ,new RowMapper() {

			public Object mapRow(ResultSet resultSet, int rowNum) throws SQLException {
				TipoCuentaSucursalBean beanResultado = new TipoCuentaSucursalBean();
	...
	public List listaFiltro(TipoCuentaSucursalBean bean, int tipoLista) {
		String query = "call TIPOPRODSUCURSALESLIS(?,?,?,?,?, ?,?,?,?,?,?,?);";
		Object[] parametros = {	Utileria.convierteEntero(bean.getInstrumentoID()),
								Utileria.convierteEntero(bean.getTipoInstrumentoID()),
	...
	public MensajeTransaccionBean baja(final TipoCuentaSucursalBean bean) {
	MensajeTransaccionBean mensaje = new MensajeTransaccionBean();
		...
		public Object doInTransaction(TransactionStatus transaction) {
			...
			mensajeBean = (MensajeTransaccionBean) ((JdbcTemplate) conexionOrigenDatosBean.getOrigenDatosMapa().get(parametrosAuditoriaBean.getOrigenDatos())).execute(
					new CallableStatementCreator() {
						public CallableStatement createCallableStatement(Connection arg0) throws SQLException {
							String query = "call TIPOPRODSUCURSALESBAJ(?,?,?,?,?, ?,?,?,?,?,  ?);";
							...
							loggerSAFI.info(parametrosAuditoriaBean.getOrigenDatos()+"-"+"call TIPOPRODSUCURSALESBAJ(" + sentenciaStore.toString() + ")");
