...
import org.springframework.web.servlet.mvc.AbstractCommandController;
	...
	public class InversionesVigRepControlador extends AbstractCommandController{
		...
		public InversionesVigRepControlador() {
		...
		protected ModelAndView handle(HttpServletRequest request,
			...
			switch(tipoReporte){
				case Enum_Con_TipRepor.ReporPantalla:
					 htmlString = inversionServicio.reporteInversionVig(inversionBean, nombreReporte);
				break;
					
				case Enum_Con_TipRepor.ReporPDF:
					ByteArrayOutputStream htmlStringPDF = reporteInversionVigPDF(inversionBean, nombreReporte, response);
				break;
					
				case Enum_Con_TipRepor.ReporExcel:		
					 List<InversionBean>listaReportes = reporteInversionesVigentesExcel(inversionBean,response);
				break;
			}
		...
		public ByteArrayOutputStream reporteInversionVigPDF(InversionBean inversionBean, String nombreReporte, HttpServletResponse response){
			ByteArrayOutputStream htmlStringPDF = null;
			try {
				htmlStringPDF = inversionServicio.repInversionVigPDF(inversionBean, nombreReporte);
		...
		public List <InversionBean> reporteInversionesVigentesExcel(InversionBean inversionBean, HttpServletResponse response){
			List<InversionBean> listaInversiones = null;
			listaInversiones = inversionServicio.listaInversionesVigentesExcel(inversionBean,  response); 	
			...
			if(listaInversiones != null){
				...
				hoja = (SXSSFSheet) libro.createSheet("Reporte de Inversiones Vigentes.");
				...
				celda = fila.createCell((short)1);
				celda.setCellValue("REPORTE DE INVERSIONES VIGENTES AL DÍA " + inversionBean.getFechaInicio());
				celda.setCellStyle(estiloNegritaEncabezado);
				...
				/* Valor de Usuario*/
				celda = fila.createCell((short)17);
				celda.setCellValue("INVERSIONES VIGENTES");
				celda.setCellStyle(estilo8);
				...
				String nombreArchivo = "Reporte Inversiones Vigentes";
				//Creo la cabecera
				...
				outputStream.close();
					...
					loggerSAFI.info(parametrosAuditoriaBean.getOrigenDatos()+"-"+"Error al crear el reporte Inversiones vigentes: " + e.getMessage());
					e.printStackTrace();
				}
			}
