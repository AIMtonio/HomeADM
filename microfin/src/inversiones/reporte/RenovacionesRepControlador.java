...
import org.springframework.web.servlet.mvc.AbstractCommandController;
	...
	public class RenovacionesRepControlador extends AbstractCommandController{
		...
		public RenovacionesRepControlador() {
			setCommandClass(InversionBean.class);
			setCommandName("renovaciones");
		}
		...
		protected ModelAndView handle(HttpServletRequest request,
			...
			case Enum_Con_TipRepor.ReporPantalla:
				 htmlString = inversionServicio.reporteRenovacion(inversionBean, nombreReporte);
			break;
				
			case Enum_Con_TipRepor.ReporPDF:
				ByteArrayOutputStream htmlStringPDF = reporteRenovacionesPDF(inversionBean, nombreReporte, response);
			break;
			case Enum_Con_TipRepor.ReporExcel:		
				 List<InversionBean>listaReportes = reporteRenovacionesInversionDiaExcel(inversionBean,response);
			break;
		}
		...
		public ByteArrayOutputStream reporteRenovacionesPDF(InversionBean inversionBean, String nombreReporte, HttpServletResponse response){
			ByteArrayOutputStream htmlStringPDF = null;
			try {
				htmlStringPDF = inversionServicio.repRenovacionPDF(inversionBean, nombreReporte);
				response.addHeader("Content-Disposition","inline; filename=RepRenovacionesInversiones.pdf");
		...
		public List <InversionBean> reporteRenovacionesInversionDiaExcel(InversionBean inversionBean, HttpServletResponse response){
			
			List<InversionBean> listaInversiones = null;
			listaInversiones = inversionServicio.listaRenovacionesMesExcel(inversionBean, response); 	
			
			Date date = new Date();
			DateFormat hora = new SimpleDateFormat("HH:mm");
			String mes = OperacionesFechas.getNombreMes(inversionBean.getMes(), true);
			...
			if(listaInversiones != null){
				...
				hoja = (SXSSFSheet) libro.createSheet("Reporte Renovaciones del mes.");
				...
				celda = fila.createCell((short)1);
				celda.setCellValue("REPORTE DE RENOVACIONES DEL MES DE "+ mes +" DE " + inversionBean.getAnio());
				celda.setCellStyle(estiloNegritaEncabezado);
				...
				/* Valor de Usuario*/
				celda = fila.createCell((short)16);
				celda.setCellValue("RENOVACIONES");
				celda.setCellStyle(estilo8);
				...
				/* Campo inversión */
				celda = fila.createCell((short)8);
				celda.setCellValue("Inversión:");
				celda.setCellStyle(estiloNegritaIzquierda);
				hoja.addMergedRegion(new CellRangeAddress(6, 6, 8, 9));
				
				/* Valor de inversion */
				int tipoInversion = Integer.parseInt(inversionBean.getInversionRenovada());
				String tipoInversiones;
				if(tipoInversion == 0){
					tipoInversiones="RENOVADA";
				} else{
					tipoInversiones="NO RENOVADA";
				}
				celda = fila.createCell((short)10);
				celda.setCellValue(tipoInversiones);
				celda.setCellStyle(estilo8);
				...
				mes = OperacionesFechas.getNombreMes(inversionBean.getMes(), false);
				String nombreArchivo = "Reporte de Renovaciones de "+ mes +" del " + inversionBean.getAnio();
				//Creo la cabecera
				...
				outputStream.close();
					...
					loggerSAFI.info(parametrosAuditoriaBean.getOrigenDatos()+"-"+"Error al crear el reporte Renovaciones de mes: " + e.getMessage());
					e.printStackTrace();
				}
			}
