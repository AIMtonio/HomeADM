...
public class CreditosMayorInsoluto10PorcRepControlador extends AbstractCommandController{
	...
	public CreditosMayorInsoluto10PorcRepControlador (){
		...
		int tipoReporte =(request.getParameter("tipoReporte")!=null)?
			...
			switch(tipoReporte){
				case Enum_Con_TipRepor.ReporteExcel:
					reporteMayorSaldoInsoluto10Porc(tipoReporte,riesgosBean,response);
				break;
			}
			return null;
		}
	// Reporte en Excel de Créditos Mayor Saldo Insoluto 10 %
	public List reporteMayorSaldoInsoluto10Porc(int tipoReporte,UACIRiesgosBean riesgosBean,  HttpServletResponse response){
		List listaRepote=null;
		String mesEnLetras	= "";
		String anio	= "";
		String nombreArchivo = "";
		mesEnLetras = creditosMayorInsoluto10PorcServicio.descripcionMes(riesgosBean.getMes());
		anio = riesgosBean.getAnio();
		nombreArchivo = "Mayor Saldo Insoluto 10 % "+mesEnLetras +" "+anio; 
		listaRepote = creditosMayorInsoluto10PorcServicio.listaReporteMayorSaldo10Porc(tipoReporte, riesgosBean, response); 
		...
		try {
			...
			// Creacion de hoja					
			HSSFSheet hoja = libro.createSheet("Mayor Saldo Insoluto 10 %");
			...
			celdaInst.setCellStyle(estiloDatosCentrado);
			  ...
			  hoja.addMergedRegion(new CellRangeAddress(//funcion para unir celdas
				...
				celda.setCellValue("REPORTE MAYOR SALDO INSOLUTO 10 % "+mesEnLetras+" - "+riesgosBean.getAnio());
				...
				celdaSuma.setCellValue("Total 20 Créditos");
				...
				celdaCapital.setCellValue("Saldo Total de la Cartera de Crédito");
				celdaCapital = fila.createCell((short)2);
				celdaCapital.setCellValue(Utileria.convierteDoble(riesgos.getTotalCarteraCredito()));
				...
				celdaResultado.setCellValue("Resultado sobre el 10%  de la Cartera Total");
				celdaResultado = fila.createCell((short)2);
				String resPorcentual = "(C32)*0.10"; 
				...
				HSSFCell celdaDiferencia=fila.createCell((short)34);
				celdaDiferencia = fila.createCell((short)1);
				celdaDiferencia.setCellValue("Resultado Porcentual del Valor del Saldo Insoluto");
				celdaDiferencia = fila.createCell((short)2);
				String resDiferencia = "(C31/C33)*100"; 
				celdaDiferencia.setCellType (HSSFCell.CELL_TYPE_FORMULA ); 
				celdaDiferencia.setCellFormula(resDiferencia);
