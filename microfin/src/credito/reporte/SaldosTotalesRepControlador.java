...
public class SaldosTotalesRepControlador extends AbstractCommandController  {
	...
	protected ModelAndView handle(HttpServletRequest request, HttpServletResponse response, Object command, BindException errors) throws Exception {
		...
		switch (tipoReporte) {
			case Enum_Con_TipRepor.ReporPantalla :
				htmlString = creditosServicio.reporteSaldosTotalesCredito(creditosBean, nomReporte);
				break;
	...
	public ByteArrayOutputStream SaldosTotalesCreditoPDF(CreditosBean creditosBean, String nomReporte, HttpServletResponse response) {
		...
		try {
			htmlStringPDF = creditosServicio.reporteSaldosTotalesCreditoPDF(creditosBean, nomReporte);
	...
	public List SaldosTotalesCreditoExcel(int tipoLista, CreditosBean creditosBean, HttpServletResponse response) {
		...
		if (listaCreditos != null) {
			try {
				...
				celda.setCellValue(calendario.get(Calendar.HOUR_OF_DAY) + ":" + calendario.get(Calendar.MINUTE));
			    ...
			    celda.setCellValue(creditosBean.getDesGenero());
				...
				celda.setCellStyle(estiloNeg8);
				celda = fila.createCell((short) 11);
				celda.setCellValue(creditosBean.getDesClasificacion());
				...
				if(creditosBean.getEsNomina().equalsIgnoreCase("S")) {
					...
					celda.setCellStyle(estiloNeg8);
					celda = fila.createCell((short) 2);
					celda.setCellValue((!creditosBean.getNombreInstit().isEmpty()) ? creditosBean.getNombreInstit() : "TODAS");
					...
					celda.setCellStyle(estiloNeg8);
					celda = fila.createCell((short) 5);
					celda.setCellValue((!creditosBean.getDesConvenio().isEmpty()) ? creditosBean.getDesConvenio() : "TODOS");
					}
				}
				// Creacion de fila
				...
				celda = fila.createCell(numCelda++);
				celda.setCellValue("Fecha Desembolso");//5
				celda.setCellStyle(estiloNeg8);
				
				celda = fila.createCell(numCelda++);
				celda.setCellValue("Fecha de Próximo Pago");
				celda.setCellStyle(estiloNeg8);
				
				celda = fila.createCell(numCelda++);
				celda.setCellValue("Estatus de la Amortización");
				celda.setCellStyle(estiloNeg8);
				...
				celda = fila.createCell(numCelda++);
				celda.setCellValue("Capital Vigente Exigible");//20
				celda.setCellStyle(estiloNeg8);
				...
				celda = fila.createCell(numCelda++);
				celda.setCellValue("IVA notas de cargo Pag.");
				celda.setCellStyle(estiloNeg8);
				...
				for (iter = 0; iter < tamanioLista; iter++) {
					...
					celda = fila.createCell(numCelda++);
					celda.setCellValue(credito.getDesDestinoCredito());
					...
					celda = fila.createCell(numCelda++);
					celda.setCellValue(credito.getInstitFondeoID());
					...
					celda = fila.createCell(numCelda++);//64
					celda.setCellValue(Utileria.convierteDoble(credito.getGrupoID()));
				...
				//Creo la cabecera
				//response.addHeader("Content-Disposition","attachment; filename=ReporteSalTotalesCredito.xls");
				response.addHeader("Content-Disposition", "inline; filename=RepAnaliticoCartera-" + creditosBean.getFechaInicio() + ".xlsx");
	...
	public void setParametrosSisServicio(
