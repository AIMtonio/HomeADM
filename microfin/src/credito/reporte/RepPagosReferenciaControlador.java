...
public class RepPagosReferenciaControlador extends AbstractCommandController {
	...
	public RepPagosReferenciaControlador() {
	...
	protected ModelAndView handle(HttpServletRequest request, HttpServletResponse response, Object command, BindException errors) throws Exception {
		...
		case Enum_Con_TipRepor.ReporteExcel:
			List<PagosxReferenciaBean> listaReportes = pagosXReferenciaRep(tipoLista, creditosBean, response);
			break;
		}
	...
	public List<PagosxReferenciaBean> pagosXReferenciaRep(int tipoLista, CreditosBean creditosBean, HttpServletResponse response) {
		List<PagosxReferenciaBean> listaCreditos = null;
		try {
			...
			int regExport = 0;
			listaCreditos = creditosServicio.pagosXReferenciaRep(tipoLista, creditosBean, response);
			...
			estiloDatosCentrado.setFont(fontCentrado10);
			estiloDatosCentrado.setAlignment((short) CellStyle.ALIGN_CENTER);

			// Creacion de hoja
			Sheet hoja = null;
			hoja = libro.createSheet("ReportePagosReferencia");
			...
			celda.setCellValue("REPORTE DE PAGOS POR REFERENCIA DEL " + creditosBean.getFechaInicio() + " AL " + creditosBean.getFechaFinal());
			...
			celd = 0;
			celda = fila.createCell((short) celd++);
			celda.setCellValue("No. " + safilocale);
			...
			celda = fila.createCell((short) celd++);
			celda.setCellValue("Cuenta ID");
			celda.setCellStyle(estiloCentrado);

			celda = fila.createCell((short) celd++);
			celda.setCellValue("Saldo Cuenta");
			celda.setCellStyle(estiloCentrado);

			celda = fila.createCell((short) celd++);
			celda.setCellValue("Referencia Crédito");
			celda.setCellStyle(estiloCentrado);

			celda = fila.createCell((short) celd++);
			celda.setCellValue("Saldo Referencia");
			celda.setCellStyle(estiloCentrado);
			...
			if (listaCreditos != null) {
				...
				PagosxReferenciaBean credito = null;
				tamanioLista = tamanioLista - 1;
				for (iter = 0; iter < tamanioLista; iter++) {

					credito = (PagosxReferenciaBean) listaCreditos.get(iter);
					...
					celda = fila.createCell((short) celd++);
					celda.setCellValue(credito.getNombreProd());
					celda.setCellStyle(estilo10);

					celda = fila.createCell((short) celd++);
					celda.setCellValue(credito.getSucursal());
					celda.setCellStyle(estilo10);
					...
					celda = fila.createCell((short) celd++);
					celda.setCellValue(Utileria.convierteDoble(credito.getSaldoCtaAntesPag()));
					...
					celda = fila.createCell((short) celd++);
					celda.setCellValue(Utileria.convierteDoble(credito.getSaldoBloqRefere()));
					celda.setCellStyle(estiloFormatoDecimal);

					celda = fila.createCell((short) celd++);
					celda.setCellValue(credito.getFechaApli());
					celda.setCellStyle(estiloDatosCentrado);

					celda = fila.createCell((short) celd++);
					celda.setCellValue(Utileria.convierteDoble(credito.getCapitalPag()));
					celda.setCellStyle(estiloFormatoDecimal);

					celda = fila.createCell((short) celd++);
					celda.setCellValue(Utileria.convierteDoble(credito.getInteresPag()));
					celda.setCellStyle(estiloFormatoDecimal);

					celda = fila.createCell((short) celd++);
					celda.setCellValue(Utileria.convierteDoble(credito.getMoratoriosPag()));
					celda.setCellStyle(estiloFormatoDecimal);

					celda = fila.createCell((short) celd++);
					celda.setCellValue(Utileria.convierteDoble(credito.getComisionesPag()));
					celda.setCellStyle(estiloFormatoDecimal);

					celda = fila.createCell((short) celd++);
					celda.setCellValue(Utileria.convierteDoble(credito.getiVAPag()));
					celda.setCellStyle(estiloFormatoDecimal);

					celda = fila.createCell((short) celd++);
					celda.setCellValue(Utileria.convierteDoble(credito.getTotalPag()));
					celda.setCellStyle(estiloFormatoDecimal);
			...
			// Creo la cabecera
			response.addHeader("Content-Disposition", "inline; filename=ReportePagosReferencia.xlsx");
