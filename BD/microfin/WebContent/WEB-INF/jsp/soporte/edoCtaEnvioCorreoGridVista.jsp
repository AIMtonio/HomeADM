...
<script type="text/javascript" src="js/soporte/edoCtaEnvioCorreoGrid.js"></script>
...
<c:if test="${listaPaginada.nrOfElements > 0}">
	<div id="formaTabla">
		<fieldset class="ui-widget ui-widget-content ui-corner-all">
			<label for="seleccionTodosPagina">Seleccionar:</label>
			<input id="seleccionTodosPagina" name="seleccionTodosPagina" value="" tabindex="6" type="checkbox"/>
			<label for="seleccionTodosPagina">Todos p&aacute;g. actual</label>
			<br>
			<label for="seleccionTodos">Seleccionar:</label>
			<input id="seleccionTodos" name="seleccionTodos" value="" tabindex="7" type="checkbox"/>
			<label for="seleccionTodos">Todos</label>
			...
			<table id="miTabla" border="0" cellpadding="0" cellspacing="0" width="60%">
				<tr>
					...
					<td class="ui-widget ui-widget-header ui-corner-all" align="center"><label style="color: #ffffff">&nbsp;&nbsp;Seleccionar&nbsp;&nbsp;</label></td>
				...
				<c:forEach items="${listaResultado}" var="edoCtaEnvioCorreo" varStatus="status">
					<tr id="renglon${status.count}" name="renglon">
						...
						<c:if test="${edoCtaEnvioCorreo.correo != ''}">
							<c:set var="clienteCorreo" value="${clienteCorreo + 1}"/>
							<c:set var="indiceTab" value="${indiceTab + 1}"/>
							<td align="center">
								<input id="seleccion${status.count}" name="seleccion" value="${status.count}" tabindex="${indiceTab}" type="checkbox"/>
							</td>
						</c:if> 
						<c:if test="${edoCtaEnvioCorreo.correo == ''}">
							<td>
								<input size="20" type="text" value="Sin correo electrónico" readonly="readonly" style="text-align: center; font-size: smaller; border: none;" />
							</td>
						</c:if>
						<td align="center">
							<c:set var="indiceTab" value="${indiceTab + 1}"/>
