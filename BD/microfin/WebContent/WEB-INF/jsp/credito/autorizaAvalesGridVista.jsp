					...
					<c:forEach items="${listaResultado}" var="avales" varStatus="status">
						<tr id="renglon${status.count}" name="renglon">
							<td> 
						  	...
						  	<td> 
								<input id="avalID${status.count}" name="avalID" size="8" 
						  	...
						  	<input id="nombre${status.count}" name="nombre" size="50" 
										 value="${avales.nombre}" readOnly="true" /> </td> 
							<td>
...
function agregaNuevoDetalle(){
   	if(estaAutorizado=='S'){
		...
		tds += '<td><input id="avalID'+nuevaFila+'" name="avalID" size="8" value="" autocomplete="off" onKeyUp="listaAvales(\'avalID'+nuevaFila+'\');" onblur="consultaAvalGrid(this.id,2);"/></td> ';
		tds += '<td><input id="clienteID'+nuevaFila+'" name="clienteID" size="8"  value="" autocomplete="off" onKeyUp="listaClientes(\'clienteID'+nuevaFila+'\');" onblur="consultaAvalGrid(this.id,3);"/></td> ';
		tds += '<td><input id="prospectoID'+nuevaFila+'" name="prospectoID" size="8"  value="" autocomplete="off" onKeyUp="listaProspectos(\'prospectoID'+nuevaFila+'\');" onblur="consultaAvalGrid(this.id,4);"/></td> ';
