...
<tbody>
	...
	<c:forEach items="${diasInversion}" var="diaInversion" varStatus="status">
			...
			<td>
				...
				<c:when test="${status.count == 1}">
					<input type="text" size="5" name="plazoInferior"
							 ...
							 value="${diaInversion.plazoInferior}"/>
				</c:when>
				<c:otherwise>
					<input type="text" size="5" name="plazoInferior"
							 id="inferior${status.count}"
							 value="${diaInversion.plazoInferior}" readOnly="true" disabled="true" />
				</c:otherwise>
				</c:choose>
				<input type="text" size="5" name="plazoSuperior"
						 id="superior${status.count}"
						 value="${diaInversion.plazoSuperior}"
						 onblur="cambiaDiaSuperior(this)"
						 onkeypress="return validaDigitos(event)"/>
				<input type="button" name="elimina"
						 id="${status.count}"						 
						 class="btnElimina"
						 onclick="eliminaPlazo(this)"/>
			</td>
		</tr>
	</c:forEach>
</tbody>
...
<!-- 
<br>
<input type="button" name="creaDiasInv" id="creaDiasInv" value="Crea Arreglo"/>
<br>
<input type="button" name="validaDias" id="validaDias" value="validaDias"/>
<br>
 -->
</form>
</body>
...
<script type="text/javascript">
	...
	if($('#estatusTipoInver').val() == 'I'){
		deshabilitaBoton('agregaPlazo', 'submit');
	}else{
		habilitaBoton('agregaPlazo', 'submit');
	}
