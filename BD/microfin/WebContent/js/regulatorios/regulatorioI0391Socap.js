...
function agregarRegistro(){	
	...
	var tds = '<tr id="renglon' + nuevaFila + '" name="renglon">';
		...
		if(numeroFila == 0){
			...
			tds += '<td nowrap><input type="text" id="tasaInteres'+nuevaFila+'" maxlength="6" name="lTasaInteres"  style="text-align:right;" onBlur="evaluaTasa(this)" size="12" value=""/><label class="label">%</label></td>';
...
function evaluaTasa(campo){
	...
	if(tasa != "" && !isNaN(tasa)){
		...
		if(/^\d{1,3}(\.\d{1,2})?$/.test(tasa)){
			...
			$('#'+campo.id).val('');
			mensajeSis('Ingrese una Tasa Valida (Máximo 3 dígitos y 2 decimales)');
			$('#'+campo.id).focus();
		}
	}else{
		$('#'+campo.id).val('0');
