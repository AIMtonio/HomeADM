$(document).ready(function() {
	...
	$('#fechaOperacion').change(function() {
		...
		if(esFechaValida(Xfecha)){
			...
			if ( mayor(Xfecha, Yfecha) ){
				...
				consultaCredMayor55Años(this.id);
			}
		}
	});
	...
	// Funcion para generar en EXCEL de creditos otorgados a Socios Mayores de 55 Años
	function generaExcel() {	
		...
		$('#ligaGenerar').attr('href','credEdadMayor55Rep.htm?fechaOperacion='+fechaOperacion+'&tipoReporte='+tr+
				'&nombreInstitucion='+nombreInstitucion+'&nomUsuario='+nomUsuario);
	}
	
	// Función para consultar Créditos otorgados a Socios Mayores de 55 Años
	function consultaCredMayor55Años(idControl){
		var jqFecha  = eval("'#" + idControl + "'");
		var fecha = $(jqFecha).val();	 
		
		var conMayor55AñosBean  = { 
				'fechaOperacion':fecha
		};	
		setTimeout("$('#cajaLista').hide();", 200);
		if(fecha != ''){	
			bloquearPantallaCarga();
			creditosEdadMayor55Servicio.consulta(catTipoConsulta.mayor55Años,conMayor55AñosBean,function(mayor) {
			if(mayor!=null){
				$('#montoCarteraSocios').val(mayor.montoCarteraSocios);
				$('#saldoCarteraCredito').val(mayor.saldoCarteraCredito);
				$('#resultadoPorcentual').val(mayor.resultadoPorcentual);
				$('#parametroPorcentaje').val(mayor.parametroPorcentaje);
				$('#difLimiteEstablecido').val(mayor.difLimiteEstablecido);
				
				$('#saldoCartera').val(mayor.saldoCartera);
				$('#saldoTotalCartera').val(mayor.saldoTotalCartera);
				$('#resultadoPorcCredito').val(mayor.resultadoPorcCredito);
				$('#parametroPorcCredito').val(mayor.parametroPorcCredito);
				$('#difLimiteEstabCredito').val(mayor.difLimiteEstabCredito);
