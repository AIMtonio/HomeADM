...
var var_montoSolicitudBase = 0;// monto de la solciitud original (sin accesorios * Com. apertura, Seguro de vida)
...
$("#solicitudCreditoID").bind('keyup',function(e) {
	if (this.value.length >= 2) {
		...
		lista('solicitudCreditoID', '1', '18',camposLista, parametrosLista,'listaSolicitudCredito.htm');
	}
});
...
$("#creditoID").blur(function() {
	...
	if($('#creditoID').val() == '' && esTab==true ){
		...
		$('#creditoID').focus();
	}else if ($('#creditoID').val() != Constantes.CADENAVACIA && $('#creditoID').val() != Constantes.ENTEROCERO && !isNaN(this.value)) {
...
$('#montoSolici').blur(function() {
	if( esTab==true){
		...
		if ($('#montoSolici').asNumber() > 0) {
			deshabilitaControl('capitalMinis1');
			$('#capitalMinis1').val($('#montoSolici').val());
			$('#totalMinistra').val($('#montoSolici').val());
			$('#diferenciaMinistra').val(0);
		}
		/*FIN MINISTRACIONES*/
		...
		if ($('#montoSolici').val() != '' ) {
			...
			if ($('#productoCreditoID').val() == '') {
				...
				if(tipoLiquidacion == 'T'){
					...
					consultaTasaCredito($('#montoSolici').asNumber(), this.id);
...
$('input[name="tipoFondeo"]').change(function(event) {
	...
	var tipoGaranFIRA = $('#tipoGarantiaFIRAID').asNumber();
	if (tipoDeFondeo == Enum_TipoFondeo.RecursosPropios) {
		...
		if(tipoGaranFIRA == Enum_TipoGarantiaFIRA.Ninguna){
...
$("#productoCreditoID").blur(function()  {
	...
	if (isNaN(this.value) ) {
		...
		if (this.value != Constantes.CADENAVACIA && parseInt(this.value) > Constantes.ENTEROCERO) {
			...
			montoOriginalSolicitud =  totalAdeudo;
...
$("#plazoID").change(function() {
	if( this.value != Constantes.CADENAVACIA && parseFloat($("#montoSolici").val()) > Constantes.ENTEROCERO){
		...
		// La tasa de interes se calcula tomando el monto total de la solicitud de credito
			...
			if(cobraAccesorios=='S' && validaAccesorios(tipoConAccesorio.plazo)==false){
...
function validaTipoComision(){
	...
	if(formaCobroComision == "P"){ // PROGRAMADO
		consultaComisionAper();

	}
	if(formaCobroComApe == 'D'){ //DEDUCIDO
	...
	if(formaCobroComApe == 'F'){//FINANCIADO
		calculoComision();
	}
}
...
function consultaSolicitudCredito(campoID) {
	...
	if ( var_solicitudCreditoID != Constantes.CADENAVACIA && !isNaN(var_solicitudCreditoID)) {
		...
		solicitudCredServicio.consulta(var_numConsultaSolCredito.Con_RenovacionesAgro, bean, { async: false, callback: function(solicitud) {
				if (solicitud != null && solicitud.solicitudCreditoID != Constantes.ENTEROCERO) {
					...
					if (parseInt(solicitud.grupoID) > Constantes.ENTEROCERO ) {
						...
						montoComApeBase = solicitud.montoComApert;
						montoIvaComApeBase = solicitud.ivaComApert;
						var JqCreditoID = '#creditoID';
						var Credito = "$("+JqCreditoID+")";
						...
						if(solicitud.lineaCreditoID > 0){
							...
							if(manejaComGarantia == 'S'){
								...
								$('#montoComGarantia').val(solicitud.montoComGarantia);
					...
					if ($('#grupoID').val() != undefined || (solicitud.grupoID > 0 && solicitud.grupoID != "" && solicitud.grupoID != null)) {
						consultaMinistracionesGrupales('ocultar');
					} else {
						consultaMinistraciones('ocultar');
					}
					$('#CAT').formatCurrency({ positiveFormat : '%n', roundToDecimalPlace : 1 });
				} else {
					mensajeSis("La Solicitud de Credito No Existe o no es Agro.");
					inicializaVariablesGlabales();
					inicializaValoresPantalla();
					inicializaCamposPantalla();
					consultaSICParam();
...
function consultaCredito(evento) {
	...
	if (esTab==true){
		if ( var_creditoID != Constantes.CADENAVACIA && !isNaN(var_creditoID)) {
			creditosServicio.consulta(var_numConsultaCredito.agropecuarios,bean, {
				async: false, callback: function(credito) {
					if (credito != null) {
						if(credito.esAgropecuario == 'S'){
							if(credito.adeudoPasivo == 'S'){
								mensajeSis("El Crédito Pasivo presenta adeudo, por tal motivo NO puede ser renovado.\n Credito Pasivo: " + credito.pasivoID);
								$("#montoSolici").val(Constantes.DECIMALCERO);
								evento.focus();
								evento.select();
							}else{

								mostrarDatosFIRA(credito.tipoGarantiaFIRAID);
								$('#acreditadoIDFIRA').val(credito.acreditadoIDFIRA);
								$('#creditoIDFIRA').val(credito.creditoIDFIRA);

								asignaInstFondeoID('institFondeoID', credito.institFondeoID);

								$('#cadenaProductivaID').val(credito.cadenaProductivaID);
								$('#ramaFIRAID').val(credito.ramaFIRAID);
								$('#subramaFIRAID').val(credito.subramaFIRAID);
								$('#actividadFIRAID').val(credito.actividadFIRAID);
								$('#tipoGarantiaFIRAID').val(credito.tipoGarantiaFIRAID);
								$('#progEspecialFIRAID').val(credito.progEspecialFIRAID);
								...
								if(credito.estatusGarantiaFIRA == Constantes.APLICADAS ){
									mensajeSis("El Crédito " + credito.creditoID + " Tiene Garantías Aplicadas.<br>Favor de consultarla en el " + "<b><a onclick=\"$('#Contenedor').load('flujoIndivReestructuraAgro.htm',function(response, status, xhr){" + "if(status == 'error') {$('#Contenedor').html(response);}});consultaSesion();\">Flujo Individual de Reestructura Agro. <img src=\"images/external.png\"></a></b>");
									$("#montoSolici").val(Constantes.DECIMALCERO);
									evento.focus();
									evento.select();
								}else{
									...
									if(SolCredito == 0){
										creditosServicio.consulta(var_numConsultaCredito.adeudoTotal,bean, {
											async: false, callback: function(adeudoCredito) {
												if (adeudoCredito != null) {
													...
													var adeudoTotal	=(adeudoCredito.adeudoTotal);
													...
													adeudoTotal = adeudoTotal.replace(re, "");

													var MontoCreditoAnterior = 	(adeudoTotal);
										...
										consultaParametrosSistema();
										var montoTotalCredito = Number(parseFloat(credito.montoCredito))+Number(parseFloat(credito.totalInteres));
										var_montoPagado = Number(montoTotalCredito.toFixed(2))-$("#montoCredAnterior").asNumber();
										montoTotalCredito = montoTotalCredito.toFixed(2);
										var_montoPagado = parseFloat(montoTotalCredito)-$("#montoCredAnterior").asNumber();
										var_porcCapCubierto = (var_montoPagado * 100) / parseFloat(montoTotalCredito);
										var_porcCapCubierto = var_porcCapCubierto.toFixed(2);

										// Solo se pueden renovar creditos individuales
										if (parseInt(credito.grupoID) <= Constantes.ENTEROCERO ) {
											// Solo se puede renovar un credito vigente o vencido
											if(credito.estatus == Constantes.ESTATUSVIGENTE || credito.estatus == Constantes.ESTATUSVENCIDO){
												// Solo se puede renovar un credito que no haya sido reestructurado
												if(credito.tipoCredito == Constantes.CREDITONUEVO || credito.tipoCredito == Constantes.CREDITORENOVACION){
													// Solo se puede renovar un credito si ya fue cubierto el porcentaje de capital, segun el parametro en param. sistema
													if(var_porcCapCubierto >= var_capCubiertoRenova){
														// Solo se puede renovar un credito si esta en el rango de renovaciones permitidas, segun el parmetro en param. sistema
														if(credito.numRenovaciones <= var_numRenovacionPer){
															...
															mensajeSis("El Crédito No se Pueve Renovar. \n"
																+ " Renovaciones Permitidas: " + var_numRenovacionPer + " \n Renovaciones Aplicadas: " + credito.numRenovaciones);
															$("#montoSolici").val(Constantes.DECIMALCERO);
															evento.focus();
															evento.select();
														}
														...
														var montoTotalCredito = Number(parseFloat(credito.montoCredito))+Number(parseFloat(credito.totalInteres));
														var_montoPagado = Number(montoTotalCredito.toFixed(2)) - Number($("#montoCredAnterior").asNumber());

														var_porcCapCubierto = (var_montoPagado * 100) / Number(montoTotalCredito.toFixed(2));

														var	var_montoRequerido = Number(montoTotalCredito.toFixed(2)) * (var_capCubiertoRenova / 100);
														var var_montoPendiente = var_montoRequerido - var_montoPagado;
														mensajeSis("No se ha Cubierto el Porcentaje de Capital para Realizar Renovación de Crédito. \n"
																+ " Requerido: " + var_capCubiertoRenova + " % \n Monto por Pagar: $" + var_montoPendiente.toFixed(2));
														$("#montoSolici").val(Constantes.DECIMALCERO);
														...
														mensajeSis("No Puede Renovar un Crédito que ha Sido Reestructurado.");
														$("#montoSolici").val(Constantes.DECIMALCERO);
														evento.focus();
														evento.select();
													}
											}else{
												mensajeSis("El Cŕedito a Renovar debe estar Vigente o Vencido.");
												$("#montoSolici").val(Constantes.DECIMALCERO);
												evento.focus();
												evento.select();
											}
										}else{
											mensajeSis("No se Permite Renovación de Crédito Grupal.");
											$("#montoSolici").val(Constantes.DECIMALCERO);
											evento.focus();
											evento.select();
										}
										...
										creditosServicio.consulta(var_numConsultaCredito.adeudoTotal,bean, {
											async: false, callback: function(adeudoCredito) {
												if (adeudoCredito != null) {
													...
													$("#montoCredAnterior").val(MontoCreditoAnterior);
													$("#montoExigible").val(MontoCreditoAnterior);
								...
								if( credito.lineaCreditoID > 0 ){
									...
									$('#forPagComAdmon').val(credito.forPagComAdmon);
									...
									if(manejaComAdmon == 'S'){
										...
										switch(forCobComAdmon){
											case cat_ForCobComisiones.Disposicion:
												montoComisiones = $('#montoCredAnterior').asNumber();
											break;
									...
									var nuevoMonto = $('#montoCredAnterior').asNumber();
									if( credito.forPagComAdmon == cat_ForPagComisiones.Financiado ){
										nuevoMonto = parseFloat($('#montoPagComAdmon').asNumber()) + parseFloat($('#montoIvaPagComAdmon').asNumber()) + parseFloat($('#montoCredAnterior').asNumber());
									}
							...
							mensajeSis("El Crédito <b>" + credito.creditoID + "</b> No es Agropecuario.<br>El trámite debe realizarse en la pantalla <b>Solicitud Renovación de Crédito</b>:<br> " + "<b><a onclick=\"$('#Contenedor').load('solicitudCreditoRenovacion.htm',function(response, status, xhr){" + "if(status == 'error') {$('#Contenedor').html(response);}});consultaSesion();\"><img src=\"images/external.png\"></a></b>");
							$('#creditoID').focus();
						}
					}
					else {
			...
			productosCreditoServicio.consulta(var_numConsultaProductoCre.principal, bean, { async: false, callback: function(prodCred) {
				if (prodCred != null) {
					if(prodCred.esAgropecuario == 'S'){
						...
						mensajeSis("El Producto de Crédito debe ser Agropecuario");
						$('#productoCreditoID').focus();
	...
	function validaMontoAdmon(){
		...
		if ($('#montoSolici').val() != '' ) {
			...
			if ($('#productoCreditoID').val() == '') {
				...
				if( $('#lineaCreditoID').val() > 0 &&
					$('#estatus').val() == 'I' &&
					montoOriginalSolicitud > $('#saldoDisponible').asNumber() ){
					mensajeSis("El Monto solicitado es mayor al saldo disponible actual de la línea de crédito seleccionada.");
					$('#montoSolici').focus();
					$('#montoSolici').select();
				}
...
function consultaComboTipoPagoCapital(datos) {
	...
	if (datos != null) {
		...
		for ( var i = 0; i < tamanio; i++) {
			...
			if(tamanio == i+1 && $('#tipoCredito').val() == "O"){
				if(esLibres == 'L'){
					dwr.util.removeAllOptions('tipoPagoCapital');
					...
					$("#frecuenciaCap").val(var_Frecuencia.LIBRE).selected = true;
					$("#tipoPagoCapital").val(var_PagoCapital.LIBRES).selected = true;
					$("#numAmortInteres").val('1'); $("#numAmortizacion").val('1');
					deshabilitaControl('numAmortInteres');
					deshabilitaControl('numAmortizacion');
					validarEsquemaCobroSeguro();
					$('#calendIrregularCheck').attr('checked',true);
					...
					$('#productoCreditoID').focus();
					$('#productoCreditoID').select();
					mensajeSis('El Producto de Crédito NO tiene Tipo de Pago de Capital LIBRES');
				}
...
function consultaComisionAper() {
	...
	if (ProdCred != '' && !isNaN(ProdCred) && esTab) {
		productosCreditoServicio.consulta(1,ProdCredBeanCon,{ async: false, callback: function(prodCred) {
					if (prodCred != null) {
						esTab = true;
						formaCobroComApe = prodCred.formaComApertura;
						...
						if (prodCred.montoComXapert > 0) {
							if (prodCred.tipoComXapert == 'P') {
								...
								var montoSolicitudBase = $('#montoSolici').val();
...
function calculoComision() {
	...
	if($('#formaComApertura').val()=='FINANCIAMIENTO' || $('#formaComApertura').val()=='DEDUCCION'){
		montoComIvaSol = Number(montoSolicitado) + (parseFloat(montoComApeBase) + parseFloat(montoIvaComApeBase));
	...
	if ($('#calendIrregularCheck').is(':checked')) {
		...
		if (ejecutarLlamada == true) {
			...
			if ($.trim($('#frecuenciaCap').val()) != Constantes.CADENAVACIA) {
				if (tipoLista == 1) {
					if ($('#frecuenciaCap').val() == var_Frecuencia.MENSUAL
						|| $('#frecuenciaCap').val() == var_Frecuencia.BIMESTRAL
							|| $('#frecuenciaCap').val() == var_Frecuencia.TRIMESTRAL
								|| $('#frecuenciaCap').val() == var_Frecuencia.TETRAMESTRAL
									|| $('#frecuenciaCap').val() == var_Frecuencia.SEMESTRAL) {
					...
					if (tipoLista == var_Simulador.TasaFija_Iguales || tipoLista == var_Simulador.TasaFija_Libres || tipoLista == var_Simulador.NOTasaFija_Iguales
							|| tipoLista ==  var_Simulador.NOTasaFija_Libres || tipoLista == var_Simulador.interesMontoOriginal) {
						if ($.trim($('#frecuenciaCap').val()) != Constantes.CADENAVACIA) {
								...
								if ($('#frecuenciaCap').val() == var_Frecuencia.MENSUAL
											|| $('#frecuenciaCap').val() == var_Frecuencia.BIMESTRAL
												|| $('#frecuenciaCap').val() == var_Frecuencia.TRIMESTRAL
													|| $('#frecuenciaCap').val() == var_Frecuencia.TETRAMESTRAL
														|| $('#frecuenciaCap').val() == var_Frecuencia.SEMESTRAL
															|| $('#frecuenciaInt').val() == var_Frecuencia.MENSUAL
																|| $('#frecuenciaInt').val() == var_Frecuencia.BIMESTRAL
																	|| $('#frecuenciaInt').val() == var_Frecuencia.TRIMESTRAL
																		|| $('#frecuenciaInt').val() == var_Frecuencia.TETRAMESTRAL
																			|| $('#frecuenciaInt').val() == var_Frecuencia.SEMESTRAL) {
									...
									|| ($('#diaPagoInteres1').is(':checked') != true && $('#diaPagoInteres2').is(':checked') != true)) {
										...
										// si el dia de pago seleccionado es dia del mes
										if ($('#diaPagoCapital2').is(':checked') == true) {
											if ($.trim($('#diaMesCapital').val()) == Constantes.CADENAVACIA || $('#diaMesCapital').val() == null
													|| $('#diaMesCapital').val() == Constantes.ENTEROCERO) {
												mensajeSis("Especifique día del Mes para Capital.");
												datosCompletos = false;
											} else {
												if (($.trim($('#diaMesInteres').val()) == Constantes.CADENAVACIA || $('#diaMesInteres').val() == null || $('#diaMesInteres').val() == Constantes.ENTEROCERO)
...
function mostrarGridLibresDetalle() {
	...
	if (numeroFila == 0) {
		tds += '<td><input type="text" id="consecutivoID'
			+ nuevaFila
			+ '" name="consecutivoID" size="4" value="1" readonly="true" disabled="true"/></td>';
		tds += '<td align="center"><input type="text" id="fechaInicio' + nuevaFila
		+ '"  name="fechaInicio" size="15" value="'
		+ $('#fechaInicioAmor').val()
		+ '" readonly="true" disabled="true"/></td>';
		tds += '<td align="center"><input type="text" id="fechaVencim'
			+ nuevaFila
			+ '" name="fechaVencim" size="15" value="" onblur="comparaFechas(this.id)"  /></td>';
		tds += '<td align="center"><input type="text" id="fechaExigible'
			+ nuevaFila
			+ '" name="fechaExigible" size="15" value=" " readonly="true" disabled="true"/></td>';
		tds += '<td><input type="text" id="capital'
			+ nuevaFila
			+ '" name="capital" size="18" style="text-align: right;" value="" esMoneda="true"'
			+ '  onblur="agregarFormatoMoneda(this.id);calculaDiferenciaSimuladorLibre()"/></td>';
		tds += '<td><input type="text" id="interes'
			+ nuevaFila
			+ '" name="interes" size="18" style="text-align: right;" value="" esMoneda="true" readonly="true"  disabled="true"/></td>';
		tds += '<td><input type="text" id="ivaInteres'
			+ nuevaFila
			+ '" name="ivaInteres" size="18" style="text-align: right;"  value=" " esMoneda="true" readonly="true" disabled="true"/></td>';
		tds += '<td><input type="text" id="totalPago'
			+ nuevaFila
			+ '" name="totalPago" size="18" style="text-align: right;"  value=" " esMoneda="true" readonly="true" disabled="true"/></td>';
		tds += '<td><input type="text" id="saldoInsoluto'
			+ nuevaFila
			+ '" name="saldoInsoluto" size="18" style="text-align: right;"  value=" " esMoneda="true" readonly="true" disabled="true"/></td>';
		...
		var valor = parseInt(document.getElementById("consecutivoID"
				+ numeroFila + "").value) + 1;
		tds += '<td><input type="text" id="consecutivoID' + nuevaFila
		+ '" name="consecutivoID" size="4" value="' + valor
		+ '" autocomplete="off" readonly="true" disabled="true"/></td>';
		tds += '<td align="center"><input type="text" id="fechaInicio' + nuevaFila
		+ '"  name="fechaInicio" size="15" value="'
		+ $('#fechaVencim' + filaAnterior).val()
		+ '" readonly="true" disabled="true"/></td>';
		tds += '<td align="center"><input type="text" id="fechaVencim'
			+ nuevaFila
			+ '" name="fechaVencim" size="15" value="" onblur="comparaFechas(this.id)" /></td>';
	...
	tds += '<td nowrap="nowrap"><input type="button" name="elimina" id="'
		+ nuevaFila
		+ '" value="" class="btnElimina" onclick="eliminarAmortizacion(this)"/>';
	tds += '<input type="button" name="agrega" id="agrega'
		+ nuevaFila
		+ '" value="" class="btnAgrega" onclick="agregarNuevaAmortizacion()"/></td>';
	tds += '</tr>';
	tds +='<tr id="filaTotales">'
		+ '<td colspan="4" align="right"><label for="lblTotales">Totales: </label></td>'
		+ '<td>';
		+ '<input id="totalCap" name="totalCap"  size="18" readOnly="true" style="text-align: right;" value="'
		+ valorSumaCapital
		+ '"esMoneda="true"/>' + '</td>'
		+ '</tr>';
	tds +='</tr>';
	tds += '<tr id="trDiferenciaCapital" >'
		...
		+ '<input type="text" id="diferenciaCapital" name="diferenciaCapital" size="18" style="text-align: right;" value="'
		+ valorDiferencia
		+ '" esMoneda="true" readonly="true" disabled="true"/>' + '</td>'
		+ '<td colspan="5"></td>' + '</tr>';
...
function agregarNuevaAmortizacion() {
	...
	if (numeroFila == 0) {
		...
		tds += '<td><input type="text" id="consecutivoID'
			+ nuevaFila
			+ '" name="consecutivoID" size="4" value="1" readonly="true" disabled="true"/></td>';
		tds += '<td align="center"><input type="text" id="fechaInicio'
			+ nuevaFila + '"  name="fechaInicio" size="15" value="'
			+ $('#fechaInicioAmor').val()
			+ '" readonly="true" disabled="true"/></td>';
		tds += '<td align="center"><input type="text" id="fechaVencim'
			+ nuevaFila
		...
		tds += '<td><input type="text" id="saldoInsoluto'
			...
			tds += '<td><input type="text" id="consecutivoID'
				+ nuevaFila
				+ '" name="consecutivoID" size="4" value="'
				+ valor
				+ '" autocomplete="off" readonly="true" disabled="true" /></td>';
			tds += '<td align="center"><input type="text" id="fechaInicio'
				+ nuevaFila + '"  name="fechaInicio" size="15" value="'
...
if (numInstituto != '' && !isNaN(numInstituto)) {
	fondeoServicio.consulta(tipoCon, instFondeoBeanCon,  {
...
setTimeout("$('#cajaLista').hide();", 200);
if (cadenaProductivaID != '' && !isNaN(cadenaProductivaID)) {
	...
	errorHandler : function(message) {
		mensajeSis("Error en Consulta de la Cadena Productiva." + message);
	}
	});
}
}
...
if (cadenaProductivaID != '' && !isNaN(cadenaProductivaID) &&
	...
	errorHandler : function(message) {
		mensajeSis("Error en Consulta de la Rama FIRA." + message);
	}
	});
}
}
...
if (cadenaProductivaID != '' && !isNaN(cadenaProductivaID) && ramaFIRAID != '' && !isNaN(ramaFIRAID) && subRamaFIRAID != '' && !isNaN(subRamaFIRAID)) {
	...
	errorHandler : function(message) {
		mensajeSis("Error en Consulta de la SubRama FIRA." + message);
	}
	});
}
}
...
if (cadenaProductivaID != '' && !isNaN(cadenaProductivaID) &&
		ramaFIRAID != '' && !isNaN(ramaFIRAID) &&
		subRamaFIRAID != '' && !isNaN(subRamaFIRAID) &&
		actividadFIRAID != '' && !isNaN(actividadFIRAID)
		) {
	...
	errorHandler : function(message) {
		mensajeSis("Error en Consulta de la Actividad FIRA." + message);
	}
	});
}
}
...
function consultProgramaFira(){
	...
	if (progEspecialFIRAID != '') {
		...
		errorHandler : function(message) {
			mensajeSis("Error en Consulta de la Programa FIRA." + message);
		}
		});
	} else {
	...
	lineaFonServicio.consulta(1, lineaFondBeanCon, {
		callback : function(lineaFond) {
			if (lineaFond != null) {
				...
				instFondeo = $('#institutFondID').asNumber();
