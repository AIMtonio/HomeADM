...
var var_IvaSucursal = parametroBean.ivaSucursal;
...
var Enum_Creditos_Con = {
		'agropecuarios' : 29
};

var Enum_Creditos_Lis = {
		'agropecuarios' : 47
};
...
$(document).ready(function() {
	...
	$('#creditoID').bind('keyup', function(e) {
		lista('creditoID', '2', Enum_Creditos_Lis.agropecuarios, 'creditoID', $('#creditoID').val(), 'ListaCredito.htm');
	});
	...
	$('#solicitudCreditoID').bind('keyup', function(e) {
		if (this.value.length >= 0) {
			...
			lista('solicitudCreditoID', '1', enum_solicitud_lis.agropecuarios, camposLista, parametrosLista, 'listaSolicitudCredito.htm');
		}
	});

	$('#solicitudCreditoID').blur(function() {

		if (esTab && ($('#solicitudCreditoID').asNumber() > '0' && $('#creditoID').asNumber() <= 0)) {
			// alta de credito con solicitud
			consultaSolicitudCred(this.id);
			solicitudCreditoBase = $('#solicitudCreditoID').asNumber();
			if (tipoPagoSeg == 'A') {
		...
		} else if (esTab && ($('#creditoID').asNumber() <= 0 && $('#solicitudCreditoID').asNumber() <= 0)) {// se trata de un credito nuevo sin solicitud
			$('#solicitudCreditoID').focus();
			mensajeSis("No se Puede dar de Alta Un Crédito Sin Solicitud.");
		...
		if(esTab && solicitudCreditoBase != $('#solicitudCreditoID').asNumber()  && $('#creditoID').asNumber() != 0 ){
			$('#solicitudCreditoID').focus();
			$('#solicitudCreditoID').select();
			mensajeSis('El Crédito: ' + $('#creditoID').val() +' es consolidado Agro por tal motivo no puede realizarse la sustitución de la Solicitud de Crédito.');
		}
	});
	...
	function validaCredito(control) {
		...
		if (numCredito != '' && $('#creditoID').asNumber() != 0 && !isNaN(numCredito) && esTab) {
			...
			if ($('#creditoID').asNumber() == '0') {
				...
				creditosServicio.consulta(Enum_Creditos_Con.agropecuarios, creditoBeanCon,{ async:false,callback: function(credito) {
					if (credito != null) {
						if (credito.esAgropecuario == 'S') {
							agregaFormatoControles('formaGenerica');
							dwr.util.setValues(credito);
							consultaSolicitudCred(this.id);
							//Se consulta ministraciones
							consultaMinistraciones('A', credito.creditoID, credito.solicitudCreditoID, credito.esConsolidacionAgro);
							montoinicial = $("#montoCredito").asNumber();
							...
							consultaCalendarioPorProductoCredito(credito.producCreditoID, credito.tipoPagoCapital, credito.frecuenciaCap, credito.frecuenciaInt, credito.plazoID, credito.tipoDispersion);
							...
							consultProgramaFira();
							asignaTipoFondeo(credito.tipoFondeo); //aeuan T_11351



							 setTimeout(function(){
							 	$('#institFondeoID').val(credito.institFondeoID);
							 	if(credito.institFondeoID == 0){
							 			$('#descripFondeo').val('');
							 	}
							 }, 200);

							consultaFechaDesembolso(credito.esConsolidacionAgro, "C");
							mostrarCreditosConsolidados(credito.esConsolidacionAgro);
							solicitudCreditoBase = credito.solicitudCreditoID;
							...
							if(credito.lineaCreditoID > 0){
								...
								$('#montoComGarantia').val(credito.montoPagComGarantiaSim);
								...
								if($('#manejaComAdmon').val() == 'S'){
									...
									if( $('#pagaIVACte').val() == 'S' ){
										montoIVAPagComAdmon = $('#montoPagComAdmon').asNumber() * var_IvaSucursal;
									}
								...
								if($('#manejaComGarantia').val() == 'S'){
									...
									if( $('#pagaIVACte').val() == 'S' ){
										montoIvaComGarantia = $('#montoComGarantia').asNumber() * var_IvaSucursal;
									}
							...
							mensajeSis("El Crédito no es de Tipo Agropecuario.");
							limpiaCamposLinea();
							$('#creditoID').focus();
						}
						agregaFormatoControles('formaGenerica');
					} else {
						mensajeSis("No Existe el Credito o no es de Tipo Agropecuario.");
						deshabilitaBoton('modifica', 'submit');
						deshabilitaBoton('agrega', 'submit');
						limpiaCamposLinea();
						$('#creditoID').focus();
					...
					mensajeSis("Error al Consultar el Crédito."+ message);
					$('#creditoID').focus();
				}
				});
			}
	...
	function consultaCliente(idControl, consultarTasaCredito) {
		...
		if (numCliente != '' && !isNaN(numCliente) && numCliente != '0') {
			clienteServicio.consulta(tipConPrincipal, numCliente, { async : false,callback : function(cliente) {
				if (cliente != null) {
					...
					if ($('#clienteID').asNumber() > 0 && $('#producCreditoID').asNumber() > 0 && $('#calificaCliente').val() != '' && $('#montoCredito').asNumber() > 0 && ($('#cicloCliente').asNumber() > 0 || $('#cicloClienteGrupal').asNumber() > 0) && consultarTasaCredito == 'S') {
	...
	function validarEventoFrecuencia() {
		...
		switch ($('#tipoPagoCapital').val()) {
			...
			case "I":
				...
				if ($('#perIgual').val() != 'S') {
					habilitarCalendarioPagosInteres();
					if ($('#frecuenciaInt').val() == 'S' || $('#frecuenciaInt').val() == 'C' || $('#frecuenciaInt').val() == 'Q' || $('#frecuenciaInt').val() == 'A' || $('#frecuenciaInt').val() == 'D') {
	...
	function consultaSolicitudCred(idControl) {
		...
		if (numSolicitud != '' && !isNaN(numSolicitud) && esTab && numSolicitud != '0') {
			...
			consultaSICParam();
			$('#creditoID').val('');
			solicitudCredServicio.consulta(tipCon, SolicitudBeanCon, {
			async : false,
			callback : function(solicitudCred) {
				if (solicitudCred != null) {
						...
						if (solicitudCred.solicitudCreditoID > 0) {
							...
							if (status == 'I' || status == 'L' || status == 'C' || status == 'R') { /*=LIBERADA,  C=CANCELACION*/
								...
								if (status != 'I') {
									...
									switch (solicitudCred.tipoCredito) {
										case var_TipoCredito.NUEVO:
											$("#tipoCredito").val(var_TipoCredito.NUEVO);
											$("#tipoCreditoDes").val(var_TipoCredito.NUEVODES);
											break;
									...
									if (lineaCreSi == "1") {
										...
										consultaCalendarioPorProductoCredito(solicitudCred.productoCreditoID, solicitudCred.tipoPagoCapital, solicitudCred.frecuenciaCap, solicitudCred.frecuenciaInt, solicitudCred.plazoID, solicitudCred.tipoDispersion);
										...
										//$('#diaPagoProd').val(solicitudCred.diaPagoCapital );
										...
										if(solicitudCred.lineaCreditoID > 0){
											consultaCliente('clienteID', consultaTasa);

											$('#lineaCreditoID').val(solicitudCred.lineaCreditoID);
											consultaLineasCreditoAgro('lineaCreditoID');

											$('#manejaComAdmon').val(solicitudCred.manejaComAdmon);
											$('#forPagComAdmon').val(solicitudCred.forPagComAdmon);
											$('#porcentajeComAdmon').val(solicitudCred.porcentajeComAdmon);

											$('#manejaComGarantia').val(solicitudCred.manejaComGarantia);
											$('#porcentajeComGarantia').val(solicitudCred.porcentajeComGarantia);
											$('#forPagComGarantia').val(solicitudCred.forPagComGarantia);

											if($('#manejaComAdmon').val() == 'S'){
												$('#montoPagComAdmon').val(solicitudCred.montoPagComAdmon);
												$('#cobroComAdmon').show();
												if( solicitudCred.comAdmonLinPrevLiq == 'S' ) {
													$('#cobroComAdmon').hide();
												}
												var montoPagComAdmon = 0.00;
												var montoComisiones = 0.00;
												var montoIVAPagComAdmon = 0.00;
												switch(solicitudCred.forCobComAdmon){
													case "D":
														montoComisiones = $('#montoCredito').asNumber();
													break;
													case "T":
												...
												montoPagComAdmon = (montoComisiones * solicitudCred.porcentajeComAdmon)/100;
												montoPagComAdmon = montoPagComAdmon.toFixed(2);
												if( solicitudCred.forPagComAdmon != 'D' ){
													montoPagComAdmon = solicitudCred.montoPagComAdmon;
												}
											...
											if($('#manejaComGarantia').val() == 'S'){
												$('#montoComGarantia').val(solicitudCred.montoPagComGarantia);
												var montoIvaComGarantia = 0.00;
												if( $('#pagaIVACte').val() == 'S' ){
													montoIvaComGarantia = $('#montoComGarantia').asNumber() * var_IvaSucursal;
												}
											...
											if( solicitudCred.comGarLinPrevLiq == 'S' ) {
												$('#comGarLinPrevLiq').val("S");
												$('#comGarLinPrevLiq').attr("checked", true);
												$('#cobroComGarantia').hide();
												if($('#manejaComGarantia').val() == 'N'){
							...
							$('#cadenaProductivaID').val(solicitudCred.cadenaProductivaID);
							$('#ramaFIRAID').val(solicitudCred.ramaFIRAID);
							$('#subramaFIRAID').val(solicitudCred.subramaFIRAID);
							$('#actividadFIRAID').val(solicitudCred.actividadFIRAID);
							$('#tipoGarantiaFIRAID').val(solicitudCred.tipoGarantiaFIRAID);
							$('#progEspecialFIRAID').val(solicitudCred.progEspecialFIRAID);
							consultaCadenaProductiva();
							consultaRamaFIRA();
							consultaSubRamaFIRA();
							consultaActividadFIRA();
							...
							consultaDeudorOriginal(solicitudCred.deudorOriginal);
							consultaFechaDesembolso(solicitudCred.esConsolidacionAgro, "S");
							mostrarCreditosConsolidados(solicitudCred.esConsolidacionAgro);

							if(solicitudCreditoBase != $('#solicitudCreditoID').val()  && $('#creditoID').val() != 0 ){
								$('#solicitudCreditoID').focus();
								$('#solicitudCreditoID').select();
								mensajeSis('El Credito: ' + $('#creditoID').val() +' es consolidado Agro por tal motivo no puede realizarse la sustitucion de la Solicitud de Credito.');
							}
					...
					consultaMinistraciones(solicitudCred.estatus, 0, $('#solicitudCreditoID').val(),solicitudCred.esConsolidacionAgro);
	...
	function consultaDeudorOriginal(deudorOriginal) {
		...
		if (deudorOriginal != '' && !isNaN(deudorOriginal)) {
			clienteServicio.consulta(1, deudorOriginal, {
				async : false,
	...
	function consultaFechaDesembolso(esConsolidacionAgro, tipoOperacion){
		var numeroConsulta = 2;
		var operacionCredito = "C";

		if(tipoOperacion == operacionCredito){
			numeroConsulta = 3;
		}
			...
			consolidacionesServicio.consultaConsolidacion(numeroConsulta, consolidacionesBean, {
				...
				callback : function(consolidacionResponse) {
					...
					if (consolidacionResponse != null) {
						...
						$('#fechaPagoMinis1').val(consolidacionResponse.fechaDesembolso);
						$('#folioConsolidaID').val(consolidacionResponse.folioConsolidaID);
						...
						mostrarCreditosConsolidado();
						consultaDeudorOriginal(consolidacionResponse.deudorOriginalID);
						...
						if ($('#producCreditoID').val() != linea.productoCreditoID) {
							if ($('#solicitudCreditoID').asNumber() == 0 && $('#lineaCreditoID').asNumber() > 0) {
								if ($('#producCreditoID').val() != linea.productoCreditoID && $('#producCreditoID').val() != '') {
									...
									if ($('#clienteID').asNumber() > 0 && $('#producCreditoID').asNumber() > 0 && $('#calificaCliente').val() != '' && $('#montoCredito').asNumber() > 0 && ($('#cicloCliente').asNumber() > 0 || $('#cicloClienteGrupal').asNumber() > 0)) {
										esTab = true;
										consultaTasaCredito($('#montoCredito').asNumber());
									}
	...
	function consultaIVASucursal() {
		...
		if (numSucursal != '' && !isNaN(numSucursal) && esTab) {
			sucursalesServicio.consultaSucursal(catTipoConsultaCredito.principal, numSucursal, {
			async : false,
			...
			if (ejecutarLlamada == true) {
				...
				if ($.trim($('#frecuenciaCap').val()) != "") {
					if (tipoLista == 1) {
						...
						if ($('#frecuenciaCap').val() == 'M' || $('#frecuenciaCap').val() == 'B' || $('#frecuenciaCap').val() == 'T' || $('#frecuenciaCap').val() == 'R' || $('#frecuenciaCap').val() == 'S') {
						...
						if (tipoLista == 2 || tipoLista == 3 || tipoLista == 4 || tipoLista == 5 || tipoLista == 11) {
							if ($.trim($('#frecuenciaCap').val()) != "") {
								if ($.trim($('#frecuenciaInt').val()) != "") {
									if ($('#frecuenciaCap').val() == 'M' || $('#frecuenciaCap').val() == 'B' || $('#frecuenciaCap').val() == 'T' || $('#frecuenciaCap').val() == 'R' || $('#frecuenciaCap').val() == 'S' || $('#frecuenciaInt').val() == 'M' || $('#frecuenciaInt').val() == 'B' || $('#frecuenciaInt').val() == 'T' || $('#frecuenciaInt').val() == 'R' || $('#frecuenciaInt').val() == 'S') {
										...
										if (($('#diaPagoCapital1').is(':checked') != true && $('#diaPagoCapital2').is(':checked') != true) || ($('#diaPagoInteres1').is(':checked') != true && $('#diaPagoInteres2').is(':checked') != true)) {
											...
											if ($('#diaPagoCapital2').is(':checked') == true) {
												if ($.trim($('#diaMesCapital').val()) == '' || $('#diaMesCapital').val() == null || $('#diaMesCapital').val() == '0') {
													...
													if (($.trim($('#diaMesInteres').val()) == '' || $('#diaMesInteres').val() == null || $('#diaMesInteres').val() == '0') && $('#diaPagoInteres2').is(':checked') == true) {
	...
	function consultaTasaCredito(idControl) {
		...
		if (cicloCteGrup != 0 && producCredesGrup == esGrup && producCredPondeGrup == esGrup) {// If para
	...
	function consultaComboFrecuenciasCredito(frecuencia, valorCap, valorInt) {
		if (frecuencia != null) {
			// se eliminan los tipos de pago que se tenian
			$('#frecuenciaCap').each(function() {
				$('#frecuenciaCap option').remove();
	...
	function consultaComboTipoDispersionCredito(tiposDispersion, valor) {
		if (tiposDispersion != null) {
			// se eliminan los tipos de pago que se tenian
			$('#tipoDispersion').each(function() {
				$('#tipoDispersion option').remove();
	...
	function calPorceGaranLiquida(garanLiquida) {
		...
		calculosyOperacionesDosDecimalesMultiplicacion($("#montoCredito").asNumber(), (garanLiquida / 100));
	}
	...
	function consultaBeneficiario(idControl) {
		...
		if (Credito != '' && !isNaN(Credito) && esTab) {
			seguroVidaServicio.consulta(catTipoConsultaCredito.foranea, SeguroVidaBean, function(seguro) {
				if (seguro != null) {
	...
	function consultaBeneficiarioSolicitud(idControl) {
		...
		setTimeout("$('#cajaLista').hide();", 200);
		if (Credito != '' && !isNaN(Credito) && esTab) {
			seguroVidaServicio.consulta(2, SeguroVidaBean, function(seguro) {
				if (seguro != null) {
	...
	function consultacicloCliente() {
		...
		'productoCreditoID' : $('#producCreditoID').asNumber(),
...
function deshabilitaControlesNoInactivo() {
	...
	deshabilitaControl('progEspecialFIRAID');
...
function mostrarGridLibresDetalle() {
	...
	tds += '<tr id="trBtnCalcular" >' + '<td  id="btnCalcularLibre" colspan="10" align="right">' + '<input type="button" class="submit" id="calcular" tabindex="37"  onclick="simuladorLibresCapFec();" value="Calcular"/>' + '</td>' + '</tr>';
...
function agregaNuevaAmort() {
	...
	tds += '</tr>';
	tds += '<tr id="trDiferenciaCapital" >' + '<td colspan="4" align="right"><label for="Diferencia">Diferencia: </label></td>' + '<td  id="inputDiferenciaCap">' + '<input type="text" id="diferenciaCapital" name="diferenciaCapital" size="18" style="text-align: right;" value="' + valorDiferencia + '" esMoneda="true" readonly="true" disabled="true"/>' + '</td>' + '<td colspan="5"></td>' + '</tr>';
	tds += '<tr id="trBtnCalcular" >' + '<td  id="btnCalcularLibre" colspan="10" align="right">' + '<input type="button" class="submit" id="calcular" value="Calcular" tabindex="37"  onclick="simuladorLibresCapFec();" />' + '</td>' + '</tr>';
...
function consultaComboCalInteres() {
	...
	formTipoCalIntServicio.listaCombo(1, {
	async : false,
	callback : function(formTipoCalIntBean) {
...
function consultaMinistraciones(estatus, creditoID, solicitudID, esConsolidacionAgro) {
	$("#gridMinistraCredAgro").html("");
	var tipoListaMin = 3;
	...
	if(creditoID == undefined || creditoID == null || creditoID == 0 || isNaN(solicitudID)){
		creditoID = 0;
	}

	if(solicitudID == undefined || solicitudID == null || solicitudID == 0 || isNaN(solicitudID)){
		solicitudID = 0;
	}
	...
	$.post("ministraCredAgroGrid.htm", beanMinistraciones, function(data) {
		if (data.length > 0) {
			...
			$("#gridMinistraCredAgro").show();
			if( esConsolidacionAgro  == 'S' ){
				$("#gridMinistraCredAgro").hide();
				$('#fechaPagoMinis1').val($('#fechaDesembolso').val());
			}
			agregaFormatoControles('formaGenerica');
		} else {
			$("#gridMinistraCredAgro").html("");
			...
			async: false, callback: function(lineaCreditoBean){
				if(lineaCreditoBean != null){
					...
					if( $('#estatus').val() == 'I' ){
						...
						if( $('#fechaVencimiento').val() > fechaVencimientoLinea ){
							banderaBoton = banderaBoton + 1;
							mensajeSis("La fecha de vencimiento del Crédito excede a la fecha de vencimiento de la línea de Crédito.");
							$('#lineaCreditoID').focus();
							$('#lineaCreditoID').select();
						}
...
function mostrarCreditosConsolidado(folioConsolidaID){
	...
	$.post("listaConsolidacionesGridAgro.htm", consolidacionesBean,function(consolidacionesResponse) {
		...
		if (consolidacionesResponse.length > 1316) {
			agregaFormatoControles('formaGenerica');
...
function mostrarCreditosConsolidados( esConsolidacionAgro){
	$('#divCreditosConsolidados').hide();

	if( esConsolidacionAgro == "S" ){
		$('#divCreditosConsolidados').show();
