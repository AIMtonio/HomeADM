...
$(document).ready(function() {
	...
	$('#creditoID').bind('keyup', function(e){
		lista('creditoID', '1', '9', 'creditoID', $('#creditoID').val(), 'ListaCredito.htm');
	});

	// Validamos el monto de notas cargos
	$('#montoNotasCargo').blur(function(){
		if ($('#montoNotasCargo').asNumber()=='0'){
			$('#porceNotasCargos').val('0.00');
		}else{
			if($('#montoNotasCargo').asNumber() > $('#totalNotaCargo').asNumber()){
				mensajeSis("El monto a condonar es mayor al \n saldo deudor por notas de Cargo $ " + $('#totalNotaCargo').val());
				existeMontoCondonar = "2";
				$('#porceNotasCargos').val("0.00");
				$('#montoNotasCargo').val("");
				$('#montoNotasCargo').focus();
			}else{
				if($('#totalNotaCargo').asNumber()>0){
					porcentaje = ($('#montoNotasCargo').asNumber()/$('#totalNotaCargo').asNumber())*100;
					porcentaje = porcentaje.toFixed(4);
					$('#porceNotasCargos').val(porcentaje);
				if($('#montoNotasCargo').asNumber() > $('#montoNotaCargoPer').asNumber() || porcentaje > $('#porceNotasCargoPer').asNumber()){
	...
	$('#montoComisiones').blur(function(){

		if ($('#montoComisiones').asNumber()=='0'){
			$('#porceComisiones').val('0.00');
		}else{
		if($('#montoComisiones').asNumber()> ($('#totalComisi').asNumber() - $('#totalNotaCargo').asNumber())){
			mensajeSis("El monto a condonar es mayor al \n saldo deudor");
			existeMontoCondonar = "2";
					$('#porceComisiones').val("0.00");
			$('#montoComisiones').val("");
			...
			if(($('#totalComisi').asNumber()- $('#totalNotaCargo').asNumber()) > 0){
				porcentaje = ($('#montoComisiones').asNumber()/($('#totalComisi').asNumber() - $('#totalNotaCargo').asNumber()))*100;
				porcentaje = porcentaje.toFixed(4);
				$('#porceComisiones').val(porcentaje);
				if($('#montoComisiones').asNumber()>$('#montoComisionesPer').asNumber() ||
						porcentaje>$('#porceComisionesPer').asNumber()){
					mensajeSis("El monto a condonar es mayor a los límites establecidos.");
							$('#porceComisiones').val("0.00");
					$('#montoComisiones').val("");
					$('#montoComisiones').focus();
	...
	$('#montoMoratorios').blur(function(){
		if ($('#montoMoratorios').asNumber()=='0'){
			$('#porceMoratorios').val('0.00');
			 ...
			 if($('#montoMoratorios').asNumber()>0){
			 	if($('#totalComisi').asNumber()>0 && ($('#montoComisiones').asNumber() ==0 || $('#montoNotasCargo').asNumber() == 0 ) ){
			 		mensajeSisRetro({
						mensajeAlert : 'De acuerdo a la Prelación es necesario primero condonar Comisiciones y Accesorios o Notas Cargos , ¿Está seguro de continuar?',
	...
	$('#formaGenerica').validate({
		rules: {
			...
			montoComisiones: {	number: true,
								required: true},
			porceComisiones: {	number: true,
								required: true},
			...
			montoNotasCargos: {	number: true,
								required: true},
			porceNotasCargos: {	number: true,
								required: true}
		},
		messages: {
			creditoID: 'Especifique Número de Crédito',
			montoComisiones: {	number: 'Soló números',
								required: 'Indique Monto Comisiones y Accesorios'},
			porceComisiones: {	number: 'Soló números',
								required: 'Indique % Comisiones y Accesorios'},
			...
			montoNotasCargos: {	number: 'Soló números',
							required: 'Indique Monto Capital'},
			porceNotasCargos: {	number: 'Soló números',
							required: 'Indique % Capital'}
		}		
	});
   				...
   				if(credito!=null){
					...
					if((listaPersBloqBean.estaBloqueado == 'S' && consultaSPL.permiteOperacion == 'S') || listaPersBloqBean.estaBloqueado == 'N'){
	   		   			...
	   		   			if(credito.esAgropecuario != 'S'){
	   						...
	   						mensajeSis("El Crédito " + credito.creditoID + " es Agropecuario.<br>Favor de consultarla en " + "<b><a onclick=\"$('#Contenedor').load('creQuitasAgroVista.htm',function(response, status, xhr){" + "if(status == 'error') {$('#Contenedor').html(response);}});consultaSesion();\"> Condonacion de Cartera Agro. <img src=\"images/external.png\"></a></b>");
							inicializaForma('formaGenerica','creditoID');
							inicializarCampos();
							deshabilitarMontos();
	   						$('#creditoID').focus();
			...
			var creditoBeanCon = { 
  			...
  			creditosServicio.consulta(Con_PagoCred,creditoBeanCon,function(credito) {
  				if(credito!=null){ 
					...
					//$('#pagoExigible').val(credito.adeudoTotal);
					...
					consultaAdeudoTotal();
					agregaFormatoControles('formaGenerica');
					$('#totalNotaCargo').val(credito.saldoNotasCargos);
  				}else{
  					deshabilitaBoton('condonar', 'submit');
		...
		if(numProductoCredito != '' && !isNaN(numProductoCredito)){
			creLimiteQuitasServicio.consulta(conLimQuitasCre,creLimQuitasBean,function(creLimiteQuitas) {
				if(creLimiteQuitas!=null){			
					$('#montoComisionesPer').val(creLimiteQuitas.limMontoAccesorios);
					$('#porceComisionesPer').val(creLimiteQuitas.limPorcenAccesorios);
					...
					$('#montoNotaCargoPer').val(creLimiteQuitas.limMontoNotasCargos);
					$('#porceNotasCargoPer').val(creLimiteQuitas.limPorcenNotasCargos);
	...
	function validarMontosCondonar() {
		...
		if($('#adeudoTotal').asNumber() ==0){
			...
			if($('#montoComisiones').asNumber()>0 ||
					$('#montoMoratorios').asNumber()>0 ||
					$('#montoInteres').asNumber()>0 ||
					$('#montoCapital').asNumber()>0 ||
					$('#montoNotasCargo').asNumber()>0 ){
	...
	function inicializarCampos() {
		...
		$('#montoNotaCargoPer').val("0.00");

		$('#porceComisionesPer').val("0.00");
		$('#porceMoratoriosPer').val("0.00");
		$('#porceInteresPer').val("0.00");
		$('#porceCapitalPer').val("0.00");
		$('#porceNotasCargoPer').val("0.00");
	...
	function deshabilitarMontos(){
		...
		deshabilitaControl('montoNotasCargo');
	}
	...
	function habilitarMontos(){
		...
		habilitaControl('montoNotasCargo');
	}
