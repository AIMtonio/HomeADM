...
$(document).ready(function() {
	...
	mostrarElementoPorClase('datosNominaE',false);
	mostrarElementoPorClase('datosNominaC',false);
	$(':text').focus(function() {
		esTab = false;
	});
	...
	$('#atrasoFinal').blur('keydown', function() {
...
function generaExcel() {
	...
	if ($('#excel').is(':checked')) {
		...
		var desGenero	= $("#sexo option:selected").html();
		...
		var nombreInstit = $('#nombreInstit').val();
		var desConvenio = $('#desConvenio').val();
		var esNomina = $('#esNomina').val();
		var desClasificacion = $("#clasificacion option:selected").html();

		if (genero == '0') {
			genero = '';
		}

		$('#ligaGenerar').attr('href', 'RepSaldosTotalesCredito.htm?'+
			...
			'&esNomina=' + esNomina+
			'&desClasificacion=' + desClasificacion+
			'&desGenero=' + desGenero
...
function consultaProducCredito(idControl) {
	...
	if (ProdCred != '' && ProdCred != '0' &&  !isNaN(ProdCred) && esTab) {
		productosCreditoServicio.consulta(catTipoConsultaCredito.principal, ProdCredBeanCon, function(prodCred) {
			if (prodCred != null) {
				...
				if(prodCred.productoNomina=="S") {
					mostrarElementoPorClase('datosNominaE',true);
					if(manejaConvenio == "S"){
						mostrarElementoPorClase('datosNominaC',true);
					}else{
						mostrarElementoPorClase('datosNominaC',false);
					}
					
					$('#esNomina').val(prodCred.productoNomina);
...
function consultaConvenioNomina(convenioID) {
	...
	setTimeout("$('#cajaLista').hide();", 200);
	if($('#convenioNominaID').asNumber()>0){
