...
$(document).ready(function(){
		...
		if(numCredito != '' && !isNaN(numCredito)){
			...
			$('#gridAmortizacion').hide();
  			...
  			$('#impTicket').hide();
   			...
   			creditosServicio.consulta(24,creditoBeanCon,function(credito) {//catTipoConsultaCredito.saldos
				if(credito!=null){
					esTab=true;	
					...
					if (credito.grupoID>0){
  					...
  					if(credito.estatus != 'V' && credito.estatus != 'B'){
						mensajeSis("El Crédito debe estar Vigente o Vencido.");
						deshabilitaBoton('pagar','submit');
						deshabilitaBoton('movimientos','submit');
						deshabilitaBoton('amortiza','submit');
						$('#creditoID').focus();
        ...
        if(numCta != '' && !isNaN(numCta)){
       		cuentasAhoServicio.consultaCuentasAho(conCta,CuentaAhoBeanCon,function(cuenta) {
                	if(cuenta!=null){                 	               		
						...
						$('#montoPagar').val(cuenta.saldoDispon);            
	...
	function imprimeTicketPago(){
		var tituloOperacion=  'PAGO DE CREDITO VERTICAL CON CARGO A CUENTA';
		var nombreGrupo = '';
		var cicloGrupo = 0;
