...
$(document).ready(function() {
	...
	$('#producCreditoID').change(function() {
		...
		if (producCreditoID != "" && producCreditoID != null) {
	...
	function pegaHtml(prodCreID, validaVacio){
		...
		if(!isNaN(prodCreID)){
			...
			$.post("limQuitasGridDetalles.htm", params, function(data){
				if(data.length >0 ){
	...
	function validaMontosGrid(){
		...
		$('input[name=elimina]').each(function() {		
			...
			var jqlimMontoNotasCargos =  eval("'#limMontoNotasCargos" + this.id + "'");
			var jqlimPorcenNotasCargos  =  eval("'#limPorcenNotasCargos" + this.id + "'");

			var jqNumMaxCondona =  eval("'#numMaxCondona" + this.id + "'");
			...
			var limMontoNotasCargos =  $(jqlimMontoNotasCargos).asNumber();
			if(limMontoNotasCargos==0)$(jqlimMontoNotasCargos).val('0.00');
			var limPorcenNotasCargos =  $(jqlimPorcenNotasCargos).asNumber();
			if(limPorcenNotasCargos==0)$(jqlimPorcenNotasCargos).val('0.00');
			...
			// NOtas cargos
			if(existeError==0){
				if(limMontoNotasCargos > 0 && limPorcenNotasCargos <= 0 ){
					mensajeSis("El Porcentaje no puede ser igual a cero si existe un Monto mayor a cero.")
					$(jqlimPorcenNotasCargos).focus();
					existeError=1;
					return;
				}
			}
	...
	function agregaFormatoGridMontos(){
		...
		$('input[name=elimina]').each(function() {		
			...
			agregaFormato('limMontoNotasCargos'+this.id);
			agregaFormatoPorcentaje('limPorcenNotasCargos'+this.id);
		});
	}
...
function agregaNuevoDetalle(){
	if(verificarVacios()){
		...
		tds +=' <td><input type="text" id="clavePuestoID'+nuevaFila+'" 	 	name="clavePuestoIDLis" onkeyup="muestralistaPuestos(\'clavePuestoID'+nuevaFila+'\')" onblur="validaPuestos(\'clavePuestoID'+nuevaFila+'\',\'descriPuesto'+nuevaFila+'\' )" size="15" value="" maxlength="15"/> ';  
		tds +='  <input type="text" id="descriPuesto'+nuevaFila+'"			name="descriPuesto" disabled="true"  size="35"  value=""/> </td>';  

		tds +=' <td><input type="text" id="limMontoCap'+nuevaFila+'" 		name="limMontoCapLis" esMoneda="true" onkeypress="return Validador(event,  this);" style="text-align:right;"  path="limMontoCap" size="15" value="" maxlength="15" /> </td>';  
		...
		tds +=' <td><input type="text" id="limMontoIntere'+nuevaFila+'" 	name="limMontoIntereLis"  esMoneda="true" onkeypress="return Validador(event,  this);" style="text-align:right;"   path="limMontoIntere"  size="15" value="" maxlength="15"/> </td>';  
		...
		tds +=' <td><input type="text" id="limMontoMorato'+nuevaFila+'" 	name="limMontoMoratoLis"  esMoneda="true" onkeypress="return Validador(event,  this);" style="text-align:right;"  path="limMontoMorato"  size="15" value="" maxlength="15"/> </td>';  
		...
		tds +=' <td><input type="text" id="limMontoAccesorios'+nuevaFila+'" name="limMontoAccesoriosLis"  esMoneda="true" onkeypress="return Validador(event,  this);" style="text-align:right;"  path="limMontoAccesorios"  size="15" value="" maxlength="15" /> </td>';  
		...
		tds +=' <td><input type="text" id="limMontoNotasCargos'+nuevaFila+'" name="limMontoNotasCargosLis"  esMoneda="true" onkeypress="return Validador(event,  this);" style="text-align:right;"  path="limMontoNotasCargos"  size="15" value="" maxlength="15" /> </td>';  
		tds +=' <td><input type="text" id="limPorcenNotasCargos'+nuevaFila+'" name="limPorcenNotasCargosLis"  onBlur="validaPorcentaje(this);" onkeypress="return Validador(event,  this);" style="text-align:right;"  path="limPorcenNotasCargos"  size="7" value="" maxlength="7" /> </td>';  		 
		...
		agregaFormato('limMontoNotasCargos'+nuevaFila);
		agregaFormatoPorcentaje('limPorcenNotasCargos'+nuevaFila);
...
function agregaEtiquetasTabla(){
	...
	tds +=' <td  class="label"> <label for="lblnumCtaAhorro">Notas de <br> cargos <br> Monto</label>  </td>';  
	tds +=' <td  class="label"> <label for="lblnumCtaAhorro">%</label>  </td>';    		 
...
function eliminaDetalle(numeroID){		
	...
	var jqLimMontoNotasCargos	 = eval("'#limMontoNotasCargos" + numeroID + "'");
	var jqLimPorcenNotasCargos	 = eval("'#limPorcenNotasCargos" + numeroID + "'");
	...
	$(jqLimMontoNotasCargos).remove();
	$(jqLimPorcenNotasCargos).remove();
...
function verificarVacios(){	
	...
	for(var i = 1; i <= numRenglones; i++){
		...
		var limMontoNotasCargos = eval("'#limMontoNotasCargos" + i + "'");
		var limPorcenNotasCargos = eval("'#limPorcenNotasCargos" + i + "'");
		...
		if($(limMontoNotasCargos).val() == ''){
			$(limMontoNotasCargos).focus();
			exito = false;
			mensajeSis('Especificar Limite Monto de Notas de Cargos');
			break;
		}
		if($(limPorcenNotasCargos).val() == ''){
			$(limPorcenNotasCargos).focus();
			exito = false;
			mensajeSis('Especificar Limite Porcentaje de Notas de Cargos');
			break;
		}
