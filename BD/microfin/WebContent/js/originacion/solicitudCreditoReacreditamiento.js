...
var Constantes = {
	...
	"SALDOINSOLUTO"			: "1",
	"MONEDAPESOS"			: "1",
	"RENOVACIONCRE"			: "REACREDITACIÓN DE CREDITO",
	"CREDITONUEVO"			: "N",
	"CREDITORENOVACION"		: "O"
};
...
$("#solicitudCreditoID").bind('keyup',function(e) {
	if (this.value.length >= 2) {
		...
		lista('solicitudCreditoID', '1', '16',camposLista, parametrosLista,'listaSolicitudCredito.htm');
	}
});
...
$("#creditoID").bind('keyup', function(e){
	lista('creditoID', '2', '51', 'creditoID', $("#creditoID").val(), 'ListaCredito.htm');
});

$("#creditoID").blur(function() {
	if($('#creditoID').val() == '' && esTab==true ){
		mensajeSis("Especificar el Crédito a reacreditar");
		$('#creditoID').focus();
	}
...
$("#formaGenerica").validate({
	...
	messages : {
		...
		relacionado			: {
								required: 'Especifique Crédito a Reacreditar',
								min: 'Especifique Crédito a Reacreditar'
							}
...
function validaTipoComision(){
	...
	if(formaCobroComision == "P"){
		consultaComisionAper();

	}
	if(formaCobroComApe == 'D'){
...
function consultaSolicitudCredito(campoID) {
	...
	if ( var_solicitudCreditoID != Constantes.CADENAVACIA && !isNaN(var_solicitudCreditoID)) {
		...
		solicitudCredServicio.consulta(var_numConsultaSolCredito.principal, bean, { async: false, callback: function(solicitud) {
				if (solicitud != null && solicitud.solicitudCreditoID != Constantes.ENTEROCERO) {
					...
					if (solicitud.flujoOrigen == 'C') {
					    ...
					    $('#solicitudCreditoID').focus();
							...
							mensajeSis("No se Permite Solicitud de Crédito Grupal para Realizar una Reacreditación de Crédito.");
							inicializaVariablesGlabales();
							inicializaValoresPantalla();
							inicializaCamposPantalla();
							consultaSICParam();
...
function consultaCredito(evento) {
	...
	if ( var_creditoID != Constantes.CADENAVACIA && !isNaN(var_creditoID)) {
		creditosServicio.consulta(var_numConsultaCredito.creditoRenovar,bean, { async: false, callback: function(credito) {
			if (credito != null) {
				...
				if((listaPersBloqBean.estaBloqueado == 'S' && consultaSPL.permiteOperacion == 'S') || listaPersBloqBean.estaBloqueado == 'N'){
						...
						if(SolCredito == 0){
							...
							creditosServicio.consulta(var_numConsultaCredito.adeudoTotal,bean, { async: false, callback: function(adeudoCredito) {
								if (adeudoCredito != null) {
									...
									var saldoNotasCargo = (adeudoCredito.saldNotasCargo);
									...
									var re = /,/gi;
									 ...
									 saldoNotasCargo = saldoNotasCargo.replace(re, "");
	
									var MontoCreditoAnterior = 	(parseFloat(saldoCapVencido)+ parseFloat(saldoCapVenNoEx)+parseFloat(saldoCapVigent)+parseFloat(saldoCapAtrasad)+parseFloat(saldoNotasCargo));
	
									montoNotasCargo = saldoNotasCargo;
							...
							var_montoPagado = parseFloat(montoCapitalizado)-$("#montoCredAnterior").asNumber();
							...
							// Solo se pueden reacreditar creditos individuales
							if (parseInt(credito.grupoID) <= Constantes.ENTEROCERO ) {
								// Solo se puede reacreditar un credito vigente
								if(credito.estatus == Constantes.ESTATUSVIGENTE){
									// Solo se puede reacreditar un credito que no haya tenido tratamiento
									if(credito.tipoCredito == Constantes.CREDITONUEVO){
										// Solo se puede reacreditar un credito si ya fue cubierto el porcentaje de capital, segun el parametro en param. sistema
										if(var_porcCapCubiertos >= var_capCubiertoRenova){
											...
											if (credito.tipoConsultaSIC != "" && credito.tipoConsultaSIC != null) {
												...
												var	var_montoRequerido = parseFloat(montoCapitalizado) * (var_capCubiertoRenova / 100);
												var var_montoPendiente = var_montoRequerido - var_montoPagado;
												mensajeSis("No se ha Cubierto el Porcentaje de Capital para Realizar la Reacreditación del Crédito. \n"
											...
											mensajeSis("No Puede Reacreditar un Crédito que ya tuvo un Tratamiento.");
											$("#montoSolici").val(Constantes.DECIMALCERO);
											evento.focus();
											evento.select();
											$('#creditoID').val("");
									...
									mensajeSis("El Cŕedito a Reacreditar debe estar Vigente.");
									$("#montoSolici").val(Constantes.DECIMALCERO);
									evento.focus();
									evento.select();
									$('#creditoID').val("");
							...
							mensajeSis("No se Permite la Reacreditación de Crédito Grupal.");
							$("#montoSolici").val(Constantes.DECIMALCERO);
							evento.focus();
							evento.select();
							$('#creditoID').val("");
					 ...
					 // ******************************************************************************** //
						if(tipoConsulta == Constantes.NUEVASOLICITUDCREDITO){
							...
							if (prodCred.esGrupal == Constantes.SI ) {
								...
								mensajeSis("El Producto de Crédito No Permite Reacreditación.");
								var_productoIDBase = Constantes.ENTEROCERO;
								$(jqControl).focus();
								$(jqControl).select();
							}
...
function validaLimiteMontoSolicitado(controlID) {
	...
	if ($('#montoSolici').asNumber()< $("#montoCredAnterior").val() && esTab==true){
		...
		mensajeSis(" El Monto Solicitado debe ser Mayor o Igual al Adeudo del Credito a Recreditar. " + "$"+montoCredAnt);
		var totalAdeudo  = $("#montoCredAnterior").val();
		$("#montoSolici").val(totalAdeudo);
...
function validaLimiteCantidadSolicitadoParcial(controlID) {
	...
	if ($('#cantidadPagar').asNumber()< $("#montoExigible").val() && esTab==true){
		...
		mensajeSis(" La Cantidad a Pagar debe ser Mayor o Igual al Exigible del Crédito a Reacreditar. " + "$"+montoExig);
		var montoExigible = $('#montoExigible').val();
		$('#cantidadPagar').val(montoExigible);
		var_montoSolicitudBase = Constantes.ENTEROCERO;
		$('#cantidadPagar').focus();
...
function validaLimiteMontoSolicitadoParcial(controlID) {
	...
	if ($('#montoSolici').asNumber()< $("#montoExigible").val() && esTab==true){
		...
		mensajeSis(" El Monto Solicitado debe ser Mayor o Igual al Exigible del Credito a Reacreditar. " + "$"+montoCredAnt);
...
function consultaParametrosSistema() {
	...
	parametrosSisServicio.consulta(var_numConsultaParamSis.principal, bean, { async: false, callback: function(parametrosSisBean) {
		if (parametrosSisBean != null) {
			...
			var_capCubiertoRenova = parametrosSisBean.capitalCubiertoReac;
