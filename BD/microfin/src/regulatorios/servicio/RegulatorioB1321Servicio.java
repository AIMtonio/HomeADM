...
import regulatorios.dao.RegulatorioB1321DAO;
import regulatorios.servicio.RegulatorioInsServicio.Enum_Lis_TipoReporte;

public class RegulatorioB1321Servicio extends BaseServicio{
	
	RegulatorioB1321DAO regulatorioB1321DAO = null;
	public static Double DOUBLE_VACIO = 0.00;
	
	public RegulatorioB1321Servicio() {
		super();
	}
	
	/* ============ case para listas de reportes regulatorios ===============*/
	public List <RegulatorioB1321Bean>listaReporteRegulatorioB1321(int tipoLista, int tipoEntidad, RegulatorioB1321Bean reporteBean, HttpServletResponse response) throws IOException{
		List<RegulatorioB1321Bean> listaReportes=null;
		
		/*SOFIPO*/
		if(tipoEntidad == RegulatorioInsServicio.Enum_Lis_TiposInstitucion.sofipo){
			switch(tipoLista){
				case Enum_Lis_TipoReporte.excel:
					listaReportes = reporteRegulatorioB1321Excel(Enum_Lis_TipoReporte.excel, reporteBean, response); 
					break;
				case Enum_Lis_TipoReporte.csv:
					listaReportes = reporteRegulatorioB1321Csv(reporteBean, Enum_Lis_TipoReporte.csv,  response); 
					break;		
			}
		}
	...
	private List reporteRegulatorioB1321Csv(RegulatorioB1321Bean reporteBean,int tipoReporte,HttpServletResponse response){
		String nombreArchivo="";
		List listaBeans = regulatorioB1321DAO.reporteRegulatorioB1321Csv(reporteBean, tipoReporte);
		nombreArchivo="R13_B1321.csv"; 
		// se inicia seccion para pintar el archivo csv
		try{
			RegulatorioB1321Bean bean;
			BufferedWriter writer = new BufferedWriter(new FileWriter(nombreArchivo));
			if (!listaBeans.isEmpty()){
				for(int i=0; i < listaBeans.size(); i++){
					bean = (RegulatorioB1321Bean) listaBeans.get(i);
	 ...
	 * @return
	 */
	public List<RegulatorioB1321Bean> reporteRegulatorioB1321Excel(int tipoLista,RegulatorioB1321Bean reporteBean,  HttpServletResponse response) throws IOException
	{
		List<RegulatorioB1321Bean> listaRegulatorioB1321Bean = null;
		String mesEnLetras			= "";
		String anio					= "";
		String nombreArchivo 		= "";
		...
		nombreArchivo 	= "R13_A1321_"+mesEnLetras+"_"+anio; 
		
		listaRegulatorioB1321Bean = regulatorioB1321DAO.reporteRegulatorioB1321Excel(reporteBean,tipoLista);
		int contador = 1;
		
		if(listaRegulatorioB1321Bean != null){
			try {
				...
				/* AGREGAR GRUPOS DE COLUMNAS */
					...
					celda = fila.createCell((short)1);
					celda.setCellValue("Reporte B-1321 Balance General.");
					celda.setCellStyle(estiloDerecha);
					//funcion para unir celdas
					hoja.addMergedRegion(new CellRangeAddress(
					...
					int rowExcel=8;
					contador=8;
					RegulatorioB1321Bean bean = null;
					CellStyle estiloDinamico = null;
					for(int x = 0; x< listaRegulatorioB1321Bean.size() ; x++ ){
						
						if (x == 0){
							estiloDinamico = estiloUpper;
						}else						
						if(x == (listaRegulatorioB1321Bean.size()-1)){
							estiloDinamico = estiloBottom;
						}else{
							estiloDinamico = estilo8;
						}
						bean= (RegulatorioB1321Bean)listaRegulatorioB1321Bean.get(x);
		...
		return listaRegulatorioB1321Bean;
	}

	public RegulatorioB1321DAO getRegulatorioB1321DAO() {
		return regulatorioB1321DAO;
	}

	public void setRegulatorioB1321DAO(RegulatorioB1321DAO regulatorioB1321DAO) {
		this.regulatorioB1321DAO = regulatorioB1321DAO;
	}
