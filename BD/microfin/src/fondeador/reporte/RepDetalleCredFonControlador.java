...
public class RepDetalleCredFonControlador extends AbstractCommandController {
	...
	public RepDetalleCredFonControlador(){
	...
	protected ModelAndView handle(HttpServletRequest request,
		...
		case Enum_Con_TipRepor.ReporPDF:
			ByteArrayOutputStream htmlStringPDF = RepDetalleFonPDF(redesCuentoBean, nombreReporte, response);
		break;
			
		case Enum_Con_TipRepor.ReporExcel:		
			 List listaReportes = detalleFondeoExcel(tipoLista,redesCuentoBean,response);
		break;
...
//		return null;
	...
	// Reporte de detalle pdf 
		public ByteArrayOutputStream RepDetalleFonPDF(RedesCuentoBean redesCuentoBean, String nombreReporte, HttpServletResponse response){
			ByteArrayOutputStream htmlStringPDF = null;
			try {
				htmlStringPDF = redesCuentoServicio.reporteDetalleFonPDF(redesCuentoBean, nombreReporte);
				response.addHeader("Content-Disposition", "inline; filename=DetalleAsigCreditos.pdf");
		...
		// Reporte de saldos capital de credito en excel
				public List  detalleFondeoExcel(int tipoLista,RedesCuentoBean redesCuentoBean,  HttpServletResponse response){
			...
			//	if(listaCreditos != null){
					...
					// inicio usuario,fecha y hora
								...
								celdaUsu.setCellValue((!redesCuentoBean.getNombreUsuario().isEmpty())?redesCuentoBean.getNombreUsuario(): "TODOS");
...
//									fechaVar= creditoHora.getFecha();
					...
					for( iter=0; iter<tamanioLista; iter ++){
						//Fecha	ID Crédito	No.Cliente	NombreCliente	Id producto	Sucursal	Monto Credito
							...
							celda=fila.createCell((short)7);
							celda.setCellValue(Utileria.convierteDoble(credito.getSaldoCapital()));
							celda.setCellStyle(estiloFormatoDecimal);
