...
public class CancelacionInvRepControlador extends AbstractCommandController{
	...
	public CancelacionInvRepControlador() {
		setCommandClass(InversionBean.class);
		setCommandName("cancelacionInv");
	}
	...
	protected ModelAndView handle(HttpServletRequest request,
		...
		switch(tipoReporte){
			case Enum_Con_TipRepor.ReporPantalla:
				 htmlString = inversionServicio.reporteCancelacionDia(inversionBean, nombreReporte);
			break;
				
			case Enum_Con_TipRepor.ReporPDF:
				ByteArrayOutputStream htmlStringPDF = reporteCancelacionDiaPDF(inversionBean, nombreReporte, response);
			break;		
			
			case Enum_Con_TipRepor.ReporExcel:		
				 List<InversionBean>listaReportes = reporteCancelacionInversionDiaExcel(inversionBean,response);
			break;
		}
	...
	public ByteArrayOutputStream reporteCancelacionDiaPDF(InversionBean inversionBean, String nombreReporte, HttpServletResponse response){
		ByteArrayOutputStream htmlStringPDF = null;
		try {
			htmlStringPDF = inversionServicio.repCancelacionPDF(inversionBean, nombreReporte);
			response.addHeader("Content-Disposition","inline; filename=RepCancelacionInversiones.pdf");
	...
	public List <InversionBean> reporteCancelacionInversionDiaExcel(InversionBean inversionBean, HttpServletResponse response){

		List<InversionBean> listaInversiones = null;
		listaInversiones = inversionServicio.listaCancelacionInversionesExcel(inversionBean,  response); 	
		...
		if(listaInversiones != null){
			...
			hoja = (SXSSFSheet) libro.createSheet("Reporte Cancelaciones de Inversión.");
			...
			celda = fila.createCell((short)1);
			celda.setCellValue("REPORTE DE CANCELACIONES DEL DÍA " + inversionBean.getFechaInicio());
			celda.setCellStyle(estiloNegritaEncabezado);
			...
			/* Valor de Usuario*/
			celda = fila.createCell((short)16);
			celda.setCellValue("CANCELACIONES");
			celda.setCellStyle(estilo8);
			...
			String nombreArchivo = "Reporte Cancelaciones del Día";
			//Creo la cabecera
			...
			outputStream.close();
				...
				loggerSAFI.info(parametrosAuditoriaBean.getOrigenDatos()+"-"+"Error al crear el reporte Cancelaciones del dia: " + e.getMessage());
				e.printStackTrace();
			}
