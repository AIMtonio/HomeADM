...
public class VencimientoInvRepControlador extends AbstractCommandController{
	...
	public VencimientoInvRepControlador() {
	...
	protected ModelAndView handle(HttpServletRequest request,
		...
		switch(tipoReporte){
			case Enum_Con_TipRepor.ReporPantalla:
				 htmlString = inversionServicio.reporteVencimientosDia(inversionBean, nombreReporte);
			break;
				
			case Enum_Con_TipRepor.ReporPDF:
				ByteArrayOutputStream htmlStringPDF = reporteVencimientoDiaPDF(inversionBean, nombreReporte, response);
			break;
			case Enum_Con_TipRepor.ReporExcel:		
				 List<InversionBean>listaReportes = reporteVenciemientoInversionDiaExcel(inversionBean,response);
			break;
		}
	...
	public ByteArrayOutputStream reporteVencimientoDiaPDF(InversionBean inversionBean, String nombreReporte, HttpServletResponse response){
		ByteArrayOutputStream htmlStringPDF = null;
		try {
			htmlStringPDF = inversionServicio.repVencimientoPDF(inversionBean, nombreReporte);
			response.addHeader("Content-Disposition","inline; filename=RepVencimientoInversiones.pdf");
	...
	public List <InversionBean> reporteVenciemientoInversionDiaExcel(InversionBean inversionBean, HttpServletResponse response){

		List<InversionBean> listaInversiones = null;
		listaInversiones = inversionServicio.listaVencimientoInversionesExcel(inversionBean, response); 	
		...
		if(listaInversiones != null){
			...
			hoja = (SXSSFSheet) libro.createSheet("Reporte Vencimiento de Inversión.");
			...
			celda = fila.createCell((short)1);
			celda.setCellValue("REPORTE DE VENCIMIENTOS DEL DÍA " + inversionBean.getFechaVencimiento());
			celda.setCellStyle(estiloNegritaEncabezado);
			...
			/* Valor de Usuario*/
			celda = fila.createCell((short)16);
			celda.setCellValue("VENCIMIENTOS");
			celda.setCellStyle(estilo8);
			...
			celda.setCellStyle(estilo8);
			hoja.addMergedRegion(new CellRangeAddress(5, 5, 6, 7));
			...
			/* Valor de modenaID */
			String estatus=inversionBean.getEstatus();
			if(estatus.equals("")){
				estatus="TODOS";
			}
			if(estatus.equals("N")){
			...
			celda.setCellValue(estatus);
			celda.setCellStyle(estilo8);
			hoja.addMergedRegion(new CellRangeAddress(5, 5, 11, 13));
			...
			String nombreArchivo = "Reporte Vencimientos del Día";
			//Creo la cabecera
