...
public class CreLimiteQuitasFiraDAO extends BaseDAO{
	public CreLimiteQuitasFiraDAO() {
		super();
	}
	...
	public CreLimiteQuitasFiraBean consultaLimQuitasProCrePuesto(CreLimiteQuitasFiraBean creLimiteQuitasBean, int tipoConsulta) {
		CreLimiteQuitasFiraBean creLimiteQuitas = new CreLimiteQuitasFiraBean();
		try{
		...
		List matches= ((JdbcTemplate)conexionOrigenDatosBean.getOrigenDatosMapa().get(parametrosAuditoriaBean.getOrigenDatos())).query(query,parametros  ,new RowMapper() {
				public Object mapRow(ResultSet resultSet, int rowNum) throws SQLException {
					CreLimiteQuitasFiraBean creLimiteQuitasBeanConsulta = new CreLimiteQuitasFiraBean();
			...
			creLimiteQuitas = matches.size() > 0 ? (CreLimiteQuitasFiraBean) matches.get(0) : null;
		}catch(Exception e){
			e.printStackTrace();
	...
	//grama Limite Quitas
	public MensajeTransaccionBean altaLimiteQuitas(final CreLimiteQuitasFiraBean creLimiteQuitasFiraBean){
		...
		mensajeResultado = (MensajeTransaccionBean) ((TransactionTemplate)conexionOrigenDatosBean.getManejadorTransaccionesMapa().get(parametrosAuditoriaBean.getOrigenDatos())).execute(
			...
			public Object doInTransaction(TransactionStatus transaction) {
				...
				CreLimiteQuitasFiraBean creLimQuitasDeListaBean = null;
				MensajeTransaccionBean mensajeBean = new MensajeTransaccionBean();
				try {

					listaDetalleGrid = (ArrayList) detalleGrid(creLimiteQuitasFiraBean);
					if(creLimiteQuitasFiraBean.getProductosAplica()!=null){
						arregloProductos = creLimiteQuitasFiraBean.getProductosAplica().split("\\,");
					}
					String prodCreIDPrincipal= creLimiteQuitasFiraBean.getProducCreditoID();
					...
					if(!listaDetalleGrid.isEmpty()) {
						for(int i=0; i < listaDetalleGrid.size(); i++){
							creLimQuitasDeListaBean = (CreLimiteQuitasFiraBean) listaDetalleGrid.get(i);
							...
							if( arregloProductos[i]!=""){
								if(Integer.parseInt(arregloProductos[i])>=100 ){
									...
									for(int j=0; j < listaDetalleGrid.size(); j++){
										creLimQuitasDeListaBean = (CreLimiteQuitasFiraBean) listaDetalleGrid.get(j);
										mensajeBean = altaMovimientosLimQuitas(creLimQuitasDeListaBean, parametrosAuditoriaBean.getNumeroTransaccion(),
												arregloProductos[i]);
										if( mensajeBean.getNumero()!=0){
											throw new Exception(mensajeBean.getDescripcion()+" <br> No. Producto Crédito: "+arregloProductos[i]);
	...
	public MensajeTransaccionBean altaMovimientosLimQuitas(final CreLimiteQuitasFiraBean creLimiteQuitasBean, final long numeroTransaccion,
			...
			mensajeBean = (MensajeTransaccionBean) ((JdbcTemplate) conexionOrigenDatosBean.getOrigenDatosMapa().get(parametrosAuditoriaBean.getOrigenDatos())).execute(
							new CallableStatementCreator() {
								public CallableStatement createCallableStatement(Connection arg0) throws SQLException {
									...
									sentenciaStore.setDouble("Par_LimMontoNotasCargos", Constantes.DOUBLE_VACIO);
									sentenciaStore.setDouble("Par_LimPorcenNotasCargos",Constantes.DOUBLE_VACIO);
									sentenciaStore.setInt("Par_NumMaxCondona",Utileria.convierteEntero(creLimiteQuitasBean.getNumMaxCondona()));
	...
	/* recorrer el gird */
	public List detalleGrid(CreLimiteQuitasFiraBean creLimiteQuitasBean ){
		...
		CreLimiteQuitasFiraBean quitasBeanSalida  = null;
		...
		for(int i=0; i<tamanio; i++){
			quitasBeanSalida = new CreLimiteQuitasFiraBean();
	...
	public List consultaGridDetallesPuestos(CreLimiteQuitasFiraBean creLimiteQuitasBean, int tipoConsulta) {
	...
	public List conProdsCreAplica(CreLimiteQuitasFiraBean creLimiteQuitasBean, int tipoConsulta) {
		List detallesPuestos = null;;
		try{
			//Query con el Store Procedure
			String query = "call CRELIMITEQUITASCON(?,?,?, ?,?,?,?,?,?,?);";
