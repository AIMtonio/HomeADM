...
public class ReporteDocVigExpControlador extends AbstractCommandController {
	...
	public ReporteDocVigExpControlador() {
	...
	private List<ClienteArchivosBean> reporteExcel(ClienteArchivosBean bean, HttpServletRequest request, HttpServletResponse response, int tipoReporte) {
		List<ClienteArchivosBean> listaReporte = null;
		try {
			listaReporte = clienteArchivosServicio.listaArchivosReportePLD(Enum_Rep_Archivo.documentosExpPLD,bean);
			...
			// Creacion de hoja					
			hoja = libro.createSheet("Documentos Vigencia");
			...
			celda.setCellValue("REPORTE DE VIGENCIA DE DOCUMENTOS DEL "+bean.getFechaInicio()+" AL "+bean.getFechaFinal());
			...
			celdaSucur.setCellValue(bean.getSucursal()+" - "+bean.getSucursalDes());
			
			celdaSucur = fila.createCell(3);
			celdaSucur.setCellValue("Nivel Riesgo:");
			celdaSucur.setCellStyle(estiloNeg10Izq);
			celdaSucur = fila.createCell(4);
			celdaSucur.setCellStyle(estilo10);
			celdaSucur.setCellValue(bean.getNivelRiesgoDes());
			
			celdaSucur = fila.createCell(6);
			celdaSucur.setCellValue("Estatus:");
			celdaSucur.setCellStyle(estiloNeg10Izq);
			celdaSucur = fila.createCell(7);
			celdaSucur.setCellStyle(estilo10);
			celdaSucur.setCellValue(bean.getEstatusDes());
			...
			celda = fila.createCell(numCelda++);
			celda.setCellValue("Fecha Vigencia");
			hoja.addMergedRegion(new CellRangeAddress(6, 7, numCelda, numCelda));
			celda.setCellStyle(estiloCentrado);
			...
			if (listaReporte != null) {
				...
				ClienteArchivosBean sitiBean = null;
				for (int iter = 0; iter < tamanioLista; iter++) {
					sitiBean = (ClienteArchivosBean) listaReporte.get(iter);
					
					numCelda = 0;
					fila = hoja.createRow(i);
					celda = fila.createCell(numCelda++);
					celda.setCellValue(sitiBean.getSucursal()+"-"+sitiBean.getSucursalDes());
					...
					celda = fila.createCell(numCelda++);
					celda.setCellValue(sitiBean.getFechaRegistro());
					celda.setCellStyle(estilo8);
					
					celda = fila.createCell(numCelda++);
					celda.setCellValue(sitiBean.getFechaExpira());
					celda.setCellStyle(estilo8);
					...
					celda = fila.createCell(numCelda++);
					celda.setCellValue(sitiBean.getEstatus());
					celda.setCellStyle(estilo8);
			...
			//Creo la cabecera
			response.addHeader("Content-Disposition", "inline; filename=ReporteVigDocCompDomi.xls");
