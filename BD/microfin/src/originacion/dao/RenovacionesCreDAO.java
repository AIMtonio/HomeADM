...
public class RenovacionesCreDAO  extends BaseDAO {
	
	ParametrosSesionBean parametrosSesionBean;
	
	public RenovacionesCreDAO() {
		super();
	}
	
	@SuppressWarnings("rawtypes")
	public List listaRenovaciones( RepRenovacionesBean repRenovacionesBean){	
		List ListaResultado=null;
		try{
		String query = "call RENOVACIONESCREREP(?,?,?,?,?, ?,?,?,?,?, "
				+ 							   "?)";
		...
		loggerSAFI.info(parametrosAuditoriaBean.getOrigenDatos()+"-"+"call RENOVACIONESCREREP(  " + Arrays.toString(parametros) + ")");
		@SuppressWarnings("unchecked")
		List matches= ((JdbcTemplate)conexionOrigenDatosBean.getOrigenDatosMapa().get(parametrosAuditoriaBean.getOrigenDatos())).query(query,parametros  ,new RowMapper() {
					public Object mapRow(ResultSet resultSet, int rowNum) throws SQLException {
						RepRenovacionesBean repRenovacionesBean= new RepRenovacionesBean();
