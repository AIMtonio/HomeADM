...
public class CreditosEdadMayor55RepControlador extends AbstractCommandController{
	...
	public CreditosEdadMayor55RepControlador (){
		...
		int tipoReporte =(request.getParameter("tipoReporte")!=null)?
			...
			switch(tipoReporte){
				case Enum_Con_TipRepor.ReporteExcel:
					reporteCredEdadMayor55(tipoReporte,riesgosBean,response);
				break;
			}
			return null;
		}
	// Reporte de Créditos Otorgados a personas Mayor de 55 Años
	public List reporteCredEdadMayor55(int tipoReporte,UACIRiesgosBean riesgosBean,  HttpServletResponse response){
		List listaRepote=null;
		String nombreArchivo = "";
		nombreArchivo = "Créditos Edad Mayor 55 Años "+riesgosBean.getFechaOperacion(); 
		listaRepote = creditosEdadMayor55Servicio.listaReporteCredEdadMayor55(tipoReporte, riesgosBean, response); 
		...
		try {
			...
			// Creacion de hoja					
			HSSFSheet hoja = libro.createSheet("Créditos Edad Mayor 55 Años");
			...
			celdaInst.setCellStyle(estiloDatosCentrado);
			  ...
			  hoja.addMergedRegion(new CellRangeAddress(//funcion para unir celdas
				...
				celdaRep.setCellValue("REPORTE CRÉDITOS EDAD MAYOR 55 AÑOS: "+riesgosBean.getFechaOperacion());
				...
				for( itera=0; itera<1; itera ++){
					...
					montoCarteraSocios 		= riesgos.getMontoCarteraSocios();
				...
				celdaMonto.setCellValue("Monto de Cartera de Socio > 55 Años");
				celdaMonto.setCellStyle(estilo8);
				celdaMonto = fila.createCell((short)2);
				celdaMonto.setCellValue(Double.parseDouble(montoCarteraSocios));
				celdaMonto.setCellStyle(estiloFormatoDecimal);
				celdaMonto = fila.createCell((short)4);
				celdaMonto.setCellValue("Saldo de Cartera de Socio > 55 Años");
				celdaMonto.setCellStyle(estilo8);
				...
				celdaTotalCred.setCellValue(Double.parseDouble(saldoCarteraCredito));
