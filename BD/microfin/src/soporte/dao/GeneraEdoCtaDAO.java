...
import soporte.dao.EdoCtaClienteDAO.Enum_Con_EdoCta;
import soporte.dao.EdoCtaClienteDAO.Enum_Con_Pre;
...
public class GeneraEdoCtaDAO extends BaseDAO{

	public GeneraEdoCtaDAO(){
		super();
	}
	...
	public static interface Enum_Con_ParamGene {
		int proveedorTim = 35;
		int credProSW = 36;
	}
	...
	public MensajeTransaccionBean generaracionEdoCtaProveedor(final GeneraEdoCtaBean generaEdoCtaBean){
		loggerSAFI.info("GeneraEdoCtaDAO.generaracionEdoCtaProveedor");
		...
		try {
			...
			loggerSAFI.info("timbradoEdoCTA:" + parametrosSisBean.getTimbraEdoCta() + "/" + generaEdoCtaBean.getTimbraEdoCta());
	...
	public MensajeTransaccionBean generacionEdoCtaSmarterWeb (final GeneraEdoCtaBean generaEdoCtaBean) {
		...
		try {
			...
			if (generaEdoCtaBean.getRutaPDI() == null) {
				throw new Exception("La Ruta de PDI(Data Integration) no puede estar vacia");
			}
			// ---------------------- Necesarios para el EdoCta MultiBase -----------------------------------
			...
			generaEdoCtaRango = consultaRangoClientes(generaEdoCtaBean, Enum_Con_EdoCta.rango);
			generaEdoCtaRango.setSucursalInicio(generaEdoCtaBean.getSucursalInicio());
			generaEdoCtaRango.setSucursalFin(generaEdoCtaBean.getSucursalFin());
			int inicio = Integer.parseInt(generaEdoCtaRango.getClienteInicio());
			int fin = Integer.parseInt(generaEdoCtaRango.getClienteFin());

			// ---------------------- Necesarios para el EdoCta MultiBase -----------------------------------
			//Se parametriza el origenDatos para el EdoCta
			generaEdoCtaRango.setOrigenDatos( parametrosSesion.getOrigenDatos() );

			//Se parametriza el prefijo de la empresa y se le quitan espacios en blanco
			generaEdoCtaRango.setPrefijoEmpresa( generaEdoCtaBean.getPrefijoEmpresa() );

			//Se parametriza la ruta del SH para EdoCta.
			generaEdoCtaRango.setRutasSHs( rutasSH.getRutaExpPDF() );

			//Se parametriza la ruta del Data-integration
			generaEdoCtaRango.setRutaPDI( rutasSH.getRutaPDI() );
			// ---------------------- Necesarios para el EdoCta MultiBase -----------------------------------
			...
			for (int i= inicio; i<= fin; i++){
				...
				generaEdoCtaRango.setClienteInicio(String.valueOf(i));
				generaEdoCtaRango.setClienteFin(String.valueOf(aux));
	...
	public MensajeTransaccionBean timbradoRest(final GeneraEdoCtaBean generaEdoCtaBean, String urlServicio, String autentificacionCodif) throws Exception{
		...
		listaCLientesTimbrar = consultaDatosCliente(generaEdoCtaBean, Enum_Lis_EdoCta.principal);
		...
		while (clientesItera.hasNext()) {
			...
			if (!elemGeneraEdo.getCadenaCFDI().isEmpty() &&	elemGeneraEdo.getEstatus() != 2 ) {
				...
				String remplazarComillas = elemGeneraEdo.getCadenaCFDI().replace("\"", "\'");
				...
				if (!timbradoEdoCtaResponse.getCodigoRespuesta().equals(CODIGO_EXITO)) {
					...
					try {
						File archivo = new File(generaEdoCtaBean.getRutaXML()+generaEdoCtaBean.getFechaProceso()+"/"+Utileria.completaCerosIzquierda(elemGeneraEdo.getSucursalID(), 3) +"/"+ Utileria.completaCerosIzquierda(elemGeneraEdo.getClienteID(),10)+"-"+"1"+"-"+generaEdoCtaBean.getFechaProceso()+".xml" );
					...
					if (generaCBB) {
						...
						try {
							...
							out = new FileOutputStream(generaEdoCtaBean.getRutaCBB()+generaEdoCtaBean.getFechaProceso()+"/"+Utileria.completaCerosIzquierda(elemGeneraEdo.getSucursalID(), 3) +"/"+ Utileria.completaCerosIzquierda(elemGeneraEdo.getClienteID(),10)+"-"+"1"+"-"+generaEdoCtaBean.getFechaProceso()+".png");
					...
					//Actualiza registro
					String rutaXML = generaEdoCtaBean.getRutaXML()+generaEdoCtaBean.getFechaProceso()+"/"+Utileria.completaCerosIzquierda(elemGeneraEdo.getSucursalID(), 3) +"/"+ Utileria.completaCerosIzquierda(elemGeneraEdo.getClienteID(),10)+"-"+"1"+"-"+generaEdoCtaBean.getFechaProceso()+".xml";
	...
	public MensajeTransaccionBean generacionEdoCtaCrediclub(final GeneraEdoCtaBean generaEdoCtaBean) {
		loggerSAFI.info("GeneraEdoCtaDAO.generacionEdoCtaCrediclub");
		MensajeTransaccionBean mensaje = new MensajeTransaccionBean();
		try{
			//Valida que el usuario y password para conectar al PAC esten capturados
			ParametrosSisBean parametrosSisBean = new ParametrosSisBean();
	...
	public MensajeTransaccionBean realizaTimbrado(GeneraEdoCtaBean generaEdoCtaBean ){
		...
		mensajeClientes = consultaDatosCliente(generaEdoCtaBean, Enum_Lis_EdoCta.principal);
	...
	public MensajeTransaccionBean leerXML(GeneraEdoCtaBean generaEdoCtaBean) {
		...
		try {
			...
			while(itera.hasNext()){
				...
				// Se valida que tanto la Ruta CBBRet y XMLRey no esten vacias

				if (generaEdoIter.getRutaCBBRet() != null) {
					generaEdo.setRutaCBBRet(generaEdoIter.getRutaCBBRet());
				}
				if (generaEdoIter.getRutaXMLRet() != null) {
					generaEdo.setRutaXMLRet(generaEdoIter.getRutaXMLRet());
				}
	...
	public MensajeTransaccionBean generaEdoCtaUnico(GeneraEdoCtaBean generaEdoCtaBean){
		...
		//Ejecucion de Estado de Cuentas
		try{
			String[] command = {"/bin/bash", "ejec_edoctaxsucurs.sh", generaEdoCtaBean.getSucursalInicio(), generaEdoCtaBean.getSucursalFin(),
					generaEdoCtaBean.getClienteInicio(), generaEdoCtaBean.getClienteFin(), generaEdoCtaBean.getOrigenDatos(), generaEdoCtaBean.getPrefijoEmpresa(),
					generaEdoCtaBean.getRutasSHs(), generaEdoCtaBean.getRutaPDI() };
			...
			public Object doInTransaction(TransactionStatus transaction) {
				...
				try {
					...
					Object[] parametros = {
							...
							"GeneraEdoCtaDAO.modificaCliente",
			...
			public Object doInTransaction(TransactionStatus transaction) {
				...
				try {
					...
					Object[] parametros = {
							...
							parametrosAuditoriaBean.getFecha(),
							parametrosAuditoriaBean.getDireccionIP(),
							"GeneraEdoCtaDAO.modificaEstatusCliente",
							parametrosAuditoriaBean.getSucursal(),
							parametrosAuditoriaBean.getNumeroTransaccion()
							};
			...
			public Object doInTransaction(TransactionStatus transaction) {
				...
				try {
					...
					Object[] parametros = {
							...
							generaEdoCtaBean.getSucursalFin(),/*03*/
							Constantes.ENTERO_CERO,/*04*/
							...
							"GeneraEdoCtaDAO.consultaInfoCliente",/*10*/
	...
	public List<GeneraEdoCtaBean> consultaDatosCliente(GeneraEdoCtaBean generaEdoCtaBean, int tipoLista) {
		...
		Object[] parametros = {
								...
								generaEdoCtaBean.getSucursalFin(),
								Constantes.ENTERO_CERO,
								tipoLista,
			...
			public Object doInTransaction(TransactionStatus transaction) {
				...
				try {
						...
						new CallableStatementCreator() {
							...
							public Object doInCallableStatement(CallableStatement callableStatement) throws SQLException,
								...
								if(callableStatement.execute()){
									...
									mensajeTransaccion.setDescripcion(Constantes.MSG_ERROR + " .GeneraEdoCtaDAO.altaCadenaXml");
					...
					if(mensajeBean ==  null){
						...
						mensajeBean.setNumero(999);
						throw new Exception(Constantes.MSG_ERROR + " .GeneraEdoCtaDAO.altaCadenaXml");
	...
	/**
	 * Funcion para actualizar los estatus de los estados de cuenta del AnioMes en base a la tabla EDOCTADATOSCTE.
	 *
	 * @param generaEdoCta: Bean con los datos para actualizar el estatus.
	 * @return MensajeTransaccionBean
	 * @author jcardenas
	 */
	public MensajeTransaccionBean actualizaEstEdoCtaPorFecha(final GeneraEdoCtaBean generaEdoCta) {
		MensajeTransaccionBean mensaje = new MensajeTransaccionBean();
		transaccionDAO.generaNumeroTransaccion();
		mensaje = (MensajeTransaccionBean) ((TransactionTemplate)conexionOrigenDatosBean.getManejadorTransaccionesMapa().get(parametrosAuditoriaBean.getOrigenDatos())).execute(new TransactionCallback<Object>() {
			@SuppressWarnings("unchecked")
			public Object doInTransaction(TransactionStatus transaction) {
				...
				try {
					...
					mensajeBean = (MensajeTransaccionBean) ((JdbcTemplate) conexionOrigenDatosBean.getOrigenDatosMapa().get(parametrosAuditoriaBean.getOrigenDatos())).execute(
						new CallableStatementCreator() {
							public CallableStatement createCallableStatement(Connection arg0) throws SQLException {
								...
								sentenciaStore.setInt("Par_NumAct",Enum_Act_EdoCta.estEdoCtaXPer);
							...
							public Object doInCallableStatement(CallableStatement callableStatement) throws SQLException,
								...
								if(callableStatement.execute()){
									...
									mensajeTransaccion.setNumero(999);
									mensajeTransaccion.setDescripcion(Constantes.MSG_ERROR + " .GeneraEdoCtaDAO.actualizaEstEdoCtaPorFecha");
									mensajeTransaccion.setNombreControl(Constantes.STRING_VACIO);
									mensajeTransaccion.setConsecutivoString(Constantes.STRING_VACIO);
								}
					...
					if(mensajeBean ==  null){
						mensajeBean = new MensajeTransaccionBean();
						mensajeBean.setNumero(999);
						throw new Exception(Constantes.MSG_ERROR + " .GeneraEdoCtaDAO.actualizaEstEdoCtaPorFecha");
					}else if(mensajeBean.getNumero()!=0){
						throw new Exception(mensajeBean.getDescripcion());
					}
				} catch (Exception e) {
	...
	public MensajeTransaccionBean consultaInfoClienteCrediclub(final GeneraEdoCtaBean generaEdoCtaBean) {
		...
		//Ejecucion de Estado de Cuentas
		try{
			ProcessBuilder pb = new ProcessBuilder("/bin/bash", "./EdoctaSafi_SoloCadenaCFDI.sh", generaEdoCtaBean.getFechaProceso(), generaEdoCtaBean.getSucursalInicio(), generaEdoCtaBean.getSucursalInicio(), Constantes.ENTERO_CERO+"" );
	...
	public MensajeTransaccionBean realizaTimbradoCrediclub(final GeneraEdoCtaBean generaEdoCtaBean) {
		...
		try{
			...
			mensaje.setNumero(0);
			mensaje.setDescripcion("Timbrado de Estados de Cuenta ha Finalizado Correctamente");
		}catch(IllegalThreadStateException e){
			mensaje.setNumero(Integer.valueOf("001"));
			mensaje.setDescripcion("Error al Generar Cadenas CFDI del cliente solicitado");
		}catch(Exception e){
