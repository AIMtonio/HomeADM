...
-- HISPLDDETECPERS
DELIMITER ;
DROP TABLE IF EXISTS `HISPLDDETECPERS`;
DELIMITER $$

CREATE TABLE `HISPLDDETECPERS` (
  `IDPLDhist` bigint(20) NOT NULL COMMENT 'ID consecutivo.',
  ...
  PRIMARY KEY (`IDPLDhist`,`IDPLDDetectPers`),
  KEY `PLDDETECPERS_IDX_1` (`ClavePersonaInv`),
  KEY `PLDDETECPERS_IDX_2` (`ListaPLDID`),
  KEY `PLDDETECPERS_IDX_3` (`TipoListaID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='Tabla histórica de los Clientes encontrados en Listas Negras y Personas Bloqueadas.'$$
