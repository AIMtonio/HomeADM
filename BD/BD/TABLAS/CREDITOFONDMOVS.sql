...
-- CREDITOFONDMOVS
DELIMITER ;
DROP TABLE IF EXISTS `CREDITOFONDMOVS`;
DELIMITER $$


CREATE TABLE `CREDITOFONDMOVS` (
  ...
  `FechaOperacion` date DEFAULT NULL COMMENT 'Fecha Operacion ',
  `FechaAplicacion` date DEFAULT NULL COMMENT 'Fecha de Aplicacion',
  `TipoMovFonID` int(4) DEFAULT NULL COMMENT 'Tipo de Movimiento de Fondeo tabla TIPOSMOVSFONDEO',
  ...
  KEY `FondFechaAplica` (`CreditoFondeoID`,`FechaOperacion`),
  PRIMARY KEY (RegistroID)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='Detalle de Movimientos de Credito Fondeo'
