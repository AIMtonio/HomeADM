...
-- ---------------------------------------------------------------------------------
-- SPEIRECEPCIONESPENHIS
DELIMITER ;
DROP TABLE IF EXISTS SPEIRECEPCIONESPENHIS;
DELIMITER $$

CREATE TABLE SPEIRECEPCIONESPENHIS (
	...
	CONSTRAINT `fk_SPEIRECEPCIONESPENHIS_1` FOREIGN KEY (`TipoPagoID`) REFERENCES `TIPOSPAGOSPEI` (`TipoPagoID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  	CONSTRAINT `fk_SPEIRECEPCIONESPENHIS_2` FOREIGN KEY (`InstiRemitenteID`) REFERENCES `INSTITUCIONESSPEI` (`InstitucionID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  	CONSTRAINT `fk_SPEIRECEPCIONESPENHIS_3` FOREIGN KEY (`InstiReceptoraID`) REFERENCES `INSTITUCIONESSPEI` (`InstitucionID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  	CONSTRAINT `fk_SPEIRECEPCIONESPENHIS_4` FOREIGN KEY (`AreaEmiteID`) REFERENCES `AREASEMITESPEI` (`AreaEmiteID`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='His:Tabla historica para almacenar las recepciones de SPEI antes de ser efectuadas realmente.'$$
