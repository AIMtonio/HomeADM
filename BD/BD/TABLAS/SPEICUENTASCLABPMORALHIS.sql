...
-- ---------------------------------------------------------------------------------
-- SPEICUENTASCLABPMORALHIS
DELIMITER ;
DROP TABLE IF EXISTS SPEICUENTASCLABPMORALHIS;
DELIMITER $$

CREATE TABLE SPEICUENTASCLABPMORALHIS (
	...
	KEY `fk_SPEICUENTASCLABPMORALHIS_1` (ClienteID),
	CONSTRAINT `fk_SPEICUENTASCLABPMORALHIS_1` FOREIGN KEY (ClienteID) REFERENCES CLIENTES (ClienteID) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='His: Tabla historica de las cuentas clabes de personaS morales para STP'$$
