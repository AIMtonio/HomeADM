...
-- HISSOCIODEMOVIV
DELIMITER ;
DROP TABLE IF EXISTS `HISSOCIODEMOVIV`;DELIMITER $$

CREATE TABLE `HISSOCIODEMOVIV` (
  ...
  KEY `fk_HISSOCIODEMOVIV_1_idx` (`TipoViviendaID`),
  KEY `fk_HISSOCIODEMOVIV_2_idx` (`TipoMaterialID`),
  CONSTRAINT `fk_HISSOCIODEMOVIV_1` FOREIGN KEY (`TipoViviendaID`) REFERENCES `TIPOVIVIENDA` (`TipoViviendaID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `fk_HISSOCIODEMOVIV_2` FOREIGN KEY (`TipoMaterialID`) REFERENCES `TIPOMATERIALVIV` (`TipoMaterialID`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='Registro Historico de Datos de Vivienda'$$