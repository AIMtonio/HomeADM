...
-- ---------------------------------------------------------------------------------
-- SPEICUENTASCLABEPFISICA
DELIMITER ;
DROP TABLE IF EXISTS `SPEICUENTASCLABEPFISICA`;
DELIMITER $$


CREATE TABLE SPEICUENTASCLABEPFISICA (
	...
	CONSTRAINT `FK_SPEICUENTASCLABEPFISICA_1` FOREIGN KEY (`ClienteID`) REFERENCES `CLIENTES` (`ClienteID`),
	CONSTRAINT `FK_SPEICUENTASCLABEPFISICA_2` FOREIGN KEY (`EstadoID`) REFERENCES `ESTADOSREPUB` (`EstadoID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='TB: Tabla donde se almacenan las Cuentas Clabe para STP de personas fisicas.'$$
