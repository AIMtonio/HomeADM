...
-- CONSTANCIARETENCION
DELIMITER ;
DROP TABLE IF EXISTS `CONSTANCIARETENCION`;
DELIMITER $$


CREATE TABLE `CONSTANCIARETENCION` (
  ...
  KEY `INDEX_CONSTANCIARETENCION_1` (`ClienteID`),
  KEY `INDEX_CONSTANCIARETENCION_2` (`Anio`),
  KEY `INDEX_CONSTANCIARETENCION_3` (`Mes`),
  KEY `INDEX_CONSTANCIARETENCION_4` (`TipoInstrumentoID`),
  KEY `INDEX_CONSTANCIARETENCION_5` (`InstrumentoID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='Tabla para almacenar informacion de cuentas, inversiones y cedes para la constancia de retencion'$$