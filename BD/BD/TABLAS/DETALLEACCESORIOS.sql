...
-- DETALLEACCESORIOS
DELIMITER ;
DROP TABLE IF EXISTS `DETALLEACCESORIOS`;
DELIMITER $$


CREATE TABLE `DETALLEACCESORIOS` (
  ...
  `TipoFormaCobro` char(1) DEFAULT '' COMMENT 'Indica la forma de Cobro del Accesorio\nA: Anticipado\nD: Deduccion\nF: Financiamiento',
  `TipoPago` char(1) DEFAULT '' COMMENT 'Tipo de Pago cuando la forma de cobro sea FINANCIAMIENTO\nM: Monto Fijo\nP: Porcentaje',
  `BaseCalculo` char(1) DEFAULT '' COMMENT 'Base de Calculo cuando la forma de cobro sea FINANCIAMIENTO y el tipo de pago sea PORCENTAJE\n1: Indica que es sobre el monto original del credito',
  ...
  `NumProyInteres` INT(11) NOT NULL DEFAULT '0' COMMENT 'Numero de Veces que se ha Realizado la Proyeccion de Intereses\nen un Pago Anticipado',
  ...
  PRIMARY KEY (`CreditoID`,`SolicitudCreditoID`,`NumTransacSim`,`AccesorioID`,`AmortizacionID`),
  INDEX `INDEX_DETALLEACCESORIOS_1` (`NumTransacSim`)
