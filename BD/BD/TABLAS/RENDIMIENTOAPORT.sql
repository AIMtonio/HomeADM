...
-- RENDIMIENTOAPORT
DELIMITER ;
DROP TABLE IF EXISTS `RENDIMIENTOAPORT`;DELIMITER $$

CREATE TABLE `RENDIMIENTOAPORT` (
  ...
  `TipoAportacionID` int(11) DEFAULT NULL COMMENT 'Tipo Aportación',
  ...
  KEY `fk_RENDIMIENTOAPORT_2_idx` (`TipoAportacionID`),
  CONSTRAINT `fk_RENDIMIENTOAPORT_1` FOREIGN KEY (`AportacionID`) REFERENCES `APORTACIONES` (`AportacionID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `fk_RENDIMIENTOAPORT_2` FOREIGN KEY (`TipoAportacionID`) REFERENCES `TIPOSAPORTACIONES` (`TipoAportacionID`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='Tab: Rendimiento Diario de Aportaciones.'$$