...
-- CATTIPOEGRESOS
DELIMITER ;
DROP TABLE IF EXISTS `CATTIPOEGRESOS`;DELIMITER $$

CREATE TABLE `CATTIPOEGRESOS` (
  `Numero` int(11) NOT NULL COMMENT 'Consecutivo',
  `Tipo` char(1) NOT NULL COMMENT 'Tipo \nE: Egreso\n',
  `Descripcion` varchar(100) NOT NULL COMMENT 'Descripcion del Engreso',
  `Estatus` char(1) DEFAULT NULL COMMENT 'Estatus del Engreso\nA: Activo\nI: Inactivo',
