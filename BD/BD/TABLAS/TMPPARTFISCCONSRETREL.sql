-- Creacion de tabla TMPPARTFISCCONSRETREL

DELIMITER ;

DROP TABLE IF EXISTS TMPPARTFISCCONSRETREL;

DELIMITER $$

CREATE TABLE `TMPPARTFISCCONSRETREL` (
  ...
  KEY `INDEX_TMPPARTFISCCONSRETREL_1` (`ConstanciaRetID`),
  KEY `INDEX_TMPPARTFISCCONSRETREL_2` (`Anio`),
  KEY `INDEX_TMPPARTFISCCONSRETREL_3` (`ClienteID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='Tmp: Tabla temporal para el Registro de Aportantes que se encuentra como Persona Relacionada de Otros Aportantes'$$
