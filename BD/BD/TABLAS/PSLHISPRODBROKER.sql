...
-- PSLHISPRODBROKER
DELIMITER ;
DROP TABLE IF EXISTS `PSLHISPRODBROKER`;DELIMITER $$

CREATE TABLE `PSLHISPRODBROKER` (
  `FechaPaseHis` datetime NOT NULL COMMENT 'Fecha en la que se paso el producto del catalogo al historico',
  ...
  `TipoServicio` int(11) NOT NULL COMMENT 'ID del Tipo de servicio',
  ...
  KEY `INDEX_PSLHISPRODBROKER_1` (`ProductoID`),
  KEY `INDEX_PSLHISPRODBROKER_2` (`ServicioID`),
  KEY `INDEX_PSLHISPRODBROKER_3` (`FechaCatalogo`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='His: Tabla para almacenar el historico de productos del Broker'$$