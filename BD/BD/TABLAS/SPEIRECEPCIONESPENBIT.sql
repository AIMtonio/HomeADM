...
-- ---------------------------------------------------------------------------------
-- SPEIRECEPCIONESPENBIT
DELIMITER ;
DROP TABLE IF EXISTS SPEIRECEPCIONESPENBIT;
DELIMITER $$

CREATE TABLE SPEIRECEPCIONESPENBIT (
	SpeiRecepcionPenID 			BIGINT(20) 		NOT NULL COMMENT 'Identificador de la tabla de Recepciones pendientes.',
	...
	CodigoError 				VARCHAR(30)		NOT NULL COMMENT 'Codigo de Error por el cual no se pudo concretar la recepcion.',
	MensajeError 				VARCHAR(400)	NOT NULL COMMENT 'Motivo por el cual no se pudo concretar la recepcion.',
	...
	CONSTRAINT `fk_SPEIRECEPCIONESPENBIT_1` FOREIGN KEY (`TipoPagoID`) REFERENCES `TIPOSPAGOSPEI` (`TipoPagoID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  	CONSTRAINT `fk_SPEIRECEPCIONESPENBIT_2` FOREIGN KEY (`InstiRemitenteID`) REFERENCES `INSTITUCIONESSPEI` (`InstitucionID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  	CONSTRAINT `fk_SPEIRECEPCIONESPENBIT_3` FOREIGN KEY (`InstiReceptoraID`) REFERENCES `INSTITUCIONESSPEI` (`InstitucionID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  	CONSTRAINT `fk_SPEIRECEPCIONESPENBIT_4` FOREIGN KEY (`AreaEmiteID`) REFERENCES `AREASEMITESPEI` (`AreaEmiteID`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='Tab:Tabla para almacenar las recepciones pendientes de SPEI que no se pudieron concretar.'$$
