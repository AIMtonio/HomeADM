-- Creacion de tabla TMPCONSRETRELFISCAL

DELIMITER ;

DROP TABLE IF EXISTS TMPCONSRETRELFISCAL;

DELIMITER $$

CREATE TABLE `TMPCONSRETRELFISCAL` (
  ...
  KEY `INDEX_TMPCONSRETRELFISCAL_1` (`ConstanciaRetID`),
  KEY `INDEX_TMPCONSRETRELFISCAL_2` (`Anio`),
  KEY `INDEX_TMPCONSRETRELFISCAL_3` (`ClienteID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='Tmp: Tabla temporal para el Registro de Relacionados Fiscales para Generar Constancia de Retencion'$$
