-- Creacion de tabla TMPPARTFISCCONSRETCTE

DELIMITER ;

DROP TABLE IF EXISTS TMPPARTFISCCONSRETCTE;

DELIMITER $$

CREATE TABLE `TMPPARTFISCCONSRETCTE` (
  ...
  KEY `INDEX_TMPPARTFISCCONSRETCTE_1` (`ConstanciaRetID`),
  KEY `INDEX_TMPPARTFISCCONSRETCTE_2` (`Anio`),
  KEY `INDEX_TMPPARTFISCCONSRETCTE_3` (`ClienteID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='Tmp: Tabla temporal para el Registro de Aportantes y sus Personas Relacionadas'$$
