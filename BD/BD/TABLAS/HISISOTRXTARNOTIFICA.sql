...
-- ISOTRXTARNOTIFICA
DELIMITER ;
DROP TABLE IF EXISTS `HISISOTRXTARNOTIFICA`;

DELIMITER $$
CREATE TABLE `HISISOTRXTARNOTIFICA` (
	HistoricoID				BIGINT(20)  	NOT NULL AUTO_INCREMENT COMMENT 'ID de Tabla',
	...
	KEY `IDX_HISISOTRXTARNOTIFICA_2` (`TarjetaID`),
	KEY `IDX_HISISOTRXTARNOTIFICA_3` (`CuentaAhoID`),
	KEY `IDX_HISISOTRXTARNOTIFICA_4` (`OperacionPeticionID`),
	KEY `IDX_HISISOTRXTARNOTIFICA_5` (`PIDTarea`),
	CONSTRAINT `FK_HISISOTRXTARNOTIFICA_1` FOREIGN KEY (`CuentaAhoID`) REFERENCES `CUENTASAHO` (`CuentaAhoID`),
	CONSTRAINT `FK_HISISOTRXTARNOTIFICA_2` FOREIGN KEY (`OperacionPeticionID`) REFERENCES `RELOPERAPETICIONISOTRX` (`OperacionPeticionID`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COMMENT='Tb: Tabla de Histórica Notificación para la Tarea: TAREA_ISOTRX_NOTIFICASALDO.'$$