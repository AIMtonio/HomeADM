...
-- FUNCIONDEUCRENOIVA
DELIMITER ;
DROP FUNCTION IF EXISTS `FUNCIONDEUCRENOIVA`;
DELIMITER $$


CREATE FUNCTION `FUNCIONDEUCRENOIVA`(
    /* DEVUELVE EL MONTO TOTAL DE CREDITO SIN INCLUIR EL IVA*/
    ...
    SELECT   ROUND( IFNULL(
                    SUM(ROUND(SaldoCapVigente,2) + ROUND(SaldoCapAtrasa,2) +
                        ROUND(SaldoCapVencido,2) + ROUND(SaldoCapVenNExi,2) +
                          ...
                          ROUND(SaldoComFaltaPa,2) +  ROUND(SaldoComServGar,2) + ROUND(SaldoSeguroCuota,2) +
