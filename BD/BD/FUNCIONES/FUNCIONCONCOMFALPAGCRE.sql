...
-- ---------------------------------------------------------------------------------
-- FUNCIONCONCOMFALPAGCRE
DELIMITER ;
DROP FUNCTION IF EXISTS `FUNCIONCONCOMFALPAGCRE`;
DELIMITER $$


CREATE FUNCTION `FUNCIONCONCOMFALPAGCRE`(
...
IF(Var_CreEstatus != Cadena_Vacia) THEN
	...
	SELECT ROUND(IFNULL(SUM(ROUND(SaldoComFaltaPa,2) + ROUND(ROUND(SaldoComFaltaPa,2) * Var_ValIVAGen,2)
                            ...
                            + ROUND(SaldoSeguroCuota,2) + ROUND(SaldoIVASeguroCuota,2) + ROUND(SaldoComisionAnual,2) + + ROUND(SaldoComisionAnualIVA,2)),
                        Entero_Cero), 2)
