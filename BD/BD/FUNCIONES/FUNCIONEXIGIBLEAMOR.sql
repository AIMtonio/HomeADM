...
-- FUNCIONEXIGIBLEAMOR
DELIMITER ;
DROP FUNCTION IF EXISTS `FUNCIONEXIGIBLEAMOR`;
DELIMITER $$

CREATE FUNCTION `FUNCIONEXIGIBLEAMOR`(
	...
	IF(Var_CreEstatus != Cadena_Vacia) THEN
		...
		SELECT CobraComAnual, SaldoComAnual
		  INTO Var_CobraComAnual, Var_SaldoComAnual
			...
			IF(IFNULL(Var_CobraComAnual,Cadena_Vacia) = ConstanteSI AND IFNULL(Var_SaldoComAnual,Entero_Cero)>Entero_Cero)THEN
		...
		SELECT  ROUND(IFNULL(
				 ...
				 + ROUND(SaldoMoratorios + SaldoMoraVencido + SaldoMoraCarVen,2) +
				  ROUND(ROUND(SaldoMoratorios * Var_ValIVAIntMo,2)+
					...
					ROUND(SaldoNotCargoConIVA,2) + ROUND(ROUND(SaldoNotCargoConIVA,2) * Var_IVASucurs,2),
						   Entero_Cero)
					, 2)
