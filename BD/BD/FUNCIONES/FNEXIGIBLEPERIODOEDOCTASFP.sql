...
-- ---------------------------------------------------------------------------------
-- FNEXIGIBLEPERIODOEDOCTASFP
DELIMITER ;
DROP FUNCTION IF EXISTS `FNEXIGIBLEPERIODOEDOCTASFP`;DELIMITER $$

CREATE FUNCTION `FNEXIGIBLEPERIODOEDOCTASFP`(
	-- FUNCION DEVUELVE EL MONTO A PAGAR EN EL PERIODO DE UN CREDITO EN BASE A UNA FECHA DE CORTE PARA EL ESTADO DE CUENTA DEL CLIENTE SOFIEXPRESS
	Par_CreditoID		BIGINT(12),				-- Identificador del credito
	Par_FechaCorte		DATE,					-- Fecha de corte que indicara el periodo a tomar en cuenta para obtener el monto a pagar
	Par_Devuelve		VARCHAR(2)				-- Indica si se requiere desglosar el monto a pagar que devuelve la funcion. C: Devuelve solo capital, I: Devuelve solo intereses, V: Devuelve solo los IVAs, T: Devuelve el total del monto
...
IF(Var_CreEstatus != Cadena_Vacia) THEN
	...
	SELECT  ROUND(IFNULL(
					SUM(IF( FIND_IN_SET('C',Par_Devuelve) > 0 || FIND_IN_SET('T',Par_Devuelve) > 0,
						...
						+
						IF( FIND_IN_SET('I',Par_Devuelve) > 0 || FIND_IN_SET('T',Par_Devuelve) > 0,
						...
						+
						IF( FIND_IN_SET('V',Par_Devuelve) > 0 || FIND_IN_SET('T',Par_Devuelve) > 0,
