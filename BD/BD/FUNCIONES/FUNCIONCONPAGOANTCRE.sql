...
-- ---------------------------------------------------------------------------------
-- FUNCIONCONPAGOANTCRE
DELIMITER ;
DROP FUNCTION IF EXISTS `FUNCIONCONPAGOANTCRE`;
DELIMITER $$


CREATE FUNCTION `FUNCIONCONPAGOANTCRE`(
    ...
    SELECT   ROUND(IFNULL(
                    SUM(ROUND(SaldoCapVigente,2) + ROUND(SaldoCapAtrasa,2) +
                        ROUND(SaldoCapVencido,2) + ROUND(SaldoCapVenNExi,2) +
                          ...
                          ROUND(SaldoOtrasComis,2) + IFNULL((ROUND(FNEXIGIBLEIVAACCESORIOS(Par_CreditoID, AmortizacionID,Var_ValIVAGen, Var_CliPagIVA),2)),Decimal_Cero) +
                          ROUND(SaldoIntOtrasComis,2) + ROUND(FNEXIGIBLEIVAINTERESACCESORIOS(Par_CreditoID,AmortizacionID,Var_ValIVAGen,Var_CliPagIVA),2) +
