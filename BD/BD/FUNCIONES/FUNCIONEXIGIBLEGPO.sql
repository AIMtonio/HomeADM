...
-- FUNCIONEXIGIBLEGPO
DELIMITER ;
DROP FUNCTION IF EXISTS `FUNCIONEXIGIBLEGPO`;DELIMITER $$

CREATE FUNCTION `FUNCIONEXIGIBLEGPO`(
...
DECLARE Var_TotalProyectado     DECIMAL(14,2);
DECLARE Var_TotalExigible       DECIMAL(14,2);
...
IF(IFNULL(Var_GrupoID,Entero_Cero)>Entero_Cero)THEN
    ...
    IF(Var_CicloGrupo = Var_CicloActual) THEN
        SELECT  SUM(FUNCIONCONPAGOANTCRE(Cre.CreditoID)),
                ...
                INTO Var_TotalDeuda, Var_TotalProyectado
...
END IF;
    ...
    SET Var_TotalProyectado := IFNULL(Var_TotalProyectado, Entero_Cero);

    SET Var_TotalExigible   := Var_TotalDeuda - Var_TotalProyectado;
    SET Var_TotalExigible   := IFNULL(Var_TotalExigible, Entero_Cero);

RETURN Var_TotalExigible;
END$$