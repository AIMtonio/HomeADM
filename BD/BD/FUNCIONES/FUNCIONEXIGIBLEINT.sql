...
-- FUNCIONEXIGIBLEINT
DELIMITER ;
DROP FUNCTION IF EXISTS `FUNCIONEXIGIBLEINT`;
DELIMITER $$


CREATE FUNCTION `FUNCIONEXIGIBLEINT`(
...
if(Var_CreEstatus != Cadena_Vacia) then
    ...
    select  round(sum(round(SaldoInteresPro,2) + round(SaldoInteresAtr,2) +
                      round(SaldoInteresVen,2) + round(SaldoIntNoConta,2)),  2)
