-- TD_FUNCIONLIMITEMONTO
DELIMITER ;
DROP FUNCTION IF EXISTS `TD_FUNCIONLIMITEMONTO`;
DELIMITER $$

CREATE FUNCTION `TD_FUNCIONLIMITEMONTO`(
  -- -----------------------------------------------------------
  --  Obtiene el limite de Tarjeta
  -- -----------------------------------------------------------
    Par_TarjetaDeb      CHAR(16),  -- Tarjeta de debito
...
BEGIN
    ...
    IF (Par_NumCon = Con_DispoDiario) THEN
        SELECT DisposiDiaNac    INTO Val_MontoLimite
            FROM TARDEBLIMITES
            WHERE TarjetaDebID = Par_TarjetaDeb AND Vigencia > Var_FechaSistema;
    ...
    IF (Par_NumCon = Con_DispoMes) THEN
        ...
        SELECT DisposiMesNac
        INTO Val_MontoLimite
        FROM TARDEBLIMITES
