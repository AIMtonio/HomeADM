...
-- ---------------------------------------------------------------------------------
-- FUNCIONIVAINTERESCRE
DELIMITER ;
DROP FUNCTION IF EXISTS `FUNCIONIVAINTERESCRE`;DELIMITER $$

CREATE FUNCTION `FUNCIONIVAINTERESCRE`(
...
if(Var_CreEstatus != Cadena_Vacia) then
 ...
 select  round(ifnull(
    ...
    Set Var_ValIVAIntOr = ifnull(Var_ValIVAIntOr, Decimal_Cero);

end if;

return Var_ValIVAIntOr;
