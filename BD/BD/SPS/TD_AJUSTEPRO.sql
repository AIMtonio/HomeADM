-- TD_AJUSTEPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `TD_AJUSTEPRO`;

DELIMITER $$
CREATE PROCEDURE `TD_AJUSTEPRO`(
	-- Store Procedure de Ajuste de Compra de tarjetas de debito por medio de Web Service
	...
	Par_TransLinea					CHAR(1),		-- Indica si puede realizar compras en linea
...
TerminaStore: BEGIN
	...
	DECLARE Var_TransactionAmount	DECIMAL(12,2);		-- Monto TransactionAmount
	DECLARE Var_CashbackAmount		DECIMAL(12,2);		-- Monto CashbackAmount
	DECLARE Var_AdditionalAmount	DECIMAL(12,2);		-- Monto AdditionalAmount

	DECLARE Var_SurchargeAmounts	DECIMAL(12,2);		-- Monto SurchargeAmounts
	DECLARE Var_LimiteCompraDiario	DECIMAL(12,2);		-- Monto Limite de Compra Diario
	DECLARE Var_LimiteCompraMes		DECIMAL(12,2);		-- Monto Limite de compras al mes
	DECLARE Var_MontoCompraDiario	DECIMAL(12,2);		-- Monto de Compra Diario
	...
	DECLARE Var_TotalReversa		DECIMAL(16,2);		-- Total de Reversa
	...
	DECLARE Con_CompraDiario		INT(11);			-- Consulta para obtener el limite de monto diario para compra
	DECLARE Con_CompraMes			INT(11);			-- Consulta para obtener el limite de monto mensual para compra
	...
	ManejoErrores:BEGIN
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen	= CONCAT('El SAFI ha tenido un problema al concretar la operacion. Disculpe las molestias que ',
									 'esto le ocasiona. Ref: SP-TD_AJUSTEPRO');
		...
		INTO 	Var_TarjetaDebitoID,	Var_EstatusTarjeta,	Var_CuentaAhoID,	Var_ClienteID,
				Var_BloquearPos,
		...
		SELECT RespaldoID,			TransactionAmount,		CashbackAmount,		AdditionalAmount,		SurchargeAmounts
		INTO Par_FolioDevolucionID,	Var_TransactionAmount,	Var_CashbackAmount,	Var_AdditionalAmount,	Var_SurchargeAmounts
		...
		SET Var_TransactionAmount := IFNULL(Var_TransactionAmount, Decimal_Cero);
		SET Var_CashbackAmount 	  := IFNULL(Var_CashbackAmount, Decimal_Cero);
		SET Var_AdditionalAmount  := IFNULL(Var_AdditionalAmount, Decimal_Cero);
		SET Var_AdditionalAmount  := IFNULL(Var_AdditionalAmount, Decimal_Cero);
		SET Var_TotalReversa	  := Var_TransactionAmount + Var_CashbackAmount + Var_AdditionalAmount + Var_SurchargeAmounts;
		...
		IF( Par_TranCode <> Code_DebitAdjustment ) THEN
			SET Par_NumErr	:= 1205;
		...
		-- Segmento de Validaciones de la Cuenta de Ahorro asociada a la Tarjeta de Debito
		...
		IF( Var_CuentaAhoID = Entero_Cero ) THEN
			SET Par_NumErr	:= 1224;
			SET Par_ErrMen	:= CONCAT('La Tarjeta de Debito: ',Par_CardNumber,' no tiene asociado una cuenta de ahorro.');
			SET Var_Control	:= 'cardNumber';
			SET Par_NumCodigoErr := 14;
			LEAVE ManejoErrores;
		END IF;
		...
		IF( Par_FolioDevolucionID = Entero_Cero ) THEN
			SET Par_NumErr	:= 1228;
			SET Par_ErrMen	:= 'No Existe una Referencia de la Operacion a Devolver.';
			SET Var_Control	:= 'cardNumber';
			SET Par_NumCodigoErr := 12;
			LEAVE ManejoErrores;
		END IF;
		...
		SET Var_DesPoliza		 := CONCAT("AJUSTE ",Var_Referencia);
		...
		CALL TD_CONTACTAAHORROPRO (
			...
			Var_TotalReversa,		Par_MonedaID,				Var_DesPoliza,				Var_Referencia,			Alta_Enc_Pol_SI,
		...
		CALL POLIZATARJETAPRO (
			...
			Con_OperacPOS,		Par_MonedaID,		Var_TotalReversa,		Entero_Cero,		Var_DesPoliza,
		...
		CALL TD_TARJETADEBITOACT (
			Par_CardNumber,		Var_TotalReversa,	Nat_Abono,			Act_MovCompra,
		...
		-- Una vez realizado la reversa, se verifica si el monto de Ajuste no excede los limites de la tarjeta
		SELECT	IFNULL(TD_FUNCIONLIMITEMONTO(Tar.TarjetaDebID, Tar.ClienteID, Tar.TipoTarjetaDebID, Con_CompraDiario), Decimal_Cero),
				IFNULL(TD_FUNCIONLIMITEMONTO(Tar.TarjetaDebID, Tar.ClienteID, Tar.TipoTarjetaDebID, Con_CompraMes), Decimal_Cero),
				IFNULL(Tar.MontoCompraDiario, Decimal_Cero ),	IFNULL(Tar.MontoCompraMes, Decimal_Cero)
		INTO 	Var_LimiteCompraDiario,
				Var_LimiteCompraMes,
				Var_MontoCompraDiario,	Var_MontoCompraMes
		FROM TARJETADEBITO Tar
		WHERE TarjetaDebID = Par_CardNumber;
		...
		IF( Var_LimiteCompraDiario > Decimal_Cero ) THEN
			IF( (Var_TotalCompra + Var_MontoCompraDiario) > Var_LimiteCompraDiario ) THEN
				SET Par_NumErr	:= 1332;
				SET Par_ErrMen	:= 'La Tarjeta de Debito ha Excedido el Limite de Compra Diario.';
				SET Var_Control	:= 'cardNumber';
				SET Par_NumCodigoErr := 61;
				LEAVE ManejoErrores;
			END IF;

			-- Se obtiene el Monto Disponible para compras al dia
		...
		IF( Var_LimiteCompraMes > Decimal_Cero) THEN
			...
			IF( (Var_TotalCompra + Var_MontoCompraMes) > Var_LimiteCompraMes) THEN
				SET Par_NumErr	:= 1334;
				SET Par_ErrMen	:= 'La Tarjeta de Debito ha Excedido el Limite de Compra Mensual.';
				SET Var_Control	:= 'cardNumber';
				SET Par_NumCodigoErr := 65;
				LEAVE ManejoErrores;
			END IF;

			-- Se obtiene el Monto Disponible para compras al Mes
		...
		-- Se efectua el Cargo por el Nuevo Monto
		CALL TD_COMPRANORMALRPRO(Par_TranCode,			Par_CardNumber,			Par_Reference,			Var_TotalCompra,		Entero_Cero,
								Entero_Cero,			Par_MerchantType,		Par_MonedaID,			Aud_NumTransaccion,		Par_TransLinea,
								Var_FechaAplicacion,	Origen_WebService,		Par_TerminalName,		Var_NumTransaccion,		Var_SaldoContableAct,
								Var_SaldoDisponibleAct,	Var_CodigoRespuesta,	Var_FechaOperacion,		Par_TarDebMovID,		Par_AuthorizationNumber,
								Var_PolizaID,			Salida_NO,				Par_NumErr,				Par_ErrMen,				Par_EmpresaID,
								Aud_Usuario,			Aud_FechaActual,		Aud_DireccionIP,		Aud_ProgramaID,			Aud_Sucursal,
								Aud_NumTransaccion);

		IF( Par_NumErr <> Entero_Cero ) THEN
			SET Par_NumErr := 1299;
			SET Par_NumCodigoErr := Var_CodigoRespuesta;
			LEAVE ManejoErrores;
		END IF;
		...
		SET Par_ErrMen := CONCAT('Ajuste Realizado Correctamente: ',Par_CodigoTransaccionID,'.');
