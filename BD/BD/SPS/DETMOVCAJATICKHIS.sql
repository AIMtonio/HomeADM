...
-- DETMOVCAJATICKHIS
DELIMITER ;
DROP PROCEDURE IF EXISTS `DETMOVCAJATICKHIS`;DELIMITER $$

CREATE PROCEDURE `DETMOVCAJATICKHIS`(
    Par_SucursalID      	int,
    Par_CajaID          	int,
    Par_Fecha           	date,
...
DECLARE Estatus_Recepcion 	int;
DECLARE Estatus_Envio     	int;
...
select	MontoEnFirme, CT.DescCorta as Movimiento, CM. Referencia,CM. FechaActual
        from `HIS-CAJASMOVS` CM
		inner join CAJATIPOSOPERA CT on CM.TipoOperacion = CT.Numero
		inner join MONEDAS M on CM.MonedaID = M.MonedaId
        	where CM.Fecha         = Par_Fecha
...
CASE Par_TipoOperacion
	WHEN Ope_EnvEfecBanc THEN Set Var_Estatus:='E';
	WHEN Ope_RecEfectBan THEN Set Var_Estatus:='R';
	WHEN Ope_EntEnvBanc  THEN Set Var_Estatus:='E';
	WHEN Ope_SalEfecBanc THEN Set Var_Estatus:='R';
	else  Set Var_Estatus:='';
END CASE;
...
case Par_TipoOperacion
	...
	when Ope_EnvEfecBanc  then Set Var_DescCorta := concat('Referencia',LPAD('Monto ', 29, ' '));
	...
	when Ope_SalRecTrCaja then Set Var_DescCorta := concat('Referencia',LPAD('Hora',7, ' '),LPAD('Monto ', 20, ' '));
	...
	when Ope_SalEfecCam   then Set Var_DescCorta := concat('Referencia',LPAD('Hora',9, ' '),LPAD('Monto', 20, ' '));
