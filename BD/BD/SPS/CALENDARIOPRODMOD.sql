...
-- CALENDARIOPRODMOD
DELIMITER ;
DROP PROCEDURE IF EXISTS `CALENDARIOPRODMOD`;
DELIMITER $$

CREATE PROCEDURE `CALENDARIOPRODMOD`(
/* MODIFICACION DEL ESQUEMA DE CALENDARIOS DE PRODUCTO DE CREDITO.
 * */
...
ManejoErrores: BEGIN
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									'Disculpe las molestias que esto le ocasiona. Ref: SP-CALENDARIOPRODMOD');
       ...
       IF(IFNULL( Par_ProduCredID, Entero_Cero)) = Entero_Cero THEN
		...
		UPDATE CALENDARIOPROD SET
			FecInHabTomar	  = Par_FecHabTom,
			AjusFecExigVenc	  = Par_AjFecExVen,
			PermCalenIrreg	  = Par_PermCalIrr,
			AjusFecUlAmoVen	  = Par_AjFecUAmVe,
			TipoPagoCapital	  = Par_TipoPagCap,

			IguaCalenIntCap	  = Par_IgCalInCap,
			Frecuencias		  = Par_Frecuenc,
			PlazoID			  = Par_PlazoID,
			DiaPagoCapital	  = Par_DiaPagoCap,
			DiaPagoInteres	  = Par_DiaPagoInt,

			TipoDispersion	  = Par_TipoDisp,
			DiaPagoQuincenal  = Par_DiaPagoQuincenal,
			DiasReqPrimerAmor = Par_DiasReqPrimerAmor,
		...
		SET Par_ErrMen := CONCAT('Calendario por Producto Modificado Exitosamente: ' ,Par_ProduCredID) ;
