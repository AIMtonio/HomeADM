DELIMITER ;
DROP PROCEDURE IF EXISTS `BITACORACTACLABEALT`; 
DELIMITER $$

CREATE  PROCEDURE `BITACORACTACLABEALT`(
	...
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									'Disculpe las molestias que esto le ocasiona. Ref: SP-BITACORACTACLABEALT');
	   ...
	   SET Var_ID := (SELECT MAX(BitacoraID) FROM BITACORACTACLABE);
	   SET Var_ID := IFNULL(Var_ID, Entero_Cero);
	   SET Var_ID := Var_ID + 1;

	   INSERT INTO BITACORACTACLABE(
			BitacoraID,			 Transaccion, 			Fecha, 				CuentaCLABE, 		EmpresaID,
			Usuario, 			 FechaActual, 			DireccionIP, 		ProgramaID, 		Sucursal,
			NumTransaccion)
		VALUES
			(Var_ID,	 		Par_NumTransaccion,		Par_FechaHora,		Par_CuentaClabe,	Aud_EmpresaID,
