-- BANCUENTASDESTINOALT

DELIMITER ;

DROP PROCEDURE IF EXISTS BANCUENTASDESTINOALT;

DELIMITER $$

CREATE PROCEDURE `BANCUENTASDESTINOALT`(
...
TerminaStore: BEGIN
	...
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			SET Par_ErrMen = CONCAT('Estimado Usuario(a), Ha ocurrido una falla en el sistema,',
									...
									'esto le ocasiona. Ref: SP-BANCUENTASDESTINOALT.');
	...
	IF(Par_TipoCuenta = Cta_Externa) THEN
		...
		SELECT	Ins.InstitucionID, 		Ins.NombreCorto, 		Ins.Folio
			INTO Par_InstitucionID, 	Var_NombreCorto, 		Var_Folio
	...
	ELSE
		...
		SET Par_InstitucionID := Var_InstitucionID;
	...
	SET Aud_FechaActual := NOW();
		...
		END IF;
         ...
         CALL CUENTASTRANSFERALT (
        	Par_ClienteID,      	 Par_CuentaInterna,  	Par_InstitucionID,		Par_CuentaCLABE,	Var_Beneficiario,
