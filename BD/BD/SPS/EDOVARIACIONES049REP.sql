...
-- ---------------------------------------------------------------------------------
-- EDOVARIACIONES049REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `EDOVARIACIONES049REP`;

DELIMITER $$
CREATE PROCEDURE `EDOVARIACIONES049REP`(
	-- SP para generar el Estado de Variacion de Capital del Cliente: ATE
...
TerminaStore: BEGIN
	...
	DECLARE Var_ParticipacionControladora		CHAR(1);-- Determina si el campo ParticipacionControladora es evaluado
	DECLARE Var_CapitalSocial					CHAR(1);-- Determina si el campo CapitalSocial es evaluado
	DECLARE Var_AportacionesCapital				CHAR(1);-- Determina si el campo AportacionesCapital es evaluado
	DECLARE Var_PrimaVenta 						CHAR(1);-- Determina si el campo PrimaVenta es evaluado
	DECLARE Var_ObligacionesSubordinadas		CHAR(1);-- Determina si el campo ObligacionesSubordinadas es evaluado

	DECLARE Var_IncorporacionSocFinancieras		CHAR(1);-- Determina si el campo IncorporacionSocFinancieras es evaluado
	DECLARE Var_ReservaCapital					CHAR(1);-- Determina si el campo ReservaCapital es evaluado
	DECLARE Var_ResultadoEjerAnterior			CHAR(1);-- Determina si el campo ResultadoEjerAnterior es evaluado
	DECLARE Var_ResultadoTitulosVenta			CHAR(1);-- Determina si el campo ResultadoTitulosVenta es evaluado
	DECLARE Var_ResultadoValuacionInstrumentos	CHAR(1);-- Determina si el campo ResultadoValuacionInstrumentos es evaluado

	DECLARE Var_EfectoAcomulado 				CHAR(1);-- Determina si el campo EfectoAcomulado es evaluado
	DECLARE Var_BeneficioEmpleados 				CHAR(1);-- Determina si el campo BeneficioEmpleados es evaluado
	DECLARE Var_ResultadoMonetario				CHAR(1);-- Determina si el campo ResultadoMonetario es evaluado
	DECLARE Var_ResultadoActivos				CHAR(1);-- Determina si el campo ResultadoActivos es evaluado
	DECLARE Var_ParticipacionNoControladora		CHAR(1);-- Determina si el campo ParticipacionNoControladora es evaluado

	DECLARE Var_CapitalContable					CHAR(1);-- Determina si el campo CapitalContable es evaluado
	...
	SET Var_NumCliente		:= IFNULL(FNPARAMGENERALES('CliProcEspecifico'), 49);

	SELECT JefeContabilidad,	GerenteGeneral,	   IFNULL(DirectorFinanzas, Cadena_Vacia)
	INTO   Var_ContaGeneral,	Var_DirectGeneral, Var_DirectorFinanza
	FROM PARAMETROSSIS
	...
	UPDATE TMPEDOVARIACIONES SET
		...
		ParticipacionControladora = CapitalContable
	WHERE NumeroTransaccion = Aud_NumTransaccion;
	...
	SELECT	Descripcion,					ParticipacionControladora,		CapitalSocial,		AportacionesCapital,	PrimaVenta,
			...
			UPPER(CONCAT("DEL 01 de ENERO AL ", FUNCIONLETRASFECHA(Par_FechaActual))) AS TextoFecha,
			CONCAT("Lic. ",Var_DirectorFinanza) AS Par_DirectorFinanzas,
			CONCAT("Lic. ",Var_ContaGeneral) AS Par_JefeContabilidad,
			CONCAT("Lic. ",Var_DirectGeneral) AS Par_GerenteGral
	FROM TMPEDOVARIACIONES
	WHERE NumeroTransaccion = Aud_NumTransaccion;
