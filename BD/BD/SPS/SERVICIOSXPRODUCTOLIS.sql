-- SP SERVICIOSXPRODUCTOLIS
DELIMITER ;
DROP PROCEDURE IF EXISTS SERVICIOSXPRODUCTOLIS;

DELIMITER $$
CREATE PROCEDURE SERVICIOSXPRODUCTOLIS(
...
TerminaStore : BEGIN
	...
	SELECT PC.ProducCreditoID,
		PC.Descripcion,
		CASE
			WHEN IFNULL(SxP.ServicioID,0) != 0 THEN
				EstatusSi
			ELSE
				EstatusNo
		END AS Estatus
	FROM PRODUCTOSCREDITO PC
	LEFT JOIN SERVICIOSXPRODUCTO SxP ON PC.ProducCreditoID = SxP.ProducCreditoID
									AND SxP.ServicioID = Par_ServicioID;
