...
-- EDORESINTERNO049REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `EDORESINTERNO049REP`;

DELIMITER $$
CREATE PROCEDURE `EDORESINTERNO049REP`(
	-- SP para generar el Estado de Resultados del Cliente: ATE
...
TerminaStore: BEGIN
	...
	IF( Par_TipoConsulta = Por_Fecha ) THEN
		...
		SET Var_FechaCadena := UPPER(CONCAT("Del 01 de enero al ",FUNCIONLETRASFECHA(Par_Fecha)));
	...
	IF( Par_TipoConsulta = Por_Periodo ) THEN
		...
		SELECT UPPER(CONCAT("Del 01 de enero al ",FUNCIONLETRASFECHA(Fin)))
	...
	IF( IFNULL(Aud_NumTransaccion, Entero_Cero) > Entero_Cero ) THEN
		...
		DROP TEMPORARY TABLE IF EXISTS TMP_EDORESULTADOSCICLO;
		CREATE TEMPORARY TABLE TMP_EDORESULTADOSCICLO (
		...
		INSERT INTO TMP_EDORESULTADOSCICLO(RegistroID, Columna, Monto)
