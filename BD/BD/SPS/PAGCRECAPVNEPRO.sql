...
-- PAGCRECAPVNEPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `PAGCRECAPVNEPRO`;
DELIMITER $$

CREATE PROCEDURE `PAGCRECAPVNEPRO`(
...
TerminaStore: BEGIN
	...
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			SET Par_ErrMen = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
	               			 'Disculpe las molestias que esto le ocasiona. Ref: SP-PAGCRECAPVNEPRO');
	...
	IF (Var_CreEstatus != Estatus_Suspendido) THEN
		SET Mov_AboConta	:= Con_CapVenNoExi;	-- No Concepto: 4
	END IF;
	...
	END IF;
		...
		SET Par_NumErr	:= 0;
		SET Par_ErrMen	:= 'Pago de Capital Vencido No Exigible realizado Exitosamente.';
