...
-- EDOCTADETINVERPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `EDOCTADETINVERPRO`;DELIMITER $$

CREATE PROCEDURE `EDOCTADETINVERPRO`(
	-- SP para obtener los Datos de las inversiones del cliente para el Estado de Cuenta
	...
	DECLARE Var_CliProEspEdoCta			INT(11);					-- Almacena el Numero de Cliente para Procesos Especificos
	...
	DECLARE DetInverEdoCta				INT(11);					-- Identificador Datos del detalle de inversiones para el Estado de Cuenta
	DECLARE Con_EdoCtaGral				VARCHAR(20);				-- Numero de Cliente para Proceso de estado de cuenta especifico
	DECLARE	NumClienteNuevo				INT(11);					-- Numero para todos los clientes nuevos Procesos Especificos: 99

	-- Asignacion de constantes
	SET Entero_Cero 					:= 0;						-- Entero Cero
	SET Cadena_Vacia					:= '';						-- Cadena Vacia
	SET DetInverEdoCta					:= 19;						-- Identificador Datos del detalle de inversiones para el Estado de Cuenta
	SET Con_EdoCtaGral					:= 'EdoCtaGeneral';			-- Numero de Cliente para Proceso de estado de cuenta especifico
	SET	NumClienteNuevo					:= 99;						-- Numero para todos los clientes nuevos Procesos Especificos: 99
	...
	ManejoErrores:BEGIN
		...
		IF(Var_CliProEspEdoCta = NumClienteNuevo) THEN
			SET Var_ProcPersonalizado := (SELECT NomProc FROM CATPROCEDIMIENTOS WHERE ProcedimientoID = DetInverEdoCta AND CliProEspID = Var_CliProEspEdoCta);
		ELSE
			LEAVE ManejoErrores;
		END IF;
