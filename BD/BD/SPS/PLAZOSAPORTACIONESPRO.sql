...
-- ---------------------------------------------------------------------------------
-- PLAZOSAPORTACIONESPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `PLAZOSAPORTACIONESPRO`;DELIMITER $$

CREATE PROCEDURE `PLAZOSAPORTACIONESPRO`(
-- =================================================================
-- ----------- SP PARA PROCESOS DE PLAZOS DE APORTACIONES ----------
-- =================================================================
  ...
  SET SalidaSI      := 'S';       -- Salida si
    SET Cons_InstAport    := 31;        -- Tipo de instrumento: Aportacion
  ...
  ManejoErrores:BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
      BEGIN
        ...
        SET Par_ErrMen = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
                    'Disculpe las molestias que esto le ocasiona. Ref: SP-PLAZOSAPORTACIONESPRO');
      END;
    ...
    -- LLAMADA PARA REGISTRO EN TABLA PLAZOSAPORTACIONES
    CALL PLAZOSAPORTACIONESALT(Par_TipoAportacionID,Par_PlazosInferior,Par_PlazosSuperior, SalidaNO,Par_NumErr,
                Par_ErrMen, Par_Empresa,Aud_Usuario,Aud_FechaActual,Aud_DireccionIP,
                                Aud_ProgramaID,Aud_Sucursal,Aud_Sucursal);

        -- VERIFICAR QUE NO HAYA REGRESADO UN ERROR
    IF (Par_NumErr <> Entero_Cero) THEN
      SET Var_Control := 'tipoAportacionID';
      LEAVE ManejoErrores;
    END IF;

    -- LLAMADA PARA REGISTRO EN TABLA PLAZOS POR PRODUCTOS
    CALL PLAZOSPORPRODUCTOALT(Par_PlazosSuperior,Cons_InstAport,Par_TipoAportacionID, SalidaNO,Par_NumErr,
                Par_ErrMen, Par_Empresa,Aud_Usuario,Aud_FechaActual,Aud_DireccionIP,
                                Aud_ProgramaID,Aud_Sucursal,Aud_Sucursal);
    ...
    SET Par_NumErr    := 000;
        SET Par_ErrMen    := "Rangos de Plazo(s) Agregado(s) Exitosamente.";
    SET Var_Control   := 'tipoAportacionID';
