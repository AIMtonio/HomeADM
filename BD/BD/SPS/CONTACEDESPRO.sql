...
-- CONTACEDESPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `CONTACEDESPRO`;DELIMITER $$

CREATE PROCEDURE `CONTACEDESPRO`(
# ========================================================
# ----- SP PARA REALIZAR LA CONTABILIDAD DE LOS CEDES-----
# ========================================================
	Par_CedeID			INT(11),
	...
	DECLARE Var_CedeStr		VARCHAR(20);
	...
	ManejoErrores:BEGIN
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
								'Disculpe las molestias que esto le ocasiona. Ref: SP-CONTACEDESPRO');
		...
		ELSE
			SET Des_Concepto:= (SELECT Descripcion
									FROM 	CONCEPTOSCEDE
									WHERE	ConceptoCedeID	= Par_ConceptoCede);
		END IF;

		SET Var_CedeStr		:=  CONCAT("Cede.",	CONVERT(Par_CedeID, CHAR));
		...
		IF(Par_Naturaleza = Nat_Abono)THEN
			IF (Par_AltaMovAho = AltaMovAho_SI) THEN
				...
				CALL CUENTASAHORROMOVALT(
					...
					Des_Concepto,		Par_CedeID,				Par_TipoMovAho,		Salida_NO,		Par_NumErr,
			...
			CALL POLIZACEDESPRO(
				Var_Poliza,			Par_Fecha,			Par_ClienteID,		Par_ConceptoCede,	Par_CedeID,
				Par_MonedaID,		Mon_Cargo,			Mon_Abono,			Des_Concepto,		Var_CedeStr,
		...
		SET Par_NumErr := 0;
		SET Par_ErrMen := CONCAT('Proceso Exitoso para el CEDE ',Par_CedeID,'.');
