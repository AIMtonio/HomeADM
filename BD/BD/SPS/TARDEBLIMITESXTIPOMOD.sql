...
-- ---------------------------------------------------------------------------------
-- TARDEBLIMITESXTIPOMOD
DELIMITER ;
DROP PROCEDURE IF EXISTS `TARDEBLIMITESXTIPOMOD`;DELIMITER $$

CREATE PROCEDURE `TARDEBLIMITESXTIPOMOD`(
...
if(ifnull(Par_DisposicionesDia, Cadena_Vacia) = Cadena_Vacia) then
               select '002' as NumErr,
                'Especifique el Numero de Disposiones al Di­a' as ErrMen,
...
if(ifnull(Par_MontoMaxComprasMensual,Cadena_Vacia) = Cadena_Vacia) then
  ...
  if(ifnull(Par_BloqueoPOS , Cadena_Vacia ) = Cadena_Vacia) then
	...
	SELECT
		'000' as NumErr ,
		concat('Limites por Tipo de Tarjeta Modificados Exitosamente: ', Par_TipoTarjetaDebID) as ErrMen,
		'tipoTarjetaDebID' as control,
