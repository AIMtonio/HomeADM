...
-- ---------------------------------------------------------------------------------
-- TIPOPRODSUCURSALESALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `TIPOPRODSUCURSALESALT`;
DELIMITER $$


CREATE PROCEDURE `TIPOPRODSUCURSALESALT`(
# ================================================================================
...
TerminaStore:BEGIN 	#bloque main del sp
	...
	ManejoErrores:BEGIN 	#bloque para manejar los posibles errores
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION
			BEGIN
				...
				SET Par_ErrMen = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
										'Disculpe las molestias que esto le ocasiona. Ref: SP-TIPOPRODSUCURSALESALT');
		...
		ELSEIF  Par_TipoInstrumentoID=TipoInstrumentoCEDES THEN
		...
		IF (Par_TipoInstrumentoID =TipoInstrumentoCEDES) THEN

			IF NOT EXISTS(SELECT TipoCedeID
						FROM TIPOSCEDES
						WHERE TipoCedeID = Par_InstrumentoID)THEN
				SET Par_NumErr  := '007';
				SET Par_ErrMen  := 'NO existe el Tipo de CEDES.';
				SET varControl  := 'tipoCedeID' ;
				LEAVE ManejoErrores;
			END IF;
			...
			FROM 	TIPOSCEDES
			WHERE	TipoCedeID	= Par_InstrumentoID;
		...
		IF(Par_Estatus = Estatus_Activo)THEN
			...
			SET Var_TipCuentaSucID	:= (SELECT IFNULL(MAX(TipProdSucID),Entero_Cero)+Entero_Uno
			 ...
			 INSERT INTO TIPOPRODSUCURSALES(
