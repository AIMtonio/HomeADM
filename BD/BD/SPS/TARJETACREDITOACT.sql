...
-- TARJETACREDITOACT
DELIMITER ;
DROP PROCEDURE IF EXISTS `TARJETACREDITOACT`;

DELIMITER $$
CREATE PROCEDURE `TARJETACREDITOACT`(
	-- Store Procedure que Actualiza campos de las Tarjetas de Credito
	Par_TarjetaCreditoID	CHAR(16),		-- Numero de Tarjeta de Credito
	...
	ManejoErrores:BEGIN
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen	= CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									 'Disculpe las molestias que esto le ocasiona. Ref: SP-TARJETACREDITOACT');
			SET Var_Control = 'SQLEXCEPTION';
		END;

		SET Par_TarjetaCreditoID := IFNULL(Par_TarjetaCreditoID, Entero_Cero);
		...
		IF( Par_TarjetaCreditoID = Entero_Cero) THEN
			SET Par_NumErr:= 1401;
		...
		SET Var_TarjetaCreditoID := IFNULL(Var_TarjetaCreditoID, Entero_Cero);

		IF( Var_TarjetaCreditoID = Entero_Cero) THEN
			SET Par_NumErr:= 1402;
			SET Par_ErrMen:= CONCAT('El Numero de Tarjeta de Credito:', Par_TarjetaCreditoID,' No Existe');
			SET Var_Control	:= 'tarjetaCredID';
			LEAVE ManejoErrores;
		END IF;
		...
		IF( Par_NumActualizacion = Act_MovCompra ) THEN
			...
			SET Par_ErrMen:= CONCAT('Actualizacion de Tarjeta de Credito: ',Par_TarjetaCreditoID,' Exitoso');
			LEAVE ManejoErrores;
		END IF;
		...
		IF( Par_NumActualizacion = Act_MovConsulta ) THEN
			...
			IF( Par_NatMovimiento = Nat_Cargo ) THEN
				...
				SELECT	NoConsultaSaldoMes,
						IFNULL(TC_FNNUMCONSULTAS(Tar.TarjetaCredID, Tar.ClienteID, Tar.TipoTarjetaCredID, Con_OpeConsulta), Entero_Cero)
				...
				IF( Var_NumConsultaMes > Entero_Cero AND (Var_NoConsultaSaldoMes + Entero_Uno) > Var_NumConsultaMes ) THEN
					SET Par_NumErr	:= 1207;
					SET Par_ErrMen	:= CONCAT('La Tarjeta: ',Par_TarjetaCreditoID,' alcanzo el Numero de Consultas Permitidas.');
					SET Var_Control	:= 'numActualizacion';
					LEAVE ManejoErrores;
				END IF;
			END IF;
