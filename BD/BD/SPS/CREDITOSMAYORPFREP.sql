...
-- CREDITOSMAYORPFREP
DELIMITER ;
DROP PROCEDURE IF EXISTS `CREDITOSMAYORPFREP`;
DELIMITER $$

CREATE PROCEDURE `CREDITOSMAYORPFREP`(
	-- SP para generar el reporte de mayor saldo insoluto Persona Fisica
...
DECLARE Var_PorcentajeCredPF    DECIMAL(10,2);	    -- Porcentaje Creditos Mayor Saldo Insoluto Persona Fisica
DECLARE Var_PorcentualCredPF    DECIMAL(10,2);      -- Resultado Porcentual (Creditos Mayor Saldo Insoluto Persona Fisica)
DECLARE Var_DifLimCredPF   		DECIMAL(10,2);      -- Diferencia entre el Parametro (Creditos Mayor Saldo Insoluto Persona Fisica) y el Resultado Porcentual
...
DECLARE ParamCredPF     	INT(11);

DECLARE EstatusVigente  	CHAR(1);
DECLARE EstatusVencida  	CHAR(1);
DECLARE FisActEmpres    	CHAR(1);
DECLARE FisSinActEmpres 	CHAR(1);
...
SET ParamCredPF         := 11;                  -- CatParamRiesgosID: Parametro Creditos Mayor Saldo Insoluto Persona Fisica

SET EstatusVigente      := 'V';					-- Estatus Credito: Vigente
SET EstatusVencida      := 'B';					-- Estatus Credito: Vencido
SET FisActEmpres        := 'A';					-- Tipo Persona: Persona Fisica con Actividad Empresarial
SET FisSinActEmpres     := 'F';                 -- Tipo Persona: Persona Fisica
...
SET Var_PorcentajeCredPF   := (SELECT Porcentaje FROM PARAMUACIRIESGOS WHERE CatParamRiesgosID = ParamCredPF);
SET Var_PorcentajeCredPF   := IFNULL(Var_PorcentajeCredPF,Decimal_Cero);
...
-- Mayor Saldo Insoluto Persona Fisica (Grid)
...
-- Consulta Parametro % Mayor Saldo Insoluto Persona Fisica
IF(Par_NumRep = Rep_Parametro) THEN
	DROP TEMPORARY TABLE IF EXISTS TMPCREDITOSMAYORSALDOPF;
	CREATE TEMPORARY TABLE  TMPCREDITOSMAYORSALDOPF(
			ClienteID			INT(11),
            CreditoID           BIGINT(12),
	...
	INSERT INTO TMPCREDITOSMAYORSALDOPF()
	SELECT Cli.ClienteID,Cre.CreditoID, SUM(Sal.SalCapVigente + Sal.SalCapAtrasado + Sal.SalCapVencido + Sal.SalCapVenNoExi +
					Sal.SalIntOrdinario + Sal.SalIntProvision + Sal.SalIntAtrasado +
					Sal.SalIntVencido + Sal.SalMoratorios +
					Sal.SaldoMoraVencido) AS SaldoInsoluto,Cre.SucursalID,
					Var_CapitalNeto, Var_PorcentajeCredPF
		FROM CLIENTES Cli
			INNER JOIN SALDOSCREDITOS Sal
				ON Sal.ClienteID = Cli.ClienteID
			AND Sal.EstatusCredito IN(EstatusVigente,EstatusVencida)
	...
	SELECT SUM(SaldoInsoluto) INTO Var_TotSaldoInsoluto FROM TMPCREDITOSMAYORSALDOPF;

	SET Var_PorcentualCredPF   := (Var_CapitalNeto * Var_PorcentajeCredPF) / 100;
	SET Var_PorcentualCredPF   := IFNULL(Var_PorcentualCredPF, Decimal_Cero);

	SELECT 	Var_TotSaldoInsoluto,	Var_CapitalNeto,	Var_CapitalNeto as Var_CapitalNetoMes, 	Var_PorcentualCredPF,
			Var_PorcentajeCredPF;

	DROP TABLE TMPCREDITOSMAYORSALDOPF;
	DROP TABLE TMPSALDOCAPITALNETO;
