...
-- ---------------------------------------------------------------------------------
-- TMPPLANPAGOSGRUPALREP
DELIMITER ;
DROP PROCEDURE IF EXISTS `TMPPLANPAGOSGRUPALREP`;
DELIMITER $$

CREATE PROCEDURE `TMPPLANPAGOSGRUPALREP`(
/* ========= SP QUE MUESTRA EL PLAN DE PAGOS (AMORTIZACIONES) GRUPALES ==========*/
    Par_GrupoID         INT,				-- Parametro Grupo ID
...
DECLARE Var_TipoCredito         VARCHAR(250);
DECLARE Var_MontoCreditoLetra   VARCHAR(200);
...
DECLARE Tipo_EncaAlternativa    INT;
...
SET Est_Activo          := 'A';
SET Int_Presiden        := 1;
SET Tipo_DatosCred      := 1;
SET Tipo_DatosGrupo    	:= 2;
SET Lis_Integra    		:= 3;
SET Tipo_Amortizacion   := 4;
SET Tipo_Enca           := 5;
...
SET Tipo_Alternativa    := 8;
SET Tipo_EncaAlternativa := 9;
...
IF(Par_TipoReporte = Tipo_Amortizacion) THEN
	...
	IF EXISTS (SELECT Estatus
				FROM CREDITOS
					WHERE GrupoID = Par_GrupoID
                    -- Se agrega que tome igual los Vencidos.
					AND Estatus in(EstatusAutorizado,EstatusInactivo,'V', 'B','P'))THEN
        SELECT	MIN(Amo.AmortizacionID) AS AmortizacionID,					MIN(Amo.FechaInicio) AS FechaInicio,					MIN(Amo.FechaVencim) AS FechaVencim,				Amo.FechaExigible,						SUM((Amo.Capital+Amo.Interes+Amo.IVAInteres+Amo.MontoOtrasComisiones+Amo.MontoIvaOtrasComisiones)) AS montoCuota,
                SUM(Amo.Interes) AS Interes,		SUM(Amo.IVAInteres) AS IVAinteres, SUM(Amo.MontoOtrasComisiones) AS OtrasComi, SUM(Amo.MontoIvaOtrasComisiones) AS IVAOtrasComi,	SUM(Amo.Capital) AS Capital,	FORMAT(SUM(Cre.MontoCredito),2) AS MontoCredito,
		...
		ELSE
			SELECT	MIN(Pag.AmortizacionID) AS AmortizacionID,					MIN(Pag.FechaInicio) AS FechaInicio,					MIN(Pag.FechaVencim) AS FechaVencim,				Pag.FechaExigible,						SUM((Pag.Capital+Pag.Interes+Pag.IVAInteres)) AS montoCuota,
                SUM(Pag.Interes) AS Interes,		SUM(Pag.IVAInteres) AS IVAinteres,	SUM(Pag.Capital) AS Capital,SUM(Amo.MontoOtrasComisiones) AS OtrasComi, SUM(Amo.MontoIvaOtrasComisiones) AS IVAOtrasComi,		FORMAT(SUM(Cre.MontoCredito),2) AS MontoCredito,
                MIN(Gru.NombreGrupo) AS NombreGrupo,					MIN(Cre.TasaFija) AS TasaFija,						MIN(Gru.CicloActual) AS CicloActual,				NombreInstitu,							DireccionInstitu,
                Sucurs,								MIN(Cre.FactorMora) AS FactorMora
            FROM	 PAGARECREDITO		Pag,
                ...
                AND Amo.CreditoID          = Cre.CreditoID
                AND Pag.CreditoID          = Amo.CreditoID
                AND Gru.EstatusCiclo = EstCerrado
                GROUP BY Pag.FechaExigible,Pag.AmortizacionID;
		END IF;
END IF;
