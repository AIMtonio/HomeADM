...
-- PAGOCREDCONTAGROPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `PAGOCREDCONTAGROPRO`;

DELIMITER $$
CREATE PROCEDURE `PAGOCREDCONTAGROPRO`(
	/* SP DE PROCESO QUE REALIZA VALIDACIONES PARA PAGO DE CREDITO CONTINGENTE */
	...
	ManejoErrores:BEGIN
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
								'Disculpe las molestias que esto le ocasiona. Ref: SP-PAGOCREDCONTAGROPRO');
		END;

		SET Aud_FechaActual     := NOW();
		SET Var_AltaPoliza		:= Constante_NO;
		SET Var_Poliza 			:= Par_Poliza;
		SET Par_TotalAdeudo		:= IFNULL(Par_TotalAdeudo,Entero_Cero);
		SET Par_MontoPagar		:= IFNULL(Par_MontoPagar,Entero_Cero);
		SET Par_PagoExigible	:= IFNULL(Par_PagoExigible,Entero_Cero);

		IF( Par_TotalAdeudo = Entero_Cero ) THEN
			SET Par_NumErr := 1;
		...
		IF(Par_MontoPagar<=Par_PagoExigible) THEN
			-- Procedimiento del Pago del Credito contingente "ordinario"
			CALL PAGOCREDITOCONTPRO(
		...
		ELSE-- SI el monto ingresado es MAYOR al pago exigible Y sea MENOR al total adeudo y
			...
			IF((Par_MontoPagar>Par_PagoExigible)AND(Par_MontoPagar<Par_TotalAdeudo))THEN
				...
				IF(Var_DiferenciaPago>Decimal_Cero) THEN-- si la diferencia es mayor a cero
					...
					CALL PREPAGOCREDITOCONTPRO(
						...
						Par_ErrMen,			Par_Consecutivo,	Par_ModoPago,			Par_Origen,			Aud_Usuario,
			...
			ELSE -- Si no, se comprueba que el monto a pagar sea igual al total del adeudo
				...
				-- Procedimiento del Pago del Credito "finiquito"
					CALL PAGOCREDITOCONTPRO(
						Par_CreditoID,		Par_CuentaAhoID,	Par_MontoPagar,		Par_MonedaID,		Constante_NO,
						Constante_SI,		Par_EmpresaID,		Constante_NO,		Var_AltaPoliza,		Var_MontoPagado,
						Var_Poliza,			Par_NumErr,			Par_ErrMen,			Par_Consecutivo,	Par_ModoPago,
						Par_Origen,			Aud_Usuario,		Aud_FechaActual,	Aud_DireccionIP,	Aud_ProgramaID,
						Aud_Sucursal,		Aud_NumTransaccion	);
				...
				ELSE
					IF(Par_MontoPagar>Par_TotalAdeudo)THEN
						SET Par_NumErr := 4;
						SET Par_ErrMen := CONCAT('El Monto No debe ser Mayor al Monto del Adeudo Total del Credito Contingente.');
		...
		SET Par_ErrMen		:= 'Pago Contingente Aplicado Exitosamente';
