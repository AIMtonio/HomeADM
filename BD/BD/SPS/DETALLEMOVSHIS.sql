...
-- DETALLEMOVSHIS
DELIMITER ;
DROP PROCEDURE IF EXISTS `DETALLEMOVSHIS`;DELIMITER $$

CREATE PROCEDURE `DETALLEMOVSHIS`(
    Par_SucursalID      int,
    Par_CajaID          int,
    Par_Fecha           date,
...
OPEN CURSORDETALLEMOVS;
		BEGIN
			...
			FETCH CURSORDETALLEMOVS into
				...
				set Var_Descripcion := concat(Var_Descripcion, LPAD(Var_Hora,8,' '));
				set Var_Descripcion := concat(Var_Descripcion,  LPAD(format(Var_Monto, 2), 31, ' '));
