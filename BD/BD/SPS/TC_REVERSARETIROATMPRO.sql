...
-- ---------------------------------------------------------------------------------
-- TC_REVERSARETIROATMPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `TC_REVERSARETIROATMPRO`;

DELIMITER $$
CREATE PROCEDURE `TC_REVERSARETIROATMPRO`(
	-- Store Procedure de Reversa Total de Retiro de ATM Para tarjetas de credito por medio de Web Service
	...
	ManejoErrores:BEGIN
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen	= CONCAT('El SAFI ha tenido un problema al concretar la operacion. Disculpe las molestias que ',
					 				'esto le ocasiona. Ref: SP-TC_REVERSARETIROATMPRO');
		...
		IF( Par_EsReversaParcial = Con_ReversaParcialSI ) THEN
			...
			SET Var_MontoReversa := Par_AmountDispensed;
		END IF;
		...
		CALL TC_CONTALINEAPRO (
			...
			Mov_CapOrdinario,		Nat_Abono,			Par_CodigoTransaccionID,	AltaMovBitacoraNO,		Cadena_Vacia,
			Par_TranCode,
		...
		CALL TC_CONTALINEAPRO (
			...
			Entero_Cero,			Nat_Abono,			Par_CodigoTransaccionID,	AltaMovBitacoraNO,		Cadena_Vacia,
			Par_TranCode,
			Salida_NO,				Par_NumErr,			Par_ErrMen,					Var_Consecutivo,		Par_EmpresaID,
			Aud_Usuario,			Aud_FechaActual,	Aud_DireccionIP,			Aud_ProgramaID,			Aud_Sucursal,
			Aud_NumTransaccion);
