...
-- ---------------------------------------------------------------------------------
-- SUBCTASUCURARRENDAALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `SUBCTASUCURARRENDAALT`;DELIMITER $$

CREATE PROCEDURE `SUBCTASUCURARRENDAALT`(
	...
	Par_SucursalID			INT(11),		-- Indica la moneda de arrendamiento
	...
	ManejoErrores:BEGIN
		DECLARE EXIT HANDLER FOR SQLEXCEPTION
			BEGIN
				...
				SET	Par_ErrMen	= CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
														'Disculpe las molestias que esto le ocasiona. Ref: SP-SUBCTASUCURARRENDAALT');
		...
		INSERT	INTO	SUBCTASUCURARRENDA(
			ConceptoArrendaID,		SucursalID,			SubCuenta,			EmpresaID,		Usuario,
			FechaActual,			DireccionIP,		ProgramaID,			Sucursal,		NumTransaccion
		) VALUES (
			Par_ConceptoArrendaID,	Par_SucursalID,		Par_SubCuenta,		Aud_EmpresaID,	Aud_Usuario,
		...
		SET	Par_NumErr	:= 000;
		SET	Var_Control	:= 'conceptoArrendaID';
		SET	Par_ErrMen	:= CONCAT('SubCuenta Sucursal Agregada Exitosamente: ',Par_SubCuenta);
