...
-- ---------------------------------------------------------------------------------
-- CONTRATOSCREDAGROREP
DELIMITER ;
DROP PROCEDURE IF EXISTS `CONTRATOSCREDAGROREP`;DELIMITER $$

CREATE PROCEDURE `CONTRATOSCREDAGROREP`(
	...
	IF (Par_TipoReporte = Con_IntegraGrupo) THEN
		SELECT
			Ing.GrupoID,	Cli.NombreCompleto,		Cli.ClienteID,		Cli.CURP,		Cli.Telefono,
			Dir.DireccionCompleta
		FROM INTEGRAGRUPOSCRE as Ing
		LEFT JOIN CLIENTES AS Cli ON Ing.ClienteID = Cli.ClienteID
		LEFT JOIN DIRECCLIENTE AS Dir ON Cli.ClienteID = Dir.ClienteID AND Dir.Oficial=DirOficial
		WHERE GrupoID = Par_GrupoID AND Ing.Estatus   = Est_Activo ;
	END IF;
	...
	IF(Par_TipoReporte = FirmaDeudores) THEN
		...
		BEGIN
			...
			IF(contador = 1)THEN
				...
				SET Tmp_Descripcion2 = CONCAT(Cadena_Vacia)
				WHERE Folio = Var_NumRegTemporal-6;
	...
	IF (Par_TipoReporte = TipoCttoGpalSTF) THEN
		...
		SELECT	Sol.CreditoID,	Sol.ClienteID, SUM(Amo.Capital),SUM(Amo.Capital + Amo.Interes + Amo.IVAInteres)
			FROM	SOLICITUDCREDITO Sol,
					AMORTICREDITO	Amo
		...
		SELECT	IF(TipCobComMorato = N_VecesLaTasa,(Cre.TasaFija*PRO.FactorMora)/100 ,PRO.FactorMora )
