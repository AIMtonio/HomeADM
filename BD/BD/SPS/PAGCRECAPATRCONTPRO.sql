...
-- PAGCRECAPATRCONTPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `PAGCRECAPATRCONTPRO`;DELIMITER $$

CREATE PROCEDURE `PAGCRECAPATRCONTPRO`(
/*SP para el Pago de capital atrasado Creditos contingentes*/
	...
	SET Con_CapAtrasado 	:= 82;			-- Concepto Contable: Resultados. Recup. Capital Cartera Castigada
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
        BEGIN
            ...
            SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al
                concretar la operacion. Disculpe las molestias que ', 'esto le ocasiona. Ref: SP-PAGCRECAPATRCONTPRO');
        END;
		...
		IF (Par_Monto > Decimal_Cero) THEN
			...
			CALL  CONTACREDITOSCONTPRO (
				...
				Entero_Cero,            Par_Poliza,         AltaPolCre_SI,      AltaMovCre_SI,      Con_CapAtrasado,
			...
			CALL  CONTACREDITOSCONTPRO (
				...
				Mov_CapAtrasado,        Nat_Cargo,          AltaMovAho_NO,      Cadena_Vacia,       Cadena_Vacia,
				Salida_NO,				Par_NumErr,         Par_ErrMen,         Par_Consecutivo,    Par_EmpresaID,
                Par_ModoPago,			Aud_Usuario,		Aud_FechaActual,	Aud_DireccionIP,    Aud_ProgramaID,
                Aud_Sucursal,       	Aud_NumTransaccion);
