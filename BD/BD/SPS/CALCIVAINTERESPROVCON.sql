...
-- ---------------------------------------------------------------------------------
-- CALCIVAINTERESPROVCON
DELIMITER ;
DROP PROCEDURE IF EXISTS `CALCIVAINTERESPROVCON`;
DELIMITER $$


CREATE PROCEDURE `CALCIVAINTERESPROVCON`(
	-- --------------------------------------------------------------------------
	-- SP para calcular el iva del los intereses de las amortizaciones de credito
	-- --------------------------------------------------------------------------
...
TerminaStore: BEGIN
	...
	END AS SaldoIVAInteres,
		...
		(SaldoComServGar + SaldoOtrasComis) AS SaldoOtrasComis,
		(ROUND(SaldoOtrasComis * Var_IVA, 2) + ROUND(SaldoComServGar * Var_IVA, 2)) AS SaldoIVAOtrCom,
