...
-- ---------------------------------------------------------------------------------
-- BALANCEINTERNO004REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `BALANCEINTERNO004REP`;

DELIMITER $$
CREATE PROCEDURE `BALANCEINTERNO004REP`(
/*sp para obetner el balance general*/
  Par_Ejercicio       INT,
  Par_Periodo         INT,
  Par_Fecha           DATE,
    ...
    IF (Var_FechaSaldos = Fecha_Vacia) THEN
      ...
      INSERT INTO TMPCONTABLE
        ...
        WHERE Fin.EstadoFinanID = Tif_Balance
          ...
          AND NumClien = NumCliente
          GROUP BY Fin.ConceptoFinanID, Fin.NombreCampo, Cue.Naturaleza, Fin.Desplegado;
