DELIMITER ;
DROP PROCEDURE IF EXISTS `TD_RETIROATMPRO`;
DELIMITER $$

CREATE PROCEDURE `TD_RETIROATMPRO`(
	-- Store Procedure de Retiro de ATM Para tarjetas de Debito por medio de Web Service
...
TerminaStore: BEGIN
	...
	DECLARE Var_Clasificacion		CHAR(1);			-- Clasificacion del Producto de Debito
	...
	DECLARE Mov_CapRetiro			INT(11);			-- Constante Movimiento de Retiro
	...
	ManejoErrores:BEGIN
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen	= CONCAT('El SAFI ha tenido un problema al concretar la operacion. Disculpe las molestias que ',
					 				'esto le ocasiona. Ref: SP-TD_RETIROATMPRO');
		...
		INTO 	Var_CuentaAhoID,	Var_TipoTarjetaDeb,	Var_ClienteID,	Var_TarjetaDebID,		Var_EstatusTarjeta,
				Var_BloquearATM,
		...
		SET Var_TotalRetiro				:= Par_TransactionAmount;
		...
		IF( Var_MontoDisponible <= Decimal_Cero ) THEN
			SET Par_NumErr	:= 1227;
			SET Par_ErrMen	:= CONCAT('La Cuenta de Ahorro Asociado a la Tarjeta: ',Par_CardNumber,' no cuenta con Saldo disponible.');
			SET Var_Control	:= 'cardNumber';
		...
		CALL TD_CONTACTAAHORROPRO (
			...
			Var_TotalRetiro,		Par_MonedaID,				Var_DesPoliza,				Var_Referencia,			Alta_Enc_Pol_SI,
			ConConta_TarDeb,		Var_PolizaID,				AltaPol_SI,					AltaMov_SI,				Mov_CapRetiro,
			Nat_Cargo,				Par_CodigoTransaccionID,	AltaMovBitacoraNO,			Cadena_Vacia,			Par_TranCode,
