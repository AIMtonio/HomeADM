...
-- ---------------------------------------------------------------------------------
-- BALANCEINTERNO022REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `BALANCEINTERNO022REP`;
DELIMITER $$

CREATE PROCEDURE `BALANCEINTERNO022REP`(
/* SP DE REPORTE PARA BALANCE GENERAL DE ALTERNATIVA 19 */
...
SET Ubi_Actual      := 'A';
SET Ubi_Histor      := 'H';
SET Tif_Balance     := 1;       -- TIPO DE REPORTE FINANCIERO: BALANCE GENERAL
SET Con_ResulNeto   := 45;      -- CORRESPONDE A LA TABLA DE CONCEPESTADOSFIN PARA RESULTADO NETO
SET NumCliente		:= 22;		-- CORRESPONDE A PARAMGENERALES PARA ALTERNATIVA19
SET Con_NO          := 'N';
...
IF (Par_TipoConsulta = Por_Fecha) THEN
	...
	ELSE
        ...
        INSERT INTO TMPSALDOCONTABLE (
			NumeroTransaccion, 	CuentaContable, SaldoInicialDeu, SaldoInicialAcr)
        SELECT
			Aud_NumTransaccion, Sal.CuentaCompleta, SUM(CASE WHEN Tmp.Naturaleza = VarDeudora  THEN
                                        Sal.SaldoFinal
                                    ELSE
                                        Entero_Cero
