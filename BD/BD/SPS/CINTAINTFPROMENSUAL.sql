...
-- CINTAINTFPROMENSUAL
DELIMITER ;
DROP PROCEDURE IF EXISTS `CINTAINTFPROMENSUAL`;
DELIMITER $$


CREATE PROCEDURE `CINTAINTFPROMENSUAL`(
...
TerminaStore: BEGIN
	...
	SELECT  Cli.ClienteID,					Cli.ApellidoPaterno,										Cli.ApellidoMaterno,							Cadena_Vacia AS Adicional,
			Cli.PrimerNombre,				Cli.SegundoNombre,											Cli.TercerNombre,								DATE_FORMAT(Cli.FechaNacimiento,FormatoFecha)AS FechaNacimiento,
			Cli.RFC,						Cli.Titulo AS Prefijo,										Cli.EstadoCivil,								Cli.Sexo,
			Cadena_Vacia AS FDef,			Cadena_Vacia AS InDef,
            IF(/*if(*/ (IFNULL(Dir.Calle,Cadena_Vacia)=Cadena_Vacia AND IFNULL(Dir.NumeroCasa,Entero_Cero)=Entero_Cero)OR(IFNULL(Dir.Calle,Cadena_Vacia) LIKE CalleConocida OR IFNULL(Dir.Calle,Cadena_Vacia) LIKE CalleConocido)/*) if*/ ,
			   /*then*/ DomicilioConocido,
			   /*else*/ CONCAT(LEFT(IFNULL(Dir.Calle,Cadena_Vacia),(40-LENGTH(IF(IFNULL(Dir.NumeroCasa,Entero_Cero)=Entero_Cero,SinNumero,CONCAT(EtiquetaNumero,Dir.NumeroCasa))))
			...
			Est.EqBuroCred AS Estado,		Cadena_Vacia AS member,										Dir.CP,											IF(IFNULL(Cli.Telefono,Entero_Cero)!=Entero_Cero,Cli.Telefono,Cli.TelefonoCelular) AS Telefono,
			Cadena_Vacia AS DirComplemento,
			LEFT(IF(Cli.LugardeTrabajo=Cadena_Vacia,Cli.NombreCompleto,Cli.LugardeTrabajo),40)  AS Empresa,
            IF(/*if(*/ (IFNULL(Dir.Calle,Cadena_Vacia)=Cadena_Vacia AND IFNULL(Dir.NumeroCasa,Entero_Cero)=Entero_Cero)OR(IFNULL(Dir.Calle,Cadena_Vacia) LIKE CalleConocida OR IFNULL(Dir.Calle,Cadena_Vacia) LIKE CalleConocido)/*) if*/ ,
			   /*then*/ DomicilioConocido,
			   /*else*/ CONCAT(LEFT(IFNULL(Dir.Calle,Cadena_Vacia),(40-LENGTH(IF(IFNULL(Dir.NumeroCasa,Entero_Cero)=Entero_Cero,SinNumero,CONCAT(EtiquetaNumero,Dir.NumeroCasa))))
			   ),IF(IFNULL(Dir.NumeroCasa,Entero_Cero)=Entero_Cero,SinNumero,CONCAT(EtiquetaNumero,Dir.NumeroCasa)))) AS CalleNumero2,
            TRIM(LEFT(CONCAT(Col.TipoAsenta,' ',Col.Asentamiento),40))  AS Colonia2,					TRIM(LEFT(Mun.Nombre,40)) AS Municipio2,		TRIM(LEFT(Loc.NombreLocalidad,40))  AS Ciudad2,
            Est.EqBuroCred AS Estado2,		Dir.CP AS CP2,												IF(IFNULL(Cli.Telefono,Entero_Cero)!=Entero_Cero,Cli.Telefono,Cli.TelefonoCelular) AS Telefono2,
			Cadena_Vacia  AS DirComplemento2,Cadena_Vacia AS Salario,									EqC.CreditoIDCte,								Cre.Estatus,
			Cre.CreditoID,	        		DATE_FORMAT(Det.FechaPago,FormatoFecha) AS UltimoPago,		Cadena_Vacia AS Incumplimiento,					Cre.NumAmortizacion,
			Cadena_Vacia AS FrecuenciaBC, 	Entero_Cero AS Saldo,										Cre.MontoCredito,								Entero_Cero AS Vencido,
			...
			Pro.DiasPasoAtraso,				Entero_Cero AS  SaldoTotal,									Cadena_Vacia AS MOP,							Pro.TipoContratoBCID,
			Cadena_Vacia AS Moneda,			Pro.EsGrupal,												Pro.EsRevolvente,								Cli.TipoPersona,
			Cadena_Vacia AS ClaveObervacion,Entero_Cero AS MontoUltPago,								Det.Transaccion,								IFNULL(Cre.PlazoID,Entero_Cero) AS PlazoID,
			Cadena_Vacia AS Nacionalidad,	Cli.LugarNacimiento, 										Entero_Cero AS DiasAtraFecFinCre, 				Cli.PaisResidencia,
            Cadena_Vacia AS NacDomicilio,	Cli.Correo AS CorreoElect,									Cadena_Vacia AS FechaUltPagoVen,				Cli.OcupacionID,
	...
	GROUP BY Cre.CreditoID,			Cli.ClienteID,			Cli.ApellidoPaterno,	Cli.ApellidoMaterno,	Cli.PrimerNombre,		Cli.SegundoNombre,
			 Cli.TercerNombre,		Cli.FechaNacimiento,	Cli.RFC,				Cli.Titulo,				Cli.EstadoCivil,		Cli.Sexo,
			 Dir.Calle,				Dir.NumeroCasa,			Col.TipoAsenta,			Col.Asentamiento,		Mun.Nombre,				Loc.NombreLocalidad,
			 Est.EqBuroCred,		Dir.CP,					Cli.Telefono,			Cli.TelefonoCelular,	EqC.CreditoIDCte,		Cre.Estatus,
			 Cre.CreditoID,	 		Cre.NumAmortizacion,	Cre.MontoCredito, 		Cre.FechaInicio,		Cre.FechTerminacion,	Cli.OcupacionID,
			 Pro.DiasPasoAtraso,	Pro.TipoContratoBCID,	Pro.EsGrupal,			Pro.EsRevolvente,		Cli.TipoPersona,		Cre.PlazoID,
			 Cli.LugarNacimiento, 	Cli.PaisResidencia,		Cli.Correo,				Det.Transaccion,		Det.FechaPago);
	...
	INSERT INTO TMPACTFECHAPAGOCRED(
	   ...
	   GROUP BY Amo.CreditoID, Cre.CreditoID);
	...
	INSERT INTO TMPACTFECHAPAGOCRED(
		...
		GROUP BY CreditoID, Det.CreditoID);
...
-- ===== La Fecha de Incumplimiento se actualizan de acuerdo al comportamiento del Cliente durante el mes que se reporta
	...
	INSERT INTO TMPSUMASALDOCAPCRED(
		SELECT CreditoIDSAFI,
			   IFNULL(ROUND((Sal.salCapVigente + Sal.SalCapAtrasado +
				Sal.SalCapVencido + Sal.SalCapVenNoExi)),Entero_Cero) AS Saldo,
			   IFNULL(ROUND((Sal.salCapAtrasado)+(Sal.SalCapVencido)
