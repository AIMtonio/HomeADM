...
-- CONTRATOINDIV010REP
DELIMITER ;
DROP PROCEDURE IF EXISTS CONTRATOINDIV010REP;

DELIMITER $$
CREATE PROCEDURE `CONTRATOINDIV010REP`(
	-- SP QUE OBTIENE TODOS LOS DATOS UTILIZADOS PARA EL CONTRATO DE MICROCREDITOS (EXCLUSIVO CONSOL)
...
TerminaStore: BEGIN
	...
	DECLARE Var_Generales			INT(11);		-- Constante: Consulta General
	DECLARE Var_Cuotas				INT(11);		-- Constante: Conuslta las Cuotas de un Credito
	...
	IF(Par_TipoConsulta = Var_Generales) THEN
        ...
        ELSE
             ...
             SELECT 	C.NombreCompleto,		E.EscrituraPublic,	E.FechaEsc,			E.Notaria,				M.Nombre,
						Es.Nombre, 				D.FolioMercantil,	N.Titular,			FNFECHACOMPLETA(C.FechaNacimiento,3)
				INTO	Var_NomApoderadoLegal, 	Var_NumEscPub, 		Var_FechaEscPub, 	Var_NumNotariaPub,		Var_NomMunicipioEscPub,
						Var_NomEstadoEscPub, 	Var_FolioMercantil,	Var_NombreNotario,	Var_FechaNacRepLegal
					FROM DIRECTIVOS D
		...
		SELECT	Var_CreditoID AS CreditoID,						Var_MontoTotal AS MontoTotal,				Var_PorcBonif AS PorcBonif,					Var_MontoBonif AS MontoBonif,					Var_Plazo AS Plazo,
                ...
                UPPER(Var_DestinoCredito) AS DestinoCredito,	Var_MontoLetra AS MontoLetra;
    ...
    -- Lista los porcentajes de garantía Fira
    ...
    -- Lista de garantías usufructuarias.
