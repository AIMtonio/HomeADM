-- SP CARCREDITOSUSPENDIDOREVERPRO

DELIMITER ;

DROP PROCEDURE IF EXISTS CARCREDITOSUSPENDIDOREVERPRO;

DELIMITER $$


CREATE PROCEDURE CARCREDITOSUSPENDIDOREVERPRO(
    ...
    DECLARE EstatuRegistrado			CHAR(1);		-- Estatus Registrado
	DECLARE EstatuPagado				CHAR(1);		-- Estatus Pagado Del credito
	DECLARE EstatuCancelado				CHAR(1);		-- Estatus Cnacelado Del credito

	DECLARE Var_DescripCan				VARCHAR(80);	-- Descripcion del Traspaso al Suspendido
	...
	DECLARE Var_EstatusCreditoAnt		CHAR(1);		-- Estatus Antes de la Suspención
	DECLARE Var_FechaSistema			DATE;			-- Fecha del Sistema
    DECLARE Var_FechaSuspension			DATE;			-- Fecha de la Suspension del crédito
    ...
    DECLARE Var_ValidaCreQuit			INT(10);		-- Variable de validacion condonaciones realizadas después de la suspensión
	DECLARE Var_ValidaDetPag			INT(10);		-- Variable de validacion pagos realizadas después de la suspensión
	...
	SET EstatuRegistrado				= 'R';			-- Estatus Registrado
    SET EstatuPagado					= 'P';			-- Estatus Pagado Del credito
	SET EstatuCancelado					= 'C';			-- Estatus Cnacelado Del credito

	SET Var_DescripCan					:= "REVERSA DE SUSPENCION DEL CREDITO";		-- Descripcion del Traspaso al Suspendido
	...
	SET Con_PasoCarSusp					:= 1103;	    -- Concepto Contable: Reversa de Traspaso a Cartera Suspendida
...
ManejoErrores:BEGIN
	...
	BEGIN
		SET Par_NumErr = 999;
		SET Par_ErrMen = concat("El SAFI ha tenido un problema al concretar la operación. Disculpe las molestias que esto le ocasiona. Ref: SP-CARCREDITOSUSPENDIDOREVERPRO");
    ...
    IF(Var_FechaSistema = Entero_Cero) THEN
		SET Par_NumErr := 1;
	...
	IF(Var_EstatusCredito != EstatuSuspendido) THEN
		SET Par_NumErr := 3;
		SET Par_ErrMen := 'El Numero de Credito Especificado no se Encuentra Suspendido.';
		SET Var_Control := 'estatus';
		LEAVE ManejoErrores;
	END IF;

    -- Obtenemos el Estatus y FechaSuspension del Crédito que Tenia Antes de la Suspensión
    SELECT EstatusCredito, FechaSuspencion
        INTO Var_EstatusCreditoAnt,Var_FechaSuspension
    FROM CARCREDITOSUSPENDIDO
        WHERE CreditoID = Par_CreditoID
			AND Estatus = EstatuRegistrado;

	IF(Var_FechaSistema != Var_FechaSuspension) THEN
		SET Par_NumErr := 4;
		SET Par_ErrMen := 'No se puede dar reversa a la suspensión de un crédito realizado en una fecha diferecte a la actual.';
		SET Var_Control := 'fechaSuspension';
		LEAVE ManejoErrores;
	END IF;

    -- Valida que no exista pago de credito después de la suspensión
    SELECT 1
		INTO Var_ValidaDetPag
    FROM DETALLEPAGCRE DP
		INNER JOIN CARCREDITOSUSPENDIDO CC ON DP.CreditoID = CC.CreditoID
            WHERE DP.FechaPago >= CC.FechaSuspencion
				...
				AND CC.CreditoID = Par_CreditoID limit 1;

	SET Var_ValidaDetPag :=IFNULL(Var_ValidaDetPag,Entero_Cero);

    IF(Var_ValidaDetPag > Entero_Cero) THEN
		SET Par_NumErr := 5;
		SET Par_ErrMen := 'No se puede dar reversa. Existen pagos realizados posterior a la suspensión.';
		SET Var_Control := 'validaDetPag';
		LEAVE ManejoErrores;
	END IF;

    SELECT 1
		INTO Var_ValidaCreQuit
    FROM CREQUITAS CR
    ...
    SET Var_ValidaCreQuit :=IFNULL(Var_ValidaCreQuit,Entero_Cero);

    IF(Var_ValidaCreQuit > Entero_Cero) THEN
		SET Par_NumErr := 6;
		SET Par_ErrMen := 'No se puede dar reversa. Existen condonaciones realizadas posterior a la suspensión.';
		SET Var_Control := 'validaCreQuit';
		LEAVE ManejoErrores;
	END IF;
	...
	IF(Var_SaldoCapVigente > Entero_Cero) THEN
		...
		CALL CONTACREDITOPRO (	Par_CreditoID,			Entero_Cero,			Entero_Cero,		Var_ClienteID,			Var_FechaSistema,
								...
								Var_SubClasif,			Var_SucursalCliente,	Var_DescripCan,		Var_Referencia,			AltaPoliza_NO,
	...
	IF(Var_SaldoIntNoConta > Entero_Cero) THEN
		-- Realizamos el cargo por el monto de Interes No Contabilizado en la cuenta de orden de cartera vencido
		CALL CONTACREDITOPRO (	Par_CreditoID,			Entero_Cero,			Entero_Cero,		Var_ClienteID,			Var_FechaSistema,
		...
		-- Realizamos el Abono por el monto de Interes No Contabilizado en la cuenta de orden de cartera Suspendido
		CALL CONTACREDITOPRO (	Par_CreditoID,			Entero_Cero,			Entero_Cero,		Var_ClienteID,			Var_FechaSistema,
								Var_FechaSistema,		Var_SaldoIntNoConta,	Var_MonedaID,		Var_ProductoCreditoID,	Var_Clasificacion,
								Var_SubClasif,			Var_SucursalCliente,	Var_DescripCan,		Var_Referencia,			AltaPoliza_NO,
		...
		-- Realizamos el Cago por el monto de Interes No Contabilizado en la cuenta correlativa de cartera vencido
		CALL CONTACREDITOPRO (	Par_CreditoID,			Entero_Cero,			Entero_Cero,		Var_ClienteID,			Var_FechaSistema,
								Var_FechaSistema,		Var_SaldoIntNoConta,	Var_MonedaID,		Var_ProductoCreditoID,	Var_Clasificacion,
								Var_SubClasif,			Var_SucursalCliente,	Var_DescripCan,		Var_Referencia,			AltaPoliza_NO,
	...
	IF(Var_SaldoMoraCarVen > Entero_Cero) THEN
		-- Realizamos el cargo por el monto Total de  Moratorio de cartera vencida  en la Cuenta de orden
		CALL CONTACREDITOPRO (	Par_CreditoID,			Entero_Cero,			Entero_Cero,		Var_ClienteID,			Var_FechaSistema,
		...
		-- Realizamos el Movimiento de abono a la cuenta correlativa de  Moratorio de cartera vencida .
		CALL CONTACREDITOPRO (	Par_CreditoID,			Entero_Cero,			Entero_Cero,		Var_ClienteID,			Var_FechaSistema,
								Var_FechaSistema,		Var_SaldoMoraCarVen,	Var_MonedaID,		Var_ProductoCreditoID,	Var_Clasificacion,
								Var_SubClasif,			Var_SucursalCliente,	Var_DescripCan,		Var_Referencia,			AltaPoliza_NO,
								Entero_Cero,			Var_Poliza,				AltaPolCre_SI,		AltaMovCre_NO,			Con_CorIntMor,
	...
	-- Registro del Estatus a Cancelado
	UPDATE CARCREDITOSUSPENDIDO
		SET Estatus 		= EstatuCancelado,
			Usuario			= Aud_Usuario,
	...
	-- Reaslizamos el cambio de estatus a paso reversa
	UPDATE CREDITOS SET
		Estatus			= Var_EstatusCreditoAnt,
	...
	WHERE CreditoID = Par_CreditoID;
		...
		SET Par_NumErr	:= 0;
		SET Par_ErrMen	:='Proceso de Reversa Realizado Exitosamente.';
