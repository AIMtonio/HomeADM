...
-- TARDEBABONOCREPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `TARDEBABONOCREPRO`;

DELIMITER $$
CREATE PROCEDURE `TARDEBABONOCREPRO`(
	# =====================================================================================
	# ----- SP QUE REGISTRA LOS MOVIMIENTOS DE ABONO Y PAGO DE CREDITO (OXXO) -------
	# =====================================================================================
...
TerminaStore: BEGIN
    ...
    SET DescripcionMov		:= 'ABONO A PAGO DE CREDITO';
    SET Con_DescriMov		:= 'APLICACION DE PAGO A CREDITO';
	SET Con_PagoCre			:= 'ABONO A CREDITO';
	SET TipoMovAhoID		:= 12;
    ...
    SET Var_DesAhorro 		:= CONCAT("ERROR EN TRASPASO DE CUENTA");
	SET Con_NO				:= 'N';
	SET Aplica_Cuenta		:= 'S';
...
# ===== TRANSFERENCIA ENTRE LA CUENTA DE BANCOS Y CUENTA EJE =====
	IF (Par_MontoTransaccion <> Decimal_Cero ) THEN
		...
		VALUES(
				Var_CuentaOrigen,    	Var_NumTransaccion, 	Var_FechaSistema,	Nat_Abono,      	Par_MontoTransaccion,
		...
		IF (Par_MontoTransaccion <> Decimal_Cero ) THEN
			...
			CALL POLIZASAHORROPRO(
				...
				Var_CuentaOrigen,   Par_MonedaID,   	Entero_Cero,        Par_MontoTransaccion,     	Con_DescriMov,
			...
			CALL POLIZATARJETAPRO(
				...
				Con_OperaPagTar,    Par_MonedaID,       Par_MontoTransaccion,	Entero_Cero,    	Con_DescriMov,
		...
		-- Saldo a reportar a la transacción
		-- La transferencia es un movimiento interno, por lo que se debe reportar el saldo de la cuenta al
		-- momento del abono
...
# Valida Cuenta Destino -------
	IF Var_CuentaDestino <> Var_CuentaOrigen AND Var_CuentaDestino <> Entero_Cero THEN
			...
			IF(Var_NumCtaDest = Entero_Cero) THEN
				# Se agrega la cuenta destino para la tranferencia interna
			...
			# SI EL TRASPASO A LA CUENTA CREDITU RED FALLA SE REALIZA EL BLOQUEO DEL SALDO A LA CUENTA EJE(Cuenta ligada a la tarjeta)
			IF(Par_NumErr <> Entero_Cero) THEN
				CALL BLOQUEOSPRO(
					Entero_Cero, 	 Nat_Mov, 			Var_CuentaOrigen,	Var_Fecha,			Par_MontoTransaccion,
					Fecha_Vacia,  	TipBloqTarjeta,		Var_DesAhorro,      Var_NumTransaccion, Cadena_Vacia,
		   ...
		   IF(Par_NumErr = Entero_Cero) THEN
			...
			END IF;
			-- ================ FIN DE BLOQUEO DE SALDO A LA CUENTA EJE
	ELSE -- La Cuenta destino no existe, se bloquea el saldo
		CALL BLOQUEOSPRO(
						Entero_Cero, 	 Nat_Mov, 			Var_CuentaOrigen,	Var_Fecha,			Par_MontoTransaccion,
						Fecha_Vacia,  	TipBloqTarjeta,		Var_DesAhorro,      Var_NumTransaccion, Cadena_Vacia,
						Cadena_Vacia,  	Salida_NO,  		Par_NumErr,      	Par_ErrMen,         Aud_EmpresaID,
