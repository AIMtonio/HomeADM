...
-- ---------------------------------------------------------------------------------
-- EDORESINTERNOFIRAREP
DELIMITER ;
DROP PROCEDURE IF EXISTS `EDORESINTERNOFIRAREP`;DELIMITER $$

CREATE PROCEDURE `EDORESINTERNOFIRAREP`(
	/*REPORTE FINANCIERO DE ESTADO DE RESULTADOS PARA FIRA, ESTE LLAMA A LOS PROCEDIMIENTOS ESPECIFICOS DE CADA CLIENTE*/
	...
	ManejoErrores: BEGIN
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									'Disculpe las molestias que esto le ocasiona. Ref: SP-EDORESINTERNOFIRAREP');
			SET Var_Control := 'sqlException';
		END;
		/*SE OBTIENE EL NUMERO DE CLIENTE ESPECIFICO*/
		SET Var_CliProEsp			:= (SELECT ValorParametro
											FROM PARAMGENERALES
		...
		SET Var_ProcPersonalizado	:=(SELECT NomProc
										FROM CATPROCEDIMIENTOS
											WHERE ProcedimientoID = Con_EdoResFIRA
		...
		IF(Var_ProcPersonalizado != Cadena_Vacia) THEN
			SET Var_Llamada 	:= CONCAT(' CALL ', Var_ProcPersonalizado,' (',Par_Ejercicio,',',Par_Periodo,',',"'",Par_Fecha,"'",",","'",Par_TipoConsulta,"'",",","'",Par_SaldosCero,"'",",","'",Par_Cifras,"'",",",Par_CCInicial,",",Par_CCFinal,",",Aud_EmpresaID,",",Aud_Usuario,",","'",Aud_FechaActual,"'",",","'",Aud_DireccionIP,"'",",","'",Aud_ProgramaID,"'",",","'",Aud_Sucursal,"'",",","'",Aud_NumTransaccion,"'",");");
