-- SP ESQUEMAQUINQUENIOSALT

DELIMITER ;

DROP PROCEDURE IF EXISTS ESQUEMAQUINQUENIOSALT;

DELIMITER $$

CREATE PROCEDURE `ESQUEMAQUINQUENIOSALT`(
	Par_InstitNominaID		INT(11),			-- Numero de Institucion Nomina
	Par_ConvenioNominaID    BIGINT UNSIGNED,	-- Numero de Convenio Nomina
    Par_SucursalID			VARCHAR(500),		-- Numero de Sucursales
	Par_QuinquenioID		VARCHAR(500),		-- Numero Quinquenios
	Par_PlazoID				VARCHAR(5000),		-- Numero Plazos
...
ManejoErrores:BEGIN
	...
	DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
		...
		SET Par_ErrMen = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
								'Disculpe las molestias que esto le ocasiona. Ref: SP-ESQUEMAQUINQUENIOSALT');
	...
	SET Var_EsqQuinquenioID := (SELECT IFNULL(MAX(EsqQuinquenioID),Entero_Cero)+1 FROM ESQUEMAQUINQUENIOS);


	SET Aud_FechaActual := NOW();

	INSERT INTO ESQUEMAQUINQUENIOS (
		EsqQuinquenioID,		InstitNominaID,			ConvenioNominaID,		SucursalID,			QuinquenioID, 
		PlazoID,				EmpresaID,				Usuario,				FechaActual,		DireccionIP,	
		ProgramaID,				Sucursal,				NumTransaccion)
	VALUES(
		Var_EsqQuinquenioID,	Par_InstitNominaID,		Par_ConvenioNominaID,	Par_SucursalID,		Par_QuinquenioID,
		Par_PlazoID,			Par_EmpresaID,			Aud_Usuario,			Aud_FechaActual,	Aud_DireccionIP,
		Aud_ProgramaID,			Aud_Sucursal,			Aud_NumTransaccion);

	SET Par_NumErr	:= 	Entero_Cero;
	SET Par_ErrMen	:= 'Esquema de Quinquenios Grabado Exitosamente.';
	SET Var_Control	:= 'institNominaID';
