...
-- ---------------------------------------------------------------------------------
-- SPEIRECEPCIONESPENBITALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `SPEIRECEPCIONESPENBITALT`;
DELIMITER $$

CREATE PROCEDURE `SPEIRECEPCIONESPENBITALT`(
	...
	Par_CodigoError 			VARCHAR(30),		-- Codigo del error por el cual se bitacorea
	Par_MensajeError 			VARCHAR(400),		-- Mensaje del error por el cual se bitacorea
...
TerminaStore: BEGIN
	...
	ManejoErrores:BEGIN
		DECLARE EXIT HANDLER FOR SQLEXCEPTION
			BEGIN
				...
				SET Par_ErrMen	= CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									'Disculpe las molestias que esto le ocasiona. Ref: SP-SPEIRECEPCIONESPENBITALT');
				SET Var_Control = 'SQLEXCEPTION';
			END;

		IF(IFNULL(Par_CodigoError, Cadena_Vacia) = Cadena_Vacia) THEN
			SET Par_NumErr 	:= 1;
			SET Par_ErrMen 	:= 'Especifique el codigo de error por registrar en bitacora.';
			SET Var_Control	:= 'codigoError';
			LEAVE ManejoErrores;
		END IF;

		IF(IFNULL(Par_MensajeError, Cadena_Vacia) = Cadena_Vacia) THEN
			SET Par_NumErr 	:= 1;
			SET Par_ErrMen 	:= 'Especifique el mensaje de error por registrar en bitacora.';
			SET Var_Control	:= 'mensajeError';
			LEAVE ManejoErrores;
		END IF;
		...
		IF(IFNULL(Var_SpeiRecPenID, Entero_Cero) = Entero_Cero) THEN
			SET Par_NumErr 	:= 3;
			SET Par_ErrMen 	:= 'No se encontro la recepcion para registrar en bitacora.';
			SET Var_Control	:= 'SpeiRecepcionPenID';
			LEAVE ManejoErrores;
		END IF;

		INSERT INTO SPEIRECEPCIONESPENBIT (
					...
					PIDTarea,							CodigoError,							MensajeError,							FechaProceso,						EmpresaID,
			...
			SELECT 	SpeiRecepcionPenID,					TipoPagoID,								TipoCuentaOrd,							CuentaOrd,							NombreOrd,
					...
					Firma,								Folio,									FolioBanxico,							FolioPaquete,						FolioServidor,
					Topologia,							Empresa,								NumTransaccionReg,						NumTransaccionRec,					FolioSpeiRecID,
					PIDTarea,							Par_CodigoError,						Par_MensajeError,						FechaProceso,						EmpresaID,
					Usuario,							FechaActual,							DireccionIP,							ProgramaID,							Sucursal,
					NumTransaccion
				FROM SPEIRECEPCIONESPEN
				WHERE SpeiRecepcionPenID = Par_SpeiRecPenID; 
		...
		SET Par_NumErr	:= 000;
		SET Par_ErrMen	:= CONCAT('Bitacora registrada Exitosamente: ', CONVERT(Par_SpeiRecPenID, CHAR));
