-- SP CONSTANCIARETCTERELACT

DELIMITER ;

DROP PROCEDURE IF EXISTS CONSTANCIARETCTERELACT;

DELIMITER $$

CREATE PROCEDURE `CONSTANCIARETCTERELACT`(
	-- SP creado para actualizar informacion del timbrado de la Constancia de Retencion de Relacionados Fiscales
...
TerminaStore:BEGIN
	...
	DECLARE Var_Control			CHAR(20);		-- Control de Errores
	DECLARE Var_CadenaCFDI		VARCHAR(5000);	-- Numero de Registros
	...
	SET Act_Estatus		:= 2;			 -- Actualiza Estatus del Timbrado
      ...
      DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
		  ...
		  SET Par_ErrMen  = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
							'Disculpe las molestias que esto le ocasiona. Ref: SP-CONSTANCIARETCTERELACT');
		  SET Var_Control = 'SQLEXCEPTION';
		END;

    -- 1.- Actualiza datos CFDI
	IF (Par_NumAct = Act_DatosCFDI)THEN
		SET Aud_FechaActual := (SELECT DATE_FORMAT(CURRENT_TIMESTAMP(), '%Y-%m-%d'));

        SET Var_CadenaCFDI :=(SELECT CadenaCFDI FROM CONSTANCIARETCTEREL WHERE ConstanciaRetID = Par_ConstanciaRetID);

        IF(Var_CadenaCFDI != Cadena_Vacia)THEN
			...
			WHERE ConstanciaRetID = Par_ConstanciaRetID
	...
	-- 2.- Actualiza Estatus del Timbrado
