...
-- BALANZACONTA028REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `BALANZACONTA028REP`;

DELIMITER $$
CREATE PROCEDURE `BALANZACONTA028REP`(
	-- Store Procedure: Para generar la Balanza Contable del Cliente: FINANCIERA TAMAZULA
...
TerminaStore: BEGIN
	...
	DROP TABLE IF EXISTS TMPCUENTASMAYOR;
	CREATE TEMPORARY TABLE TMPCUENTASMAYOR(
		CuentaCompleta	 	VARCHAR(50),
		Nomenclatura		VARCHAR(50),
		KEY `INDEX_TMPCUENTASMAYOR_1` (`CuentaCompleta`));

	INSERT INTO TMPCUENTASMAYOR VALUES
	('130000000000','1300'),
	('130002100000','13000210'),
	('135002100000','13500210'),
	('135000000000','1350'),
	('240110000000','24011006'),
	('240110000000','24011007'),
	('240110000000','24011008'),
	('510502100000','51050210'),
	('649000000000','6490'),
	('770002100000','77000210'),
	('780302100000','78030210'),
	('780303000000','78030316'),
	('780303000000','78030317'),
	('139001000000','139001'),
	('240120000000','2401200000'),
	('870002100000','87000210'),
	('510602100000','51060210'),
	('880302100000','88030210'),
	('120000000000','1200'),
	('240100000000','24010'),
	('240100000000','24011'),
	('240100000000','24019');

	DELETE FROM TMPSALDOSBALANZA WHERE NumTransaccion = Aud_NumTransaccion;
	...
	INSERT INTO TMPBALCUENTACONTABLE (
		NumTransaccion,	CuentaCompleta, CentroCosto, CuentaMayor, Descripcion,
		Naturaleza,		Grupo,			MonedaID)
	SELECT	Aud_NumTransaccion, Cue.CuentaCompleta,
			Cen.CentroCostoID,
			CASE WHEN Cue.Grupo = Tip_Encabezado AND Var_NumSegConta = 1 AND
					  Cue.CuentaCompleta NOT IN ('505090000000','505010000000') THEN
	...
	UPDATE TMPBALCUENTACONTABLE SET
		CuentaMayor = '505090'
	WHERE CuentaCompleta = '505090000000'
	  AND NumTransaccion = Aud_NumTransaccion;

	UPDATE TMPBALCUENTACONTABLE SET
		CuentaMayor = '505010'
	WHERE CuentaCompleta = '505010000000'
	  AND NumTransaccion = Aud_NumTransaccion;
	...
	-- insertamos las sumatorias
	INSERT INTO TMPSALDOSBALANZA (
		CuentaCompleta,	SaldoIniDeudor,	SaldoIniAcredor,	Cargos,
		Abonos,			SaldoDeudor,	SaldoAcreedor,		NumTransaccion)
	SELECT  Tmp.CuentaCompleta,		SUM(Bal.SaldoInicialDeu),	SUM(Bal.SaldoInicialAcre),	SUM(Bal.Cargos),	SUM(Bal.Abonos),
			SUM(Bal.SaldoDeudor),	SUM(Bal.SaldoAcreedor),		Aud_NumTransaccion
	FROM TMPCUENTASMAYOR Tmp
	INNER JOIN TMPBALANZACONTABLE Bal
	WHERE Bal.NumeroTransaccion = Aud_NumTransaccion
	  AND Bal.CuentaContable LIKE CONCAT(Tmp.Nomenclatura,'%')
	  AND Bal.Grupo = Tip_Detalle
	GROUP BY Tmp.CuentaCompleta;

	-- Actualizamos los saldos de la balanza
	UPDATE 	TMPSALDOSBALANZA Tmp ,TMPBALANZACONTABLE Bal SET
		Bal.SaldoInicialDeu = Tmp.SaldoIniDeudor,
		Bal.SaldoInicialAcre = Tmp.SaldoIniAcredor,
		Bal.Cargos = Tmp.Cargos,
		Bal.Abonos = Tmp.Abonos,
		Bal.SaldoDeudor = Tmp.SaldoDeudor,
		Bal.SaldoAcreedor = Tmp.SaldoAcreedor
	WHERE Bal.NumeroTransaccion = Aud_NumTransaccion
	  AND Bal.NumeroTransaccion = Tmp.NumTransaccion
	  AND Bal.CuentaContable = Tmp.CuentaCompleta;
