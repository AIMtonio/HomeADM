...
-- REGULATORIOA1011REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `REGULATORIOA1011REP`;
DELIMITER $$

CREATE PROCEDURE `REGULATORIOA1011REP`(
	/* Genera el regulatorio A1011 para Sofipo */
...
TerminaStore:BEGIN
	...
	SET NumReporte			:= '1011';		-- Clave del reporte
	...
	DROP TABLE IF EXISTS TMPREGULATORIOA1011;
	DROP TABLE IF EXISTS TMPREGULATORIO;

	CREATE TEMPORARY TABLE TMPREGULATORIOA1011(
	...
	CREATE TEMPORARY TABLE TEMPCONCEPESTADOSFINCATMINIMO(
		...
		BalanceSubsidiaria1		DECIMAL(16,2)
	);

	INSERT INTO  TMPREGULATORIOA1011
	...
	IF(Var_NumConceptos > Entero_Cero) THEN
	    ...
	    SELECT BalanceSubsidiaria1 INTO Var_Resultado FROM TMPREGULATORIOA1011 WHERE ClaveEntidad = Con_Resultado;

		UPDATE TMPREGULATORIOA1011 SET
			BalanceSubsidiaria1 = BalanceSubsidiaria1 + Var_Resultado
		WHERE ClaveEntidad IN(Con_CapGanado,Con_CapContable);

	END IF;

    CREATE TEMPORARY TABLE TMPREGULATORIO SELECT * FROM TMPREGULATORIOA1011;
	...
	IF Par_NumRep = Rep_Excel THEN
		...
		FROM TMPREGULATORIOA1011 ORDER BY ClaveEntidad;
