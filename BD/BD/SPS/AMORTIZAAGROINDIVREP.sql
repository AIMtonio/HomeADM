...
-- ---------------------------------------------------------------------------------
-- AMORTIZAAGROINDIVREP
DELIMITER ;
DROP PROCEDURE IF EXISTS `AMORTIZAAGROINDIVREP`;DELIMITER $$

CREATE PROCEDURE `AMORTIZAAGROINDIVREP`(
/*SP para el reporte de amortizaciones de creditos agropecuarios*/
...
IF (Par_TipoReporte = Tipo_DatosCred) THEN
 ...
 END IF;
		...
		IF(Var_TipoCredito != CredReestructura) THEN
			...
			WHERE Amo.CreditoID = Par_CreditoID;
		ELSE
			SELECT COUNT(Amo.CreditoID) INTO Var_NumCuotas
...
IF(	Par_TipoReporte	=	Tipo_PlanPagos	)	THEN
	...
	SET Var_TotInteres		:= 	IFNULL(Var_TotInteres, Entero_Cero);
	SET Var_TasaFlat		:=	( ( (Var_TotInteres / Var_NumAmort) / Var_MontoCred ) / Var_Periodo ) * 30 * 100;
...
IF(Par_TipoReporte = Tipo_AmortizacionZafy) THEN
	...
	IF (Var_TipoCredito = CredReestructura AND Var_EstatusSol = EstDesembolsada)THEN
		...
		SELECT (@ROW:= @ROW+1) AS AmortizacionID, DATE_FORMAT(Amo.FechaExigible, '%d/%m/%Y') AS FechaExigible,
			...
			Amo.SaldoIVAComFalP+ Var_MontoSeguroCuota+ Var_IVASeguroCuota) AS montoCuota, Amo.Interes, Amo.IVAInteres,
