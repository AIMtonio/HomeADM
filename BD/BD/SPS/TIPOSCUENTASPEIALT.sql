...
-- TIPOSCUENTASPEIALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `TIPOSCUENTASPEIALT`;DELIMITER $$

CREATE PROCEDURE `TIPOSCUENTASPEIALT`(
...
ManejoErrores: BEGIN
    ...
    BEGIN
        ...
        set Par_ErrMen = concat('Estimado Usuario(a), Ha ocurrido una falla en el sistema, ' ,
                                 ...
                                 'esto le ocasiona. Ref: SP-TIPOSCUENTASPEIALT');
...
if exists (select TipoCuentaID
			from TIPOSCUENTASPEI
            where TipoCuentaID = Par_TipoCuentaID)then

	call TIPOSCUENTASPEIACT(
		Par_TipoCuentaID,   Par_Descripcion,   ActRegistro,      SalidaNO,        Par_NumErr,
...
insert into TIPOSCUENTASPEI(
	TipoCuentaID,	    Descripcion,		    Estatus,    	    EmpresaID,		Usuario,
    FechaActual,	    DireccionIP,		    ProgramaID,	        Sucursal,	    NumTransaccion)

values (
	Par_TipoCuentaID,	Par_Descripcion,	    Var_Estatus,        Par_EmpresaID,	 Aud_Usuario,
    Aud_FechaActual,    Aud_DireccionIP,	    Aud_ProgramaID,	    Aud_Sucursal,    Aud_NumTransaccion);


        set Par_NumErr := 000;
		set Par_ErrMen := concat('Registro agregado exitosamente: ', convert(Par_TipoCuentaID, char));
		set Var_Control:= 'tipoCuentaID' ;
