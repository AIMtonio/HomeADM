...
-- CATTIPOINGRESOSMOD
DELIMITER ;
DROP PROCEDURE IF EXISTS `CATTIPOINGRESOSMOD`;DELIMITER $$

CREATE PROCEDURE `CATTIPOINGRESOSMOD`(
	/*SP para modificar registros de la tabla CATTIPOINGRESOS */
	...
	ManejoErrores: BEGIN
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen	:= CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
								'Disculpe las molestias que esto le ocasiona. Ref: SP-CATTIPOINGRESOSMOD');
		...
		SET	Par_NumErr			:= 0;
		SET	Par_ErrMen			:= CONCAT('Ingreso Modificado Exitosamente: ', CONVERT(Par_Numero,CHAR));
		SET Var_Control			:= 'tipo';
