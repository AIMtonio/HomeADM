...
-- INVERPAGOCIERRE
DELIMITER ;
DROP PROCEDURE IF EXISTS `INVERPAGOCIERRE`;
DELIMITER $$


CREATE PROCEDURE `INVERPAGOCIERRE`(
-- --------------------------------------------------------------------------------------
...
TerminaStore: BEGIN
	...
	DECLARE Var_EsAutomatico	CHAR(1);		-- Indica si el crédito es Automatico
    DECLARE Var_TipoAutomatico	CHAR(1);		-- Indica el tipo de Credito Automatico que corresponde  -- Alter
    ...
    SET ProcesoCierre	:=	'C';
 	...
 	SET TipoInversion	:= 'I';		-- Automatico por Inversion
		...
		Transaccion:BEGIN
			...
			IF(IFNULL(Var_InversionID, Entero_Cero) != Entero_Cero)THEN
				SELECT Creditos INTO Var_NumCred FROM  TMPINVERCRED
                    ...
                    SELECT EsAutomatico,	TipoAutomatico INTO Var_EsAutomatico, Var_TipoAutomatico
                FROM CREDITOS
                WHERE CreditoID = Var_CredID;

				SET Var_EsAutomatico := IFNULL(Var_EsAutomatico, No_constante);
                SET Var_TipoAutomatico := IFNULL(Var_TipoAutomatico, Cadena_Vacia);
				...
				END IF;
                 ...
                 IF(Var_TipoAutomatico <> TipoInversion) THEN
