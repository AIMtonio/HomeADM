...
-- ---------------------------------------------------------------------------------
-- CATINSTGRDVALORESALT
DELIMITER ;
DROP PROCEDURE IF EXISTS CATINSTGRDVALORESALT;

DELIMITER $$
CREATE PROCEDURE `CATINSTGRDVALORESALT`(
	-- Store Procedure: De Alta del Catalogo de Instrumentos del menu de Guarda Valores
...
TerminaStore: BEGIN
	...
	DECLARE Par_CatInsGrdValoresID	INT(11);		-- ID de Tabla
	...
	ManejoErrores:BEGIN
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									'Disculpe las molestias que esto le ocasiona. Ref: SP-CATINSTGRDVALORESALT');
		...
		CALL FOLIOSAPLICAACT('CATINSTGRDVALORES', Par_CatInsGrdValoresID);
		SET Aud_FechaActual := NOW();

		INSERT INTO CATINSTGRDVALORES (
			CatInsGrdValoresID,		NombreInstrumento,		Descripcion,		Estatus,		EmpresaID,
			Usuario,				FechaActual,			DireccionIP,		ProgramaID,		Sucursal,
			NumTransaccion)
		VALUES(
			Par_CatInsGrdValoresID,	Par_NombreInstrumento,	Par_Descripcion,	Par_Estatus,	Aud_EmpresaID,
