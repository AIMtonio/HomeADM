-- SP CONSTANCIARETIMBRACFDI015PRO

DELIMITER ;

DROP PROCEDURE IF EXISTS CONSTANCIARETIMBRACFDI015PRO;

DELIMITER $$

CREATE PROCEDURE `CONSTANCIARETIMBRACFDI015PRO`(
...
TerminaStore:BEGIN
	...
	ManejoErrores:BEGIN #bloque para manejar los posibles errores
		DECLARE EXIT HANDLER FOR SQLEXCEPTION
		 BEGIN
			...
			SET Par_ErrMen = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
							'Disculpe las molestias que esto le ocasiona. Ref: SP-CONSTANCIARETIMBRACFDI015PRO');
		...
		IF ((Par_TipoPersona = PersonaFisica OR Par_TipoPersona = PersonaFisAct) AND Par_RFC != Cadena_Vacia)THEN
		...
		IF (Par_TipoPersona = PersonaMoral AND Par_RFC != Cadena_Vacia)THEN
		...
		-- Si el RFC es vacio se setea el RFC Generico
