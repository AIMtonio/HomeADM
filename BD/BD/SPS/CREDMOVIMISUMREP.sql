...
-- CREDMOVIMISUMREP
DELIMITER ;
DROP PROCEDURE IF EXISTS `CREDMOVIMISUMREP`;
DELIMITER $$


CREATE PROCEDURE `CREDMOVIMISUMREP`(
# ================================================================
...
TerminaStore: BEGIN
      ...
      DECLARE EXIT HANDLER FOR SQLEXCEPTION
                BEGIN
                    ...
                    SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. Disculpe las molestias que ',
                         'esto le ocasiona. Ref: SP-CREDMOVIMISUMREP');
	...
	(SELECT CreditoID, FechaPago AS Fecha,Pago AS Descripcion, SUM(MontoTotPago) AS Monto, SUM(MontoCapOrd+MontoCapAtr+MontoCapVen) AS PagoCapital,
		...
		SUM(MontoComision+MontoGastoAdmon+MontoAccesorios) AS PagoComisiones,CASE WHEN SUM(MontoIVA) > Entero_Cero THEN SUM(MontoComision+MontoGastoAdmon) * Var_IVA ELSE Decimal_Cero END AS IVAComisiones,Cadena_Vacia,Cadena_Vacia,
