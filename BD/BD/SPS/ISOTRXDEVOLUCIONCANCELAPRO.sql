DELIMITER ;
DROP PROCEDURE IF EXISTS ISOTRXDEVOLUCIONCANCELAPRO;
DELIMITER $$

CREATE PROCEDURE ISOTRXDEVOLUCIONCANCELAPRO(
	...
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
								'Disculpe las molestias que esto le ocasiona. Ref: SP-ISOTRXDEVOLUCIONCANCELAPRO');
		...
		-- SE AGREGAN LAS OPERACIONES VALIDAS
		...
		CALL ISOTRXTARJETADEBVAL(
				Par_TipoOperacion,	Par_NumeroTarjeta,		Var_ClienteID,		Var_CuentaAhoID,	Cadena_Vacia,
				Entero_Cero,		Entero_Cero,			Par_GiroComercial,
		...
		IF(FIND_IN_SET(Var_TipoOperacionID,Var_OperacionExistente) = Entero_Cero)THEN
			SET Par_NumErr	:= 1324;
			SET Par_ErrMen	:= 'Tipo de Operación no disponible para cancelacion o devolucion.';
			SET Var_Control	:= 'numeroTarjeta';
			LEAVE ManejoErrores;
		END IF;
