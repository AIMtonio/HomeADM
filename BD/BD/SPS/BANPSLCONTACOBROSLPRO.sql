-- BANPSLCONTACOBROSLPRO

DELIMITER ;

DROP PROCEDURE IF EXISTS BANPSLCONTACOBROSLPRO;

DELIMITER $$

CREATE PROCEDURE BANPSLCONTACOBROSLPRO(
...
TerminaStore:BEGIN
	...
	ManejoErrores:BEGIN
		DECLARE EXIT HANDLER FOR SQLEXCEPTION
			BEGIN
				SET Par_NumErr 	= 999;
       			SET Par_ErrMen  = CONCAT(	'El SAFI ha tenido un problema al concretar la operacion. ',
											'Disculpe las molestias que esto le ocasiona. Ref: SP-BANPSLCONTACOBROSLPRO');
        	...
        	CALL PSLCONTACOBROSLPRO(    Par_PolizaID,                   Var_MonedaId,               Par_ProductoID,                 Par_ServicioID,                 Par_ClasificacionServ,
                                        Par_TipoUsuario,                Par_NumeroTarjeta,          Par_ClienteID,                  Par_CuentaAhoID,                Par_Producto,
                                        Par_FormaPago,                  Par_Precio,                 Par_Telefono,                   Par_Referencia,                 Par_ComisiProveedor,
                                        Par_ComisiInstitucion,          Par_IVAComision,            Par_TotalComisiones,            Par_TotalPagar,                 Var_FechaDate,
                                        Entero_Cero,                 	Entero_Cero,                Entero_Cero,                   	Par_Canal,                      Par_NumPro,
                                        SalidaNO,                       Par_NumErr,                 Par_ErrMen,                     Aud_EmpresaID,                  Aud_Usuario,
                                        Var_FechaDate,                	Aud_DireccionIP,            Aud_ProgramaID,                 Aud_Sucursal,                   Aud_NumTransaccion);
