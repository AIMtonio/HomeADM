DELIMITER ;
DROP PROCEDURE IF EXISTS `BITACORAPAGMOVILHEADALT`;
DELIMITER $$

CREATE PROCEDURE `BITACORAPAGMOVILHEADALT`(
	...
	Par_Fecha                 DATE,       		-- 'Parametro de auditoria'
...
TerminaStore: BEGIN
	...
	ManejoErrores:BEGIN
		...
		IF(Par_Fecha = Fecha_Vacia ) THEN
			SET Par_NumErr  := 2;
			SET Par_ErrMen  := 'La Fecha es obligatorio';
			LEAVE ManejoErrores;
		END IF;
        ...
        INSERT INTO BITACORAPAGMOVILHEAD(
			DispositivoID, 		 	 Fecha,       		 	 ClaveProm,  		 	 EmpresaID, 		 	 Usuario,
			FechaActual,   		 	 DireccionIP, 		 	 ProgramaID, 		 	 Sucursal,  		 	 NumTransaccion
        )
        VALUES(
			Par_DispositivoID, 		 Par_Fecha,       		 Par_ClaveProm,  		 Aud_EmpresaID, 		 Aud_Usuario,
			Aud_FechaActual,   		 Aud_DireccionIP, 		 Aud_ProgramaID, 		 Aud_Sucursal,  		 Aud_NumTransaccion
        );

        SET Var_Consecutivo := LAST_INSERT_ID();
