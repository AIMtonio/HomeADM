...
-- REGULATORIOA0417REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `REGULATORIOA0417REP`;DELIMITER $$

CREATE PROCEDURE `REGULATORIOA0417REP`(
	-- Genera la llamada al Reporte por tipo de Entidad --
    -- Reemplaza al sp REGCARTERA417REP              --
	Par_Fecha           DATETIME,
...
-- Variables
	...
	SET NumReporte			:= 'A0417';				-- Clave del reporte
	SET Entero_Cero			:= 0;
	SET Cadena_Vacia 		:= '';
	...
	SET Var_Llamada := CONCAT(' CALL ',Var_NombreReporte,
		'("',Par_Fecha,'",',Par_NumReporte,',',Par_EmpresaID,',',Aud_Usuario,',',"'",Aud_FechaActual,"'",',',"'",Aud_DireccionIP,"'",',',"'",IFNULL(Aud_ProgramaID,''),"'",',',Aud_Sucursal,',',Aud_NumTransaccion,');');
