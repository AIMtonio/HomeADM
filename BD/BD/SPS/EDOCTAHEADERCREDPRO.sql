...
-- EDOCTAHEADERCREDPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `EDOCTAHEADERCREDPRO`;DELIMITER $$

CREATE PROCEDURE `EDOCTAHEADERCREDPRO`(
	-- SP para obtener los Datos para el encabezado de los creditos del cliente para el Estado de Cuenta
	...
	DECLARE HeaderCredEdoCta			INT(11);					-- Identificador Datos del encabezado de creditos del Cliente para el Estado de Cuenta
	...
	SET HeaderCredEdoCta				:= 20;						-- Identificador Datos del encabezado de creditos del cliente para el Estado de Cuenta
	...
	ManejoErrores:BEGIN
		...
		IF(Var_CliProEspEdoCta = NumClienteNuevo) THEN
			SET Var_ProcPersonalizado := (SELECT NomProc FROM CATPROCEDIMIENTOS WHERE ProcedimientoID = HeaderCredEdoCta AND CliProEspID = Var_CliProEspEdoCta);
		ELSE
			LEAVE ManejoErrores;
		END IF;
