...
-- PAGCRECAPATRPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `PAGCRECAPATRPRO`;
DELIMITER $$

CREATE PROCEDURE `PAGCRECAPATRPRO`(
	-- PAGO CREDITO CAPITAL ATRASADO
...
TerminaStore: BEGIN
	...
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			SET Par_ErrMen = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
	               			 'Disculpe las molestias que esto le ocasiona. Ref: SP-PAGCRECAPATRPRO');
	...
	END IF;
		...
		SET Par_NumErr	:= 0;
		SET Par_ErrMen	:= 'Pago de Capital Atrasado realizado Exitosamente.';
