...
-- ---------------------------------------------------------------------------------
-- EDOCTARESUMCREDITOS015PRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `EDOCTARESUMCREDITOS015PRO`;
DELIMITER $$


CREATE PROCEDURE `EDOCTARESUMCREDITOS015PRO`(
-- SP que genera informacion de Resumen de Creditos para el estado de cuenta de SOFI EXPRESS
...
SET Con_SI				:= 'S';				-- Tipo de Salida: SI

SET Con_NO				:= 'N';				-- Tipo de Salida: SI
...
SET Var_FechaCorte:= (SELECT MAX(FechaCorte)
	...
	SELECT Par_AnioMes, 	Par_SucursalID, 	ClienteID, 		CreditoID, 			Con_Entero_Cero,
	...
	UPDATE EDOCTARESUMCREDITOS Res, TMP_EDOCTADETALLEPAGCRE Tmp
		SET Res.Capital = IFNULL(ROUND((Tmp.MontoCapOrd + Tmp.MontoCapAtr + Tmp.MontoCapVen), 2),Con_Moneda_Cero),
			Res.Interes = IFNULL(ROUND((Tmp.MontoIntOrd + MontoIntAtr + Tmp.MontoIntVen), 2),Con_Moneda_Cero),
			Res.Moratorios = IFNULL(ROUND(Tmp.MontoIntMora, 2),Con_Moneda_Cero),
			Res.ComFaltaPago = Con_Moneda_Cero,
			Res.OtrasCom = IFNULL(ROUND((Tmp.MontoComision + Tmp.MontoGastoAdmon), 2),Con_Moneda_Cero),

			Res.IVAS = IFNULL(ROUND(Tmp.MontoIVA, 2),Con_Moneda_Cero)
	...
	UPDATE EDOCTARESUMCREDITOS Edo, CREDITOS Cre
		SET Edo.MontoOtorgado= Cre.MontoCredito,
		Edo.TasaFija=Cre.TasaFija,
		Edo.TasaMoratoria=Cre.FactorMora
	WHERE Edo.CreditoID=Cre.CreditoID;
	...
	UPDATE EDOCTARESUMCREDITOS Edo, AMORTICREDITO Cre
		...
		WHERE Edo.CreditoID=Cre.CreditoID AND Cre.Estatus= EstatusAtrasado;


	UPDATE EDOCTARESUMCREDITOS Edo
		 INNER JOIN MONEDAS Mon ON Edo.MonedaID= Mon.MonedaID
	SET Edo.DescMoneda = Mon.DescriCorta
	 WHERE Edo.MontoExigible = Con_Entero_Cero;
	...
	UPDATE EDOCTARESUMCREDITOS edo
	 INNER JOIN TMPINTERESCOMCRED tmp ON edo.CreditoID=tmp.CreditoID
		SET edo.InteresaPagar = IFNULL(tmp.InteresaPagar, Con_Moneda_Cero),
			edo.Comisiones = IFNULL(tmp.Comisiones, Con_Moneda_Cero);
	...
	SELECT ACre.CreditoID, ACre.ClienteID, Cli.PagaIVA, ProdCre.CobraIVAInteres, ProdCre.CobraIVAMora,
	 ...
	 WHERE ACre.ClienteID = Edo.ClienteID
	...
	-- Se crea la tabla temporar para registrar el numero de amortizaciones
	...
	-- Se obtiene el numero maximo de la aportizacion pagada
