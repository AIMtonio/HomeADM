...
-- ---------------------------------------------------------------------------------
-- DEMTAREASEJECUTADASALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `DEMTAREASEJECUTADASALT`;DELIMITER $$

CREATE PROCEDURE `DEMTAREASEJECUTADASALT`(
	-- SP para dar de alta una tarea en la bitacora DEMTAREASEJECUTADAS
	...
	ManejoErrores: BEGIN
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen	= CONCAT(	'El SAFI ha tenido un problema al concretar la operacion. ',
										'Disculpe las molestias que esto le ocasiona. Ref: SP-DEMTAREASEJECUTADASALT');
			SET Var_Control	= 'sqlException';
		END;


		-- Se inician con las validaciones
		IF(Par_TareaID = Cadena_Vacia) THEN
			SET Par_NumErr	:= 1;
			SET	Par_ErrMen	:= 'Para registrar la tarea en la bitacora de tareas es necesario que especifique el ID de la tarea';
			LEAVE ManejoErrores;
		END IF;

		IF(Par_PidTarea = Cadena_Vacia) THEN
			SET Par_NumErr	:= 1;
			SET	Par_ErrMen	:= 'Para registrar la tarea en la bitacora de tareas es necesario que especifique el PID de la tarea';
			LEAVE ManejoErrores;
		END IF;

		INSERT INTO DEMTAREASEJECUTADAS(
			TareaID,		CodigoResuesta,		MensajeRespuesta,		PidTarea,			FechaInicio,
			FechaFin,		EmpresaID,			Usuario,				FechaActual,		DireccionIP,
			ProgramaID,		Sucursal,			NumTransaccion)
		SELECT
			Par_TareaID,	Par_CodigoResuesta,	Par_MensajeRespuesta,	Par_PidTarea,		Par_FechaInicio,
			Par_FechaFin,	Par_EmpresaID,		Aud_Usuario,			Aud_FechaActual,	Aud_DireccionIP,
			Aud_ProgramaID,	Aud_Sucursal,		Aud_NumTransaccion;

		-- El registro se inserto exitosamente
		SET Par_NumErr		:= 0;
		SET	Par_ErrMen	:= CONCAT('La tarea[',Par_TareaID,'] se registro correctamente en la bitacora PID[',Par_PidTarea,']');
