...
-- CINTAINTFMENSUALSUSP014PRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `CINTAINTFMENSUALSUSP014PRO`;
DELIMITER $$


CREATE PROCEDURE `CINTAINTFMENSUALSUSP014PRO`(
/* SP PARA CINTA DE BURO DE CREDITO MENSUAL FORMATO CSV CLIENTE TLR*/
...
TerminaStore: BEGIN
	...
	SELECT  MAX(Cli.ClienteID),					IFNULL(MAX(Cli.ApellidoPaterno),Cadena_Vacia) AS ApellidoPaterno,		IFNULL(MAX(Cli.ApellidoMaterno),Cadena_Vacia) AS ApellidoMaterno,							Cadena_Vacia AS Adicional,
            ...
            IFNULL(MAX(Cli.Titulo),Cadena_Vacia) AS Prefijo,
           ...
           IF((IFNULL(MAX(Dir.Calle),Cadena_Vacia)=Cadena_Vacia AND IFNULL(MAX(Dir.NumeroCasa),Entero_Cero)=Entero_Cero)OR(IFNULL(MAX(Dir.Calle),Cadena_Vacia) LIKE CalleConocida OR IFNULL(MAX(Dir.Calle),Cadena_Vacia) LIKE CalleConocido),
			...
			MAX(Est.EqBuroCred) AS Estado,		Cadena_Vacia AS member,										MAX(Dir.CP),									SUBSTRING(REPLACE(IF(IFNULL(MAX(Cli.Telefono),Cadena_Vacia)!=Cadena_Vacia,MAX(Cli.Telefono),MAX(Cli.TelefonoCelular)), ' ', ''), 1, 10) AS Telefono,
			...
			Cadena_Vacia AS Salario,		MAX(EqC.CreditoIDCte),						MAX(Cre.Estatus),
			Cre.CreditoID,
			...
			Fecha_Vaciaddmmyyyy AS Incumplimiento, MAX(Cre.NumAmortizacion),
			...
			Entero_Cero AS Saldo,			MAX(Cre.MontoCredito),								Entero_Cero AS Vencido,
			DATE_FORMAT(MAX(Cre.FechaInicio), FormatoFecha) AS FechaInicio,
			Entero_Cero AS	ImportePagos,

            DATE_FORMAT(Cre.FechTerminacion,FormatoFecha) AS FechaCierre,
			...
			MAX(Cli.OcupacionID),
            Entero_Cero AS PlazoCredito,	MAX(Cre.MonedaID) AS MonedaID,	MAX(Pro.GraciaMoratorios),			MAX(Pro.DiasPasoAtraso),
            MAX(Cre.FechTerminacion),		MAX(Cre.FechaInicio),			Entero_Cero AS InteresReportado,	Cadena_Vacia AS Cuenta,
	...
	INSERT INTO TMPACTFECHAPAGOCRED
		SELECT Amo.CreditoID, MAX(Amo.AmortizacionID), Fecha_Vacia, Entero_Cero, Fecha_Vacia
	...
	UPDATE TMPACTFECHAPAGOCRED Tmp, AMORTICREDITO Amo SET
		Tmp.FechaPago 	= Amo.FechaExigible,
		Tmp.FechaCierre	= Amo.FechaExigible,
	...
	UPDATE TMPDATOSCTESCREDPAG Det, TMPACTFECHAPAGOCRED Tem SET
		Det.FechaCierre = DATE_FORMAT(Tem.FechaCierre, FormatoFecha),
        Det.FecTerminacion = Tem.FechaCierre
	WHERE Det.CreditoIDSAFI = Tem.CreditoID
      AND Det.Estatus=EstatusPagado
      AND Det.FechaCierre =  Fecha_Vaciaddmmyyyy;
	...
	INSERT INTO TMPDIASATRASOCRE
		SELECT	Tem.CreditoIDSAFI, Amo.FechaExigible
	FROM AMORTICREDITO Amo,
		 TMPDATOSCTESCREDPAG Tem
	WHERE Tem.CreditoIDSAFI = Amo.CreditoID
		AND Amo.FechaExigible <=Par_FechaCorteBC
          AND Amo.Estatus != EstatusPagado
          GROUP BY Tem.CreditoIDSAFI, Amo.FechaExigible;
    ...
    -- Actualiza el prefijo en base al sexo
    UPDATE TMPDATOSCTESCREDPAG SET
		Prefijo = CASE WHEN Sexo = SexoFemenino THEN PrefijoSRITA
				  ELSE PrefijoSR END
	WHERE Prefijo = Cadena_Vacia;
