...
-- ---------------------------------------------------------------------------------
-- BALANCEINTERNO016REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `BALANCEINTERNO016REP`;
DELIMITER $$

CREATE PROCEDURE `BALANCEINTERNO016REP`(
/*sp para obtener balance general*/
  Par_Ejercicio       INT,
  Par_Periodo         INT,
  Par_Fecha           DATE,
...
TerminaStore: BEGIN
  ...
  DECLARE For_DepAcumulada    VARCHAR(200);
  DECLARE Des_DepAcumulada    VARCHAR(200);
  DECLARE Con_DepAcumulada    INT(5);
  ...
  SELECT CuentaContable, Desplegado  INTO For_DepAcumulada, Des_DepAcumulada
    FROM CONCEPESTADOSFIN
    WHERE EstadoFinanID = Tif_Balance
      AND ConceptoFinanID = Con_DepAcumulada
      AND NumClien = NumCliente;

  SET For_DepAcumulada   := IFNULL(For_DepAcumulada, Cadena_Vacia);
  SET Des_DepAcumulada   := IFNULL(Des_DepAcumulada, Cadena_Vacia);
  ...
  SET Var_UltPeriodoCie    := IFNULL(Var_UltPeriodoCie, Entero_Cero);
    ...
    IF (Par_Ejercicio <> Entero_Cero AND Par_Periodo = Entero_Cero) THEN
    SET Par_TipoConsulta := Por_FinPeriodo;
  ELSEIF(Par_Ejercicio=Var_UltEjercicioCie AND Par_Periodo=12) THEN
    SET Par_TipoConsulta := Por_FinPeriodo;
  END IF;
  ...
  IF (Par_TipoConsulta = Por_Fecha) THEN
    ...
    IF (Var_FechaSaldos = Fecha_Vacia) THEN
      ...
      END IF;
       ...
       IF(For_DepAcumulada != Cadena_Vacia) THEN

        CALL `EVALFORMULACONTAPRO`(
          For_DepAcumulada,   Ubi_Actual,     Por_Fecha,          Par_Fecha,        Var_UltEjercicioCie,
          Var_UltPeriodoCie,  Par_Fecha,      Par_AcumuladoCta, Par_CCInicial,    Par_CCFinal,
          Par_EmpresaID,    Aud_Usuario,    Aud_FechaActual,    Aud_DireccionIP,  Aud_ProgramaID,
          Aud_Sucursal,     Aud_NumTransaccion);
        ...
        INSERT INTO TMPBALANZACONTA	(
				...
				CuentaMayor,					CentroCosto)
		VALUES(
				Aud_NumTransaccion, "ActDepAcuTrans",       Cadena_Vacia,     Entero_Cero,    Entero_Cero,
				Entero_Cero,    Entero_Cero,          Par_AcumuladoCta,   Entero_Cero,    Des_DepAcumulada,
				Cadena_Vacia,   Cadena_Vacia);
      END IF;
    ...
    ELSE
      ...
      IF(For_ResulNeto != Cadena_Vacia) THEN
       ...
       IF(For_DepAcumulada != Cadena_Vacia) THEN
        ...
        CALL `EVALFORMULACONTAPRO`(
          For_DepAcumulada,       Var_Ubicacion,  Por_Fecha,          Par_Fecha,      Var_UltEjercicioCie,
          Var_UltPeriodoCie,    Var_FecIniPer,  Par_AcumuladoCta,   Par_CCInicial,    Par_CCFinal,
          Par_EmpresaID,      Aud_Usuario,  Aud_FechaActual,    Aud_DireccionIP,  Aud_ProgramaID,
          Aud_Sucursal,     Aud_NumTransaccion);
  ...
  ELSEIF(Par_TipoConsulta = Por_Peridodo) THEN
    ...
    INSERT INTO TMPBALANZACONTA	(
      ...
      SELECT  Aud_NumTransaccion, Fin.NombreCampo,    Cadena_Vacia,
        ...
        SET Var_FecIniPer = DATE_FORMAT(Par_Fecha,'%Y-%m-01');

      IF(For_ResulNeto != Cadena_Vacia) THEN

        CALL `EVALFORMULACONTAPRO`(
          For_ResulNeto,  Cadena_Vacia, Por_Peridodo,       Par_Fecha,      Var_UltEjercicioCie, -- se agrega variable Var_UltEjercicioCie layala t15652
          Var_UltPeriodoCie,  Var_FecIniPer,    Par_AcumuladoCta,   Par_CCInicial,    Par_CCFinal, -- se agrega variable Var_UltPeriodoCie layala t15652
          Par_EmpresaID,  Aud_Usuario,  Aud_FechaActual,    Aud_DireccionIP,  Aud_ProgramaID,
          Aud_Sucursal,   Aud_NumTransaccion);
      ...
      IF(For_DepAcumulada != Cadena_Vacia) THEN
        ...
        CALL `EVALFORMULACONTAPRO`(
          For_DepAcumulada, Cadena_Vacia, Por_Peridodo,       Par_Fecha,      Par_Ejercicio,
  ...
  ELSEIF(Par_TipoConsulta = Por_FinPeriodo) THEN
    ...
    INSERT INTO TMPBALANZACONTA	(
      ...
      IF(For_ResulNeto != Cadena_Vacia) THEN
        ...
        SET Par_AcumuladoCta := (SELECT SUM(SaldoFinal)*-1
                    FROM SALDOCONTACIERREJER
                    WHERE CuentaCompleta LIKE '51%')
                  + (SELECT SUM(SaldoFinal)
                    FROM SALDOCONTACIERREJER
                    WHERE CuentaCompleta LIKE '52%');
