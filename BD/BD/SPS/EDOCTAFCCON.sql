...
-- EDOCTAFCCON
DELIMITER ;
DROP PROCEDURE IF EXISTS `EDOCTAFCCON`;DELIMITER $$

CREATE PROCEDURE `EDOCTAFCCON`(
...
select distinct(ClienteID),
       ...
       concat(RutaPDFEdoCta, Var_StrAnioMes,'/', LPAD(convert(SucursalID,char), 3, 0) ,'/',LPAD(convert(ClienteID,char), 10, 0),
    ...
    from EDOCTARESUMCTA Edo,
         PARAMETROSSIS Par
    where Edo.SucursalID=Par_Sucursal;
