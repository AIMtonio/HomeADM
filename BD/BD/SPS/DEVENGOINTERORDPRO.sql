...
-- GENERAINTERECREPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `DEVENGOINTERORDPRO`;

DELIMITER $$
CREATE PROCEDURE `DEVENGOINTERORDPRO`(
-- SP para devengo de interes ordinario
    Par_CreditoID       BIGINT,
    Par_AmortizacionID  INT,
    Par_MontoInteres       DECIMAL(16,2),
    Par_PolizaID        BIGINT,
...
DECLARE Var_IntereOrd   DECIMAL(12,4);
...
SET Mov_IntPro    := 14;                  -- Tipo de Movimiento de Credito: Ordinarios
...
SET Con_CueOrdInt   := 11;                  -- Concepto Contable: Cuentas de Orden de Moratorios
SET Con_CorOrdInt   := 12;                  -- Concepto Contable: Correlativa de Orden de Moratorios
...
SET Con_IntDeven   := 19;                  -- Concepto Contable: Interes Ordinario Devengado
SET Con_IntIngreso := 5;         -- Concepto Contable: Ingreso por Interes Ordinario
...
SET Des_CieDia      := 'PROYECCION INTERES ORDINARIO';
SET Ref_GenInt      := 'GENERACION INTERES ORDINARIO';
...
ManejoErrores:BEGIN
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
      ...
      SET Par_ErrMen  := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
              'Disculpe las molestias que esto le ocaciona. Ref: SP-DEVENGOINTERORDPRO');
    END;
  ...
  SET Var_TipoContaMora := IFNULL(Var_TipoContaMora, Cadena_Vacia);
   ...
   SELECT  Cre.CreditoID,   Cre.EmpresaID,           
    ...
    IF( Var_Estatus = Estatus_Vigente) THEN
      SET Mov_AboConta    := Con_IntIngreso;
    ...
    END IF; -- Endif de Var_IntereOrd (Moratorios) mayor que cero

    SET Par_NumErr := 0;
    SET Par_ErrMen := 'Devengo Interes Ordinario Realizado Exitosamente';
