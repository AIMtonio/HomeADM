...
-- CUENTASMAYORMONALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `CUENTASMAYORMONALT`;DELIMITER $$

CREATE PROCEDURE `CUENTASMAYORMONALT`(
...
insert into CUENTASMAYORMON
			values ( Par_ConceptoMonID, Aud_EmpresaID, Par_Cuenta,	Par_Nomenclatura,
					Par_NomenclaturaCR,	Aud_Usuario,		Aud_FechaActual,
					Aud_DireccionIP,		Aud_ProgramaID,		Aud_Sucursal,
					Aud_NumTransaccion
					);
select '000' as NumErr,
	  concat("Cuenta Agregada: ", convert(Par_ConceptoMonID, CHAR))  as ErrMen,
	  'conceptoMonID' as control;
