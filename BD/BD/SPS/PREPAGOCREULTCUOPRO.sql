...
-- PREPAGOCREULTCUOPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `PREPAGOCREULTCUOPRO`;

DELIMITER $$
CREATE PROCEDURE `PREPAGOCREULTCUOPRO`(
	-- SP DE PROCESO QUE REALIZA EL PREPAGO ULTIMAS CUOTAS
...
DECLARE AltaMov_NO		CHAR(1);		-- Alta de Movimiento: NO
...
SET AltaMov_NO			:= 'N';		-- Alta de Movimiento: NO
SET CalculoSalGlob		:= 2;		-- Calculo de Interes por Saldos Globales (Monto Original)
SET TipoActInteres		:= 4;		-- Tipo de Actualizacion (intereses)
...
ManejoErrores: BEGIN
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			SET Par_ErrMen = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
								'Disculpe las molestias que esto le ocasiona. Ref: SP-PREPAGOCREULTCUOPRO');
		END;
	...
	SET Var_TotCapVigente	:= (SELECT SUM(IFNULL(SaldoCapVigente, Entero_Cero) + IFNULL(SaldoCapVenNExi, Entero_Cero))
									FROM AMORTICREDITO
										WHERE CreditoID = Par_CreditoID AND Estatus IN (Esta_Atrasado, Esta_Vigente,Esta_Vencido));
	...
	SELECT  Cli.SucursalOrigen,     Cre.ClienteID,          Pro.ProducCreditoID,    Des.Clasificacion, 		Cre.NumAmortizacion,
			...
			Pro.EsRevolvente,       Pro.TipoCalInteres,		Cre.MontoCredito,		Pro.EsReestructura,		Res.EstatusCreacion,
            Cre.FrecuenciaCap,		Cre.FrecuenciaInt,		Cre.FechaInicio,		Cre.SaldoIntNoConta
			INTO
            ...
            Var_TipoCalInteres,		Var_MontoCredito,		Var_EsReestruc,			Var_EstCreacion,		Var_Frecuencia,
            Var_FrecInteres,		Var_FechaInicioCred,	Var_IntNoCont
	...
	-- Esta validacion solo aplica para prepagos a ultimas cuotas
	IF(Par_MontoPagar >= Var_TotCapVigente) THEN
		SET Par_NumErr      := 10;
		SET Par_ErrMen      := 'No Puede PrePagar el Total del Capital, Por Favor Seleccione la Opcion Total Adeudo.' ;
		LEAVE ManejoErrores;
	END IF;
	...
	ELSE
		...
		UPDATE AMORTICREDITO Amo SET
			...
			AND (SaldoCapVigente + SaldoCapAtrasa + SaldoCapVencido + SaldoCapVenNExi +
				   SaldoInteresOrd + SaldoInteresAtr + SaldoInteresVen + SaldoInteresPro +
				   SaldoIntNoConta + SaldoMoratorios + SaldoMoraVencido + SaldoMoraCarVen +
				   SaldoComFaltaPa + SaldoComServGar + SaldoOtrasComis + SaldoNotCargoRev + SaldoNotCargoSinIVA + SaldoNotCargoConIVA) = Entero_Cero
			  AND Estatus 	!= Esta_Pagado
