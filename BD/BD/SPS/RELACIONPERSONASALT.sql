...
DELIMITER ;
DROP PROCEDURE IF EXISTS `RELACIONPERSONASALT`;
...
-- --------------------------------------------------------------------------------
DELIMITER $$
CREATE PROCEDURE `RELACIONPERSONASALT`(
	/* SP PARA EL ALTA DE PERSONAS RELACIONADAS A LA EMPRESA */
...
TerminaStore: BEGIN
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
		...
		concretar la operacion. Disculpe las molestias que ', 'esto le ocasiona. Ref: SP-RELACIONPERSONASALT');
	...
	IF(Var_NumRegistros = Entero_Cero) THEN
		...
		SET Par_EmpleadoID := (SELECT IFNULL(MAX(PersonaID),Entero_Cero) + 1
							FROM RELACIONPERSONAS);

		# SE INSERTAN LAS PERSONAS RELACIONADAS A LA EMPRESA
		INSERT INTO RELACIONPERSONAS
		(	`PersonaID`, 		`NombrePersona`,		`CURP`,					`RFC`,				`PuestoID`,
			PorcAcciones,
			`EmpresaID`, 		`Usuario`,				`FechaActual`,			`DireccionIP`,		`ProgramaID`,
			`Sucursal`, 		`NumTransaccion`
		)VALUES(
			Par_EmpleadoID,		Par_NombreEmpleado,		Par_CURPRelacionado,	Par_RFCRelacionado,	Par_PuestoIDEmp,
            Par_PorcAcciones,
