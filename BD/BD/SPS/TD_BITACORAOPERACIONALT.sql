-- TD_BITACORAOPERACIONALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `TD_BITACORAOPERACIONALT`;

DELIMITER $$
CREATE PROCEDURE `TD_BITACORAOPERACIONALT`(
	-- Store Procedura para el Alta de las operaciones de tarjetas de debito por medio de Web Service
	...
	Par_Balance					DECIMAL(12,2),	-- Saldo Contable de la Cuenta de Ahorro

	Par_BalanceAvailable		DECIMAL(12,2),	-- Saldo Disponible de la Cuenta de Ahorro
...
TerminaStore: BEGIN
	...
	DECLARE Var_MerchantType	CHAR(4);		-- Giro del comercio valores aceptados(Catalogo)
	...
	ManejoErrores:BEGIN
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen	= CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									'Disculpe las molestias que esto le ocasiona. Ref: SP-TD_BITACORAOPERACIONALT');
		...
		IF(Par_ProdIndicator = Prod_ATM)THEN
			SET Var_MerchantType := Con_GiroATM;
			ELSE
				SET Var_MerchantType := Par_MerchantType;
		END IF;

		-- ID de Tabla
		SET Par_RespaldoID 	:= (SELECT IFNULL(MAX(RespaldoID),Entero_Cero)+1 FROM TD_BITACORAOPERACION FOR UPDATE);

		INSERT INTO TD_BITACORAOPERACION (
		...
		VALUES (
			...
			Par_TransactionDate,	Var_MerchantType,			Par_Reference,			Par_TerminalID,				Par_TerminalName,
		...
		IF( Par_FolioDevolucionID > Entero_Cero ) THEN
			...
			IF( Var_RespaldoID = Entero_Cero) THEN
				SET Par_NumErr:= 1201;
				SET Par_ErrMen:= 'El Numero de Respaldo de Tarjeta de Debito No Existe';
				LEAVE ManejoErrores;
			END IF;

			UPDATE TD_BITACORAOPERACION SET
