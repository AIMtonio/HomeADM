...
-- ---------------------------------------------------------------------------------
-- TC_VENTAGENERICAATMPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `TC_VENTAGENERICAATMPRO`;

DELIMITER $$
CREATE PROCEDURE `TC_VENTAGENERICAATMPRO`(
	-- Store Procedure de Venta Generica de ATM Para tarjetas de credito por medio de Web Service
...
TerminaStore: BEGIN
	...
	DECLARE Con_BloqATM				INT(11);			-- Consulta para obtener si se bloquea ATM
	...
	DECLARE Act_MovRetiro			INT(11);			-- Constante Actualiza Movimiento Retiro ATM
	...
	ManejoErrores:BEGIN
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen	= CONCAT('El SAFI ha tenido un problema al concretar la operacion. Disculpe las molestias que ',
					 				'esto le ocasiona. Ref: SP-TC_VENTAGENERICAATMPRO');
		...
		-- Segmento de Validaciones de la Operacion de Compra/Venta por ATM
		...
		IF( Par_ProdIndicator <> Prod_ATM ) THEN
			SET Par_NumErr	:= 1203;
			SET Par_ErrMen	:= 'El Indicador no es Valido para la Operacion de Compra/Venta ATM.';
			SET Var_Control	:= 'prodIndicator';
			LEAVE ManejoErrores;
		END IF;
		...
		IF( Par_TranCode <> Code_PagoServicio ) THEN
			SET Par_NumErr	:= 1205;
			SET Par_ErrMen	:= 'Codigo de transaccion no es Valido para la Operacion de Compra/Venta ATM.';
			SET Var_Control	:= 'tranCode';
			LEAVE ManejoErrores;
		END IF;
		...
		IF( (Var_MontoDisponible - Var_TotalCompra) < Decimal_Cero ) THEN
			SET Par_NumErr	:= 1325;
		...
		IF( Var_LimiteDispuestoDiario <> Decimal_Cero ) THEN
			...
			IF( (Var_TotalCompra + Var_MontoDispuestoDiario) > Var_LimiteDispuestoDiario ) THEN
				SET Par_NumErr	:= 1226;
			...
			IF( Var_TotalCompra > Var_MontoDispuestoLibre ) THEN
				SET Par_NumErr	:= 1227;
				SET Par_ErrMen	:= 'Ha excedido el limite de Disposicion Diario.';
				SET Var_Control	:= 'cardNumber';
				LEAVE ManejoErrores;
		...
		IF( Var_LimiteDispuestoMes <> Decimal_Cero ) THEN
			...
			IF( (Var_TotalCompra + Var_MontoDispuestoMes) > Var_LimiteDispuestoMes ) THEN
				SET Par_NumErr	:= 1228;
		...
		SET Var_DesPoliza	:= CONCAT("COMPRA/VENTA GENERICA ATM ",Var_Referencia);
		...
		CALL TC_CONTALINEAPRO (
			...
			Var_TotalCompra,		Par_MonedaID,			Var_ProductoCreditoID,		Var_Clasificacion,		Con_SubClasificacion,
		...
		CALL POLIZATARJETAPRO (
			...
			Con_OperacATM,		Par_MonedaID,		Entero_Cero,			Var_TotalCompra,	Var_DesPoliza,
		...
		CALL TARJETACREDITOACT (
			Par_CardNumber,		Var_TotalCompra,	Nat_Cargo,			Act_MovRetiro,
		...
		SET Par_ErrMen := CONCAT('Compra/Venta por ATM realizada Correctamente: ',Par_CodigoTransaccionID,'.');
