...
-- REGB242100006REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `REGB242100006REP`;
DELIMITER $$


CREATE PROCEDURE `REGB242100006REP`(
# =======================================================
...
TerminaStore: BEGIN
	...
	IF( Par_NumReporte = Rep_Excel) THEN
		SELECT 	Var_Periodo AS Periodo,		Var_ClaveEntidad AS ClaveEntidad,		For_0841 AS Formulario,					Localidad,  		MunicipioID,
                ...
                Moneda, 					PersJuridica,    						Genero,									SaldoFinal
		FROM TMPREGB0841;
	...
	IF( Par_NumReporte = Rep_Csv) THEN
		SELECT  CONCAT(
			...
			IFNULL( SaldoFinal,Cadena_Vacia)) AS Valor
		FROM TMPREGB0841;
    END IF;
END TerminaStore$$