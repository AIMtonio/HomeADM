...
-- POLIZACREDITOPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `POLIZACREDITOPRO`;
DELIMITER $$

CREATE PROCEDURE `POLIZACREDITOPRO`(
...
TerminaStore: BEGIN
 ...
 /* Asignacion de Constantes */
	...
	ManejoErrores:BEGIN
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION
			BEGIN
				...
				SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
					'Disculpe las molestias que esto le ocasiona. Ref: SP-POLIZACREDITOPRO');
		...
		ELSE
			...
			IF LOCATE(For_Fondeador, Var_Cuenta) > 0 THEN
				...
				# Termina consulta de institucion de fondeo
	...
	-- Se comenta por motivos de cierre dia, no descomentar.
	/*IF (Salida_SI = Salida_SI) THEN
