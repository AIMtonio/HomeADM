...
-- ---------------------------------------------------------------------------------
-- REGCONSOLIDACIONCARTALIQACT
DELIMITER ;
DROP PROCEDURE IF EXISTS `REGCONSOLIDACIONCARTALIQACT`;

DELIMITER $$
CREATE PROCEDURE `REGCONSOLIDACIONCARTALIQACT`(
...
TerminaStore: BEGIN
	...
	DECLARE	EstatusCred			CHAR(1);		-- Estatus del credito tras la actulizacion;
	...
	ManejoErrores: BEGIN
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen  = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									 'Disculpe las molestias que esto le ocasiona. Ref: SP-REGCONSOLIDACIONCARTALIQACT');
