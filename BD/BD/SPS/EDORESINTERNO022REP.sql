...
-- EDORESINTERNO022REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `EDORESINTERNO022REP`;
DELIMITER $$


CREATE PROCEDURE `EDORESINTERNO022REP`(
/* REPORTE FINANCIERON DE ESTADO DE RESULTADOS PARA ALTERNATIVA 19*/
...
TerminaStore: BEGIN
  ...
  SELECT
    FechaSistema,        EjercicioVigente,    PeriodoVigente
  INTO
  ...
  IF (Par_TipoConsulta = Por_Fecha) THEN
    ...
    ELSE
      ...
      IF(Var_FechaSaldos != Par_Fecha) THEN
        ...
        SELECT
          CuentaCompleta,   FechaCorte,   SUM(SaldoFinal)
          FROM SALDOSCONTABLES
        ...
        SET
          Tmp.SaldoInicialDeu = IF(Naturaleza = VarDeudora,Sal.SaldoFinal,Entero_Cero),
          Tmp.SaldoInicialAcr = IF(Naturaleza = VarAcreedora,Sal.SaldoFinal,Entero_Cero)
