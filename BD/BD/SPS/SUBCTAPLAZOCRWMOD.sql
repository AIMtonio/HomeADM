...
-- SUBCTAPLAZOCRWMOD
DELIMITER ;
DROP PROCEDURE IF EXISTS `SUBCTAPLAZOCRWMOD`;

DELIMITER $$
CREATE PROCEDURE `SUBCTAPLAZOCRWMOD`(
/* MODIFICACIÓN DE SUBCUENTAS PLAZOS CROWDFUNDING. */
...
ManejoErrores: BEGIN
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									'Disculpe las molestias que esto le ocasiona. Ref: SP-SUBCTAPLAZOCRWMOD');
	...
	SET	Par_ErrMen := CONCAT("Subcuenta Modificada Exitosamente. Num. Retiros: ", CONVERT(Par_NumRetiros, CHAR));
	SET Var_Control:= 'numRetiros';
