...
-- PAGONOMINAWSLIS
DELIMITER ;
DROP PROCEDURE IF EXISTS `PAGONOMINAWSLIS`;
DELIMITER $$

CREATE PROCEDURE `PAGONOMINAWSLIS`(
...
TerminaStore: BEGIN
	...
	IF( Par_NumConsulta = Con_Principal ) THEN
		...
		SET Var_Sentencia := ' SELECT Inst.InstitNominaID AS InstitucionID, 		Inst.NombreInstit as NombreInstitucion, 			Cre.CreditoID, 		Cli.ClienteID, ';
		SET Var_Sentencia := CONCAT(Var_Sentencia,'Cli.NombreCompleto as NombreCliente,		IFNULL(Nom.NoEmpleado, "") AS NominaEmpleadoID, 		Cre.MontoCuota, ');
        ...
        SET Var_Sentencia := CONCAT(Var_Sentencia,'    INNER JOIN INSTITNOMINA Inst ON Inst.InstitNominaID = Cre.InstitNominaID ');
        SET Var_Sentencia := CONCAT(Var_Sentencia,'    INNER JOIN CONVENIOSNOMINA Con ON Con.ConvenioNominaID = Cre.ConvenioNominaID AND Con.InstitNominaID  = Inst.InstitNominaID ');
        SET Var_Sentencia := CONCAT(Var_Sentencia,'    INNER JOIN NOMINAEMPLEADOS Nom ON Nom.InstitNominaID = Inst.InstitNominaID AND Nom.ClienteID = Cli.ClienteID ');
        SET Var_Sentencia := CONCAT(Var_Sentencia,'WHERE Cre.Estatus IN ("',Con_Vigente,'","',Con_Vencido,'")');
        SET Var_Sentencia := CONCAT(Var_Sentencia,'AND Inst.InstitNominaID = "',Par_InstitucionID,'" ');
        ...
        SET Var_Sentencia := CONCAT(Var_Sentencia,'ORDER BY Cre.FechaInicio,  Cre.CreditoID;');
