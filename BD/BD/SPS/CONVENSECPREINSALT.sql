...
-- CONVENSECPREINSALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `CONVENSECPREINSALT`;DELIMITER $$

CREATE PROCEDURE `CONVENSECPREINSALT`(
...
SET PreAsamSecc			:= 'PAS'; -- Preinscriocion a asamblea seccional
	...
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
				BEGIN
					...
					SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
											 'Disculpe las molestias que esto le ocasiona. Ref: SP-CONVENSECPREINSALT');
...
-- Validar que el socio solo pueda preinscribirse en una sola y unica fecha de seccional para asisitir.

IF(Par_TipoRegistro = PreAsamSecc)THEN
	...
	IF EXISTS (SELECT NoSocio
	   ...
	   WHERE NoSocio = Par_NoSocio AND YEAR(FechaAsamblea)=YEAR(Par_Fecha) AND SucursalID = Par_SucursalID)THEN
		SET Par_NumErr	:= 14;
		SET Par_ErrMen	:='El socio ya se encuentra preinscrito';
		SET Var_Control	:=  'tipoRegistro' ;
		LEAVE ManejoErrores;
	END IF;
...
IF(Par_TipoRegistro = PreAsamGral)THEN
...
IF(Par_TipoRegistro = PreAsamGral || Par_TipoRegistro = PreAsamSecc)THEN
	...
	-- * Validar que el Socio sea Ahorrador Mayor.
	IF(IFNULL(Var_EsMenorEdad, Var_NO)!= Var_NO) THEN
	SET Par_NumErr := 008;
	SET Par_ErrMen := 'El socio Es Menor.';
	SET Var_Control	:= 'tipoRegistro';
	LEAVE ManejoErrores;
	END IF;
...
SET NumConvenPre:= (SELECT IFNULL(MAX(ConvenPreID),Entero_Cero) + 1
FROM CONVENSECPREINS);
...
INSERT INTO CONVENSECPREINS (
	ConvenPreID,			NoTarjeta,				NoSocio,			NombreCompleto,			FechaRegistro,
	FechaAsamblea,			TipoRegistro,			SucursalID,			EmpresaID,				Usuario,
	FechaActual,			DireccionIP,			ProgramaID,			Sucursal,				NumTransaccion)

VALUES (
	NumConvenPre,			Par_NoTarjeta,			Par_NoSocio,		Par_NombreCompleto,		Var_FechaRegistro,
	Par_Fecha,				Par_TipoRegistro,		Par_SucursalID,		Par_EmpresaID,	   		Aud_Usuario,
	Aud_FechaActual,	   	Aud_DireccionIP,		Aud_ProgramaID,		Aud_Sucursal,	   		Aud_NumTransaccion);

SET Par_NumErr := 000;
SET Par_ErrMen := CONCAT("Registro Agregado Exitosamente: ", CONVERT(NumConvenPre, CHAR));
SET Var_Control := 'tipoRegistro' ;
SET Var_Consecutivo := NumConvenPre;
