...
-- CUENTASMAYORKUBOALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `CUENTASMAYORKUBOALT`;DELIMITER $$

CREATE PROCEDURE `CUENTASMAYORKUBOALT`(
...
insert into CUENTASMAYORKUBO
			values ( Par_ConceptoKuboID,    Par_Cuenta,          Par_Nomenclatura,       Par_NomenclaturaCR, 	Aud_EmpresaID,
              Aud_Usuario,		        Aud_FechaActual,			Aud_DireccionIP,		     Aud_ProgramaID,		    Aud_Sucursal,
              Aud_NumTransaccion
					);
select '000' as NumErr,
	  concat("Cuenta Agregada: ", convert(Par_ConceptoKuboID, CHAR))  as ErrMen,
	  'conceptoKuboID' as control;
