...
-- GENERACOMAPERPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `GENERACOMAPERPRO`;
DELIMITER $$


CREATE PROCEDURE `GENERACOMAPERPRO`(
	/*SP para Generar los asientos contables de la Comision por Apertura de Credito*/
...
TerminaStore: BEGIN
	...
	DECLARE CURSORCOMISIONAPERTURA CURSOR FOR
		SELECT Cre.CreditoID,				Amo.AmortizacionID,			Amo.FechaInicio,		Amo.FechaVencim,			Amo.FechaExigible,
			...
			Des.Clasificacion,				Des.SubClasifID,			Cre.SucursalID,			Cre.MontoComApert,			Cre.IVAComApertura,
			Cre.ComAperCont,				Cre.IVAComAperCont,			Cli.PagaIVA,			Cre.NumAmortizacion, 		Sol.NumAmortizacion,
            Cre.TipoCredito,      			Cre.ComAperReest,			Cre.IVAComAperReest
		FROM AMORTICREDITO Amo INNER JOIN
			 ...
			 DESTINOSCREDITO Des ON Cre.DestinoCreID = Des.DestinoCreID LEFT JOIN
             SOLICITUDCREDITO Sol ON Cre.CreditoID = Sol.Relacionado
		WHERE
			Cre.MontoComApert > Decimal_Cero AND
			(Amo.Estatus	= Estatus_Vigente
				OR Amo.Estatus	= Estatus_Atrasada
                OR Amo.Estatus	= Estatus_Pagada
			...
			AND Amo.FechaLiquida != Cre.FechaInicio;
	...
	SET Aud_ProgramaID			:= 'GENERACOMAPERPRO';
	...
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
								'Disculpe las molestias que esto le ocasiona. Ref: SP-GENERACOMAPERPRO');
		END;
	...
	BEGIN
		...
		LOOP
			FETCH CURSORCOMISIONAPERTURA INTO
				...
				Var_ClasifCre,			Var_SubClasifID,			Var_SucursalCred,		Var_MontoComAp,		Var_IVAComAp,
				Var_MontoCont,			Var_MontoIVACont,			Var_CliPagIVA,			Var_NumAmort,		Var_NumAmortRees,
                Var_TipoCredito,        Var_ComReest,				Var_IVAComReest;
			...
			Transaccion: BEGIN
                ...
                IF(Var_TipoCredito = TipoReestructura) THEN
					SET Var_NumAmort 	:= IFNULL(Var_NumAmortRees, Entero_Cero);
				...
				IF(Var_AmortizacionID != Var_NumAmort) THEN
					SET Var_MontoAmort			:= IF(Var_NumAmort != Entero_Cero, ROUND((Var_MontoComAp)/Var_NumAmort,2), Entero_Cero);	# Obtiene el monto que se abona a la cuenta de Com. por Apert
