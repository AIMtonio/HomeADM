...
-- ---------------------------------------------------------------------------------
-- BALGENERALFIRA010REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `BALGENERALFIRA010REP`;
DELIMITER $$


CREATE PROCEDURE `BALGENERALFIRA010REP`(
/* SP DE REPORTE PARA BALANCE GENERAL DE CONSOL PARA LA CARTERA AGRO (FIRA) */
...
TerminaStore: BEGIN
  ...
  SET Var_ConsecID := (SELECT COUNT(*) FROM CONCEEDOSFINFIRA
	  WHERE EstadoFinanID = TipoRepBalance
  ...
  INSERT INTO CONCEEDOSFINFIRA
	  SELECT EstadoFinanID, 	ConceptoFinanID, 	NumClien, 		Var_ConsecID,		Descripcion,
		...
		WHERE NumClien = NumCliente
			AND EstadoFinanID = TipoRepBalance;
  ...
  SET Var_UltPeriodoCie    := IFNULL(Var_UltPeriodoCie, Entero_Cero);
	...
	/* CONSULTA DEL REPORTE POR FECHA */
      ...
      SELECT
		...
		FROM CONCEEDOSFINFIRA Con
			LEFT OUTER JOIN  SALDOSCONTABLES Sal
				...
				AND Con.EstadoFinanID = TipoRepBalance
  ...
  IF(Par_Cifras = Cifras_Miles) THEN
    UPDATE TMPBALANZACONTAFIRA SET
      SaldoDeudor = ROUND(SaldoDeudor/1000.00, 2)
      WHERE NumeroTransaccion = Aud_NumTransaccion;
  END IF;

	-- SE GUARDAN LAS CUENTAS QUE SON PARTE DE UNA FÓRMULA.
  ...
  INSERT INTO REPMONITOREOFIRA(
    ...
    FROM CONCEPESTADOSFIN AS EDO INNER JOIN TMPBALANZACONTAFIRA BAL ON (EDO.NombreCampo = BAL.CuentaContable)
      WHERE EDO.EstadoFinanID = TipoRepBalance
