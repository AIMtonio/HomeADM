...
-- APORTPROVISIONEXTPRO

DELIMITER ;
DROP PROCEDURE IF EXISTS `APORTPROVISIONEXTPRO`;

DELIMITER $$
CREATE PROCEDURE `APORTPROVISIONEXTPRO`(
/** ===================================================================
...
TerminaStore: BEGIN
	...
	DECLARE Var_PaisResidencia		INT(11);				-- ID del Pais de Residencia.
	...
	DECLARE CURSORINVER CURSOR FOR
		SELECT  AportacionID,		Monto,                  tasaFV,         Tasa,           TasaNeta,
				...
				TipoPeriodo,		PagoIntCapitaliza, 		SaldoCap,		DiasAmo,		TasaISRExt,
				PaisResidencia
	   ...
	   DECLARE EXIT HANDLER FOR SQLEXCEPTION
			BEGIN
				...
				SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. Disculpe las molestias que ',
										'esto le ocasiona. Ref: SP-APORTPROVISIONEXTPRO');
			END;
		...
		INSERT INTO TMPRENDIMIENTOAPORT(
			...
			SaldoISR,				PagaISR,				EmpresaID,				SucursalOrigen,			DiasAmo,
			ClienteID,				FechaPago,				FechaPagoAmo,			FechaInicioAmo,			FechaVencimAmo,
			InteresAmo,				IntRetenerAmo,			SaldoProvAmo,			AmortizacionID,			TipoPersona,
			TipoPeriodo,			PagoIntCapitaliza,		SaldoCap,				PaisResidencia,
			TasaISR,				TasaISRExt)
		SELECT
			ap.AportacionID,		Par_Fecha,              ap.Monto,				ap.TipoAportacionID,	ap.TipoPagoInt,
			...
			amo.SaldoISR,			cte.PagaISR,        	ap.EmpresaID,			cte.SucursalOrigen,		amo.Dias,
			...
			amo.TipoPeriodo,		ap.PagoIntCapitaliza,	amo.SaldoCap,			cte.PaisResidencia,
			FNTASAISREXT(cte.PaisResidencia,1,Fecha_Vacia,Entero_Cero),
			FNTASAISREXT(cte.PaisResidencia,2,Par_Fecha,ap.TasaISR)
			...
			BEGIN
				...
				CICLOCURSORINVER:LOOP
					...
					FETCH CURSORINVER INTO
						...
						Var_TipoPeriodo, 		Var_CapitalizaInt,		Var_SaldoCap,		Var_DiasAmo,		Var_NuevaTasaISR,
						Var_PaisResidencia;
					START TRANSACTION;
					Transaccion:BEGIN
						...
						SET Var_ExisteCambioAport	:=FNEXISTECAMBIOISREXT(Par_Fecha, Par_Fecha, Var_PaisResidencia);
						SET Var_ExisteCambioAmort	:=FNEXISTECAMBIOISREXT(Var_FechaInicioAmo, VarFecPagoAmo, Var_PaisResidencia);
						SET Var_ActRetencion		:=IFNULL(Var_ExisteCambioAport,Const_NO);
						SET Var_FechaCambioTasa		:=FNFECHACAMBIOTASAEXT(1,Var_FechaInicioAmo, VarFecPagoAmo, Var_PaisResidencia);
						...
						IF(Var_ActRetencion = Cons_SI) THEN
							SET Var_FechaFin1erPer	:= (SELECT FechaFin1erPer FROM TMP_AMORTIAPORTISREXT WHERE NumTransaccion = Aud_NumTransaccion AND AportacionID = Var_AportacionID AND AmortizacionID = Var_AmortizacionID);
							SET Var_FechaFin2doPer	:= (SELECT FechaFin2doPer FROM TMP_AMORTIAPORTISREXT WHERE NumTransaccion = Aud_NumTransaccion AND AportacionID = Var_AportacionID AND AmortizacionID = Var_AmortizacionID);
							SET Var_Dias1erPer		:= (SELECT Dias1erPer FROM TMP_AMORTIAPORTISREXT WHERE NumTransaccion = Aud_NumTransaccion AND AportacionID = Var_AportacionID AND AmortizacionID = Var_AmortizacionID);
							SET Var_ISR1erPer		:= (SELECT ISR1erPer FROM TMP_AMORTIAPORTISREXT WHERE NumTransaccion = Aud_NumTransaccion AND AportacionID = Var_AportacionID AND AmortizacionID = Var_AmortizacionID);
							SET Var_Dias2doPer		:= (SELECT Dias2doPer FROM TMP_AMORTIAPORTISREXT WHERE NumTransaccion = Aud_NumTransaccion AND AportacionID = Var_AportacionID AND AmortizacionID = Var_AmortizacionID);
							SET Var_ISR2doPer		:= (SELECT ISR2doPer FROM TMP_AMORTIAPORTISREXT WHERE NumTransaccion = Aud_NumTransaccion AND AportacionID = Var_AportacionID AND AmortizacionID = Var_AmortizacionID);
						...
						SET Var_TotalAmo := (SELECT COUNT(*) FROM AMORTIZAAPORT WHERE AportacionID = Var_AportacionID);
								...
								IF (Var_TipoPagoInt IN (Cons_PagoProg,Cons_PagoVenc) ) THEN
									...
									ELSE
										IF(Var_ExisteCambioAmort = Cons_SI AND Par_Fecha > Var_FechaCambioTasa)THEN
											SET Var_Dias2doPer	:= (SELECT Dias2doPer FROM TMP_AMORTIAPORTISREXT WHERE AportacionID = Var_AportacionID AND AmortizacionID = Var_AmortizacionID);
											SET Var_ISR2doPer	:= (SELECT ISR2doPer FROM TMP_AMORTIAPORTISREXT WHERE AportacionID = Var_AportacionID AND AmortizacionID = Var_AmortizacionID);
											SET Var_IsrDia		:= ROUND((Var_ISR2doPer / Var_Dias2doPer),2);
										ELSE
						...
						-- -------------------------------------------------------------------------------------------
							IF(Var_TasaFV = TasaFija) THEN
								...
								IF (Var_TipoPagoInt IN (Cons_PagoProg,Cons_PagoVenc)) THEN
									...
									SET Var_IsrDia := ROUND((Var_ISRCuota - IFNULL(Var_SaldoISR, Entero_Cero)),2);
								END IF;
							ELSE
		...
		SET Par_ErrMen  := 'Proceso Realizado Exitosamente: APORTPROVISIONEXTPRO';
