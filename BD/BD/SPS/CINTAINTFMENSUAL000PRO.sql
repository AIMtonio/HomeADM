...
-- ---------------------------------------------------------------------------------
-- CINTAINTFMENSUAL000PRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `CINTAINTFMENSUAL000PRO`;
DELIMITER $$

CREATE PROCEDURE `CINTAINTFMENSUAL000PRO`(
...
TerminaStore: BEGIN
  ...
  DECLARE ConsConsolidado       VARCHAR(5);         /*Cadena Consolidado*/
    /* CLAVES MOPS */
  ...
  WHERE Tem.Estatus <> EstatusCastigado;
	...
	DROP TEMPORARY TABLE IF EXISTS TMPEXIGIBLESNOTASCARGOINTFMEN;
	CREATE TEMPORARY TABLE TMPEXIGIBLESNOTASCARGOINTFMEN (
		CreditoID				BIGINT(12),
		Monto					DECIMAL(14,2),
	...
	DROP TEMPORARY TABLE IF EXISTS TMPPAGOSNOTASCARGOINTFMEN;
	CREATE TEMPORARY TABLE TMPPAGOSNOTASCARGOINTFMEN (
		CreditoID				BIGINT(12),
		Monto					DECIMAL(14,2),
		PRIMARY KEY (CreditoID)
	);
	...
	INSERT INTO TMPEXIGIBLESNOTASCARGOINTFMEN (	CreditoID,			Monto,									MontoExigible	)
	...
	INSERT INTO TMPPAGOSNOTASCARGOINTFMEN (	CreditoID,			Monto	)
								SELECT		DET.CreditoID,		ROUND(SUM(ROUND(DET.MontoNotasCargo, 2)), 2)
									FROM	TMPEXIGIBLESNOTASCARGOINTFMEN TMP
	...
	UPDATE TMPEXIGIBLESNOTASCARGOINTFMEN NTC
		INNER JOIN TMPPAGOSNOTASCARGOINTFMEN PAG ON NTC.CreditoID = PAG.CreditoID
	SET	NTC.MontoExigible = ROUND(NTC.Monto - PAG.Monto, 2);

	UPDATE TMPEXIGIBLESNOTASCARGOINTFMEN
	SET	MontoExigible = Entero_Cero
	WHERE MontoExigible < Entero_Cero;

	UPDATE TMPSALDOSCREDITOCINTABC TMP
		INNER JOIN TMPEXIGIBLESNOTASCARGOINTFMEN NTC ON NTC.CreditoID = TMP.CreditoID
  ...
  INSERT INTO TMPFECHAINCUMP
    ...
    FROM TMPDATOSCTESCREDPAG Dat
        INNER JOIN AMORTICREDITO Amo ON Dat.CreditoIDSAFI = Amo.CreditoID
