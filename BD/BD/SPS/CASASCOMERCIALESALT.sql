...
-- CASASCOMERCIALESALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `CASASCOMERCIALESALT`;
DELIMITER $$

CREATE PROCEDURE `CASASCOMERCIALESALT`(
	-- =====================================================================================
	-- ----------------- SP PARA DAR DE ALTA CASAS COMERCIALES  ---------------------
	-- =====================================================================================
...
TerminaStore: BEGIN
    ...
    DECLARE Var_ResultUno           DECIMAL(14,2);
    DECLARE Var_ResultDos           DECIMAL(14,2);
    DECLARE Var_ResultTres          DECIMAL(14,2);
    DECLARE Var_ResultCuatro        DECIMAL(14,2);
    DECLARE Var_ResultCinco         DECIMAL(14,2);
    DECLARE Var_ResultSeis          DECIMAL(14,2);
    DECLARE Var_ResultSiete         DECIMAL(14,2);
    DECLARE Var_ResultOcho          DECIMAL(14,2);
    DECLARE Var_ResultNueve         DECIMAL(14,2);
    DECLARE Var_ResultDiez          DECIMAL(14,2);
    DECLARE Var_ResultOnce          DECIMAL(14,2);
    DECLARE Var_ResultDoce          DECIMAL(14,2);
    DECLARE Var_ResultTrece         DECIMAL(14,2);
    DECLARE Var_ResultCatorce       DECIMAL(14,2);
    DECLARE Var_ResultQuince        DECIMAL(14,2);
    DECLARE Var_ResultDieSeis       DECIMAL(14,2);
    DECLARE Var_ResultDieSiete      DECIMAL(14,2);
    ...
    ManejoErrores: BEGIN
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION
			BEGIN
				...
				SET Par_ErrMen	= CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									'Disculpe las molestias que esto le ocasiona. Ref: SP-CASASCOMERCIALESALT');
        ...
        SET Var_CasaID := (SELECT MAX(CasaComercialID) FROM CASASCOMERCIALES);
        SET Var_CasaID := IFNULL(Var_CasaID,Entero_Cero) + Entero_Uno;

        INSERT INTO CASASCOMERCIALES (
            CasaComercialID,    NombreCasaCom,      TipoDispersionCasa,       InstitucionID,          CuentaCLABE,
            Estatus,            RFC,                EmpresaID,                Usuario,                FechaActual,
            DireccionIP,        ProgramaID,         Sucursal,                   NumTransaccion
        )VALUES(
            Var_CasaID,         Par_NombreCasa,    Par_TipoDispersion,      Par_InstitucionID,          Par_CuentaClabe,
            Par_Estatus,        Par_RFC,           Aud_EmpresaID,           Aud_Usuario,                Aud_FechaActual,
            Aud_DireccionIP,    Aud_ProgramaID,    Aud_Sucursal,            Aud_NumTransaccion);

        SET Par_NumErr	:= Entero_Cero;
		SET Par_ErrMen	:= CONCAT("Casa Comercial Agregada Exitosamente: ",CONVERT(Var_CasaID,CHAR));
