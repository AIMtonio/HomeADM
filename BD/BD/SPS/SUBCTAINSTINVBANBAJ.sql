...
-- SUBCTAINSTINVBANBAJ
DELIMITER ;
DROP PROCEDURE IF EXISTS `SUBCTAINSTINVBANBAJ`;DELIMITER $$

CREATE PROCEDURE `SUBCTAINSTINVBANBAJ`(
    ...
    ManejoErrores: BEGIN
		...
		BEGIN
			...
			SET Par_ErrMen = concat('Estimado Usuario(a), Ha ocurrido una falla en el sistema, ' ,
									 ...
									 'esto le ocasiona. Ref: SP-SUBCTAINSTINVBANBAJ');
		...
		IF(ifnull(Par_ConceptoInvBanID, Entero_Cero))= Entero_Cero then
				Set Par_NumErr	:= '002';
				Set Par_ErrMen	:= 'La Instuccion esta Vacia';
 				Set Var_Control	:= 'ConceptoInvBanID';
			LEAVE TerminaStore;
		END IF;

		IF NOT EXISTS( SELECT ConceptoInvBanID FROM SUBCTAINSTINVBAN WHERE ConceptoInvBanID=Par_ConceptoInvBanID
