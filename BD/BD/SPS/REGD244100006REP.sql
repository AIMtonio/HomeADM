...
-- REGD244100006REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `REGD244100006REP`;DELIMITER $$

CREATE PROCEDURE `REGD244100006REP`(
# ------------------------------------------------------------------------------------------- #
...
# ------------------------------------------------------------------------------------------- #
   ...
   */
    ...
    SELECT reg.CuentaAhoID,		SUM(reg.CantidadMovimiento)  CantidadMov,		COUNT(*) AS NumMov,
           CASE WHEN FIND_IN_SET(reg.TipoMovAhoID,Var_InverDeposito)  > Entero_Cero THEN OperDeposito
               WHEN FIND_IN_SET(reg.TipoMovAhoID,Var_InverRetiro)    > Entero_Cero THEN OperRetiro END AS TipoMovimiento,
		...
		WHERE ti.TipoCuentaID <> Tipo_CtaInst
		GROUP BY reg.CuentaAhoID, reg.TipoMovAhoID, cu.ClienteID
		ORDER BY reg.CuentaAhoID;
   ...
   */
    ...
    SELECT DepositoVista,		OperRetiro,		SUM(det.MontoOpe) monto,		COUNT(det.TarDebMovID) operaciones,
        COUNT(DISTINCT(det.TarjetaDebID)) socios FROM TmpCompras det;
