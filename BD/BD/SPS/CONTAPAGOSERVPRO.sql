...
-- CONTAPAGOSERVPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `CONTAPAGOSERVPRO`;
DELIMITER $$

CREATE PROCEDURE `CONTAPAGOSERVPRO`(
...
SET Procedimiento       := 'CONTAPAGOSERVPRO';
SET Var_Tercero         := 'T';
SET Var_Interno         := 'I';
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
        BEGIN
            ...
            SET Par_ErrMen = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
                                    'Disculpe las molestias que esto le ocasiona. Ref: SP-CONTAPAGOSERVPRO');
        END;
...
IF (Par_AltaEncPol = AltaEncPol_Si) THEN
    ...
    CALL MAESTROPOLIZAALT(
        ...
        Var_ConceptoConDes, Salida_NO,      Aud_Usuario,            Aud_FechaActual,    Aud_DireccionIP,
...
IF(Par_AltaDetPol = AltaDetPol_Si) THEN
    ...
    IF(Var_Origen = Var_Tercero )THEN
        IF(Var_CobraComision = Var_SI )THEN
            SET Var_MontoComision := IFNULL(Var_MontoComision,Par_Comision );
        ...
        IF(IFNULL(Par_MontoServicio, Decimal_Cero))= Decimal_Cero THEN
            IF (Par_Salida = Salida_SI) THEN
                SELECT '001' AS NumErr,
                     'El monto del servicio esta vacia.' AS ErrMen,
                     'comision' AS control,
                     Entero_Cero AS consecutivo;
            ELSE
                SET Par_NumErr := 1;
        ...
        CALL DETALLEPOLIZAALT(
                ...
                Aud_FechaActual,        Aud_DireccionIP,    Aud_ProgramaID,     Aud_Sucursal,       Aud_NumTransaccion);
    ELSE
        SET Var_MontoServicio := IFNULL(Par_MontoServicio,Decimal_Cero );
...
END IF;
    ...
    SELECT ServReactivaCte INTO Var_ServReactivaCte
                                FROM PARAMETROSSIS
                                WHERE ServReactivaCte = Par_CatalogoServID;
    SET Var_ServReactivaCte := IFNULL(Var_ServReactivaCte,Entero_Cero);

    IF(Par_CatalogoServID = Var_ServReactivaCte)THEN
        SET Var_MonTotalReactivaCli = Par_MontoServicio + Par_IvaServicio;

        CALL COBROREACTIVACLIALT (
                    Par_ClienteID,              Par_CajaID,     Par_SucursalID, Par_MonedaID,   Par_Poliza,
                    Var_MonTotalReactivaCli,    Salida_NO,     Par_NumErr,     Par_ErrMen,     Par_EmpresaID,
                    Aud_Usuario,               Aud_FechaActual,Aud_DireccionIP,Aud_ProgramaID, Aud_Sucursal,
                    Aud_NumTransaccion);
