...
-- REGULATORIOC0452REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `REGULATORIOC0452REP`;DELIMITER $$

CREATE PROCEDURE `REGULATORIOC0452REP`(
	/* Genera la llamada al regulatorio C0452 para Sofipo */
...
-- Variables
	...
	SET NumReporte			:= 'C0452';					-- Clave del reporte
	SET Entero_Cero			:= 0;
	SET Cadena_Vacia 		:= '';
	...
	SET Resp_SI				:= 'S';
	SET Resp_SI				:= 'N';
	SET Var_FechaCon		:= CONCAT(Par_Anio,'-',CASE WHEN Par_Mes < 10 THEN '0' ELSE '' END,Par_Mes,'-01');
    SET Var_FechaCon		:= last_day(Var_FechaCon);
	-- Numero del Cliente
	...
	SET Var_Llamada := CONCAT(' CALL ',Var_NombreReporte,
		'("',Var_FechaCon,'",',Par_NumRep,',',Entero_Cero,',',Par_EmpresaID,',',Aud_Usuario,',',"'",Aud_FechaActual,"'",',',"'",Aud_DireccionIP,"'",',',"'",Aud_ProgramaID,"'",',',Aud_Sucursal,',',Aud_NumTransaccion,');');
