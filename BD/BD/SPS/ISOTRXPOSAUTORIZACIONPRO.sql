DELIMITER ;
DROP PROCEDURE IF EXISTS ISOTRXPOSAUTORIZACIONPRO;
DELIMITER $$

CREATE PROCEDURE ISOTRXPOSAUTORIZACIONPRO(
	-- Descripcion: Proceso operativo que realiza los reversos de movimientos re y pre autorizacion.
...
TerminaStore: BEGIN
	...
	DECLARE Var_SaldoAuxiliar			DECIMAL(12,2);	-- Variable para almacenar el auxiliar de saldo
	...
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
								'Disculpe las molestias que esto le ocasiona. Ref: SP-ISOTRXPOSAUTORIZACIONPRO');
		...
		CALL ISOTRXTARJETADEBVAL(
				Par_TipoOperacion,	Par_NumeroTarjeta,		Var_ClienteID,		Var_CuentaAhoID,
				Cadena_Vacia,		Entero_Cero,			Entero_Cero,		Par_GiroComercial,
		...
		WHERE TarjetaDebID = Par_NumeroTarjeta
			...
			AND FechaHrOpe = Var_FechaHrOpe
			AND Estatus = Est_Procesado
			AND TipoOperacionID IN(Tran_PreAutorizacion,Tran_ReAutorizacion)
			AND IFNULL(EstatusConcilia,Cadena_Vacia) <> Est_Conciliado;
		...
		-- Despues de realizar el movimiento de desbloqueo se deberá aplicar un cargo a la cuenta del cliente
		...
		IF(Var_TipoOperacionID IN(Tran_PreAutorizacion,Tran_ReAutorizacion) )THEN
			...
			IF(Var_ExisteOperacion > Entero_Cero)THEN
				...
				SET Var_SaldoAuxiliar	:= Decimal_Cero;

				-- ======================================== TRANSACCION PARA DESBLOQUEAR EL SALDO ========================================
				...
				WHILE (Var_Contador <= Var_RegistroID) DO
					...
					SET Var_SaldoAuxiliar	:= IFNULL(Var_SaldoAuxiliar, Decimal_Cero);

					-- SE DESBLOQUEA EL SALDO DE LA CUENTA
					CALL BLOQUEOSPRO(
						Var_FolioBloqueo,		Nat_Desbloqueo,			Var_CuentaAhoID,		Var_FechaSistema,			Var_Saldo,
						Var_FechaSistema,		Cons_MovTarjeta,		Var_Descripcion,		Var_CodigoAutorizacion,		Var_ClaveUsuario,
					...
					SET Var_SaldoAuxiliar	:= Var_SaldoAuxiliar + Var_Saldo;
					SET Var_Saldo			:= Decimal_Cero;
					SET Var_FolioBloqueo	:= Entero_Cero;
				END WHILE;

				-- ======================================== TRANSACCION PARA EL CARGO ========================================
				...
				IF(Var_SaldoAuxiliar > Var_SaldoDisponible)THEN
					SET Par_NumErr	:= 1002;
				...
				-- Se realiza afetacion por el nuevo monto de Var_SaldoAuxiliar
				CALL ISOTRXCONTAPRO(
						Var_CuentaAhoID,		Var_ClienteID,				Aud_NumTransaccion,			Var_FechaSistema,		Var_FechaSistema,
						Var_SaldoAuxiliar,		Var_Descripcion,			Par_CodigoAutorizacion,		Mov_CompraDebito,		Var_MonedaID,
