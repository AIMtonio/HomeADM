...
-- ---------------------------------------------------------------------------------
-- TIPOTARJETASUBBINLIS
DELIMITER ;
DROP PROCEDURE IF EXISTS `TIPOTARJETASUBBINLIS`;
DELIMITER $$

CREATE PROCEDURE `TIPOTARJETASUBBINLIS`(
...
SET Aud_FechaActual := NOW();
    ...
    IF(Par_NumLis = Lis_Principal) THEN
      SELECT Tar.NumSubBIN,Pat.NombrePatroc,Tar.Estatus
        FROM TIPOTARJETADEB Tar  INNER JOIN CATALOGOPATROCINADOR Pat WHERE Tar.PatrocinadorID = Pat.PatrocinadorID 
        AND Tar.Estatus=EstatusA
        AND Tar.TarBinParamsID=Par_TarBinParamsID;
    END IF;
