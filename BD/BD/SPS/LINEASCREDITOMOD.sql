...
-- LINEASCREDITOMOD
DELIMITER ;
DROP PROCEDURE IF EXISTS `LINEASCREDITOMOD`;

DELIMITER $$
CREATE PROCEDURE `LINEASCREDITOMOD`(
	-- Store procedure para la modificacion de lineas de credito
	...
	ManejoErrores:BEGIN
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen	= CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									 'Disculpe las molestias que esto le ocasiona. Ref: SP-LINEASCREDITOMOD');
		...
		IF( Par_LineaCreditoID = Entero_Cero ) THEN
			SET Par_NumErr	:= 001;
			SET Par_ErrMen	:= 'El Numero de Linea esta Vacio.';
			SET Var_Control	:= 'lineaCreditoID';
			LEAVE ManejoErrores;
		END IF;

		IF(NOT EXISTS(SELECT LineaCreditoID FROM LINEASCREDITO WHERE LineaCreditoID = Par_LineaCreditoID)) THEN
			SET Par_NumErr	:= 002;
		...
		SET Var_TipoComAnual	:= IFNULL(Var_TipoComAnual, Con_NO);
		...
		UPDATE LINEASCREDITO SET
			...
			FolioContrato			= CASE WHEN Par_EsAgropecuario = Con_NO
												THEN Par_FolioContrato
			...
			CobraComAnual			= Var_CobraComAnual,
			TipoComAnual			= Var_TipoComAnual,

			ValorComAnual			= Var_ValorComAnual,
			Solicitado				= Par_Solicitado,
			EsAgropecuario			= Par_EsAgropecuario,
			TipoLineaAgroID			= Par_TipoLineaAgroID,
