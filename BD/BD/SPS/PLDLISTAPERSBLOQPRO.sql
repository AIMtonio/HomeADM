...
-- PLDLISTAPERSBLOQPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `PLDLISTAPERSBLOQPRO`;DELIMITER $$

CREATE PROCEDURE `PLDLISTAPERSBLOQPRO`(
/* SP DE PROCESO DE CARGA DE LISTAS DE PERSONAS BLOQUEADAS*/
	...
	ManejoErrores: BEGIN
		...
		SET Var_PersonaBloqID := (SELECT MAX(PersonaBloqID) FROM PLDLISTAPERSBLOQ);
		SET Var_PersonaBloqID := IFNULL(Var_PersonaBloqID, Entero_Cero);
		SET Aud_FechaActual := NOW();

		INSERT INTO PLDLISTAPERSBLOQ(
			PersonaBloqID,													PrimerNombre,
			...
			NumTransaccion)
		SELECT
			(IdListaNegraTemp+Var_PersonaBloqID),								IFNULL(LEFT(TRIM(UPPER(IF(Tmp.Nombre IS NULL,Razonsoc, Tmp.Nombre))),100),Cadena_Vacia),
			'',																	'',
		...
		SET	Par_NumErr := 0;
		SET	Par_ErrMen := CONCAT('Proceso de Carga de Listas de Personas Bloqueadas Exitoso.');
