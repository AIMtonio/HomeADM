...
-- TC_RETIROATMPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `TC_RETIROATMPRO`;

DELIMITER $$
CREATE PROCEDURE `TC_RETIROATMPRO`(
	-- Store Procedure de Retiro de ATM Para tarjetas de credito por medio de Web Service
...
TerminaStore: BEGIN
	...
	DECLARE Var_MontoDispuestoLibre		DECIMAL(12,2);	-- Monto de Dispuesto libre
	DECLARE Var_MontoDispuestoMes		DECIMAL(12,2);	-- Monto Dispuesto al mes
	...
	DECLARE Var_TotalRetiro				DECIMAL(16,2);	-- Total de Retiro
	...
	ManejoErrores:BEGIN
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen	= CONCAT('El SAFI ha tenido un problema al concretar la operacion. Disculpe las molestias que ',
					 				'esto le ocasiona. Ref: SP-TC_RETIROATMPRO');
		...
		SELECT 	LineaTarCredID,		TipoTarjetaCredID,	ClienteID,		TarjetaCredID,			Estatus,
				...
				IFNULL(TC_FUNCIONLIMITEMONTO(Tar.TarjetaCredID, Tar.ClienteID, Tar.TipoTarjetaCredID, Con_DispoDiario), Decimal_Cero),
				IFNULL(Tar.MontoDispoMes, Decimal_Cero),
				IFNULL(TC_FUNCIONLIMITEMONTO(Tar.TarjetaCredID, Tar.ClienteID, Tar.TipoTarjetaCredID, Con_DispoMes), Decimal_Cero),
				IFNULL(Tar.NoDispoDiario, Entero_Cero),
				IFNULL(TC_FNNUMCONSULTAS(Tar.TarjetaCredID, Tar.ClienteID, Tar.TipoTarjetaCredID, Con_OpeDisposicion), Entero_Cero)
		...
		SET Var_MontoDispuestoMes		:= IFNULL(Var_MontoDispuestoMes, Entero_Cero);
		...
		SET Var_TotalRetiro				:= Par_TransactionAmount + Par_CashbackAmount + Par_AdditionalAmount + Par_SurchargeAmounts;
		...
		-- Segmento de Validaciones de la Operacion de Retiro ATM
		...
		IF( Par_ProdIndicator <> Prod_ATM ) THEN
			SET Par_NumErr	:= 1203;
			SET Par_ErrMen	:= 'El Indicador no es Valido para la Operacion de Retiro ATM.';
			SET Var_Control	:= 'prodIndicator';
			LEAVE ManejoErrores;
		END IF;
		...
		IF( Par_TranCode <> Code_CashAdvance ) THEN
			SET Par_NumErr	:= 1205;
			SET Par_ErrMen	:= 'Codigo de transaccion no es Valido para la Operacion de Retiro ATM.';
			SET Var_Control	:= 'tranCode';
			LEAVE ManejoErrores;
		END IF;
		...
		IF( Var_MontoDisponible <= Decimal_Cero ) THEN
			SET Par_NumErr	:= 1227;
			SET Par_ErrMen	:= CONCAT('La Linea de Credito Asociada al Tarjeta: ',Par_CardNumber,' no cuenta con credito disponible.');
			SET Var_Control	:= 'cardNumber';
			LEAVE ManejoErrores;
		END IF;

		IF( (Var_MontoDisponible - Var_TotalRetiro) < Decimal_Cero ) THEN
			SET Par_NumErr	:= 1328;
		...
		IF ( Par_AdditionalAmount >= Par_TransactionAmount) THEN
			SET Par_NumErr	:= 1329;
			SET Par_ErrMen	:= 'El Monto de la Comision Supera el Monto de la Operacion.';
			SET Var_Control	:= 'additionalAmount';
			LEAVE ManejoErrores;
		END IF;
		...
		IF( Var_LimiteDispuestoDiario > Decimal_Cero ) THEN
			...
			IF( (Var_TotalRetiro + Var_MontoDispuestoDiario) > Var_LimiteDispuestoDiario ) THEN
				SET Par_NumErr	:= 1230;
			...
			IF( Var_TotalRetiro > Var_MontoDispuestoLibre ) THEN
				SET Par_NumErr	:= 1231;
				SET Par_ErrMen	:= 'Ha excedido el limite de Disposicion Diario.';
				SET Var_Control	:= 'cardNumber';
				LEAVE ManejoErrores;
		...
		IF (Var_LimiteDispuestoMes > Decimal_Cero) THEN
			...
			IF( (Var_TotalRetiro + Var_MontoDispuestoMes) > Var_LimiteDispuestoMes ) THEN
				SET Par_NumErr	:= 1232;
			...
			SET Var_MontoDispuestoLibre := Var_LimiteDispuestoMes - Var_MontoDispuestoMes;
		...
		IF( Par_CashbackAmount > Entero_Cero ) THEN
		...
		SET Var_DesPoliza	:= CONCAT("RETIRO ATM ",Var_Referencia);
		...
		CALL TC_CONTALINEAPRO (
			...
			Var_TotalRetiro,		Par_MonedaID,			Var_ProductoCreditoID,		Var_Clasificacion,		Con_SubClasificacion,
			...
			Mov_CapOrdinario,		Nat_Cargo,				Par_CodigoTransaccionID,	AltaMovBitacoraNO,		Cadena_Vacia,
			Par_TranCode,
		...
		CALL TC_CONTALINEAPRO (
			...
			Entero_Cero,			Nat_Cargo,				Par_CodigoTransaccionID,	AltaMovBitacoraNO,		Cadena_Vacia,
			Par_TranCode,
			Salida_NO,				Par_NumErr,				Par_ErrMen,					Var_Consecutivo,		Par_EmpresaID,
			Aud_Usuario,			Aud_FechaActual,		Aud_DireccionIP,			Aud_ProgramaID,			Aud_Sucursal,
			Aud_NumTransaccion);
		...
		CALL POLIZATARJETAPRO (
			...
			Con_OperacATM,		Par_MonedaID,		Entero_Cero,			Var_TotalRetiro,	Var_DesPoliza,
		...
		CALL TARJETACREDITOACT (
			Par_CardNumber,		Var_TotalRetiro,	Nat_Cargo,			Act_MovRetiro,
		...
		SET Par_ErrMen := CONCAT('Retiro ATM realizado Correctamente: ',Par_CodigoTransaccionID,'.');
