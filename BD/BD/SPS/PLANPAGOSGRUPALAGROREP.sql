...
-- ---------------------------------------------------------------------------------
-- PLANPAGOSGRUPALAGROREP
DELIMITER ;
DROP PROCEDURE IF EXISTS `PLANPAGOSGRUPALAGROREP`;DELIMITER $$

CREATE PROCEDURE `PLANPAGOSGRUPALAGROREP`(
	Par_GrupoID         INT,
	Par_TipoReporte     INT,
	...
	SELECT	CicloActual INTO Var_CicloGruAc
		...
		IF(Var_CicloGruAc>1) THEN
				...
				SELECT	Cr.CreditoID INTO Var_NumCredAnt
						...
						AND H.Ciclo = Var_CicloAnt
						AND G.GrupoID = H.GrupoID
						ORDER BY H.SolicitudCreditoID DESC
						LIMIT 1;
...
IF(Par_TipoReporte = Tipo_Amortizacion) THEN
	...
	IF EXISTS (SELECT Estatus
				FROM CREDITOS
					WHERE GrupoID = Par_GrupoID
					AND Estatus in(EstatusAutorizado,EstatusInactivo,'V'))THEN/*quitar la V*/
		SELECT	MIN(Amo.AmortizacionID),					MIN(Amo.FechaInicio),					MIN(Amo.FechaVencim),				Amo.FechaExigible,						SUM((Amo.Capital+Amo.Interes+Amo.IVAInteres)) AS montoCuota,
		...
		ELSE -- si el Credito ya fue desembolsado la informacion se obtiene de la tabla PAGARECREDITO
			SELECT	MIN(Pag.AmortizacionID),					MIN(Pag.FechaInicio),					MIN(Pag.FechaVencim),				Pag.FechaExigible,						SUM((Pag.Capital+Pag.Interes+Pag.IVAInteres)) AS montoCuota,
...
IF (Par_TipoReporte = Tipo_AmortSTF) THEN			-- Hecho por Daniel
	  ...
	  INTO	Var_FecMinist,									DirPreGru
		FROM	INTEGRAGRUPOSCRE	InGruCre,
			...
			WHERE	InGruCre.GrupoID		=	Par_GrupoID
				...
				ORDER BY InGruCre.SolicitudCreditoID ASC
				LIMIT 1;
	...
	INTO	Var_NomProduc,			Var_CAT,		Var_NumRECA
		FROM	PRODUCTOSCREDITO	ProdCre,
			...
			WHERE	Cred.GrupoID 			=	Par_GrupoID
				...
				ORDER BY Cred.SolicitudCreditoID ASC
		limit 1;
...
IF(Par_TipoReporte = Tab_AmortSTF) then
	IF EXISTS(SELECT Estatus
		...
		SELECT	MIN(Amo.AmortizacionID), 		Amo.FechaExigible, 	SUM(Amo.SaldoCapital) AS SaldoCapital, 	SUM(Amo.Capital) AS Capital,	SUM(Amo.Interes) AS SaldoInteresOrd,
		...
		FROM	AMORTICREDITOAGRO		Amo,
			...
			GROUP BY FechaExigible;
	ELSE  -- si el Credito ya fue desembolsado la informacion se obtiene de la tabla PAGARECREDITO
		SELECT	MIN(Pag.AmortizacionID), 		Pag.FechaExigible, 	SUM(Pag.Capital) AS SaldoCapital, 	SUM(Pag.Capital) AS Capital,	SUM(Pag.Interes) AS SaldoInteresOrd,
