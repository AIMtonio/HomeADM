...
-- ---------------------------------------------------------------------------------
-- BALANCEINTERNO049REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `BALANCEINTERNO049REP`;

DELIMITER $$
CREATE PROCEDURE `BALANCEINTERNO049REP`(
	-- SP para generar el Balance General del Cliente: ATE
...
TerminaStore: BEGIN
	...
	DECLARE Por_Periodo			CHAR(1);			-- Constante si el balance se genera por periodo
	...
	SELECT FechaSistema,	 EjercicioVigente, PeriodoVigente, JefeContabilidad, GerenteGeneral,    DirectorFinanzas
	INTO   Var_FechaSistema, Var_EjercicioVig, Var_PeriodoVig, Var_ContaGeneral, Var_DirectGeneral, Var_DirectorFinanza
	FROM PARAMETROSSIS;

	SET Var_ContaGeneral 	:= IFNULL(Var_ContaGeneral, Cadena_Vacia);
	SET Var_DirectGeneral 	:= IFNULL(Var_DirectGeneral, Cadena_Vacia);
	SET Var_DirectorFinanza	:= IFNULL(Var_DirectorFinanza, Cadena_Vacia);
		...
		IF (Var_FechaSaldos = Fecha_Vacia) THEN
			...
			SELECT 	Aud_NumTransaccion,	MAX(Fin.NombreCampo),Cadena_Vacia,	Entero_Cero, Entero_Cero,
					...
					SUM(CASE WHEN Cue.Naturaleza = Con_Acreedora THEN
						...
						END),
					Entero_Cero, Entero_Cero,
					MAX(Fin.Desplegado), MIN(Pol.CuentaContable), Cadena_Vacia
			...
			UPDATE TMPBALANZACONTA Pol
			INNER JOIN CUENTASCONTABLES Cue ON Pol.CuentaMayor = Cue.CuentaCompleta SET
				Pol.SaldoDeudor = ( CASE WHEN Cue.Naturaleza = Con_Deudora THEN
										IFNULL(Pol.Cargos, Entero_Cero) - IFNULL(Pol.Abonos, Entero_Cero)
									ELSE
										IFNULL(Pol.Abonos, Entero_Cero) - IFNULL(Pol.Cargos, Entero_Cero)
							  		END),
				Pol.Cargos = Entero_Cero,
				Pol.Abonos = Entero_Cero
			...
			IF(Var_NumeroRegistros > Entero_Cero) THEN
				...
				WHILE( Var_Contador <= Var_NumeroRegistros) DO
					...
					IF(Var_CuentaContable != Cadena_Vacia) THEN
						...
						select Var_CuentaContable,	Var_Ubicacion,		Por_Fecha,			Par_Fecha,			Var_UltEjercicioCie,
	...
	IF( Par_TipoConsulta = Por_Periodo ) THEN
		...
		SELECT  UPPER(CONCAT("Del 01 al ",FUNCIONLETRASFECHA(Fin)))
		...
		IF(Var_NumeroRegistros > Entero_Cero) THEN
			...
			WHILE( Var_Contador <= Var_NumeroRegistros) DO
				...
				IF(Var_CuentaContable != Cadena_Vacia) THEN
					CALL `EVALFORMULACONTAPRO`(
						Var_CuentaContable,	Cadena_Vacia,		Por_Periodo,		Par_Fecha,			Par_Ejercicio,
	...
	IF( IFNULL(Aud_NumTransaccion, Entero_Cero) > Entero_Cero ) THEN
		...
		DROP TEMPORARY TABLE IF EXISTS TMP_BALANCECICLO;
		CREATE TEMPORARY TABLE TMP_BALANCECICLO (
			RegistroID	INT(11)			NOT NULL COMMENT 'Llave primaria',
			Columna		VARCHAR(25) 	NOT NULL COMMENT 'Cuenta Contable',
	 		Monto 		DECIMAL(16,4)	DEFAULT NULL COMMENT 'Saldo Deudor',
	 		PRIMARY KEY(RegistroID));

		SET @RegistroID := Entero_Cero;
		INSERT INTO TMP_BALANCECICLO(RegistroID, Columna, Monto)
		SELECT @RegistroID := (@RegistroID + Entero_Uno),
				CuentaContable,	SaldoDeudor
		FROM TMPBALANZACONTA
		WHERE NumeroTransaccion = Aud_NumTransaccion;
		...
		INTO Var_NumeroRegistros
		FROM TMP_BALANCECICLO;

		SET Var_NumeroRegistros := IFNULL(Var_NumeroRegistros, Entero_Cero);

		WHILE ( Var_Contador <= Var_NumeroRegistros ) DO

			SET Var_Monto 		:= Entero_Cero;
			SET Var_Columna 	:= Var_Columna;

			SELECT Columna,		Monto
			INTO   Var_Columna, Var_Monto
			FROM TMP_BALANCECICLO
