...
-- CREPROXPAGCON
DELIMITER ;
DROP PROCEDURE IF EXISTS `CREPROXPAGCON`;
DELIMITER $$


CREATE PROCEDURE `CREPROXPAGCON`(
        ...
        ELSE
            ...
            IF Var_AmortizacionID != Entero_Cero THEN
                SELECT  FechaExigible,
                        (ROUND(SaldoCapVigente, 2) + ROUND(Interes, 2) + ROUND(IVAInteres, 2)+ROUND(SaldoCapVencido,2)+ROUND(SaldoCapVenNExi,2) + ROUND(MontoOtrasComisiones,2))
                INTO
