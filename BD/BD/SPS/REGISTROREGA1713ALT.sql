...
-- REGISTROREGA1713ALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `REGISTROREGA1713ALT`;DELIMITER $$

CREATE PROCEDURE `REGISTROREGA1713ALT`(
/*=======================================================================
--------- SP QUE ANADE UN REGISTRO PARA EL REGULATORIO A1713 -----------
========================================================================*/
  ...
  ManejoErrores: BEGIN
	...
	BEGIN
		...
		SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al
					concretar la operacion.Disculpe las molestias que ', 'esto le ocasiona. Ref: SP-REGISTROREGA1713ALT');
		SET Var_Control := 'SQLEXCEPTION';
    END;


    SELECT (MAX(RegistroID)+Entero_Uno) INTO Var_Consecutivo
      FROM  REGISTROREGA1713 WHERE Fecha = Par_Fecha;
    ...
    SET Var_Consecutivo := IFNULL(Var_Consecutivo,Entero_Uno);
	 ...
	 IF IFNULL(Par_Fecha,Fecha_Vacia) = Fecha_Vacia THEN
      ...
      IF EXISTS (SELECT RFC FROM REGISTROREGA1713 WHERE RFC = Par_RFC AND Fecha = Par_fecha) THEN
		SET Par_NumErr  := 27;
      ...
      IF EXISTS (SELECT CURP FROM REGISTROREGA1713 WHERE CURP = Par_CURP AND Fecha = Par_fecha) THEN
		SET Par_NumErr  := 28;
	 ...
	 IF IFNULL(Par_ManifestCumpID,Entero_Cero) = Entero_Cero THEN
	  ...
	  INSERT INTO REGISTROREGA1713
		(Fecha,             RegistroID,         TipoMovimientoID,       NombreFuncionario,      RFC,
		 ...
		 Sucursal,          NumTransaccion,		CausaBajaID)
		VALUES
		(Par_Fecha,             Var_Consecutivo,         Par_TipoMovimientoID,       Par_NombreFuncionario,      Par_RFC,
		 Par_CURP,              Par_Profesion,          Par_Telefono,               Par_Email,                  Par_PaisID,
		 Par_EstadoID,          Par_MunicipioID,        Par_LocalidadID,            Par_ColoniaID,              Par_CodigoPostal,
		 Par_Calle,             Par_NumeroExt,          Par_NumeroInt,              Par_FechaMovimiento,        Par_FechaInicioGes,
		 Par_FechaFinGestion,   Par_OrganoID,           Par_CargoID,                Par_PermanenteID,           Par_ManifestCumpID,
		 Par_EmpresaID,         Aud_Usuario,            Aud_FechaActual,        	Aud_DireccionIP, 			Aud_ProgramaID,
         Aud_Sucursal,          Aud_NumTransaccion,		Par_CausaBajaID);
