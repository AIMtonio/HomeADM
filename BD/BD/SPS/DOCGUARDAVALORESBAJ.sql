...
-- DOCGUARDAVALORESBAJ
DELIMITER ;
DROP PROCEDURE IF EXISTS `DOCGUARDAVALORESBAJ`;

DELIMITER $$
CREATE PROCEDURE `DOCGUARDAVALORESBAJ`(
	-- Store Procedure: De Baja para Documentos para el menu de Guarda Valores
	...
	ManejoErrores:BEGIN
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									'Disculpe las molestias que esto le ocasiona. Ref: SP-DOCGUARDAVALORESBAJ');
		...
		DELETE FROM DOCGUARDAVALORES WHERE ParamGuardaValoresID = Par_ParamGuardaValoresID;
		...
		SET Par_ErrMen := 'Parametros de Documentos Eliminados Correctamente';
		SET Var_Control:= 'paramGuardaValoresID';
