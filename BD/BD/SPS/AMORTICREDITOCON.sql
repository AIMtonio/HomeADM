...
-- AMORTICREDITOCON
DELIMITER ;
DROP PROCEDURE IF EXISTS `AMORTICREDITOCON`;

DELIMITER $$
CREATE PROCEDURE `AMORTICREDITOCON`(
...
TerminaStore: BEGIN
	...
	DECLARE Con_PagareTfijaAsefimex	INT(11);
	...
	DECLARE Con_PagIndGrupConsol INT(11);
    ...
    DECLARE Var_Telefono			VARCHAR(60);			-- Campo para guardar el numero telefonico del cliente
    DECLARE Var_LugarAsefimex		VARCHAR(100);			-- Lugar de la sucursal del cliente Asefimex
	...
	SET Con_PagIndGrupConsol :=13;           -- Consulta Pagare por integrante de Consol.
	SET Con_PagareTfijaAsefimex	:=14;			-- Consulta tasa fija para el cliente Asefimex
	...
	IF(Par_NumCon = Con_PagareTfija) THEN
		...
		SELECT ValorParametro
			INTO Var_LugarAsefimex
		FROM PARAMGENERALES
			WHERE LlaveParametro = 'LugarExpAsefimex';
		...
		SELECT	DISTINCT
                        ...
                        IFNULL(Cli.Telefono,Cadena_Vacia)
				INTO 	IDCliente,		NombreCliente,			Var_TasaFija,				MontoCred,			FechaVencCred,
						FactorM,		FechaMinis,				DireccionCte, Var_TotalCredito, Var_Telefono
			FROM CREDITOS Cre
		...
		IF (Var_EstatusCredito = EstatusAutorizado OR Var_EstatusCredito = EstatusInactivo )THEN
			...
			SELECT AmortizacionID,	FechaInicio,	FechaVencim,			FechaExigible,	(Capital+Interes+IVAInteres+MontoOtrasComisiones+MontoIVAOtrasComisiones+MontoIntOtrasComis+MontoIVAIntComisi) AS montoCuota,
				   ...
				   SaldoCapital,	Var_EstadoSuc , Var_MuniSuc,			Mora ,			FijaTasa,
					...
					(SELECT MAX(FechaVencim)	FROM AMORTICREDITO WHERE CreditoID = Par_CreditoID)AS Par_FechaultimaAmortizacion,
		 ...
		 ELSE
			...
			SELECT PAG.AmortizacionID,	PAG.FechaInicio,PAG.FechaVencim,	PAG.FechaExigible,	(PAG.Capital+PAG.Interes+PAG.IVAInteres) AS montoCuota,
				   ...
				   NombreCliente,		Var_TasaFija,	FORMAT(Var_TasaFija,2) AS TasaOrdi,			FORMAT(MontoCred,2),	FechaVencCred,
                   fechaPTF,			Sucurs,			FactorM,			FechaMinis,				DireccionCte,		 PAG.CreditoID,  	Var_NCuotas 	AS NCuotas,
				   Var_EstadoSuc,		Var_MuniSuc,	Mora,		 		FijaTasa,

					CONVPORCANT(MontoCred, '$','Peso', 'Nacional')AS Montocletra,
                    FUNCIONNUMEROSLETRAS(MontoCred) AS MotoEnLetras,
					Var_DescripcionProCre,
					CONVPORCANT(ROUND((FijaTasa)/12,2),'%','2','') AS TasaOrdinariaMensual,
					CONVPORCANT(ROUND((FijaTasa)*1.16/12,2),'%','2','') AS tasaAnualizadaMensual,
					CONVPORCANT(ROUND(FijaTasa,2),'%','2','') AS TasaOrdinariaAnual,
					...
					CONVPORCANT(ROUND(((FijaTasa)*3/360)*1.16*MontoCred/100,2),'$','peso','Nacional') AS ValorMoraDiaria,
					FORMATEAFECHACONTRATO((SELECT FechaSistema FROM PARAMETROSSIS)) AS FechaSis,
                    (SELECT FechaSistema FROM PARAMETROSSIS) AS FechaSistema,
					(SELECT pro.ProducCreditoID FROM PRODUCTOSCREDITO pro, CREDITOS cre WHERE pro.ProducCreditoID = cre.ProductoCreditoID AND cre.CreditoID = Par_CreditoID) AS ProductoCre
			 ,(SELECT MAX(FechaVencim)	FROM PAGARECREDITO WHERE CreditoID = Par_CreditoID)AS Par_FechaultimaAmortizacion,
				(SELECT RegistroRECA FROM CREDITOS cr , PRODUCTOSCREDITO  pc WHERE cr.CreditoID = Par_CreditoID AND cr.ProductoCreditoID = pc.ProducCreditoID) AS Var_Reca, Var_TxtComision AS TxtComision,
				LOWER(FORMATEAFECHACONTRATO((SELECT FechaSistema FROM PARAMETROSSIS))) AS FechaActual,
                Var_Telefono AS Telefono,
                FORMAT((SELECT SaldoInteresOrd FROM AMORTICREDITO AMO WHERE AMO.AmortizacionID = PAG.AmortizacionID AND AMO.CreditoID = Par_CreditoID),2) AS SaldoInteresOrd,
                IFNULL(Var_LugarAsefimex, Cadena_Vacia) AS LugarAsefimex
			FROM PAGARECREDITO PAG
				WHERE PAG.CreditoID = Par_CreditoID;
	...
	IF(Par_NumCon = Con_Totalvigente) THEN
		...
		SET Var_TotCapVigente	:= (SELECT SUM(SaldoCapVigente) FROM AMORTICREDITO
	...
	IF(Par_NumCon = Con_SalFinPlazoWS) THEN
		...
		CREATE TEMPORARY TABLE TMPAMORTICREDITO(
		...
		CREATE INDEX TMPAMORTICREDITO_IDX1 ON TMPAMORTICREDITO (AmortizacionID, CreditoID);
		...
		SELECT  AmortizacionID,  CreditoID, Estatus,
				...
				ROUND(Capital + Interes + IVAInteres+MontoOtrasComisiones+MontoIVAOtrasComisiones+MontoIntOtrasComis+MontoIVAIntComisi,2) AS montoCuota
	...
	IF(Par_NumCon = Con_PagAlternativa) THEN
		...
		IF (Var_EstatusCredito = EstatusAutorizado OR Var_EstatusCredito = EstatusInactivo )THEN
		 ...
		 END IF;
	END IF;
	IF(Par_NumCon = Con_PagIndGrupConsol) THEN
		SET Var_DescripcionProCre:=
			(	SELECT pro.Descripcion FROM PRODUCTOSCREDITO pro, CREDITOS cre
				WHERE pro.ProducCreditoID = cre.ProductoCreditoID AND cre.CreditoID = Par_CreditoID);
		SET NumInstitucion  	:= (SELECT InstitucionID FROM PARAMETROSSIS);
		...
		IF (Var_EstatusCredito = EstatusAutorizado OR Var_EstatusCredito = EstatusInactivo )THEN
		 ...
		 END IF;
            ...
            SELECT AmortizacionID,	FechaInicio,	FechaVencim,			FechaExigible,	(Capital+Interes+IVAInteres+MontoOtrasComisiones+MontoIVAOtrasComisiones+MontoIntOtrasComis+MontoIVAIntComisi) AS montoCuota,
				   ...
				   NombreCliente,	Var_TasaFija,	FORMAT(MontoCred,2),	FechaVencCred,	 fechaPTF,
				   Sucurs,			FactorM,		FechaMinis,				DireccionCte,	CreditoID,  Var_NCuotas 	AS NCuotas,
				   SaldoCapital,	Var_EstadoSuc , Var_MuniSuc, Var_DirecSuc, Var_PlazoCredito,	Mora ,			FijaTasa,
					FNLETRACAPITAL(CONVPORCANT(MontoCred, '$','Peso', 'N.'))AS Montocletra,
					Var_DescripcionProCre,
					CONVPORCANT(ROUND(FijaTasa*0.16,2),'%','2','') AS taiva,
					CONVPORCANT(ROUND(FijaTasa*0.16/12,2),'%','2','') AS taivaMensual,
					replace(CONVPORCANT(ROUND((FijaTasa)/12,2),'%','2',''),"por ciento","")  AS TasaOrdinariaMensual,
					CONVPORCANT(ROUND((FijaTasa)*1.16/12,2),'%','2','') AS tasaAnualizadaMensual,
					CONVPORCANT(ROUND(FijaTasa,2),'%','2','') AS TasaOrdinariaAnual,
					CONVPORCANT(ROUND((FijaTasa)*3/360,2),'%','2','') AS TasaMoraDiaria,
					CONVPORCANT(ROUND((FijaTasa)*3/12,2),'%','2','')AS TasaMoraMensual,
					...
					(SELECT RegistroRECA FROM CREDITOS cr , PRODUCTOSCREDITO  pc WHERE cr.CreditoID = Par_CreditoID AND cr.ProductoCreditoID = pc.ProducCreditoID) AS Var_Reca, Var_TxtComision AS TxtComision,
					LOWER(FORMATEAFECHACONTRATO((SELECT FechaSistema FROM PARAMETROSSIS))) AS FechaActual,
                   	concat (right(FechaInicio,2),' (',SUBSTRING(FUNCIONSOLONUMLETRAS(day(FechaInicio)), 1, length(FUNCIONSOLONUMLETRAS(day(FechaInicio)))-1),') de ',FUNCIONMESNOMBRE(FechaInicio),' del ',
             			year(FechaInicio),' (',SUBSTRING(FUNCIONSOLONUMLETRAS(year(FechaInicio)), 1, length(FUNCIONSOLONUMLETRAS(year(FechaInicio)))-1),')') as FechaEnLetras,
                 	concat (right(FechaVencCred,2),' (',lower(SUBSTRING(FUNCIONSOLONUMLETRAS(day(FechaVencCred)), 1, length(FUNCIONSOLONUMLETRAS(day(FechaVencCred)))-1)),') de ',lower(FUNCIONMESNOMBRE(FechaVencCred)),' del ',
             			year(FechaVencCred),' (',lower(SUBSTRING(FUNCIONSOLONUMLETRAS(year(FechaVencCred)), 1, length(FUNCIONSOLONUMLETRAS(year(FechaVencCred)))-1)),')') as FechaEnLetrasVen
			FROM AMORTICREDITO
				WHERE CreditoID = Par_CreditoID;
		 ELSE
			SELECT prod.ProducCreditoID, prod.CobraFaltaPago, prod.CriterioComFalPag, prod.TipCobComFalPago, prod.PerCobComFalPag
			...
			SET Var_NCuotas := (SELECT COUNT(AmortizacionID)
				FROM PAGARECREDITO PAG
				WHERE PAG.CreditoID = Par_CreditoID);
			SELECT PAG.AmortizacionID,	PAG.FechaInicio,PAG.FechaVencim,	FNFECHATEXTO(PAG.FechaExigible) as FechaExigible,	(PAG.Capital+PAG.Interes+PAG.IVAInteres) AS montoCuota,
				   PAG.Interes,			PAG.IVAInteres,	PAG.Capital,		DireccionInstitu,	 NombreInstitu,
				   NombreCliente,		Var_TasaFija,	FORMAT(MontoCred,2),	FechaVencCred,	 	 fechaPTF,
	...
	IF(Par_NumCon = Con_PagareTfijaAsefimex) THEN
		SET Var_DescripcionProCre:=
			(	SELECT pro.Descripcion FROM PRODUCTOSCREDITO pro, CREDITOS cre
				WHERE pro.ProducCreditoID = cre.ProductoCreditoID AND cre.CreditoID = Par_CreditoID);
		SET NumInstitucion  	:= (SELECT InstitucionID FROM PARAMETROSSIS);
		...
		IF (Var_EstatusCredito = EstatusAutorizado OR Var_EstatusCredito = EstatusInactivo )THEN
			...
			SELECT AmortizacionID,	FechaInicio,	FechaVencim,			FechaExigible,	(Capital+Interes+IVAInteres+MontoOtrasComisiones+MontoIVAOtrasComisiones+MontoIntOtrasComis+MontoIVAIntComisi) AS montoCuota,
				   Interes,	IVAInteres,		Capital,				DireccionInstitu,	 NombreInstitu,
				   NombreCliente,	Var_TasaFija,	FORMAT(Var_TasaFija,2) AS TasaOrdi,		FORMAT(MontoCred,2),	FechaVencCred,	 fechaPTF,
				   Sucurs,			FactorM,		FechaMinis,				DireccionCte,	CreditoID,  Var_NCuotas 	AS NCuotas,
				   SaldoCapital,	Var_EstadoSuc , Var_MuniSuc,			Mora ,			FijaTasa,
					...
					CONVPORCANT(ROUND((FijaTasa)*1.16/12,2),'%','2','') AS tasaAnualizadaMensual,
					CONVPORCANT(ROUND(FijaTasa,2),'%','2','') AS TasaOrdinariaAnual,
					CONVPORCANT(ROUND((FijaTasa)/360,2),'%','2','') AS TasaMoraDiaria,
					ROUND(Mora/12,2) AS TasaMoraMensual,
					CONVPORCANT(ROUND(((FijaTasa)/360)*1.16*MontoCred/100,2),'$','peso','Nacional') AS ValorMoraDiaria,
					FORMATEAFECHACONTRATO((SELECT FechaSistema FROM PARAMETROSSIS)) AS FechaSis,
					(SELECT FechaSistema FROM PARAMETROSSIS) AS FechaSistema,
					(SELECT pro.ProducCreditoID FROM PRODUCTOSCREDITO pro, CREDITOS cre WHERE pro.ProducCreditoID = cre.ProductoCreditoID AND cre.CreditoID = Par_CreditoID) AS ProductoCre,
					(SELECT MAX(FechaVencim)	FROM AMORTICREDITO WHERE CreditoID = Par_CreditoID)AS Par_FechaultimaAmortizacion,
