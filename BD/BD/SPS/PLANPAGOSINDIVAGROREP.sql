...
-- ---------------------------------------------------------------------------------
-- PLANPAGOSINDIVAGROREP
DELIMITER ;
DROP PROCEDURE IF EXISTS `PLANPAGOSINDIVAGROREP`;DELIMITER $$

CREATE PROCEDURE `PLANPAGOSINDIVAGROREP`(
Par_GrupoID         INT,
...
IF (Par_TipoReporte = Tipo_DatosCred) THEN
	...
	SELECT
		Cre.ValorCAT,  ROUND(Cre.PorcGarLiq,2), Pro.FactorRiesgoSeguro, Pro.RegistroRECA, Cre.FechaVencimien,
		CONCAT(PrimerNombre,
	...
	SELECT
		SUM(Cre.MontoSeguroVida), SUM(Cre.MontoCredito)  INTO Var_MontoSeguro, Var_SumMonCred
		FROM INTEGRAGRUPOSCRE Ing,
		CREDITOS Cre
		WHERE Ing.GrupoID   = Par_GrupoID
		AND Ing.Estatus   = Est_Activo
	...
	SELECT
	Var_NumCredAnt, 	Var_MontoCredAnt,	Var_Plazo,      	Var_FechaVenc,  Var_DesFrec,
