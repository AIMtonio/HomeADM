...
-- ESQUEMACOMANUALMOD
DELIMITER ;
DROP PROCEDURE IF EXISTS `ESQUEMACOMANUALMOD`;
DELIMITER $$


CREATE PROCEDURE `ESQUEMACOMANUALMOD`(
	/*SP PARA LA MODIFICACION DE LOS ESQUEMAS DE COBRO DE COMISION ANUAL DE LOS CREDITOS*/
	...
	ManejoErrores:BEGIN
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									'Disculpe las molestias que esto le ocasiona. Ref: SP-ESQUEMACOMANUALMOD');
		...
		UPDATE ESQUEMACOMANUAL SET
			CobraComision		=		Par_CobraComision,
			TipoComision		=		Par_TipoComision,
			BaseCalculo			=		Par_BaseCalculo,
			MontoComision		=		Par_MontoComision,
			PorcentajeComision	=		Par_PorcentajeComision,
			DiasGracia			=		Par_DiasGracia,
			Empresa				=		Aud_Empresa,
			Usuario				=		Aud_Usuario,
		...
		SET Par_NumErr := 000;
		SET Par_ErrMen := CONCAT('Esquema Modificado Exitosamente para el Producto: ',Par_ProducCreditoID,'.');
		SET Var_Control := 'producCreditoID';
