...
-- CATTIPOEGRESOSALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `CATTIPOEGRESOSALT`;DELIMITER $$

CREATE PROCEDURE `CATTIPOEGRESOSALT`(
	/*SP para dar de alta los registros de Egresos para el CATALOGO EGRESOS*/
	...
	ManejoErrores: BEGIN
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen	:= CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
								'Disculpe las molestias que esto le ocasiona. Ref: SP-CATTIPOEGRESOSALT');
        ...
        IF EXISTS(SELECT Numero FROM CATTIPOEGRESOS WHERE Numero = Par_Numero) THEN
			SET Par_NumErr		:= 005;
		...
		SET	Par_NumErr			:= 0;
		SET	Par_ErrMen			:= CONCAT('Egreso Agregado Exitosamente: ', CONVERT(Par_Numero,CHAR));
		SET Var_Control			:= 'tipo';
