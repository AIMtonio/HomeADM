...
-- CONVENIOMEXIREP
DELIMITER ;
DROP PROCEDURE IF EXISTS `CONVENIOMEXIREP`;
DELIMITER $$

CREATE PROCEDURE `CONVENIOMEXIREP`(
...
TerminaStore: BEGIN
	...
	DECLARE Cons_Ven			CHAR(1);
	DECLARE Cons_Pro			CHAR(1);
	...
	SET Cons_Ven		:= 'V';			-- Constante Vencimiento
	SET Cons_Pro		:= 'E';			-- Constante Programado
	...
	IF(Par_NumCon = Con_Principal)THEN
		...
		SET Var_CuentaAhoID	  := Par_CuentaAhoID;

		SELECT FechaApertura
			INTO Var_FechaConvenio
		FROM CUENTASAHO
		WHERE CuentaAhoID = Par_CuentaAhoID;
		...
		WHERE 	cli.ClienteID	= Par_ClienteID AND ide.Oficial = Cons_SI
		LIMIT 1; -- Limita a 1 por si hay más de una identificación oficial.
	...
	IF(Par_NumCon = Con_Amortiza)THEN
		SET	Var_Titulo	:= (SELECT (CASE
										WHEN (TipoPagoInt = Cons_Ven) THEN 'APORTACIONES AL VENCIMIENTO'
										WHEN (TipoPagoInt = Cons_Pro AND PagoIntCapitaliza=Cons_SI) THEN 'INTERESES  CAPITALIZABLES  MENSUALMENTE  VENCIDOS'
										WHEN (TipoPagoInt = Cons_Pro AND PagoIntCapitaliza=Cons_NO) THEN 'INTERESES  PAGADEROS  MENSUALMENTE  VENCIDOS'
										ELSE ''
									END) FROM APORTACIONES WHERE AportacionID = Par_AportacionID);
