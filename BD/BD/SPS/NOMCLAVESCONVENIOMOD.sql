-- SP NOMCLAVESCONVENIOMOD

DELIMITER ;

DROP PROCEDURE IF EXISTS NOMCLAVESCONVENIOMOD;

DELIMITER $$


CREATE PROCEDURE NOMCLAVESCONVENIOMOD(
	-- Stored Procedure para Modificar las claves presupuestales por convenio de nomina
	...
	DECLARE Var_ConvenioNominaID		BIGINT(20);		-- Variable de convenio
...
ManejoErrores:BEGIN
	...
	DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
		SET Par_NumErr = 999;
		SET Par_ErrMen = concat("El SAFI ha tenido un problema al concretar la operación. Disculpe las molestias que esto le ocasiona. Ref: SP-NOMCLAVESCONVENIOMOD");
		SET Var_Control = 'sqlException';
	END;

	IF(Par_NomClaveConvenioID = Entero_Cero) THEN
		SET Par_NumErr := 1;
		SET Par_ErrMen := 'Especifique el Numero de Clave Convenio a Modificar.';
	...
	IF(IFNULL(Var_NomClaveConvenioID,Entero_Cero) = Entero_Cero) THEN
		SET Par_NumErr := 2;
		SET Par_ErrMen := 'El Numero de Clave Convenio a Modificar No Existe.';
		SET Var_Control := 'nomClaveConvenioID';
		LEAVE ManejoErrores;
	END IF;
	...
	IF (Par_ConvenioNominaID = Entero_Cero) THEN
		SELECT COUNT(NomClaveConvenioID)
			...
			FROM NOMCLAVESCONVENIO
			WHERE InstitNominaID = Par_InstitNominaID
			AND NomClaveConvenioID <> Par_NomClaveConvenioID;
	END IF;
	...
	UPDATE NOMCLAVESCONVENIO
		SET	InstitNominaID			= Par_InstitNominaID,
			ConvenioNominaID		= Par_ConvenioNominaID,
			NomClavePresupID		= Var_ClavesFinales,
		...
		SET Par_NumErr	:= 0;
		SET Par_ErrMen	:= 'Claves por Convenio de Nomina Modificadas Correctamente';
