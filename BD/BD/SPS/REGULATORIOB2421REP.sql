...
-- REGULATORIOB2421REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `REGULATORIOB2421REP`;DELIMITER $$

CREATE PROCEDURE `REGULATORIOB2421REP`(

	/* Genera la llamada al regulatorio B2421 para Sofipo o Socap */
	...
	SET NumReporte			:= 'B2421';					-- Clave del reporte
	...
	SET Resp_SI				:= 'S';						-- Cosntante si
	SET Resp_SI				:= 'N';						-- Constante no
	...
	SET Var_Llamada := CONCAT(' CALL ',Var_NombreReporte,
		'(',Par_Anio,',',Par_Trimestre,',',Par_NumRep,',',Par_EmpresaID,',',Aud_Usuario,',',"'",Aud_FechaActual,"'",',',"'",Aud_DireccionIP,"'",',',"'",Aud_ProgramaID,"'",',',Aud_Sucursal,',',Aud_NumTransaccion,');');
