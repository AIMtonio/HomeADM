-- SP EDOCTADETCRE024PRO

DELIMITER ;

DROP PROCEDURE IF EXISTS EDOCTADETCRE024PRO;

DELIMITER $$

CREATE PROCEDURE `EDOCTADETCRE024PRO`(
...
TerminaStore: BEGIN
	...
	SELECT  Par_AnioMes,        Par_SucursalID,            	Con_Entero_Cero,    			Mov.CreditoID,       	 MAX(Amo.FechaVencim) ,
			Var_TipoInteres,    'INTERESES ORDINARIOS',   	ROUND(SUM(Mov.Cantidad),2),     Con_Moneda_Cero, 		 IFNULL(Mov.PolizaID,Con_PolizaCero),
	...
	SELECT  Par_AnioMes,        Par_SucursalID,            		Con_Entero_Cero,    			Mov.CreditoID,       	MAX(Mov.FechaOperacion),
			Var_TipoInteres,    'INTERES DEVENGADO',   	ROUND(SUM(Mov.Cantidad),2),     Con_Moneda_Cero,  		IFNULL(Mov.PolizaID,Con_PolizaCero),
            Con_Entero_Cero,	Con_Entero_Cero,				Con_Entero_Cero,				Con_Entero_Cero
	FROM CREDITOSMOVS Mov
	INNER JOIN AMORTICREDITO Amo ON Amo.CreditoID = Mov.CreditoID AND Amo.AmortizacionID = Mov.AmortiCreID AND Amo.FechaVencim > Par_FecFinMes
	WHERE Mov.FechaOperacion >= Par_FecIniMes
	...
	SELECT  Par_AnioMes,     	Par_SucursalID,            	Con_Entero_Cero,    				Mov.CreditoID,        	Mov.FechaOperacion ,
			Var_TipoIVAs,       'IVA DE LOS INTERESES',	ROUND(SUM(Mov.Cantidad),2),      	Con_Moneda_Cero, 		IFNULL(Mov.PolizaID,Con_PolizaCero),
			Con_Entero_Cero,	Con_Entero_Cero,			Con_Entero_Cero,					Con_Entero_Cero
	FROM CREDITOSMOVS Mov
	...
	INSERT INTO EDOCTADETCRE(
		...
		NumAmortizaciones,	AmoCubiertas,	AmoPorCubrir,	CapitalCubierto)
	SELECT  Par_AnioMes,        Par_SucursalID,            		Con_Entero_Cero,    		Mov.CreditoID,       					MAX(Mov.FechaOperacion),
			Var_TipoInteres,    'IVA DE LOS INTERESES MORATORIOS',   	ROUND((SUM(Mov.Cantidad)*(IFNULL(Res.ValorIVAMora, 0.0))),2),      	Con_Moneda_Cero,
	...
	WHERE Mov.FechaOperacion >= Par_FecIniMes
	  ...
	  AND Mov.NatMovimiento = NatCargo
	  AND (Mov.Descripcion = 'CIERRE DIARO CARTERA'  AND Mov.Referencia ='GENERACION INTERES MORATORIO')
	...
	INSERT INTO EDOCTADETCRE(
		...
		NumAmortizaciones,	AmoCubiertas,	AmoPorCubrir,	CapitalCubierto)
	SELECT  Par_AnioMes,        Par_SucursalID,            	Con_Entero_Cero,    			CreditoID,       	MAX(FechaOperacion),
			Var_TipoInteres,    'COMISION POR FALTA DE PAGO',   			ROUND(SUM(Cantidad), 2),      	Con_Moneda_Cero,  	IFNULL(PolizaID,Con_PolizaCero),
			Con_Entero_Cero,	Con_Entero_Cero,			Con_Entero_Cero,				Con_Entero_Cero
	FROM CREDITOSMOVS
	WHERE FechaOperacion >= Par_FecIniMes
	  AND FechaOperacion <= Par_FecFinMes
	...
	INSERT INTO EDOCTADETCRE(
		...
		NumAmortizaciones,	AmoCubiertas,	AmoPorCubrir,	CapitalCubierto)
	SELECT  Par_AnioMes,        Par_SucursalID,            	Con_Entero_Cero,    			CreditoID,       	MAX(FechaOperacion),
			Var_TipoInteres,    'COMISION POR APERTURA',   			ROUND(SUM(Cantidad), 2),      	Con_Moneda_Cero,  	IFNULL(PolizaID,Con_PolizaCero),
			Con_Entero_Cero,	Con_Entero_Cero,			Con_Entero_Cero,				Con_Entero_Cero
	FROM CREDITOSMOVS
	WHERE FechaOperacion >= Par_FecIniMes
	  AND FechaOperacion <= Par_FecFinMes
	...
	INSERT INTO EDOCTADETCRE(
		...
		NumAmortizaciones,	AmoCubiertas,	AmoPorCubrir,	CapitalCubierto)
	SELECT  Par_AnioMes,        Par_SucursalID,            	Con_Entero_Cero,    			CreditoID,       	MAX(FechaOperacion),
			Var_TipoInteres,    'COMISION POR GASTOS DE ADMINISTRACION',  ROUND( SUM(Cantidad), 2),      	Con_Moneda_Cero,  	IFNULL(PolizaID,Con_PolizaCero),
			Con_Entero_Cero,	Con_Entero_Cero,			Con_Entero_Cero,				Con_Entero_Cero
	FROM CREDITOSMOVS
	WHERE FechaOperacion >= Par_FecIniMes
	  AND FechaOperacion <= Par_FecFinMes
	...
	INSERT INTO EDOCTADETCRE(
		...
		NumAmortizaciones,	AmoCubiertas,	AmoPorCubrir,	CapitalCubierto)
	SELECT  Par_AnioMes,        Par_SucursalID,            		Con_Entero_Cero,    		Mov.CreditoID,       MAX(Mov.FechaOperacion),
			Var_TipoInteres,    'IVA DE LAS COMISIONES',   ROUND((SUM(Mov.Cantidad)*(IFNULL(Res.ValorIVAAccesorios, 0.0))), 2),      	Con_Moneda_Cero,
