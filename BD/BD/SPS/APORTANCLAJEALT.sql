...
-- APORTANCLAJEALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `APORTANCLAJEALT`;DELIMITER $$

CREATE PROCEDURE `APORTANCLAJEALT`(
# ===========================================================
# ----------- SP QUE REALIZA LOS ANCLAJES DE APORTACIONES-----------
# ===========================================================
	Par_AportacionOriID         INT(11),            -- Numero de Aportacion original
	Par_AportacionAncID         INT(11),            -- Numero de Aportacion que se esta anclando
	Par_MontoTotal              DECIMAL(14,2),      -- Monto que sumariza la Aportacion original con la inversion anclada.
	Par_MontoTotalAnclar        DECIMAL(14,2),      -- Monto de la Aportacion que se esta anclando.
	Par_Plazo                   INT(11),            -- Plazo de la Nueva Inversipon

	Par_Tasa                    DECIMAL(14,4),      -- Tasa que corresponde a la Aportacion con anclaje
	Par_FechaAnclaje            DATE,               -- Fecha en que se esta anclando la Aportacion
	...
	DECLARE Var_AportacionRenovada	INT(11);
	...
	DECLARE Var_DiasPeriodo         INT(11);			-- Almacena los dias por periodo de aportaciones
	...
	SET Var_AportacionAncID			:=  0;              -- Valor Aportacion Anclada
    ...
    SET Cons_Indistinto             := 'I';             -- Capitaliza pagos: Indistinto
    SET Var_AperturaAport			:= 'FA';			-- Apertura DEFAULT Fecha Actual
	...
	ManejoErrores:BEGIN
		DECLARE EXIT HANDLER FOR SQLEXCEPTION
			BEGIN
				...
				SET Par_ErrMen  := CONCAT('El SAFI ha tenido un problema al concretar la operacion. Disculpe las molestias que ',
										 'esto le ocasiona. Ref: SP-APORTANCLAJEALT');
		...
		-- OBTENIENDO LOS DATOS DE LA Aportacion MADRE
		SELECT  AP.CuentaAhoID,       AP.ClienteID,     AP.TipoAportacionID,  AP.FechaInicio,       AP.FechaVencimiento,
				AP.Plazo,             AP.Estatus,       AP.Reinvertir,        AP.AportacionRenovada,      AP.MonedaID,
				AP.SaldoProvision,    AP.Monto,         AP.Reinversion,       AP.InteresGenerado,   AP.InteresRecibir,
				AP.TasaFija,          AP.TasaBase,      AP.SobreTasa,         AP.PisoTasa,          AP.TechoTasa,
				AP.CalculoInteres,    AP.ValorGat,      AP.ValorGatReal,      AP.TasaNeta,          AP.TipoPagoInt,
				AP.CajaRetiro,		  AP.DiasPeriodo,	AP.PagoIntCal,		  AP.DiasPago,			AP.PagoIntCapitaliza,
				AP.OpcionAport,		  AP.CantidadReno,	AP.InvRenovar, 		  AP.Notas
		 INTO   Var_CuentaAho,          Var_Cliente,        Var_TipAportacion,            Var_FechaIni,           Var_FechaVen,
				Var_Plazo,              Var_EstatusAportacion,    Var_Reinvertir,         Var_AportacionRenovada,       Var_MonedaID,
				...
				Var_CajaRetiro,			Var_DiasPeriodo,	Var_PagoIntCal,			Var_DiaPago,			Var_Capitaliza,
				Var_OpcionAport,		Var_CantidadReno,	Var_InvRenovar,			Var_Notas
		   FROM  APORTACIONES AP
			WHERE AP.AportacionID = Par_AportacionOriID;

		-- OBTENIENDO LOS DATOS DEL TIPO Aportacion MADRE
		SELECT  	Anclaje,        TasaMejorada,       EspecificaTasa,     TasaFV,         MontoMinimoAnclaje,
			...
			WHERE 	TipoAportacionID = Var_TipAportacion;
		...
		IF(IFNULL(Par_AportacionOriID,Entero_Cero) = Entero_Cero) THEN
			SET Par_NumErr  :=  3;
			SET Par_ErrMen  :=  'La Aportacion Madre se Encuentra Vacia.';
			SET Var_Control :=  'aportacionOriID';
			LEAVE ManejoErrores;
		END IF;
		...
		IF(IFNULL(Par_MontoTotalAnclar,Decimal_Cero) < Var_MinimoAnclaje) THEN
			SET Par_NumErr  :=  5;
			SET Par_ErrMen  :=  CONCAT('El Monto Minimo de Contratacion para el anclaje de una Aportacion es: ', FORMAT(Var_MinimoAnclaje,2));
		...
		SELECT  IFNULL(COUNT(AP.AportacionID),Entero_Cero)
		INTO    Var_Anclajes
			FROM APORTANCLAJE anc
				INNER JOIN APORTACIONES AP ON anc.AportacionAncID  = AP.AportacionID
									 AND AP.Estatus   = Est_Act
			WHERE anc.AportacionOriID = Par_AportacionOriID;
		...
		IF (Var_DiaInhabil = SabDom AND Var_EsHabil = No_DiaHabil) THEN
			SET Par_NumErr  :=  14;
			SET Par_ErrMen  :=  CONCAT('El Tipo de Aportacion ', Var_TipAportacion,' Tiene Parametrizado Dia Inhabil: Sabado y Domingo
								por tal Motivo No se Puede Anclar la Aportacion.');
			SET Var_Control :=  'aportacionOriID';
			LEAVE ManejoErrores;
		END IF;

		IF(Var_EstatusAportacion = Cancelado) THEN
			SET Par_NumErr  :=  15;
			SET Par_ErrMen  :=  'La Aportacion ha sido cancelada y no puede ser Modificada.';
			SET Var_Control :=  'aportacionOriID';
			LEAVE ManejoErrores;
		END IF;
		...
		-- DANDO DE ALTA EL ANCLAJE EN LA TABLA DE 'APORTACIONES'
		CALL APORTACIONESALT (
			Var_TipAportacion,		Var_CuentaAho,      	Var_Cliente,        	VarFechaSistema,        Var_FechaVen,
			...
			Var_CajaRetiro,			Par_Plazo, 				Var_DiaPago,			Var_Capitaliza,			Var_OpcionAport,
            Var_CantidadReno,		Var_InvRenovar,			Var_Notas,				Var_AperturaAport,		Var_AportacionAncID,
		...
		-- DANDO DE ALTA EL ANCLAJE EN LA TABLA 'APORTANCLAJE'
		SET Var_AportAnclajeID := (SELECT IFNULL(MAX(AportAnclajeID), Entero_Cero) + Entero_Uno FROM APORTANCLAJE);

		INSERT INTO APORTANCLAJE(
			AportAnclajeID,			AportacionOriID,		AportacionAncID,            MontoConjunto,              TotalRecibir,
		...
		VALUES(
			Var_AportAnclajeID,     Par_AportacionOriID,	Var_AportacionAncID,		Par_MontoTotal,             Par_TotalRecibir,
		...
		SET Par_NumErr  :=  0;
		SET Par_ErrMen  :=  CONCAT('Aportacion Agregada Exitosamente: ', CONVERT(Var_AportacionAncID, CHAR));
		SET Var_Control :=  'aportAnclajeID';
	...
	IF(Par_Salida = Salida_SI)THEN
		SELECT  Par_NumErr      AS NumErr,
				...
				Var_AportacionAncID   AS consecutivo;
	END IF;
