...
-- PARAMETROSCRWMOD
DELIMITER ;
DROP PROCEDURE IF EXISTS `PARAMETROSCRWMOD`;
DELIMITER $$

CREATE PROCEDURE `PARAMETROSCRWMOD`(
...
ManejoErrores: BEGIN
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									'Disculpe las molestias que esto le ocasiona. Ref: SP-PARAMETROSCRWMOD');
	...
	UPDATE PARAMETROSCRW SET
		FormulaRetencion	= Par_FormReten,
		TasaISR				= Par_TasaISR,
		PorcISRMoratorio	= Par_PorcISRMora,
		PorcISRComision 	= Par_PorcISRComi,
		MinPorcFonProp 		= Par_MinPorFonPr,

		MaxPorcPagCre		= Par_MaxPorPaCre,
		MaxDiasAtraso		= Par_MaxDiasAtra,
		DiasGraciaPrimVen	= Par_DiasGraPriV,
	...
	SET Par_NumErr := 0;
	SET Par_ErrMen := CONCAT('Parametros Crowdfunding Modificados Exitosamente: ',Par_ProductoCreditoID,'.');
	SET Var_Control:= 'productoCreditoID' ;
