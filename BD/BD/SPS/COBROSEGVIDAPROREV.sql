...
-- COBROSEGVIDAPROREV
DELIMITER ;
DROP PROCEDURE IF EXISTS `COBROSEGVIDAPROREV`;
DELIMITER $$

CREATE PROCEDURE `COBROSEGVIDAPROREV`(
...
DECLARE Var_SegCreditoID    BIGINT(12);
...
SET Con_CobroSegVida:= 35;

SET Act_SegInactivo  := 3;

SET Var_DesPagSeg   := 'REVERSA COBRO COBERTURA DE RIESGO';
...
IF(Par_MontoPago > Var_MontoSegVida) THEN
    SET Par_NumErr  := 3;
    SET Par_ErrMen  := 'El Monto de la Transaccion es mayor al Adeudo';
    LEAVE ManejoErrores;
END IF;

IF(Par_MontoPago <= Entero_Cero) THEN
    SET Par_NumErr  := 4;
    SET Par_ErrMen  := 'El monto de la Transaccion es Incorrecto';
    LEAVE ManejoErrores;
END IF;
...
CALL  CONTACREDITOPRO (
    ...
    Var_SubClasifID,    Var_SucCliente,     Var_DesPagSeg,      Var_StrSegVidaID,   AltaPoliza_SI,
...
IF(( Var_SegVidaPagado-Par_MontoPago) < Var_MontoSegVida AND Var_EstatusSeguro=Est_Vigente)  THEN

    CALL SEGUROVIDAACT (
        Entero_Cero,        Par_CreditoID,  Par_CuentaAhoID,    Act_SegInactivo, Salida_NO,
