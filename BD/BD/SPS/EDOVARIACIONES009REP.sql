...
-- ---------------------------------------------------------------------------------
-- EDOVARIACIONES009REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `EDOVARIACIONES009REP`;

DELIMITER $$
CREATE PROCEDURE `EDOVARIACIONES009REP`(
	-- SP para generar el Estado de Variacion de Capital AyE
...
TerminaStore: BEGIN
	...
	DECLARE Var_SaldoConceptoAnt		DECIMAL(16,2);	-- Saldo Anterior el Concepto Financiero
	DECLARE Var_SaldoConceptoAct		DECIMAL(16,2);	-- Saldo Actual el Concepto Financiero
	DECLARE Var_SaldoConcepto			DECIMAL(16,2);	-- Saldo Total el Concepto Financiero
	DECLARE Var_MontoCampo				DECIMAL(16,2);	-- Acomulado de la Cuenta
	DECLARE Var_AcumuladoCta 			DECIMAL(16,2);	-- Acomulado de la Cuenta
	DECLARE Var_Acumulado 				DECIMAL(16,2);	-- Acomulado de la Cuenta

	DECLARE Var_ParticipacionControladora		CHAR(1);-- Participacion Controladora
	DECLARE Var_CapitalSocial					CHAR(1);-- Capital Social
	DECLARE Var_AportacionesCapital				CHAR(1);-- Aportaciones Capital
	DECLARE Var_PrimaVenta 						CHAR(1);-- Prima Venta
	DECLARE Var_ObligacionesSubordinadas		CHAR(1);-- Obligaciones Subordinadas
	DECLARE Var_IncorporacionSocFinancieras		CHAR(1);-- Incorporacion Financieros
	DECLARE Var_ReservaCapital					CHAR(1);-- Reserva Capital
	DECLARE Var_ResultadoEjerAnterior			CHAR(1);-- Resultado Ejercicio Anterior
	DECLARE Var_ResultadoTitulosVenta			CHAR(1);-- Resultado Titulos Venta
	DECLARE Var_ResultadoValuacionInstrumentos	CHAR(1);-- Resultado Evaluacion Instrumentos
	DECLARE Var_EfectoAcomulado 				CHAR(1);-- Efecto Acomulado
	DECLARE Var_BeneficioEmpleados 				CHAR(1);-- Beneficios Empleados
	DECLARE Var_ResultadoMonetario				CHAR(1);-- Resultado Monitoreo
	DECLARE Var_ResultadoActivos				CHAR(1);-- Resultado Activos
	DECLARE Var_ParticipacionNoControladora		CHAR(1);-- participacion No Controladora
	DECLARE Var_CapitalContable					CHAR(1);-- Capital Contable
    DECLARE Var_EfectoIncorporacion				CHAR(1);-- Efecto Incorporacion
	...
	SET Cuenta_SaldoAnt		:= '30020300000000';
	SET Cuenta_SaldoAct		:= '30020100000000';
	...
	INSERT INTO TMPEDOVARIACIONES(
		...
		ResultadoMonetario,		ResultadoActivos,		ParticipacionNoControladora,	CapitalContable,				EfectoIncorporacion)
	SELECT
	...
	UPDATE TMPEDOVARIACIONES SET
		Descripcion = CONCAT('Saldo al ',FUNCIONLETRASFECHA(DATE_ADD(Var_FecConsultaAnterior, INTERVAL Entero_Cero DAY)))
	...
	WHILE (Var_Contador <= Var_ConceptoFinanIDMax) DO
		...
		-- Evaluacion de Efecto por incorporacion
		SELECT TRIM(EfectoIncorporacion), CuentaContable,		Presentacion
		INTO Var_FormulaConcepto,   Var_CuentaConcepto,	Var_Presentacion
		FROM EDOVARIACIONES
		WHERE EstadoFinanID = Con_EdoVariacion
		  AND CaTConceptos = Var_Contador
		...
		IF IFNULL(Var_FormulaConcepto,Cadena_Vacia) <> Cadena_Vacia THEN
			...
			UPDATE TMPEDOVARIACIONES SET
				EfectoIncorporacion = CASE WHEN Var_Presentacion = SaldoContableActual   THEN Var_SaldoConceptoAct
									 WHEN Var_Presentacion = SaldoInicioFin		   THEN Var_SaldoConceptoAct - Var_SaldoConceptoAnt
									 WHEN Var_Presentacion = SaldoFinInicio		   THEN Var_SaldoConceptoAnt - Var_SaldoConceptoAct
									 WHEN Var_Presentacion = SaldoContableAnterior THEN Var_SaldoConceptoAnt
								END
		...
		IF Par_Cifras = Tipo_Miles THEN
			UPDATE TMPEDOVARIACIONES SET
                ...
                EfectoIncorporacion				= ROUND((EfectoIncorporacion / Con_Mil), Con_DosDecimales)
			WHERE NumeroTransaccion = Aud_NumTransaccion
			  AND CaTConceptos = Var_Contador;
		END IF;
	...
	SELECT	ParticipacionControladora,			CapitalSocial,			AportacionesCapital,		PrimaVenta,					ObligacionesSubordinadas,
			...
			CapitalContable,					EfectoIncorporacion
	INTO	Var_ParticipacionControladora,		Var_CapitalSocial,		Var_AportacionesCapital,	Var_PrimaVenta,				Var_ObligacionesSubordinadas,
			...
			Var_CapitalContable,				Var_EfectoIncorporacion
	FROM EDOVARIACIONES
	WHERE EstadoFinanID = Con_EdoVariacion
	  AND CaTConceptos = Entero_Cero
	  AND NumeroCliente = Var_NumCliente;
	...
	WHILE (Var_Contador <= Var_ConceptoFinanIDMax) DO
        ...
        -- Columna EfectoIncorporacion
		IF( Var_EfectoIncorporacion = Con_SI ) THEN
			...
			IF( Var_MostrarFila = Con_SI ) THEN
				...
				IF( Var_FormulaReporte <> Cadena_Vacia) THEN
					...
					UPDATE TMPEDOVARIACIONES SET
						EfectoIncorporacion = Var_AcumuladoCta
					WHERE NumeroTransaccion = Aud_NumTransaccion
					  AND CaTConceptos = Var_Contador;
	...
	UPDATE TMPEDOVARIACIONES SET
		CapitalContable = CapitalSocial +		AportacionesCapital+	PrimaVenta +			ObligacionesSubordinadas + 		IncorporacionSocFinancieras +
						  ...
						  BeneficioEmpleados +	ResultadoMonetario+		ResultadoActivos +		ParticipacionNoControladora + EfectoIncorporacion,
	...
	SELECT	Descripcion,					ParticipacionControladora,		CapitalSocial,		AportacionesCapital,	PrimaVenta,
			...
			ParticipacionNoControladora,	CapitalContable, 				EfectoIncorporacion
	FROM TMPEDOVARIACIONES
	WHERE NumeroTransaccion = Aud_NumTransaccion;
