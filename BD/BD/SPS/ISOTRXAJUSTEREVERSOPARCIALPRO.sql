DELIMITER ;
DROP PROCEDURE IF EXISTS ISOTRXAJUSTEREVERSOPARCIALPRO;
DELIMITER $$

CREATE PROCEDURE ISOTRXAJUSTEREVERSOPARCIALPRO(
	-- Descripcion: Proceso operativo que realiza el ajuste y reverso parcial de las operciones de isotrx.
...
TerminaStore: BEGIN
	...
	-- Declaracion de tipo transacciones
	...
	-- Asinacion de numero de transacciones
	SET Tran_Ajuste						:= 21;
	...
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
								'Disculpe las molestias que esto le ocasiona. Ref: SP-ISOTRXAJUSTEREVERSOPARCIALPRO');
		...
		-- Se validan las operaciones para la reversa
		IF(FIND_IN_SET(Var_TipoOperacionID,Var_OperacionExistente) = Entero_Cero)THEN
			SET Par_NumErr	:= 1324;
			SET Par_ErrMen	:= 'Tipo de Operación no disponible para cancelación o devolución.';
			SET Var_Control	:= 'numeroTarjeta';
			LEAVE ManejoErrores;
		END IF;
		...
		CALL ISOTRXTARJETADEBVAL(
				...
				Nat_Reversa,		Var_MontoTransaccion,	Par_MontoRemplazo,	Par_GiroComercial,
