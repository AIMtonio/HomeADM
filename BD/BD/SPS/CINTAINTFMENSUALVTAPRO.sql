...
-- ---------------------------------------------------------------------------------
DELIMITER ;
DROP PROCEDURE IF EXISTS `CINTAINTFMENSUALVTAPRO`;
DELIMITER $$

CREATE PROCEDURE `CINTAINTFMENSUALVTAPRO`(
...
DECLARE ClaveVentaCesionNA		CHAR(2);
DECLARE ClaveVentaCesionNV		CHAR(2);
...
DECLARE Var_TipoNegVta			VARCHAR(45);
...
SET ClaveVentaCesionNV		:= 'NV';
SET ClaveVentaCesionNA		:= 'NA';
...
SELECT 	 ClaveInstitID,	    Nombre,				TipoNegVta
  INTO 	 Var_Member,		Var_NombreEmp,		Var_TipoNegVta
	FROM BUCREPARAMETROS;

SET Var_Member = Var_TipoNegVta;
...
SELECT	MAX(Cli.ClienteID) AS ClienteID,		IFNULL(MAX(Cli.ApellidoPaterno),Cadena_Vacia) AS ApellidoPaterno,   IFNULL(MAX(Cli.ApellidoMaterno),Cadena_Vacia) AS ApellidoMaterno,   Cadena_Vacia AS Adicional,
		...
		CASE WHEN LENGTH(MAX(Cli.RFC)) != 13 AND MAX(Cli.TipoPersona) != PersMoral THEN  Cadena_Vacia ELSE MAX(Cli.RFC) END AS RFC,
		IFNULL(Cli.Titulo,Cadena_Vacia) AS Prefijo,
    	CASE MAX(Cli.EstadoCivil)
		    WHEN Casado_BM		THEN Casado_BC 		WHEN Casado_BMC		THEN Casado_BC
		    WHEN Casado_BS		THEN Casado_BC 		WHEN Viudo    		THEN Viudo_BC
		    WHEN Union_libre 	THEN Union_libreBC 	WHEN EdoSeparado 	THEN Soltero_BC ELSE LEFT(MAX(Cli.EstadoCivil), 1) END AS EstadoCivil,
		MAX(Cli.Sexo) AS Sexo,					Cadena_Vacia AS FDef,												Cadena_Vacia AS InDef,
		IF((IFNULL(MAX(Dir.Calle),Cadena_Vacia)=Cadena_Vacia AND IFNULL(MAX(Dir.NumeroCasa),Entero_Cero)=Entero_Cero)OR(IFNULL(MAX(Dir.Calle),Cadena_Vacia) LIKE CalleConocida OR IFNULL(MAX(Dir.Calle),Cadena_Vacia) LIKE CalleConocido), DomicilioConocido,
    	...
    	MAX(Est.EqBuroCred) AS Estado,			Cadena_Vacia AS member,												MAX(Dir.CP) AS CP,
    	SUBSTRING(REPLACE(IF(IFNULL(MAX(Cli.Telefono),Cadena_Vacia)!=Cadena_Vacia,MAX(Cli.Telefono),MAX(Cli.TelefonoCelular)), ' ', ''), 1, 10) AS Telefono,							Cadena_Vacia AS DirComplemento,
		CASE IFNULL(REPLACE(MAX(Cli.LugardeTrabajo),Entero_Cero,Cadena_Vacia),Cadena_Vacia)
			WHEN Cadena_Vacia THEN	LEFT(IF(ifnull(REPLACE(MAX(Cli.LugardeTrabajo),Entero_Cero,Cadena_Vacia),Cadena_Vacia)=Cadena_Vacia,'TRABAJADOR INDEPENDIENTE',MAX(Cli.LugardeTrabajo)),40)
			WHEN 'ASALARIADO' THEN	'NEGOCIO PROPIO'	ELSE LEFT(MAX(Cli.LugardeTrabajo),40) END AS Empresa,
			-- INFORMACION DEL TRABAJO DEL CLIENTE O ACREDITADO
		Cadena_Vacia AS CalleNumero2,		Cadena_Vacia AS Colonia2,	Cadena_Vacia AS Municipio2,		Cadena_Vacia AS Ciudad2,		Cadena_Vacia AS Estado2,	Cadena_Vacia  AS CP2,
		...
		Cadena_Vacia  AS DirComplemento2,	Cadena_Vacia AS Salario,    MAX(EqC.CreditoIDCte) AS CreditoIDCte,               MAX(Cre.Estatus) AS Estatus,	Cre.CreditoID,
	    ...
	    CASE 	WHEN MAX(Cre.FrecuenciaCap)  = 'C' THEN 'K'	WHEN MAX(Cre.FrecuenciaCap)  = 'Q' THEN 'S'
	      		WHEN MAX(Cre.FrecuenciaCap)  = 'M' THEN 'M'	WHEN MAX(Cre.FrecuenciaCap)  = 'B' THEN 'B'
				WHEN MAX(Cre.FrecuenciaCap)  = 'E' THEN 'H'	WHEN MAX(Cre.FrecuenciaCap)  = 'A' THEN 'Y'
				WHEN MAX(Cre.FrecuenciaCap)  = 'S' OR MAX(Cre.FrecuenciaCap)  = 'D'	THEN 'W'
	      		...
	      		WHEN MAX(Cre.FrecuenciaCap)  = 'P' OR MAX(Cre.FrecuenciaCap)  = 'L'	OR   MAX(Cre.FrecuenciaCap)  = 'U' THEN 'V'	END AS FrecPagos,
		Entero_Cero AS Saldo,							MAX(Cre.MontoCredito) AS MontoCredito,	Entero_Cero AS Vencido,			DATE_FORMAT(MAX(Cre.FechaInicio), FormatoFecha) AS FechaInicio,
    	Entero_Cero AS  ImportePagos,					DATE_FORMAT(MAX(Cre.FechTerminacion),FormatoFecha) AS FechaCierre,		Entero_Cero AS NoCuotasAtraso,
    	Entero_Cero AS DiasAtraso,						MAX(Pro.DiasPasoAtraso) AS DiasPasoAtraso,								Entero_Cero AS  SaldoTotal,
    	Cadena_Vacia AS MOP,							MAX(Pro.TipoContratoBCID) AS TipoContratoBCID,							Cadena_Vacia AS Moneda,
    	MAX(Pro.EsGrupal) AS EsGrupal,					MAX(Pro.EsRevolvente) AS EsRevolvente,									MAX(Cli.TipoPersona) AS TipoPersona,
    	Cadena_Vacia AS ClaveObervacion,				Entero_Cero AS MontoUltPago,											IFNULL(MAX(Cre.PlazoID),Entero_Cero) AS PlazoID,
    	Cadena_Vacia AS Nacionalidad,					MAX(Cli.LugarNacimiento) AS LugarNacimiento,							Entero_Cero AS DiasAtraFecFinCre,
    	MAX(Cli.PaisResidencia) AS PaisResidencia,		Cadena_Vacia AS NacDomicilio,											IFNULL(MAX(Cli.Correo),Cadena_Vacia) AS CorreoElect,
	    CASE WHEN MAX(Cre.FechTraspasVenc) != Fecha_Vacia AND MAX(Cre.FechTraspasVenc) < Par_FechaCorteBC THEN DATE_FORMAT(MAX(Cre.FechTraspasVenc), FormatoFecha) ELSE Cadena_Vacia END,
		MAX(Cli.OcupacionID) AS OcupacionID,			Entero_Cero AS PlazoCredito,            								MAX(Cre.MonedaID) AS MonedaID,
		MAX(Pro.GraciaMoratorios) AS GraciaMoratorios,	MAX(Pro.DiasPasoAtraso) AS DiasPasoAtraso,								MAX(Cre.FechTerminacion) AS FechTerminacion,
	   	MAX(Cre.FechaInicio) AS FechaInicio,			Entero_Cero AS InteresReportado,										Cadena_Vacia AS Cuenta,
	...
	FROM CREDITOS Cre
		...
		INNER JOIN HISCREDITOSVENTACAR HIS ON Cre.CreditoID = HIS.CreditoID AND HIS.Tipo = 'PROCESADO' and HIS.Fecha <= Par_FechaCorteBC
	WHERE Cre.Estatus IN (EstatusVendido)
		AND Cre.FechaInicio <= Var_FechaFin
		AND (	Cre.FechTerminacion <= Par_FechaCorteBC)
        AND	Cli.TipoPersona = TipoPersFisica
	GROUP BY  Cre.CreditoID);

	/* Actualizamos la Direccion del Trabajo. REVISADO*/
	UPDATE 	TMPDATOSCTESCREDPAG Tem,	DIRECCLIENTE Dir,
			ESTADOSREPUB Est,			MUNICIPIOSREPUB Mun,
			LOCALIDADREPUB Loc,			COLONIASREPUB Col SET
		Tem.CalleNumero2 = IF((	IFNULL(Dir.Calle, Cadena_Vacia) = Cadena_Vacia AND IFNULL(Dir.NumeroCasa, Entero_Cero) = Entero_Cero) OR
								(IFNULL(Dir.Calle,Cadena_Vacia) LIKE CalleConocida OR  IFNULL(Dir.Calle,Cadena_Vacia) LIKE CalleConocido),
							DomicilioConocido, CONCAT(LEFT(IFNULL(Dir.Calle, Cadena_Vacia), (40-LENGTH(IF(IFNULL(Dir.NumeroCasa, Entero_Cero) = Entero_Cero,
							SinNumero, CONCAT(EtiquetaNumero,Dir.NumeroCasa))))),IF(IFNULL(Dir.NumeroCasa,Entero_Cero) = Entero_Cero,SinNumero,
		...
		Tem.Ciudad2		= TRIM(LEFT(Mun.Nombre,40)) ,
...
set Empresa      ='TRABAJADOR INDEPENDIENTE',
	CalleNumero2 = CalleNumero,
	Colonia2	 = Colonia,
	...
	CP2			 = CP,
	Telefono2    = Telefono,
	DirComplemento2 = DirComplemento
where Empresa='TRABAJADOR INDEPENDIENTE';

/*Actualizo las direcciones vacias */
update TMPDATOSCTESCREDPAG
set CalleNumero2 = CalleNumero,
	Colonia2	 = Colonia,
	Municipio2   = Municipio,
	Ciudad2		 = Ciudad,
	Estado2      = Estado,
	...
	Telefono2    = Telefono,
	DirComplemento2 = DirComplemento
where IFNULL(CalleNumero2,Cadena_Vacia)=Cadena_Vacia;

/* Actualizamos el nombre de el municipio. REVISADO */
UPDATE TMPDATOSCTESCREDPAG D, TEMPORAL_MUNICIP T SET
	D.Municipio 	= T.NombreCorrecto,
	D.Municipio2 	= T.NombreCorrecto,
	D.Ciudad 		= T.NombreCorrecto,
	D.Ciudad2 		=  T.NombreCorrecto
WHERE D.Municipio = T.Nombre;

/* Eliminamos Creditos que se desembolsaron posterior a la fecha corte*/
DELETE FROM TMPDATOSCTESCREDPAG WHERE CONVERT(SUBSTRING(FechaInicio,3,2),SIGNED)>MONTH(Var_FechaFin) AND SUBSTRING(FechaInicio,5,4) = YEAR(Var_FechaFin);

/* Eliminamos Creditos que se hayan liquidado anterior a la Fecha corte*/
-- DELETE FROM TMPDATOSCTESCREDPAG WHERE CONVERT(SUBSTRING(FechaCierre,3,2),SIGNED)<MONTH(Var_FechaFin) AND SUBSTRING(FechaCierre,5,4) = YEAR(Var_FechaFin);

/* Se Inicializan las Fechas de Pago vacio */
UPDATE TMPDATOSCTESCREDPAG Dat INNER JOIN CREDITOS Cre ON Dat.CreditoIDSAFI =Cre.CreditoID
...
UPDATE TMPDATOSCTESCREDPAG Det
	...
	Det.Responsabilidad = (CASE Pro.EsGrupal
								WHEN Str_NO THEN TipoRespIndividual
								WHEN Str_SI THEN TipoRespObligadoSol END),
	Det.TipoCuenta 		= (CASE Pro.EsRevolvente
								WHEN Str_NO THEN TipoCtaPagosFijos
								WHEN Str_SI THEN TipoCtaRevolvente END);

-- Actualiza creditos que no manejan linea de credito pero aparecen como Revolventes
UPDATE TMPDATOSCTESCREDPAG Det INNER JOIN CREDITOS Cre 		ON Det.CreditoIDSAFI	= Cre.CreditoID SET
	Det.TipoCuenta = TipoCtaPagosFijos
WHERE Cre.LineaCreditoID = Entero_Cero AND Det.TipoCuenta = TipoCtaRevolvente;
...
SELECT 	Tem.CreditoIDSAFI,
	CASE WHEN IFNULL(Sal.EstatusCredito, Cadena_Vacia)  = Cadena_Vacia THEN Tem.Estatus ELSE Sal.EstatusCredito END,
	...
	IFNULL(Sal.NoCuotasAtraso, Entero_Cero),	IFNULL(Sal.DiasAtraso, Entero_Cero),
...
FROM TMPDATOSCTESCREDPAG Tem
	LEFT OUTER JOIN SALDOSCREDITOS Sal ON Tem.CreditoIDSAFI = Sal.CreditoID AND Sal.FechaCorte = Par_FechaCorteBC
...
WHERE Dat.CreditoIDSAFI = Tem.CreditoID;
  ...
  INSERT INTO TMPMONTOCUOTACREDITO
	...
	-- ---------------------------------------------------------
	UPDATE TMPDATOSCTESCREDPAG Dat SET
		MOP = CASE 	WHEN Dat.DiasAtraso  BETWEEN 1 AND 29 THEN Mop02
...
*/
  ...
  INSERT INTO TMPFECHACIERREBC
	...
	update TMPDATOSCTESCREDPAG set
			...
			Vencido    		= Entero_Cero,
			NoCuotasAtraso  = Entero_Cero,
			DiasAtraso 		= Entero_Cero,
			DiasPasoAtraso 	= Entero_Cero,
			ClaveObervacion = Cadena_Vacia,
			Incumplimiento 	= Fecha_Vaciaddmmyyyy,
			FechaUltPagoVen = Fecha_Vaciaddmmyyyy
	WHERE Vencido BETWEEN 1 AND 49;
	...
	WHERE Dat.CreditoIDSAFI = Cla.CreditoID
    	...
    	-- SE ACTUALIZA LA CLAVE PARA LOS CREDITOS VENDIDOS
	UPDATE TMPDATOSCTESCREDPAG Dat
		INNER JOIN  HISCREDITOSVENTACAR Cas ON Dat.CreditoIDSAFI = Cas.CreditoID SET

		Dat.FechaCierre 	= DATE_FORMAT(Cas.Fecha, FormatoFecha),
		Dat.ClaveObervacion	= ClaveVentaCesionNA,
        Dat.ImportePagos	=  	ifnull(Cas.SaldoCapAtrasa,Entero_Cero)		+ ifnull(Cas.SaldoCapVencido,Entero_Cero)	+ ifnull(Cas.SaldoCapVenNExi,Entero_Cero) +
								ifnull(Cas.SaldoCapVigente,Entero_Cero)	+ ifnull(Cas.SaldoComFaltaPa,Entero_Cero) 	+ ifnull(Cas.SalMoraCarVen,Entero_Cero)   +
								ifnull(Cas.SalMoraVencido,Entero_Cero) 	+ ifnull(Cas.SaldoInteresAtr,Entero_Cero) 	+ ifnull(Cas.SaldoIntNoConta,Entero_Cero) +
								ifnull(Cas.SaldoInteresPro,Entero_Cero) 	+ ifnull(Cas.SaldoInteresVen,Entero_Cero) 	+ ifnull(Cas.SaldoMoratorios,Entero_Cero) +
								ifnull(Cas.SaldoOtrasComis,Entero_Cero),
        Dat.SaldoTotal		= 	ifnull(Cas.SaldoCapAtrasa,Entero_Cero)		+ ifnull(Cas.SaldoCapVencido,Entero_Cero)	+ ifnull(Cas.SaldoCapVenNExi,Entero_Cero) +
								ifnull(Cas.SaldoCapVigente,Entero_Cero)	+ ifnull(Cas.SaldoComFaltaPa,Entero_Cero) 	+ ifnull(Cas.SalMoraCarVen,Entero_Cero)   +
								ifnull(Cas.SalMoraVencido,Entero_Cero) 	+ ifnull(Cas.SaldoInteresAtr,Entero_Cero) 	+ ifnull(Cas.SaldoIntNoConta,Entero_Cero) +
								ifnull(Cas.SaldoInteresPro,Entero_Cero) 	+ ifnull(Cas.SaldoInteresVen,Entero_Cero) 	+ ifnull(Cas.SaldoMoratorios,Entero_Cero) +
								ifnull(Cas.SaldoOtrasComis,Entero_Cero),
		Dat.Vencido 		=   IFNULL(Cas.SaldoCapAtrasa, Entero_Cero) + IFNULL(Cas.SaldoCapVencido , Entero_Cero)+
								IFNULL(Cas.SaldoInteresAtr, Entero_Cero) + IFNULL(Cas.SaldoInteresVen, Entero_Cero) + IFNULL(Cas.SaldoMoratorios, Entero_Cero) +
								IFNULL(Cas.SalMoraVencido, Entero_Cero) + IFNULL(Cas.SaldoComFaltaPa, Entero_Cero) + IFNULL(Cas.SaldoOtrasComis, Entero_Cero)
		WHERE Cas.Fecha <= Par_FechaCorteBC
		and Cas.Tipo="PROCESADO";


    -- SE ACTUALIZA LA CLAVE PARA LOS CREDITOS VENDIDOS
	UPDATE TMPDATOSCTESCREDPAG Dat SET
		Dat.ClaveObervacion	= ClaveVentaCesionNV
    WHERE  FecTerminacion < Var_FechaInicial
		and Dat.Vencido > Entero_Cero;
	...
	UPDATE TMPDATOSCTESCREDPAG Dat
		INNER JOIN  HISCREDITOSVENTACAR Cas ON Dat.CreditoIDSAFI = Cas.CreditoID SET
		Dat.MOP 			= Mop01,
		Dat.ClaveObervacion	= Cadena_Vacia,
		Dat.FechaCierre 	= Fecha_Vaciaddmmyyyy
	WHERE Cas.Fecha >= Var_FechaInicial
		and Cas.Tipo="PROCESADO";
