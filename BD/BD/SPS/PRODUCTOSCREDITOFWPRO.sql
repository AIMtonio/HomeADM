...
-- PRODUCTOSCREDITOFWPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `PRODUCTOSCREDITOFWPRO`;
DELIMITER $$


CREATE PROCEDURE `PRODUCTOSCREDITOFWPRO`(
	...
	ManejoErrores: BEGIN
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen  = CONCAT(	'El SAFI ha tenido un problema al concretar la operacion. ',
										'Disculpe las molestias que esto le ocasiona. Ref: SP-PRODUCTOSCREDITOFWPRO');
			SET Var_Control = 'sqlException';
		END;

		SET Par_ProductoCreditoFWID := IFNULL(Par_ProductoCreditoFWID,Entero_Cero);
		...
		IF(Par_ProductoCreditoFWID = Entero_Cero)THEN
			CALL PRODUCTOSCREDITOFWALT (	Par_ProductoCreditoFWID,	Par_ProductoCreditoID,	Par_DestinoCreditoID,  	Par_ClasificacionDestino,   Par_PerfilID,
		...
		IF(Par_NumErr > Entero_Cero)THEN
			SET Par_NumErr	:= 3;
			SET Par_ErrMen	:= 'Error en alta de producto.';
			SET Var_Control	:= 'productoCreditoID';
			LEAVE ManejoErrores;
		END IF;

		IF(Par_ProductoCreditoFWID > Entero_Cero)THEN
			CALL PRODUCTOSCREDITOFWMOD (	Par_ProductoCreditoFWID,	Par_ProductoCreditoID,	Par_DestinoCreditoID,  	Par_ClasificacionDestino,   Par_PerfilID,
											Var_SalidaNO,				Par_NumErr,				Par_ErrMen,				Par_EmpresaID,				Aud_Usuario,
											Aud_FechaActual,   			Aud_DireccionIP,       	Aud_ProgramaID,			Aud_Sucursal,				Aud_NumTransaccion);
		END IF;
		...
		IF(Par_NumErr > Entero_Cero)THEN
			SET Par_NumErr	:= 4;
			SET Par_ErrMen	:= 'Error al modificar el producto.';
			SET Var_Control	:= 'productoCreditoID';
			LEAVE ManejoErrores;
		END IF;
		...
		SET Par_NumErr		:= 0;
		SET Par_ErrMen		:= 'Registros procedados exitosamente';
		SET Var_Control		:= 'productoCreditoID';
