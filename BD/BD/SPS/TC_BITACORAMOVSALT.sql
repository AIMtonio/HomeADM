...
-- TC_BITACORAMOVSALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `TC_BITACORAMOVSALT`;

DELIMITER $$
CREATE PROCEDURE `TC_BITACORAMOVSALT`(
/* ----------------------------------------------------------
    Registra los datos de la transaccion en
...
-- ---------------------------------------------------------- */
    ...
    INOUT Par_TarCredMovID  INT,             -- Identificador del movimiento
    ...
    ManejoErrores:BEGIN
        DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
            ...
            SET Par_ErrMen  = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
                                     'Disculpe las molestias que esto le ocasiona. Ref: SP-TC_BITACORAMOVSALT');
        END;

        SET Aud_FechaActual := NOW();

        SELECT MAX(TarCredMovID)
        INTO Var_TarCredMovID
        FROM TC_BITACORAMOVS;

        SET Var_TarCredMovID := IFNULL(Var_TarCredMovID,Entero_Cero) + 1;

        INSERT INTO TC_BITACORAMOVS (
            TarCredMovID,           OrigenOperacion,        TipoMensaje,            TipoOperacionID,        TarjetaCredID,
        ...
        VALUES (
            Var_TarCredMovID,       Par_OrigenOperacion,    Par_TipoMensaje,        Par_TipoOperacionID,    Par_TarjetaCredID,
        ...
        SET Par_TarCredMovID := Var_TarCredMovID;
