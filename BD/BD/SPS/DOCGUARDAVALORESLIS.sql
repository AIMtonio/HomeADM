...
-- DOCGUARDAVALORESLIS
DELIMITER ;
DROP PROCEDURE IF EXISTS DOCGUARDAVALORESLIS;

DELIMITER $$
CREATE PROCEDURE `DOCGUARDAVALORESLIS`(
	-- Store Procedure: Que Lista la configuracion de los Parametros de Guarda Valores
	-- Modulo Guarda Valores
	Par_DocGuardaValoresID		INT(11),			-- ID de Tabla
...
TerminaStore: BEGIN
	...
	IF( Par_NumLista = Lis_Principal ) THEN
		...
		SELECT	Doc.DocGuardaValoresID,		Doc.ParamGuardaValoresID,
				Doc.DocumentoID,			Tip.Descripcion AS NombreDocumento
		FROM DOCGUARDAVALORES Doc
		INNER JOIN TIPOSDOCUMENTOS Tip ON Tip.TipoDocumentoID = Doc.DocumentoID
