...
-- ---------------------------------------------------------------------------------
-- CUENTASMAYORINVBANBAJ
DELIMITER ;
DROP PROCEDURE IF EXISTS `CUENTASMAYORINVBANBAJ`;DELIMITER $$

CREATE PROCEDURE `CUENTASMAYORINVBANBAJ`(
	...
	ManejoErrores: BEGIN
		...
		BEGIN
			...
			SET Par_ErrMen = concat('Estimado Usuario(a), Ha ocurrido una falla en el sistema, ' ,
									 ...
									 'esto le ocasiona. Ref: SP-CUENTASMAYORINVBANBAJ');
		...
		IF(ifnull(Par_ConceptoInvBanID, Entero_Cero))= Entero_Cero then
			...
			LEAVE TerminaStore;
		END IF;
		IF NOT EXISTS( SELECT ConceptoInvBanID FROM CUENTASMAYORINVBAN 	WHERE ConceptoInvBanID=Par_ConceptoInvBanID) THEN
				Set Par_NumErr	:= '002';
