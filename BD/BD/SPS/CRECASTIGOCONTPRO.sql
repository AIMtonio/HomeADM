...
-- CRECASTIGOCONTPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `CRECASTIGOCONTPRO`;DELIMITER $$

CREATE PROCEDURE `CRECASTIGOCONTPRO`(
-- =======================================================================
-- SP QUE REALIZA EL PROCESO DE CASTIGO PARA LOS CREDITOS CONTINGENTES
-- =======================================================================
...
DECLARE Con_OrdCastigo  	INT;				-- Contante  de Orden de Cartera Castigada
...
SET Con_CtaOrdCom       := 108;
SET Con_CorComFal       := 109;
SET Con_IngresoMora		:= 6;
SET Con_IngMoraCarVen	:= 35;
SET Con_CtaOrdMor       := 106;
SET Con_CorIntMor       := 107;
SET	Con_MoraVencido		:= 34;
SET	Con_MoraVigente		:= 33;
SET Con_CtaOrdInt       := 104;
SET Con_CorIntOrd       := 105;
...
SET Con_OrdCastigo      := 102;
SET Con_CorCastigo      := 103;
	...
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
							'Disculpe las molestias que esto le ocasiona. Ref: SP-CRECASTIGOCONTPRO');
		END;

DROP TABLE IF EXISTS TMPAMORTICONT;
CREATE TEMPORARY TABLE TMPAMORTICONT(
	IDReg			INT(11),
	CreditoID		BIGINT(12),
    AmortizacionID	INT(11),
    SaldoCapVigente	DECIMAL(12),
    SaldoCapAtrasa	DECIMAL(12),
    SaldoCapVencido	DECIMAL(12),
    SaldoCapVenNExi	DECIMAL(12),
    SaldoInteresOrd	DECIMAL(12),
    SaldoInteresAtr	DECIMAL(12),
    SaldoInteresVen	DECIMAL(12),
    SaldoInteresPro	DECIMAL(12),
    SaldoIntNoConta	DECIMAL(12),
    SaldoMoratorios	DECIMAL(12),
    SaldoComFaltaPa	DECIMAL(12),
    SaldoOtrasComis	DECIMAL(12),
    MonedaID		INT(11),
    FechaInicio		DATE,
    FechaVencim		DATE,
    FechaExigible	DATE,
	Estatus			CHAR(1),
    SaldoMoraVencido	DECIMAL(12),
    SaldoMoraCarVen		DECIMAL(12)
    );

SET @cont :=0;

INSERT INTO TMPAMORTICONT
   SELECT  	(@cont:=@cont+1),		Amo.CreditoID,          Amo.AmortizacionID,     Amo.SaldoCapVigente,	Amo.SaldoCapAtrasa,
            Amo.SaldoCapVencido,    Amo.SaldoCapVenNExi,    Amo.SaldoInteresOrd,	Amo.SaldoInteresAtr,
            Amo.SaldoInteresVen,    Amo.SaldoInteresPro,    Amo.SaldoIntNoConta,	Amo.SaldoMoratorios,
            Amo.SaldoComFaltaPa,    Amo.SaldoOtrasComis,    Cre.MonedaID,			Amo.FechaInicio,
            Amo.FechaVencim,        Amo.FechaExigible,      Amo.Estatus,			Amo.SaldoMoraVencido,
			Amo.SaldoMoraCarVen
...
WHILE (@Contador < Var_NumAmorti) DO
	...
	INTO	Var_CreditoID,          Var_AmortizacionID ,    Var_SaldoCapVigente,    Var_SaldoCapAtrasa,
			...
			Var_FechaVencim,        Var_FechaExigible,      Var_AmoEstatus,			Var_SalMoraVencido,
			Var_SalMoraCarVen
	FROM TMPAMORTICONT
	WHERE  IDReg = @contador;
		...
		SET Var_CasAccesor  := Var_CasAccesor + Var_SaldoComFaltaPa + Var_SaldoOtrasComis;
    ...
    IF (Var_SaldoComFaltaPa >= Mon_MinPago) THEN
        CALL  CONTACREDITOSCONTPRO (
            ...
            Par_SalidaNO,       Par_NumErr,             Par_ErrMen,         Par_Consecutivo,    Par_EmpresaID,  	Cadena_Vacia,
