...
-- EDOCTAGRAFICAPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `EDOCTAGRAFICAPRO`;DELIMITER $$

CREATE PROCEDURE `EDOCTAGRAFICAPRO`(
	-- SP para obtener los Datos para la grafica del Cliente para el Estado de Cuenta
...
DECLARE GraficaCtaEdoCta		INT(11);					-- Identificador Datos de la grafica del Cliente para el Estado de Cuenta
...
SET GraficaCtaEdoCta			:= 16;						-- Identificador Datos la grafica del Cliente para el Estado de Cuenta
...
ManejoErrores:BEGIN
	...
	IF(Var_CliProEspEdoCta = NumClienteNuevo) THEN
		SET Var_ProcPersonalizado := (SELECT NomProc FROM CATPROCEDIMIENTOS WHERE ProcedimientoID = GraficaCtaEdoCta AND CliProEspID = Var_CliProEspEdoCta);
	ELSE
		SET Var_ProcPersonalizado := (SELECT NomProc FROM CATPROCEDIMIENTOS WHERE ProcedimientoID = GraficaCtaEdoCta AND CliProEspID = Entero_Cero);
	END IF;
