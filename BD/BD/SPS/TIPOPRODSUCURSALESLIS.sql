...
-- ---------------------------------------------------------------------------------
-- TIPOPRODSUCURSALESLIS
DELIMITER ;
DROP PROCEDURE IF EXISTS `TIPOPRODSUCURSALESLIS`;DELIMITER $$

CREATE PROCEDURE `TIPOPRODSUCURSALESLIS`(
# =================================================================
...
# =================================================================
	...
	SET	Entero_Cero						:= 0;
	SET	Lis_Principal					:= 1;
	SET Lis_PorSucursal					:= 2;
	SET Lis_PorEstado					:= 5;
	SET Lis_PorSucursalTasas			:= 7;
	SET Lis_PorEstadoTasas 				:= 10;
	SET Lis_PorPrincipalTasas 			:= 12;
	...
	SELECT	Tip.TasaCedeSucID,		Tip.TipoCedeID,
				LPAD(Tip.SucursalID, 3, '0') AS SucursalID,	Tip.EstadoID,	Estatus_Activo AS Estatus,
				Suc.NombreSucurs AS NombreSucursal,			Est.Nombre AS NombreEstado
			FROM	TASACEDESUCURSALES Tip
					INNER JOIN SUCURSALES Suc ON Tip.SucursalID = Suc.SucursalID
					INNER JOIN ESTADOSREPUB Est ON Tip.EstadoID = Est.EstadoID
			WHERE 	Tip.TipoCedeID 	= Par_InstrumentoID
			AND 	Tip.TasaCedeID	= Par_TipoInstrumentoID
			AND 	Tip.Estatus 	= Estatus_Activo;
	END IF;
