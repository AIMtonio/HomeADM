...
-- ---------------------------------------------------------------------------------
-- CALCULARCATPAGLIBPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `CALCULARCATPAGLIBPRO`;
DELIMITER $$


CREATE PROCEDURE `CALCULARCATPAGLIBPRO`(
-- ---------------------------------------------------------------- --
-- SP QUE CALCULA EL CAT PARA PAGOS DE CAPITAL UNICOS Y LIBRES      --
-- ---------------------------------------------------------------- --
    ...
    Par_ComAnualLin			DECIMAL(14,4),		-- Comisión Anual de linea de crédito
...
DECLARE Var_SumDiasPL       INT;
DECLARE Var_TotalDias       INT;
DEClARE Var_AuxConta        INT;
...
BEGIN
	...
	SELECT 	        TA.Tasa         INTO Var_TasaGarantia
		...
		and         TC.GeneraInteres =   Var_Si
		and         CA.EsPrincipal   =   Var_Si
		and         TA.MonedaID      =    1
	...
	SET Var_TotalDias := (SELECT sum(TC.Tmp_Dias) FROM TMPPAGAMORSIM TC WHERE TC.NumTransaccion = NumTransaccion);

	IF Var_TasaGarantia > 0 THEN
		SET Var_InteresGL := (Var_AportaCliente * Var_TasaGarantia * Var_TotalDias) / 36000;
    ...
    WHILE Var_AuxConta < Var_Cre_NumPag DO
        SET Var_AuxConta := Var_AuxConta + 1;
        SET Var_SumDiasPL := (SELECT sum(TMP.Tmp_Dias) FROM TMPPAGAMORSIM TMP WHERE TMP.NumTransaccion = NumTransaccion and TMP.Tmp_Consecutivo <= Var_AuxConta);

        UPDATE TMPCUOTASCALCAT
        SET TmpCuo_Xn = TmpCuoMonto /( power( (1.00 + Var_Xn),(Var_SumDiasPL / Var_Cre_Period  )))
        WHERE TmpNumTransaccion = NumTransaccion
        and TmpCuoNumero = Var_AuxConta;

        UPDATE TMPCUOTASCALCAT
        SET TmpCuo_Xn_1 = TmpCuoMonto /( power( (1.00 + Var_Xn_1),(Var_SumDiasPL / Var_Cre_Period  )))
        WHERE TmpNumTransaccion = NumTransaccion
        and TmpCuoNumero = Var_AuxConta;
