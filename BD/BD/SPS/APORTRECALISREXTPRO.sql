...
-- APORTRECALISREXTPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `APORTRECALISREXTPRO`;

DELIMITER $$
CREATE PROCEDURE `APORTRECALISREXTPRO`(
/* ===============================================================
...
DECLARE Dias_BasePerIrreg		INT(11);
...
SET Dias_BasePerIrreg		:= 31;					-- Días base para periodo irregular.
SET TasaRetencion			:= 3;					-- Para Calcula el Interés a Retener para clientes extranjeros.
SET TasaInteres				:= 1;					-- Para Calcula el Interés Generado.
SET TasaRecalculoISR		:= 4;					-- Para Calcula el Interés a Retener (Recálculo) clientes extranjeros.
	...
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
								'Disculpe las molestias que esto le ocasiona. Ref: SP-APORTRECALISREXTPRO');
		END;
		...
		DELETE FROM TMP_APORTRECALEXT WHERE NumTransaccion = Aud_NumTransaccion AND Fecha = Var_FechaCambioTasa;

		# Se obtienen las aportaciones de clientes extranjeros que posiblemente tuvieron cambio de tasa ISR.
		INSERT INTO TMP_APORTRECALEXT(
			AportacionID,		Fecha,					AmortizacionID,			ClienteID,			PaisResidencia,
			NumTransaccion,		ExisteCambioISR)
		SELECT
			ap.AportacionID,	Var_FechaCambioTasa,	amo.AmortizacionID,		cte.ClienteID,		cte.PaisResidencia,
			Aud_NumTransaccion,	FNEXISTECAMBIOISREXT(Var_FechaCambioTasa,Var_FechaCambioTasa,cte.PaisResidencia)
		...
		WHERE ap.Estatus = Est_Vigente
			...
			AND cte.PagaISR = Cons_SI
			AND Var_FechaCambioTasa BETWEEN ap.FechaInicio AND ap.FechaVencimiento
			AND Var_FechaCambioTasa BETWEEN amo.FechaInicio AND amo.FechaVencimiento
			AND cte.PaisResidencia != Var_PaisIDBase;

		# Se eliminan aquellas aportaciones que no tuvieron cambio de tasa.
		DELETE FROM TMP_APORTRECALEXT
		WHERE NumTransaccion = Aud_NumTransaccion
			AND ExisteCambioISR = Cons_NO
			AND Fecha = Var_FechaCambioTasa;

		# Si hay aportaciones con cambio de tasa.
		IF(EXISTS(SELECT * FROM TMP_APORTRECALEXT WHERE NumTransaccion = Aud_NumTransaccion AND Fecha = Var_FechaCambioTasa))THEN
			...
			SELECT
				...
				AP.NumTransaccion
			FROM APORTACIONES AP
				INNER JOIN TMP_APORTRECALEXT TMP ON AP.AportacionID = TMP.AportacionID
			WHERE TMP.NumTransaccion = Aud_NumTransaccion
				AND TMP.Fecha = Var_FechaCambioTasa
			...
			SELECT
				...
				AP.PagoIntCapitaliza,	Var_FechaCambioTasa,	Aud_NumTransaccion
			FROM APORTACIONES AP
				INNER JOIN AMORTIZAAPORT AM ON AP.AportacionID = AM.AportacionID
				INNER JOIN TMP_APORTRECALEXT TMP ON AP.AportacionID = TMP.AportacionID AND AM.AmortizacionID = TMP.AmortizacionID
			WHERE TMP.NumTransaccion = Aud_NumTransaccion
				AND TMP.Fecha = Var_FechaCambioTasa
				ORDER BY AM.AportacionID, AM.AmortizacionID;

			SET Var_ConsID := 1;
			SET Var_TotalReg := (SELECT COUNT(*) FROM TMP_AMORTIAPORTISREXT WHERE NumTransaccion = Aud_NumTransaccion AND FechaCalculo = Var_FechaCambioTasa);
			...
			WHILE(Var_ConsID <= Var_TotalReg)DO
				...
				FROM TMP_AMORTIAPORTISREXT T
				WHERE T.TmpID = Var_ConsID
					AND T.NumTransaccion = Aud_NumTransaccion
					AND T.FechaCalculo = Var_FechaCambioTasa;

				SET Var_TasaISRActual	:= (SELECT TasaISR FROM APORTACIONES WHERE AportacionID = Var_AportacionID);
				SET Var_NuevaTasaISR	:= (SELECT FNTASAISREXT(PaisResidencia,2,Par_Fecha,Var_TasaISRActual)
											FROM TMP_APORTRECALEXT WHERE AportacionID = Var_AportacionID AND AmortizacionID = Var_AmortizacionID AND Fecha = Var_FechaCambioTasa);
				...
				IF(Var_TipoPeriodo = Periodo_Reg)THEN
					 ...
					 */
					SET Var_Dias2doPer := DATEDIFF(Var_FechaPago,Var_FechaCambioTasa);
					SET Var_Dias2doPer := IF((Var_Dias2doPer+1)>Dias_BasePerReg,Dias_BasePerReg,(Var_Dias2doPer+1));
					SET Var_Dias1erPer := (Dias_BasePerReg - Var_Dias2doPer);

					SET Var_FechaFin2doPer := Var_FechaPago;
					# Cálculo del ISR del 2do. corte con la nueva tasa.
					SET Var_ISR2doPer := FNINTERESCAL(Var_TipoPeriodo,TasaRecalculoISR,Var_Interes,Var_Dias2doPer,Var_NuevaTasaISR,Entero_Cero,Entero_Cero,Entero_Cero);
					SET Var_ISR2doPer := ROUND((Var_ISR2doPer / (Var_Dias1erPer+Var_Dias2doPer)),2);
					...
					# SI HAY DÍAS CALCULADOS PARA EL PRIMER CORTE SE CALCULA EL ISR.
					IF(Var_Dias1erPer>Entero_Cero)THEN
						# Cálculo del ISR del 1er. corte con la tasa actual.
						SET Var_ISR1erPer := FNINTERESCAL(Var_TipoPeriodo,TasaRecalculoISR,Var_Interes,Var_Dias1erPer,Var_TasaISRActual,Entero_Cero,Entero_Cero,Entero_Cero);
						SET Var_ISR1erPer := ROUND((Var_ISR1erPer / (Var_Dias1erPer+Var_Dias2doPer)),2);
					ELSE
						SET Var_ISR1erPer := Entero_Cero;
					END IF;
				END IF;
				...
				IF(Var_TipoPeriodo = Periodo_Irreg)THEN
					...
					# Días base de acuerdo al mes del segundo periodo.
					SET Dias_BasePerIrreg := DAY(LAST_DAY(Var_FechaCambioTasa));

					SET Var_FechaFin2doPer := DATE_ADD(Var_FechaCambioTasa, INTERVAL (Var_Dias2doPer-1) DAY);
					# Cálculo del ISR del 2do. corte con la nueva tasa.
					SET Var_ISR2doPer := FNINTERESCAL(Var_TipoPeriodo,TasaRecalculoISR,Var_Interes,Var_Dias2doPer,Var_NuevaTasaISR,Entero_Cero,Entero_Cero,Entero_Cero);
					SET Var_ISR2doPer := ROUND(Var_ISR2doPer / (Dias_BasePerIrreg),2);

					# Días base de acuerdo al mes del primer periodo.
					SET Dias_BasePerIrreg := DAY(LAST_DAY(Var_FechaInicio));

					# RECALCULO DE LOS DÍAS DEL PRIMER CORTE Y LA FECHA FIN. (CON LA TASA ACTUAL -ANTES DEL CAMBIO DE TASA-).
					SET Var_FechaFin1erPer := DATE_ADD(Var_FechaInicio, INTERVAL Var_Dias1erPer DAY);
					# Cálculo del ISR del 1er. corte con la tasa actual.
					SET Var_ISR1erPer := FNINTERESCAL(Var_TipoPeriodo,TasaRecalculoISR,Var_Interes,Var_Dias1erPer,Var_TasaISRActual,Entero_Cero,Entero_Cero,Entero_Cero);
					SET Var_ISR1erPer := ROUND(Var_ISR1erPer / (Dias_BasePerIrreg),2);
				END IF;
				...
				WHERE TmpID = Var_ConsID
					...
					AND FechaCalculo = Var_FechaCambioTasa;

				# SE ACTUALIZA EL ISR (INTERESRETENER) Y EL INTERÉS NETO (TOTAL) LAS CUOTAS VIGENTES EN TRÁNSITO.
				UPDATE AMORTIZAAPORT AM
					INNER JOIN TMP_AMORTIAPORTISREXT TMP ON AM.AportacionID = TMP.AportacionID AND AM.AmortizacionID = TMP.AmortizacionID
				SET
				...
				WHERE TMP.NumTransaccion = Aud_NumTransaccion
					AND TMP.FechaCalculo = Var_FechaCambioTasa;
				...
				WHILE(Var_AmortSigID <= Var_TotalAmor)DO
					IF(Var_PagoIntCapitaliza = Cons_SI)THEN
						...
						SET Var_NvoInteresRet := FNINTERESCAL(Var_TipoPeriodo,TasaRetencion,Var_NvoInteresGen,Var_PlazoAmo,Var_NuevaTasaISR,Entero_Cero,Entero_Cero,Entero_Cero);
