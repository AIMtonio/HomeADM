...
-- REGD244200003REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `REGD244200003REP`;DELIMITER $$

CREATE PROCEDURE `REGD244200003REP`(
# ------------------------------------------------------------------------------------------- #
...
# ------------------------------------------------------------------------------------------- #
    ...
    DECLARE Var_MoviDepositos   VARCHAR(150);
    DECLARE Var_CanalOtros      INT;
    ...
    SET Cli_Crediclub       := 24; -- Numero de Cliente de Crediclub
    SET Var_CanalOtros      := 213;  -- Canal de operaciones "Otros"
   ...
   */
    ...
    SELECT Cue.NumeroMov,       Cue.CuentaAhoID,        SUM(Cue.CantidadMov) AS CantidadMovimiento      ,
           MIN(Cue.TipoMovAhoID),     MONTH(MIN(Cue.Fecha)),    Entero_Cero
       ...
       INSERT INTO TMPREPCUENTASMOVS(CuentaAhoID,CantidadMov,NumMov,TipoMovimiento,TipoCuenta,ClienteID,Mes)
        SELECT reg.CuentaAhoID,     SUM(reg.CantidadMovimiento) CantidadMov,        COUNT(*) AS NumMov,     OperDeposito AS TipoMovimiento,
               CASE WHEN MAX(ti.ClasificacionConta) = Tipo_Ahorro THEN DepositoAhorro ELSE DepositoVista END,
               MAX(cu.ClienteID),        MIN(reg.Mes)
   ...
   */
    ...
    SELECT Tar.TarjetaDebID,        COUNT(Tar.DetalleATMID) Frecuencia,         MIN(month(FechaTransac))
        FROM TARDEBCONCIATMDET Tar
        WHERE Tar.FechaTransac BETWEEN Var_FechaInicio AND Var_FechaFin
        AND Tar.Descripcion LIKE CONCAT('%',TipoRetiro,'%')
        GROUP BY TarjetaDebID;
   ...
   */
    ...
    CREATE TEMPORARY TABLE TmpComprasD2442
        SELECT TarDebMovID,     TarjetaDebID,       MontoOpe,       NumeroTran,     month(DATE(SUBSTR(REPLACE(FechaHrOpe,'-',''),1,8))) AS Mes
    ...
    INSERT INTO TmpComprasD2442
         SELECT FolioConcilia,      NumCuenta,      ImporteDestinoTotal,        NumTransaccion,     month(FechaProceso)
    ...
    INSERT INTO FRECUENCIAATMD2442
        SELECT det.TarjetaDebID,        COUNT(DISTINCT(det.TarDebMovID)) operaciones,       MIN(det.Mes)
        FROM TmpComprasD2442 det
        GROUP BY det.TarjetaDebID;
