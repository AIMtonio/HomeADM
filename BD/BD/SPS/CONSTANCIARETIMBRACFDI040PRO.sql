-- SP CONSTANCIARETIMBRACFDI040PRO

DELIMITER ;

DROP PROCEDURE IF EXISTS CONSTANCIARETIMBRACFDI040PRO;

DELIMITER $$

CREATE PROCEDURE `CONSTANCIARETIMBRACFDI040PRO`(
...
TerminaStore:BEGIN
	...
	DECLARE Var_InteresGravado	DECIMAL(18,2);			-- Monto de Interes Gravado
    DECLARE Var_InteresExento	DECIMAL(18,2);			-- Monto de Interes Exento
    DECLARE Var_InteresRetener	DECIMAL(18,2);			-- Monto de Interes Retener
    DECLARE Var_InteresReal		DECIMAL(18,2);			-- Monto de Interes Real
	...
	DECLARE Var_FormatoGMT 		VARCHAR(6);				-- Formato GMT
	DECLARE Var_CveRetencion	VARCHAR(2);				-- Clave de retencion.
    ...
    DECLARE Prov_SmarterWeb		CHAR(1);				-- ID para el proveedor de Smarter Web
    DECLARE Prov_HubServicios	CHAR(1);				-- ID para el proveedor Hub Servicios
    ...
    SET Long_PerFisica	:= 13;		-- Longitut RFC Persona Fisica: 13
	...
	SET Prov_SmarterWeb := 2;		-- ID para el proveedor de Smarter Web
    SET Prov_HubServicios := 3;		-- ID para el proveedor Hub Servicios
	...
	ManejoErrores:BEGIN #bloque para manejar los posibles errores
		DECLARE EXIT HANDLER FOR SQLEXCEPTION
		 BEGIN
			...
			SET Par_ErrMen = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
							'Disculpe las molestias que esto le ocasiona. Ref: SP-CONSTANCIARETIMBRACFDI040PRO');
			SET Var_Control = 'SQLEXCEPTION';
		 END;

		SELECT 	FechaSistema, 			ZonaHoraria
				INTO Var_FechaSistema, 	Var_FormatoGMT
			FROM PARAMETROSSIS LIMIT 1;
		SET Var_FormatoGMT := IFNULL(Var_FormatoGMT, Cadena_Vacia);

		-- Se obtiene los datos de la Institucion
		SELECT  Con.TimbraConsRet, Con.DivideSaldoPromCta, Con.TipoProveedorWS, Con.CveRetencion
		INTO 	Var_TimbraConsRet, Var_DivideSaldoPromCta, Var_TipoProveedorWS, Var_CveRetencion
		...
		SET Var_CveRetencion := IFNULL(Var_CveRetencion,Cadena_Vacia);
		...
		-- Validaciones especificas para el proveedor de timbrado de SmarterWeb y Hub de Servicios
		IF(Var_TipoProveedorWS IN (Prov_SmarterWeb, Prov_HubServicios)) THEN
			IF(Var_FormatoGMT = Cadena_Vacia) THEN
				SET Var_Continuar   := Entero_Uno;
				SET Par_NumErr  	:= 004;
				SET Par_ErrMen  	:= 'Parametrice la zona horaria.';
				LEAVE ManejoErrores;
			END IF;

			IF(Var_CveRetencion = Cadena_Vacia) THEN
				SET Var_Continuar   := Entero_Uno;
				SET Par_NumErr  	:= 004;
				SET Par_ErrMen  	:= 'Parametrice la clave de retencion.';
				LEAVE ManejoErrores;
			END IF;
		END IF;
		...
		SET Par_CadenaCFDI 		:= Cadena_Vacia;

		-- CUANDO LA PERSONA RELACIONADA NO SEA CLIENTE
		IF(Par_Tipo = TipoRelacion) THEN
			IF(Par_InteresReal > Decimal_Cero)THEN
				SET Var_MontoIntReal := Par_InteresReal;
			ELSE
				SET Var_Perdida 	:= Par_InteresReal * -1;
			END IF;

			SET Var_TotalOperacion	:= Par_InteresGravado + Par_InteresExento - Par_InteresRetener;
			SET Var_TotalOperacion	:= IFNULL(Var_TotalOperacion, Decimal_Cero);

			SET Var_MontIntNominal	:= Par_InteresGravado + Par_InteresExento;
			SET Var_MontIntNominal	:= IFNULL(Var_MontIntNominal, Decimal_Cero);
		END IF;

		-- CUANDO LA PERSONA RELACIONADA SEA CLIENTE
		IF(Par_Tipo = TipoCliente) THEN
			IF NOT EXISTS(SELECT CteRelacionadoID
							FROM CONSTANCIARETCTEREL
							WHERE Anio = Par_Anio
							AND CteRelacionadoID = Par_CteRelacionadoID
							AND Tipo = TipoCliente
							AND CadenaCFDI != Cadena_Vacia)THEN
				-- SE OBTIENEN LOS INTERESES CUANDO LA PERSONA RELACIONADA SEA CLIENTE
				SELECT	SUM(MontoTotGrav),		SUM(MontoTotExent),		SUM(MontoTotRet),		SUM(MontoIntReal)
				INTO    Par_InteresGravado,		Par_InteresExento,		Par_InteresRetener,		Par_InteresReal
				...
				SET Var_MontIntNominal	:= IFNULL(Var_MontIntNominal, Decimal_Cero);
			ELSE
				SET Par_InteresGravado := Decimal_Cero;
				SET Par_InteresExento  := Decimal_Cero;
				SET Par_InteresRetener := Decimal_Cero;
				SET Par_InteresReal    := Decimal_Cero;
			END IF;

		END IF;

		-- CUANDO SEA APORTANTE
		IF(Par_Tipo = TipoAportante) THEN
			-- SE OBTIENEN LOS INTERESES DEL APORTANTE
			SELECT	SUM(MontoTotGrav),		SUM(MontoTotExent),		SUM(MontoTotRet),		SUM(MontoIntReal)
			...
			-- SE OBTIENEN LOS INTERESES CUANDO EL APORTANTE SE ENCUENTRA COMO PERSONA RELACIONADA DE OTROS APORTANTES
			SELECT	SUM(MontoTotGrav),		SUM(MontoTotExent),		SUM(MontoTotRet),		SUM(MontoIntReal)
			...
			-- SE OBTIENE LA SUMA DE LOS INTERESES DEL APORTANTE
			SET Par_InteresGravado	:= Var_TotalGravado + Var_InteresGravado;
			SET Par_InteresExento	:= Var_TotalExento + Var_InteresExento;
			SET Par_InteresRetener	:= Var_TotalISR + Var_InteresRetener;
			SET Par_InteresReal		:= Var_MontoIntReal + Var_InteresReal;
		...
		IF (Var_Continuar = Entero_Cero) THEN
			...
			IF (Par_InteresRetener > Decimal_Cero) THEN
				IF(Var_TipoProveedorWS IN (Prov_SmarterWeb, Prov_HubServicios)) THEN
                    SET Var_CadenaCFDI := CONCAT('<retenciones:Retenciones', ' ', '\n');
                    SET Var_CadenaCFDI := CONCAT(Var_CadenaCFDI,'Cert=\"\"', '\n');
					SET Var_CadenaCFDI := CONCAT(Var_CadenaCFDI, 'CveRetenc=\"', Var_CveRetencion, '\" ', '\n');
					SET Var_CadenaCFDI := CONCAT(Var_CadenaCFDI, 'FechaExp=\"',Var_FechaSistema,'T',TIME(NOW()), Var_FormatoGMT,'\" ', '\n');
					SET Var_CadenaCFDI := CONCAT(Var_CadenaCFDI, 'FolioInt=\"ABC\" ', '\n');
					...
					SET Var_CadenaCFDI := CONCAT(Var_CadenaCFDI, 'montoTotExent=\"', CAST(Par_InteresExento AS CHAR), '\" ', '\n');
					SET Var_CadenaCFDI := CONCAT(Var_CadenaCFDI, 'montoTotGrav=\"', CAST(Par_InteresGravado AS CHAR), '\" ', '\n');
					SET Var_CadenaCFDI := CONCAT(Var_CadenaCFDI, 'montoTotOperacion=\"', CAST(Var_TotalOperacion AS CHAR), '\" ', '\n');
					SET Var_CadenaCFDI := CONCAT(Var_CadenaCFDI, 'montoTotRet=\"', CAST(Par_InteresRetener AS CHAR), '\" ', '\n');
					SET Var_CadenaCFDI := CONCAT(Var_CadenaCFDI, '>', '\n');
					SET Var_CadenaCFDI := CONCAT(Var_CadenaCFDI, '</retenciones:Totales>', '\n');
					SET Var_CadenaCFDI := CONCAT(Var_CadenaCFDI, '</retenciones:Retenciones>');

					-- SE OBTIENE EL VALOR DE LA CADENA CFDI
					SET Par_CadenaCFDI := Var_CadenaCFDI;
					SET Par_CadenaCFDI := IFNULL(Par_CadenaCFDI,Cadena_Vacia);
				ELSE
