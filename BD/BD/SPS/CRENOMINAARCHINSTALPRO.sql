...
-- ---------------------------------------------------------------------------------
-- CRENOMINAARCHINSTALPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `CRENOMINAARCHINSTALPRO`;

DELIMITER $$
CREATE PROCEDURE `CRENOMINAARCHINSTALPRO`(
    -- STORE PARA  CAMBIAR EL ESTATUSNOMINA, A ENVIADO, DE UN DETERMINADO CREDITO.
    ...
    Par_NumPro          INT(1),         -- Vlor que determina la secuencia del store a ejecutar.
    ...
    ManejoErrores: BEGIN
        ...
        BEGIN
            ...
            SET Par_ErrMen  =  CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
                                    'Disculpe las molestias que esto le ocasiona. Ref: SP-CRENOMINAARCHINSTALPRO');
        ...
        -- Se valida si el numero de proceso enviado no es nulo.
        IF(IFNULL(Par_NumPro, Entero_Cero) = Entero_Cero) THEN
            SET Par_NumErr  :=  002;
            SET Par_ErrMen  :=  'El numero de proceso esta vacio';
            SET Var_Control :=  'numPro';
            LEAVE ManejoErrores;
        END IF;


        IF (Par_NumPro = Con_Proceso_N) THEN

            -- Se cambia el estatus del detalle del archivo de instalacion.
            UPDATE DETALLEARCHIVOINSTAL DET
            SET
                DET.Estatus = Con_Estatus_N
            WHERE
                DET.FolioID = Par_FolioID
            AND
                DET.CreditoID = Par_CreditoID;
        ...
        IF (Par_NumPro = Con_Proceso_E) THEN

            SELECT  FechaRecepLim
              INTO  Var_FechaRecep
              FROM  CREDITOS
             WHERE  CreditoID = Par_CreditoID;

            CALL DETALLEARCHIVOINSTALALT(   Par_FolioID,        Par_CreditoID,      Con_Estatus_E,      Var_FechaRecep,      'N',
                                            Par_NumErr,         Par_ErrMen,         Aud_EmpresaID,      Aud_Usuario,         Aud_FechaActual,
                                            Aud_DireccionIP,    Aud_ProgramaID,     Aud_Sucursal,       Aud_NumTransaccion);
            ...
            UPDATE CREDITOS CR
            SET
                CR.EstatusNomina = Con_Estatus_E
            WHERE
                CR.CreditoID = Par_CreditoID;
        END IF;

        -- Proceso de Autorizacion del credito.
        IF (Par_NumPro = Con_Proceso_A) THEN

            -- Se cambia el estatus del detalle del archivo de instalacion.
            UPDATE DETALLEARCHIVOINSTAL DET
            SET
                DET.Estatus = Con_Estatus_A
            WHERE
                DET.FolioID = Par_FolioID
            AND
                DET.CreditoID = Par_CreditoID;
            ...
            UPDATE CREDITOS CR
            SET
                CR.EstatusNomina = Con_Estatus_A
            WHERE
                CR.CreditoID = Par_CreditoID;
        END IF;
