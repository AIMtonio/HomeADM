...
-- TC_AJUSTEPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `TC_AJUSTEPRO`;

DELIMITER $$
CREATE PROCEDURE `TC_AJUSTEPRO`(
	-- Store Procedure de Ajuste de Compra de tarjetas de credito por medio de Web Service
	...
	ManejoErrores:BEGIN
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen	= CONCAT('El SAFI ha tenido un problema al concretar la operacion. Disculpe las molestias que ',
									 'esto le ocasiona. Ref: SP-TC_AJUSTEPRO');
		...
		IF( (Var_MontoDisponible + Var_TotalReversa - Var_TotalCompra) > Var_MontoLinea ) THEN
			SET Par_NumErr	:= 1329;
			SET Par_ErrMen	:= 'El Monto Disponible mas el Monto de Ajuste superan el Monto asignado a la Linea de Credito de la Tarjeta.';
			SET Var_Control	:= 'transactionAmount';
			LEAVE ManejoErrores;
		END IF;
		...
		CALL TC_CONTALINEAPRO (
			...
			Var_TotalReversa,		Par_MonedaID,			Var_ProductoCreditoID,		Var_Clasificacion,		Con_SubClasificacion,
		...
		SELECT	IFNULL(TC_FUNCIONLIMITEMONTO(Tar.TarjetaCredID, Tar.ClienteID, Tar.TipoTarjetaCredID, Con_CompraDiario), Decimal_Cero),
				IFNULL(TC_FUNCIONLIMITEMONTO(Tar.TarjetaCredID, Tar.ClienteID, Tar.TipoTarjetaCredID, Con_CompraMes), Decimal_Cero),
		...
		IF( Var_LimiteCompraDiario > Decimal_Cero ) THEN
			IF( (Var_TotalCompra + Var_MontoCompraDiario) > Var_LimiteCompraDiario ) THEN
				SET Par_NumErr	:= 1332;
				SET Par_ErrMen	:= 'La Tarjeta de Credito ha Excedido el Limite de Compra Diario.';
				SET Var_Control	:= 'cardNumber';
				LEAVE ManejoErrores;
			END IF;
		...
		IF( Var_LimiteCompraMes > Decimal_Cero) THEN
			...
			IF( (Var_TotalCompra + Var_MontoCompraMes) > Var_LimiteCompraMes) THEN
				SET Par_NumErr	:= 1334;
				SET Par_ErrMen	:= 'La Tarjeta de Credito ha Excedido el Limite de Compra Mensual.';
				SET Var_Control	:= 'cardNumber';
				LEAVE ManejoErrores;
			END IF;
		...
		CALL TC_COMPRANORMALRPRO (
			...
			Var_SaldoDisponibleAct,	Var_CodigoRespuesta,	Var_FechaOperacion,		Par_TarCredMovID,		Par_AuthorizationNumber,
			Var_PolizaID,
			Salida_NO,				Par_NumErr,				Par_ErrMen,				Par_EmpresaID,			Aud_Usuario,
			Aud_FechaActual,		Aud_DireccionIP,		Aud_ProgramaID,			Aud_Sucursal,			Aud_NumTransaccion);
