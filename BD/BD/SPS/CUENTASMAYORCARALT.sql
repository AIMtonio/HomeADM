...
-- CUENTASMAYORCARALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `CUENTASMAYORCARALT`;DELIMITER $$

CREATE PROCEDURE `CUENTASMAYORCARALT`(
...
insert into CUENTASMAYORCAR
			values ( Par_ConceptoCarID,    Par_Cuenta,          Par_Nomenclatura,       Par_NomenclaturaCR, 	Aud_EmpresaID,
              Aud_Usuario,		      Aud_FechaActual,			Aud_DireccionIP,		     Aud_ProgramaID,		    Aud_Sucursal,
              Aud_NumTransaccion
					);
select '000' as NumErr,
	  concat("Cuenta Agregada Exitosamente: ", convert(Par_ConceptoCarID, CHAR))  as ErrMen,
	  'conceptoCarID' as control;
