...
-- CUENTASMAYORCRWALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `CUENTASMAYORCRWALT`;

DELIMITER $$
CREATE PROCEDURE `CUENTASMAYORCRWALT`(
/* ALTA DE CUENTAS MAYOR CROWDFUNDING. */
...
ManejoErrores: BEGIN
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									'Disculpe las molestias que esto le ocasiona. Ref: SP-CUENTASMAYORCRWALT');
	...
	INSERT INTO CUENTASMAYORCRW(
		ConceptoCRWID,		Cuenta,				Nomenclatura,		NomenclaturaCR,			EmpresaID,
		Usuario,			FechaActual,		DireccionIP,		ProgramaID,				Sucursal,
		NumTransaccion)
	VALUES (
		Par_ConceptoCRWID,	Par_Cuenta,			Par_Nomenclatura,	Par_NomenclaturaCR,		Aud_EmpresaID,
		Aud_Usuario,		Aud_FechaActual,	Aud_DireccionIP,	Aud_ProgramaID,			Aud_Sucursal,
		Aud_NumTransaccion);

	SET	Par_NumErr := Entero_Cero;
	SET	Par_ErrMen := CONCAT("Cuenta Agregada Exitosamente. Concepto: ", CONVERT(Par_ConceptoCRWID, CHAR));
	SET Var_Control:= 'conceptoCRWID';
