...
-- CAJASARQUEOREPHIS
DELIMITER ;
DROP PROCEDURE IF EXISTS `CAJASARQUEOREPHIS`;DELIMITER $$

CREATE PROCEDURE `CAJASARQUEOREPHIS`(
    Par_SucursalID      int,
    Par_CajaID          int,
    Par_Fecha           date,
...
insert into TMPARQUEO
  select  concat(RPAD(CASE WHEN Den.Valor = 1 THEN concat('M',LPAD('Monedas',11,' '))
		...
		left outer  join `HIS-BALANZADENO` AS Bal ON Den.DenominacionID = Bal.DenominacionID
		where Bal.SucursalID =Var_SucursalCaja and Bal.CajaID = Par_CajaID  and Bal.Fecha  = Par_Fecha;
...
BEGIN
    ...
    set Var_Encabezado  := RPAD(ltrim(rtrim(Var_Descripcion)), 60, ' ');
    set Var_Encabezado  := concat(Var_Encabezado,  LPAD(format(Var_NumeroMovs, 0), 14, ' '));
    set Var_Encabezado  := concat(Var_Encabezado,  LPAD(format(Var_MontoMovs, 2), 20, ' '));
