...
-- PREPAGOCRESIGCUOPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `PREPAGOCRESIGCUOPRO`;
DELIMITER $$


CREATE PROCEDURE `PREPAGOCRESIGCUOPRO`(
...
DECLARE Var_OtrasComisiones		DECIMAL(14,2);		-- Monto de Otras Comisiones
...
DECLARE Llave_CobraAccesorios	VARCHAR(100); -- Llave para Consultar Si Cobra Accesorios
...
DECLARE CURSORAMORTI CURSOR FOR
	SELECT  Amo.CreditoID,      Amo.AmortizacionID, 	Amo.SaldoCapVigente,    Amo.SaldoCapVenNExi,
			...
			Cre.CobraComAnual,	Amo.SaldoComisionAnual,	Amo.SaldoOtrasComis,	Amo.SaldoNotCargoRev,
			Amo.SaldoNotCargoSinIVA,	Amo.SaldoNotCargoConIVA
...
SET TipoActInteresCouSig := 2;			-- Tipo de Actualizacion Cuotas Siguientes
...
ManejoErrores: BEGIN
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			SET Par_ErrMen = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
								'Disculpe las molestias que esto le ocasiona. Ref: SP-PREPAGOCRESIGCUOPRO');
		END;
	...
	IF(Var_ClienteEspecifico = Var_ClienteConsol) THEN
		SET Tol_DifPago := 0.00;
	END IF;
	...
	SELECT  Cli.SucursalOrigen,     Cre.ClienteID,          Pro.ProducCreditoID,    Des.Clasificacion,
			...
			Pro.EsGrupal,			Cre.SolicitudCreditoID, Cre.TasaFija,           Cre.MonedaID,
			Cre.GrupoID,			Des.SubClasifID,        Cre.CicloGrupo,         Cre.CalcInteresID,
			FUNCIONTOTDEUDACRE(Par_CreditoID),				Cre.LineaCreditoID,		Pro.ManejaLinea,
			Pro.EsRevolvente,		Cre.CobraSeguroCuota,	Cre.CobraIVASeguroCuota,Cre.MontoCredito,
            Pro.EsReestructura,		Res.EstatusCreacion,	Cre.FrecuenciaCap,		Cre.FrecuenciaInt,
            Cre.FechaInicio,		Cre.Refinancia,			Cre.CobraAccesorios
			INTO
			Var_SucCliente,     	Var_ClienteID, 			Var_ProdCreID,      	Var_ClasifCre,
			Var_NumAmorti,			Var_EstatusCre, 		Var_CliPagIVA,  		Var_IVAIntOrd,
			Var_EsGrupal,			Var_SolCreditoID,   	Var_CreTasa,    		Var_MonedaID,
			Var_GrupoID,			Var_SubClasifID,    	Var_CicloGrupo,			Var_CalInteresID,
			Var_TotDeuda,			Var_LineaCredito,		Var_ManejaLinea,
			Var_EsRevolvente,		Var_CobraSeguroCuota, 	Var_CobraIVASeguroCuota,Var_MontoCredito,
            Var_EsReestruc,			Var_EstCreacion,		Var_Frecuencia,			Var_FrecInteres,
            Var_FechaInicioCred,	Var_Refinancia,			Var_CobraAccesorios
	...
	BEGIN
		...
		FETCH CURSORAMORTI INTO
			...
			Var_SaldoIntNoConta, 	Var_SaldoSeguroCuota,	Var_CobraComAnual,		Var_SaldoComAnual,		Var_OtrasComisiones,
			Var_SaldoNotCargoRev,	Var_SaldoNotCargoSinIVA, Var_SaldoNotCargoConIVA;

		SET	Var_SaldoSeguroCuota 	:= IFNULL(Var_SaldoSeguroCuota, Decimal_Cero);
        SET	Var_OtrasComisiones 	:= IFNULL(Var_OtrasComisiones, Decimal_Cero);
		...
		IF (Var_MontoNotaCargo >= Mon_MinPago) THEN
			...
			ELSE
				SET Var_CantidPagar		:= ROUND(Var_SaldoPago, 2) - ROUND(((Var_SaldoNotCargoConIVA)/(1 + Var_IVANotaCargo)) * Var_IVANotaCargo, 2);
				SET Var_IVACantidPagar	:= ROUND(((Var_SaldoNotCargoConIVA)/(1 + Var_IVANotaCargo)) * Var_IVANotaCargo, 2);
			END IF;

			CALL PAGCRENOTACARGOPRO(Var_CreditoID,		Var_AmortizacionID,		Var_FechaInicio,	Var_FechaVencim,	Par_CuentaAhoID,
									...
									Con_PagoCred,		Var_CuentaAhoStr,		Var_Poliza,			Par_OrigenPago,		Cons_No,
        ...
        IF(Var_CobraAccesoriosGen = Cons_Si AND Var_CobraAccesorios = Cons_Si) THEN
			-- PAGO DE OTRAS COMISIONES
			IF(Var_OtrasComisiones >= Mon_MinPago) THEN/*PAGO OTRAS COMISIONES*/
				SET Var_IVACantidPagar	:= Entero_Cero;
				SET Var_CantidPagar 	:= Var_SaldoPago;

				-- SE EJECUTA LA LLAMADA QUE REALIZA EL PAGO DE OTRAS COMISIONES (ACCESORIOS DEL CREDITO)
				...
				IF(ROUND(Var_SaldoPago,2)	<= Decimal_Cero) THEN
					LEAVE CICLO;
				END IF;	-- PAGO DE OTRAS COMISIONES
			END IF; -- FIN PAGO OTRAS COMISIONES
        END IF;
		...
		IF(Var_SaldoSeguroCuota >= Mon_MinPago AND Var_CobraSeguroCuota = SiCobSeguroCuota) THEN
			...
			IF(Var_CobraIVASeguroCuota = SiCobIVASeguroCuota) THEN
				SET Var_IVACantidPagar 		:= ROUND((Var_SaldoSeguroCuota * Var_ValIVAIntOr), 2);
			ELSE
				-- OPERATIVO
				 SET Var_IVACantidPagar 	:= Decimal_Cero;
                  -- CONTABLE
                SET Var_MontoSeguroCont 	:= ROUND((Var_SaldoSeguroCuota/(1+Var_ValIVAIntOr)),2);
				SET Var_MontoIVASeguroCont 	:= ROUND(Var_MontoSeguro-Var_MontoSeguroCont,2);
			END IF;

			-- Se obtiene el Monto a Pagar
			IF(ROUND(Var_SaldoPago,2) >= (Var_SaldoSeguroCuota + Var_ValIVAIntOr)) THEN
			...
			ELSE
				...
				ELSE
					...
					SET	Var_MontoSeguroCont		:= ROUND(Var_SaldoPago,2) -
												ROUND(((Var_SaldoPago)/(1+Var_ValIVAIntOr)) * Var_ValIVAIntOr, 2);
					SET	Var_MontoIVASeguroCont  := ROUND(((Var_SaldoPago)/(1+Var_ValIVAIntOr)) * Var_ValIVAIntOr, 2);
            ...
            ELSE
				CALL PAGCRESEGCUOTASINIVAPRO (
					...
					Var_SubClasifID,		Var_SucCliente,			Con_PagoCred,		Var_CuentaAhoStr,   Var_Poliza,
	...
	ELSE
		...
		CALL AMORTICREDITOACT(
			Par_CreditoID,		TipoActInteresCouSig,   SalidaNO,			Par_NumErr, 		Par_ErrMen,
