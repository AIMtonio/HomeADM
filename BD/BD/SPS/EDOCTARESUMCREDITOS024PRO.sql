...
-- ---------------------------------------------------------------------------------
-- EDOCTARESUMCREDITOS024PRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `EDOCTARESUMCREDITOS024PRO`;
DELIMITER $$


CREATE PROCEDURE `EDOCTARESUMCREDITOS024PRO`(
...
TerminaStore: BEGIN
    ...
    DECLARE Num_Orden			INT(11);
	DECLARE Con_ValorTres		INT(1);
	DECLARE Con_ProxCapital		CHAR(1);
	DECLARE Con_MontoProxTotal	CHAR(1);

	DECLARE Con_ProxIntOrd		CHAR(2);
	DECLARE Con_ProxIVAIntOrd	CHAR(2);
	DECLARE Con_ProxMoraOrd		CHAR(2);
	DECLARE Con_ProxIVAMoraOrd	CHAR(2);
    ...
    SET Num_Orden				:= 3;			-- Numero de orden a evaluar
	SET Con_ValorTres			:= 3;			-- Constante 3

	SET Con_ProxCapital			:= 'C';			-- Tipo de monto de proximo pago: Capital
	SET Con_MontoProxTotal		:= 'T';			-- Tipo de monto de proximo pago: Total

	SET Con_ProxIntOrd			:= 'IN';		-- Tipo de monto de proximo pago: Interes Ordinario
	SET Con_ProxIVAIntOrd		:= 'IV';		-- Tipo de monto de proximo pago: IVA de Interes Ordinario
	SET Con_ProxMoraOrd			:= 'MN';		-- Tipo de monto de proximo pago: Interes Moratorio
	SET Con_ProxIVAMoraOrd		:= 'MV';		-- Tipo de monto de proximo pago: IVA de Interes Moratorio
	...
	-- Se obtiene la fecha de corte al inicio del periodo
	...
	INSERT INTO EDOCTARESUMCREDITOSCOM
	SELECT
		ClienteID,				CreditoID,				Con_Cadena_Vacia,		Con_Entero_Cero,		Con_Cadena_Vacia,
		Con_Moneda_Cero,		Con_Moneda_Cero,		Con_Moneda_Cero,		Con_Moneda_Cero,		Con_Moneda_Cero,
		Con_Moneda_Cero,		Con_Moneda_Cero,		Con_Moneda_Cero,		Con_Moneda_Cero,		Con_Moneda_Cero,
	...
	INSERT INTO TMPAMORTIZACIONESCRE(
		CreditoID,		TotalCuotas,		Cubiertas)
	SELECT cre.CreditoID
		  ,COUNT(amor.AmortizacionID)
		  ,SUM(CASE WHEN amor.Estatus= Est_Pagado THEN 1 ELSE 0 END)
	FROM CREDITOS cre
	INNER JOIN AMORTICREDITO amor ON cre.CreditoID = amor.CreditoID
	WHERE cre.Estatus IN (Est_Vigente, EstatusVencido, Est_Pagado)
	GROUP BY cre.CreditoID;

	UPDATE EDOCTARESUMCREDITOSCOM CRE
	INNER JOIN TMPAMORTIZACIONESCRE TMP ON CRE.CreditoID=TMP.CreditoID
		SET
		CRE.TotalPagos = TMP.TotalCuotas,
		CRE.NumPagoPlazo = IF(TMP.Cubiertas = TMP.TotalCuotas, 'N/A', CONCAT(CAST((TMP.Cubiertas+Entero_Uno) AS CHAR),'/',CAST(TMP.TotalCuotas AS CHAR)) );

	UPDATE EDOCTARESUMCREDITOSCOM CRE
	SET CRE.NumeroPago = IFNULL((	SELECT group_concat(AMO.AmortizacionID SEPARATOR ',') AS NumeroPago
									FROM AMORTICREDITO AMO
									WHERE AMO.Estatus = Est_Pagado
									  AND AMO.FechaLiquida >= Par_FecIniMes
									  AND AMO.FechaLiquida <= Par_FecFinMes
									  AND AMO.ClienteID = CRE.ClienteID
									  AND AMO.CreditoID = CRE.CreditoID
									GROUP BY AMO.CreditoID),'0');

	UPDATE EDOCTARESUMCREDITOSCOM CRED, TMP_DATOSAMORTICRED AS TMP
	SET SaldoInsolutoPrin = FNCALCULARSALDOINSOLUTO(TMP.CreditoID, TMP.AmortizacionID+1)
	WHERE TMP.CreditoID = CRED.CreditoID;

	UPDATE EDOCTADATOSCTECOM COM,EDOCTARESUMCREDITOS CRE
	SET COM.FechaProxPago = CASE WHEN (SELECT 1
										FROM EDOCTARESUMCREDITOS CRE1
										WHERE CRE1.FechaProxPago = 'INMEDIATO' AND CRE1.ClienteID = COM.ClienteID limit 1) = 1 THEN 'INMEDIATO'
								 ELSE IFNULL((SELECT FechaProxPago
										FROM EDOCTARESUMCREDITOS CRE2
										WHERE CRE2.ClienteID = COM.ClienteID AND CRE2.FechaProxPago <> 'INMEDIATO' AND FechaProxPago <> ''
										ORDER BY CRE2.FechaProxPago ASC LIMIT 1),'')
								 END,
		COM.CreditoID = CRE.CreditoID
	WHERE 	COM.ClienteID = CRE.ClienteID
			AND CRE.Orden = Con_ValorTres;

	UPDATE	EDOCTARESUMCREDITOSCOM
	SET		MontoProxPago = FNEXIGIBLEPROXPAGEDOCTA(CreditoID, Par_FecFinMes, Con_MontoProxTotal),
			CapitalProxPago = FNEXIGIBLEPROXPAGEDOCTA(CreditoID, Par_FecFinMes, Con_ProxCapital),
			IntOrdProxPago = FNEXIGIBLEPROXPAGEDOCTA(CreditoID, Par_FecFinMes, Con_ProxIntOrd),
			IVAIntOrdProxPago = FNEXIGIBLEPROXPAGEDOCTA(CreditoID, Par_FecFinMes, Con_ProxIVAIntOrd),
			IntMoraProxPago = FNEXIGIBLEPROXPAGEDOCTA(CreditoID, Par_FecFinMes, Con_ProxMoraOrd),
			IVAIntMoraProxPago = FNEXIGIBLEPROXPAGEDOCTA(CreditoID, Par_FecFinMes, Con_ProxIVAMoraOrd);

	UPDATE	EDOCTARESUMCREDITOSCOM COM,  EDOCTARESUMCREDITOS RES
	SET		COM.InteresesCargados = RES.Interes + RES.Moratorios,
			COM.ComisionesCobradas = RES.ComFaltaPago + RES.OtrasCom
	WHERE COM.ClienteID = RES.ClienteID
	  AND COM.CreditoID = RES.CreditoID
	  AND RES.Orden = 3;

	UPDATE EDOCTARESUMCREDITOSCOM COM
			SET		IVAComisiCobradas = (ComisionesCobradas * (CASE WHEN (SELECT Cl1.PagaIVA FROM CLIENTES Cl1 WHERE Cl1.ClienteID=COM.ClienteID) = 'S'
															THEN (SELECT SU.IVA
															FROM SUCURSALES SU
															WHERE SU.SucursalID = (SELECT CL2.SucursalOrigen
															FROM CLIENTES CL2
															where CL2.ClienteID=COM.ClienteID))
															ELSE Con_Moneda_Cero END));

	UPDATE EDOCTARESUMCREDITOSCOM Com, EDOCTARESUMCREDITOS Edo, SALDOSCREDITOS Sal
	SET Com.SaldoInicial = IFNULL(ROUND((Sal.SalCapVigente + Sal.SalCapatrasado + Sal.SalCapVencido + Sal.SalCapVenNoExi),2)
								+ROUND(((Sal.SalIntOrdinario + Sal.SalIntAtrasado + Sal.SalIntVencido + Sal.SalIntProvision + Sal.SalIntNoConta) * (Edo.ValorIVAInt + 1)) ,2)
								+ROUND(((Sal.SalMoratorios + Sal.SaldoMoraVencido + Sal.SaldoMoraCarVen) * (Edo.ValorIVAMora + 1)), 2)
								+ROUND((Sal.SalComFaltaPago * (Edo.ValorIVAAccesorios + 1)), 2)
								+ROUND((Sal.SaldoComServGar * (Edo.ValorIVAAccesorios + 1)), 2)
								+ROUND((Sal.SalOtrasComisi * (Edo.ValorIVAAccesorios + 1)), Con_Moneda_Cero), 2)
	WHERE Sal.FechaCorte = Var_FechaCorteAnterior
	  AND Com.CreditoID = Edo.CreditoID
	  AND Edo.CreditoID = Sal.CreditoID
      AND Edo.Orden = 2;

	UPDATE EDOCTARESUMCREDITOSCOM Com, EDOCTARESUMCREDITOS Edo
	SET Com.SaldoInicial = IFNULL((Edo.MontoOtorgado), Con_Moneda_Cero)
	WHERE Com.CreditoID = Edo.CreditoID
	  AND Edo.FechaFormalizacion >= Par_FecIniMes
	  AND Edo.FechaFormalizacion <= Par_FecFinMes
	  AND Edo.Orden = 2;

	UPDATE EDOCTADATOSCTECOM Cte, EDOCTARESUMCREDITOS Edo
	SET Cte.EsSuperTasas = Con_SI
	WHERE Cte.ClienteID = Edo.ClienteID
	  AND Edo.ProductoCred LIKE '%SUPERTASAS%'
	  AND Edo.Orden = 2;
