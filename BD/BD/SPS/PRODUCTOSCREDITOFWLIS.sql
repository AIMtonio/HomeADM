-- SP PRODUCTOSCREDITOFWLIS

DELIMITER ;

DROP PROCEDURE IF EXISTS PRODUCTOSCREDITOFWLIS;

DELIMITER $$

CREATE PROCEDURE `PRODUCTOSCREDITOFWLIS`(
...
IF(Par_NumLis = Lis_Combo) THEN
    ...
    FROM PRODUCTOSCREDITO Pro INNER JOIN PRODUCTOSCREDITOFW Pbe ON Pro.ProducCreditoID=Pbe.ProductoCreditoID
    WHERE Pro.Descripcion LIKE concat("%", Par_Descripcion, "%") AND PerfilID=Par_PerfilID;
END IF;

IF(Par_NumLis = Lis_Bancas) THEN
	SELECT	Pbe.ProductoCreditoFWID,	Pbe.ProductoCreditoID, 		Pro.Descripcion AS DesCredito, 		Des.DestinoCreID, 		Des.Descripcion AS DesDestino,
			Des.Clasificacion AS ClasifChar,
