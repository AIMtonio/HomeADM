...
-- ---------------------------------------------------------------------------------
-- PAGOVERINTNOCONTCREPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `PAGOVERINTNOCONTCREPRO`;
DELIMITER $$

CREATE PROCEDURE `PAGOVERINTNOCONTCREPRO`(
...
TerminaStore: BEGIN
	...
	DECLARE Var_DivideIngresoInteres CHAR(1);
    ...
    DECLARE CURSORAMORTIZACIONES CURSOR FOR
		SELECT  Amo.AmortizacionID, 	Amo.SaldoIntNoConta, 	Amo.SaldoMoraCarVen,	Amo.FechaInicio,
				Amo.FechaVencim
	...
	SELECT FechaSistema, DivideIngresoInteres INTO Var_FechaSistema, Var_DivideIngresoInteres FROM PARAMETROSSIS WHERE EmpresaID = Par_EmpresaID;
	...
	OPEN CURSORAMORTIZACIONES;
			BEGIN
				...
				FETCH CURSORAMORTIZACIONES INTO
					Var_AmortizacionID,		Var_SaldoIntNoConta, 	Var_SaldoMoraCarVen,	Var_FechaInicio,
                    Var_FechaVencim;
