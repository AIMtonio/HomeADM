...
-- ---------------------------------------------------------------------------------
-- CREDITOSMAYOR3PORCREP
DELIMITER ;
DROP PROCEDURE IF EXISTS `CREDITOSMAYOR3PORCREP`;
DELIMITER $$

CREATE PROCEDURE `CREDITOSMAYOR3PORCREP`(
	-- SP para generar el reporte de mayor saldo insoluto 3.5 %
...
DECLARE Var_FechaInicio 		DATE;				-- Inicio de mes
DECLARE Var_FechaFin 			DATE;				-- Fin de mes
DECLARE Var_FechFin				DATE;	            -- Fin de mes del periodo contable
DECLARE Var_ReporteID			VARCHAR(10);

DECLARE Var_EjercicioVig		INT;				-- Ejercicio vigente
DECLARE Var_PeriodoVig			INT;				-- Ejercicio vigente
...
DECLARE Var_PorCredPorc     	DECIMAL(10,2);	 	-- Porcentaje Creditos Mayor Saldo Insoluto 3.5 %
DECLARE Var_Porcentual      	DECIMAL(10,2);   	-- Resultado Porcentual (Creditos Mayor Saldo Insoluto 3.5 %)
DECLARE Var_DifLimEsta   		DECIMAL(10,2);      -- Diferencia entre el Parametro (Creditos Mayor Saldo Insoluto 3.5 %) y el Resultado Porcentual
DECLARE Var_ResCapNet3Porc  	DECIMAL(14,2);
...
SET ParamCred3Porc      := 14;                  -- CatParamRiesgosID: Parametro Creditos Mayor Saldo Insoluto
...
SET Rep_Parametro       := 2;                   -- Tipo Reporte: Consulta Parametro 3.5 %
...
SET Var_PorCredPorc   	:= (SELECT Porcentaje FROM PARAMUACIRIESGOS WHERE CatParamRiesgosID = ParamCred3Porc);
SET Var_PorCredPorc   	:= IFNULL(Var_PorCredPorc,Decimal_Cero);
...
END IF;
	...
	SET Var_ResCapNet3Porc := IFNULL((Var_CapitalNeto * Var_PorCredPorc),Decimal_Cero)/100;

-- Mayor Saldo Insoluto 20 Creditos (Grid)
IF(Par_NumRep = Rep_Principal) THEN
	DROP TABLE IF EXISTS TMPCREDITOSMAYORSALDO;
	CREATE TEMPORARY TABLE  TMPCREDITOSMAYORSALDO(
			ClienteID		   INT(11),
            CreditoID          BIGINT(12),
			SaldoInsoluto      DECIMAL(12,2),
			SucursalID         INT(11),
			CapitalNetoMes     DECIMAL(12,2),
			ResultadoCapNeto   DECIMAL(12,2),
		    Porcentaje   	   DECIMAL(10,2),
			DiferenciaMonto    DECIMAL(12,2));

	INSERT INTO TMPCREDITOSMAYORSALDO()
	SELECT Cre.ClienteID,Cre.CreditoID, SUM(Sal.SalCapVigente + Sal.SalCapAtrasado + Sal.SalCapVencido +
						...
						Cre.SucursalID,Var_CapitalNeto,Var_ResCapNet3Porc,Var_PorCredPorc, Decimal_Cero
	...
	UPDATE TMPCREDITOSMAYORSALDO Tmp
		SET  DiferenciaMonto = Var_ResCapNet3Porc - SaldoInsoluto;

	 SELECT ClienteID,			CreditoID,			SaldoInsoluto,
			SucursalID,			CapitalNetoMes,		ResultadoCapNeto,
			Porcentaje,			FORMAT(DiferenciaMonto,2) AS DiferenciaMonto,
			DiferenciaMonto AS DiferenciaMontoExc
		FROM TMPCREDITOSMAYORSALDO;

	DROP TABLE TMPCREDITOSMAYORSALDO;
    DROP TABLE TMPSALDOCAPITALNETO;
END IF;

-- Consulta Parametro Mayor Saldo Insoluto 3.5 %
IF(Par_NumRep = Rep_Parametro) THEN
	SELECT Var_CapitalNeto, Var_ResCapNet3Porc, Var_PorCredPorc;
    DROP TABLE TMPSALDOCAPITALNETO;
END IF;
END TerminaStore$$