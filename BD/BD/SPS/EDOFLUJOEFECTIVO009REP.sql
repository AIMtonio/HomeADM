...
-- ---------------------------------------------------------------------------------
-- EDOFLUJOEFECTIVO009REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `EDOFLUJOEFECTIVO009REP`;

DELIMITER $$
CREATE PROCEDURE `EDOFLUJOEFECTIVO009REP`(
	-- SP para generar el Flujo de Efectivo AyE
	-- Modulo Contabilidad --> Reportes --> Reportes Financieros
...
TerminaStore: BEGIN
	...
	DECLARE Var_AcumuladoCta 			DECIMAL(14,2);	-- Acomulado de la Cuenta
	DECLARE Var_AcumuladoCtaAux			DECIMAL(14,2);	-- Acomulado de la CuentaAux
	DECLARE Var_MontoCampo				DECIMAL(14,2);	-- Acomulado de la Cuenta
	...
	CREATE TABLE TMP_EDOFINFLUJOEFECTIVOREP(
	...
	CREATE TABLE TMP_ENCEDOFINFLUJOEFECTIVO(
	...
	CREATE TABLE TMP_DETEDOFINFLUJOEFECTIVO(
		RegistroID			INT(11)			NOT NULL COMMENT 'ID de Tabla',
		ConceptoFinanID		INT(11)			NOT NULL COMMENT 'Numero Consecutivo de Concepto',
	...
	SELECT IFNULL(ClaveEntidad, Cadena_Vacia),	 IFNULL(JefeContabilidad, Cadena_Vacia),	IFNULL(GerenteGeneral, Cadena_Vacia),	IFNULL(DirectorFinanzas, Cadena_Vacia)
	...
	SET Var_Texto = CONCAT('DEL ', UPPER(FUNCIONLETRASFECHA(Var_FechaInicio)), ' AL ', UPPER(FUNCIONLETRASFECHA(Par_FechaActual)));
	...
	IF(Par_FechaAnterior <> Fecha_Vacia) THEN
		...
		UPDATE TMP_EDOFINFLUJOEFECTIVOREP SET
			SaldoFinal = CASE WHEN Presentacion = SaldoContableActual	THEN IFNULL(SaldoActual, Decimal_Cero)
							  WHEN Presentacion = SaldoInicioFin 		THEN IFNULL(SaldoActual,Decimal_Cero) - IFNULL(SaldoAnterior,Decimal_Cero)
							  WHEN Presentacion = SaldoFinInicio 		THEN IFNULL(SaldoAnterior,Decimal_Cero) - IFNULL(SaldoActual,Decimal_Cero)
							  WHEN Presentacion = SaldoContableAnterior THEN IFNULL(SaldoAnterior,Decimal_Cero)
						 END
		WHERE NumeroTransaccion = Aud_NumTransaccion;
		...
		WHILE (Var_Contador <= Var_ConceptoFinanIDMax) DO
			...
			IF(Var_ConceptoFinanID > Entero_Cero ) THEN
				...
				IF((LOCATE(Signo_Mas, Var_FormulaReporte) > Entero_Cero OR LOCATE(Signo_Menos, Var_FormulaReporte) > Entero_Cero)) THEN
					...
					-- Actualizo la tabla base
					UPDATE TMP_EDOFINFLUJOEFECTIVOREP SET
						SaldoFinal = IFNULL(Var_AcumuladoCta,Decimal_Cero)
					WHERE ConceptoFinanID = Var_ConceptoFinanID
					  AND NumeroTransaccion = Aud_NumTransaccion;
	...
	SET Var_ConsultaEdoFin := CONCAT(Var_ConsultaEdoFin,' "',Var_DirectorFinanza,'" AS Var_DirectorFinanza,',' "',Var_ContaGeneral,'" AS Var_ContaGeneral, "',Var_DirectGeneral,'" AS Var_DirectGeneral, 0.00 AS Var_DecimalCero," ',
									 Var_Texto, '" AS Var_Texto;');
