...
-- CAJASARQTICKETREP
DELIMITER ;
DROP PROCEDURE IF EXISTS `CAJASARQTICKETREP`;DELIMITER $$

CREATE PROCEDURE `CAJASARQTICKETREP`(
    Par_SucursalID      int,
    Par_CajaID          int,
    Par_Fecha           date,
...
insert into TMPARQUEO
    select  substring(concat(RPAD(LPAD(CASE WHEN Den.Valor = 1
                                THEN 'Monedas'
                   ...
                   LPAD(format(ifnull(Bal.Cantidad, 0) * Den.Valor, 2), 13, ' ')),2,40),
