...
-- ---------------------------------------------------------------------------------
-- AMORTICREDITOAGROALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `AMORTICREDITOAGROALT`;DELIMITER $$

CREATE PROCEDURE `AMORTICREDITOAGROALT`(
/* SP PARA REGISTRAR LAS AMORTIZACIONES DEL CALENDARIO DE PAGOS DE UN CREDITO */
	Par_CreditoID			BIGINT(12),							# Numero de Credito
	Par_NumTransSim			BIGINT,								# Numero de transaccion del credito
	Par_ClienteID			INT(11),							# Numero de Cliente
	Par_CuentaID			BIGINT(12),							# Numero de Cuenta de Ahorro
	Par_MontoCre			DECIMAL(12,2),						# Monto del credito

	Par_NumAlta				TINYINT UNSIGNED,					# Tipo de alta
	...
	ManejoErrores: BEGIN
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
								'Disculpe las molestias que esto le ocasiona. Ref: SP-AMORTICREDITOAGROALT');
		...
		IF(Par_NumAlta = Alta_CreNuevo)THEN
			...
			/*SE DAN DE BAJA LAS AMORTIZACIONES DEL CALENDARIO AGROPECUARIO*/
			CALL AMORTICREDITOAGROBAJ(
				Par_CreditoID,		Par_Salida,			Par_NumErr,			Par_ErrMen,		Aud_EmpresaID,
			...
			INSERT INTO AMORTICREDITOAGRO(
					...
					SaldoIVASeguroCuota,SaldoComisionAnual,		MontoPendDesembolso,	EstatusDesembolso,
			...
			SELECT
					...
					SaldoIVAComFalP,	SaldoOtrasComis,		SaldoIVAComisi,			Entero_Cero,			Tmp_Insoluto,
					SaldoMoraVencido,	SaldoMoraCarVen,		Tmp_MontoSeguroCuota,	Tmp_IVASeguroCuota,		SaldoMontoSeguroCuota,
					SaldoIVASeguroCuota,Var_SaldoComisionAnual,	Tmp_Capital,			Var_EstatusDesembolso,
                    Aud_EmpresaID,			Aud_Usuario,			Aud_FechaActual,
					Aud_DireccionIP,	Aud_ProgramaID,			Aud_Sucursal,			Par_NumTransSim
			FROM TMPPAGAMORSIM
			WHERE NumTransaccion	= Par_NumTransSim;
		END IF;
		...
		IF(Par_NumAlta = Alta_CreReestructura)THEN
			...
			SELECT	Tmp_Consecutivo +
					...
					SaldoIVAComFalP,		SaldoOtrasComis,			SaldoIVAComisi,			Entero_Cero,		Tmp_Insoluto,
					SaldoMoraVencido,		SaldoMoraCarVen,			Tmp_MontoSeguroCuota,	Tmp_IVASeguroCuota,	SaldoMontoSeguroCuota,
					SaldoIVASeguroCuota,	Var_SaldoComisionAnual,		Aud_EmpresaID,			Aud_Usuario,		Aud_FechaActual,
					Aud_DireccionIP,		Aud_ProgramaID,				Aud_Sucursal,			Par_NumTransSim
			FROM TMPPAGAMORSIM
			WHERE NumTransaccion	= Par_NumTransSim;

			CALL TMPPAGAMORSIMBAJ(
				Par_NumTransSim,	SalidaNO,			Par_NumErr,			Par_ErrMen,		Aud_EmpresaID,
				Aud_Usuario,		Aud_FechaActual,	Aud_DireccionIP,	Aud_ProgramaID,	Aud_Sucursal,
				Aud_NumTransaccion);
		END IF;
