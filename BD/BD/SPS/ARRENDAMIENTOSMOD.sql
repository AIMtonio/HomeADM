...
-- ARRENDAMIENTOSMOD
DELIMITER ;
DROP PROCEDURE IF EXISTS `ARRENDAMIENTOSMOD`;DELIMITER $$

CREATE PROCEDURE `ARRENDAMIENTOSMOD`(
# =====================================================================================
# -- STORED PROCEDURE PARA MODIFICAR LOS ARRENDAMIENTOS
# =====================================================================================
	...
	Par_MontoCuota 				DECIMAL(12,2),	-- 'Valor final de cada cuota que el cliente debera pagar ',
	...
	DECLARE Var_SeguroArrendaID		INT(11);			-- ID DE SEGURO
	DECLARE Var_SeguroVidaArrendaID	INT(11);			-- ID DE SEGURO DE VIDA
	DECLARE Var_AnualMes			INT(11);			-- ANUALIDAD MES
	...
	DECLARE Var_MontoSegAnual 		DECIMAL(16,2);		-- MONTO DEL SEGURO ANUALIZADO
	DECLARE Var_MontoSegVidaAn 		DECIMAL(16,2);		-- MONTO DEL SEGURO DE VIDA ANUALIZADO
	...
	DECLARE DECIMAL_Cero 		DECIMAL(12,2);			-- DECIMAL DERO
	...
	SET DECIMAL_Cero			:= 0; 					-- DECIMAL DERO
	...
	ManejoErrores:BEGIN
		DECLARE EXIT HANDLER FOR SQLEXCEPTION
			BEGIN
				...
				SET Par_ErrMen  = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
										 'Disculpe las molestias que esto le ocasiona. Ref: SP-ARRENDAMIENTOSMOD');
				SET Var_Control = 'sqlException';
			END;

		IF(IFNULL(Par_ArrendaID,Entero_Cero)= Entero_Cero)THEN
			SET Par_NumErr		:= 001;
			SET Var_Control		:= 'arrendaID';
			SET Par_ErrMen		:= CONCAT('Especificar Arrendamiento');
			SET VarConsecutivo	:= Par_ArrendaID;
			LEAVE ManejoErrores;
		END IF;
		...
		CALL ARRENDAAMORTIZAPRO(
			...
			Par_ClienteID,			Par_ArrendaID,			Par_EsRentaAnticipada,		Par_CantRenAdelantadas,	Par_TipoRenAdelantadas,
		...
		SET Par_OtroGastos			:= IFNULL(Par_OtroGastos      ,Entero_Cero);
		SET Var_IVAOtrosGastos		:= IFNULL(Var_IVAOtrosGastos  ,Entero_Cero);
		SET Var_TotalPagoInicial	:= IFNULL(Var_TotalPagoInicial,Entero_Cero);
		SET Par_CantCuota 			:= IFNULL(Par_CantCuota       ,Entero_Cero);
		SET Par_MontoCuota 			:= IFNULL(Par_MontoCuota      ,Entero_Cero);
		SET Par_DiaPagoProd 		:= IFNULL(Par_DiaPagoProd     ,Entero_Cero);
		SET Par_FechaInhabil		:= IFNULL(Par_FechaInhabil    ,Entero_Cero);

		-- **************************************************************************************
		-- SE ACTUALIZAN LOS VALORES EN LA TABLA DE ARRENDAMIENTOS *******************
		-- **************************************************************************************
		UPDATE ARRENDAMIENTOS SET
			LineaArrendaID          = Par_LineaArrendaID,
			ClienteID               = Par_ClienteID,
			NumTransacSim           = Par_NumTransacSim,
			TipoArrenda             = Par_TipoArrenda,
			ProductoArrendaID       = Par_ProductoArrendaID,
			MontoArrenda            = Par_MontoArrenda,
			IVAMontoArrenda         = Var_IVAMontoArrenda,
			MontoEnganche           = Var_MontoEnganche,
			IVAEnganche             = Var_IVAEnganche,
			PorcEnganche            = Par_PorcEnganche,
			MontoFinanciado         = Var_MontoFinanciado,
			SeguroArrendaID         = Par_SeguroArrendaID,
			TipoPagoSeguro          = Par_TipoPagoSeguro,
			MontoSeguroAnual        = Par_MontoSeguroAnual,
			SeguroVidaArrendaID     = Par_SeguroVidaArrendaID,
			TipoPagoSeguroVida      = Par_TipoPagoSeguroVida,
			MontoSeguroVidaAnual    = Par_MontoSeguroVidaAnual,
			MontoResidual           = Par_MontoResidual,
			FechaApertura           = Par_FechaApertura,
			FechaPrimerVen          = Par_FechaPrimerVen,
			FechaUltimoVen          = Par_FechaUltimoVen,
			Plazo                   = Par_Plazo,
			FrecuenciaPlazo         = Par_FrecuenciaPlazo,
			TasaFijaAnual           = Par_TasaFijaAnual,
			MontoRenta              = Par_MontoRenta,
			MontoSeguro             = Par_MontoSeguro,
			MontoSeguroVida         = Par_MontoSeguroVida,
			CantRentaDepo           = Par_CantRentaDepo,
			MontoDeposito           = Par_MontoDeposito,
			IVADeposito             = Var_IVADeposito,
			MontoComApe             = Par_MontoComApe,
			IVAComApe               = Var_IVAComApe,
			OtroGastos              = Par_OtroGastos,
			IVAOtrosGastos          = Var_IVAOtrosGastos,
			TotalPagoInicial        = Var_TotalPagoInicial,
			CantCuota               = Par_CantCuota,
			MontoCuota              = Par_MontoCuota,
			DiaPagoProd             = Par_DiaPagoProd,
			FechaInhabil            = Par_FechaInhabil,
			EsRenAnticipada			= Par_EsRentaAnticipada,
			TipRenAdelanta			= Par_TipoRenAdelantadas,
			NumRenAdelantada		= Par_CantRenAdelantadas,
			RentasAdelantadas		= Par_CuotasAdelantadas,
			IVARenAdelanta			= Par_IVACuotasAdelan,
			RentaAnticipada			= Par_RentaAnticipada,
			IVARentaAnticipa		= Par_IVARentaAnticipada,
		...
		SET Par_NumErr      := 000;
		SET Var_Control     := 'arrendaID';
		SET Par_ErrMen      := CONCAT('Arrendamiento modificado exitosamente: ',Par_ArrendaID);
		SET VarConsecutivo  := Par_ArrendaID;
	END ManejoErrores;
