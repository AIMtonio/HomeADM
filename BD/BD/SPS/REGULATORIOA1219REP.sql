...
-- REGULATORIOA1219REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `REGULATORIOA1219REP`;
DELIMITER $$

CREATE PROCEDURE `REGULATORIOA1219REP`(
	/* Genera el regulatorio A1219 para Sofipo */
...
TerminaStore:BEGIN
	...
	DECLARE Var_Resultado		DECIMAL(18,2);
	DECLARE Con_Resultado		VARCHAR(25);
	DECLARE Con_CapGanado		VARCHAR(25);
	DECLARE Con_CapContable		VARCHAR(25);
	DECLARE Con_Participacion	VARCHAR(25);
	...
	DECLARE CveTipoSaldo	VARCHAR(4);		-- Numero de clave del tipo de saldo
	DECLARE CveTipoSaldo135	VARCHAR(4);		-- Numero de clave del tipo de saldo
	DECLARE CveTipoSaldo136	VARCHAR(4);		-- Numero de clave del tipo de saldo
	...
	SET NumReporte			:= '1219';		-- Clave del reporte
	SET CveTipoSaldo		:= '130';		-- Numero de clave del tipo de saldo
	SET CveTipoSaldo135		:= '135';		-- Numero de clave del tipo de saldo
	SET CveTipoSaldo136		:= '136';		-- Numero de clave del tipo de saldo
	SET Rep_Excel 			:= 1 ;
	SET Rep_Csv	  			:= 2;
	SET Entero_Uno 			:= 1;
	SET Entero_Cero			:= 0;
	...
	SET Var_Reporte			:='A1219';
	SET Con_UbiActual 		:= 'A';
	SET Con_UbiHistorica 	:= 'H';
	SET Con_Fecha 			:= 'F';
	SET Con_Resultado 		:= '421100000000';
	SET Con_CapGanado 		:= '400000000000';
	SET Con_CapContable		:= '420000000000';
	SET Con_Participacion	:= '450000000000';
	...
	DROP TABLE IF EXISTS TMPREGULATORIOA1219;
	DROP TABLE IF EXISTS TMPREGULATORIOA;
	DROP TABLE IF EXISTS TMPREGULATORIOB;

	CREATE TEMPORARY TABLE TMPREGULATORIOA1219(
	...
	INSERT INTO  TMPREGULATORIOA1219
	...
	WHERE Concepto.NumClien = Var_NumCliente
		...
		AND (Concepto.CuentaFija NOT LIKE ('5%') AND Concepto.CuentaFija NOT LIKE('6%'));
	...
	IF(Var_NumConceptos > Entero_Cero) THEN
		...
		SELECT BalanceSubsidiaria1 INTO Var_Resultado FROM TMPREGULATORIOA1219 WHERE ClaveEntidad = Con_Resultado;

		UPDATE TMPREGULATORIOA1219 SET
			BalanceSubsidiaria1 = BalanceSubsidiaria1 + Var_Resultado
		WHERE ClaveEntidad IN(Con_CapGanado,Con_CapContable);

		SELECT BalanceSubsidiaria1 INTO Var_Resultado FROM TMPREGULATORIOA1219 WHERE ClaveEntidad = Con_CapGanado;

		UPDATE TMPREGULATORIOA1219 SET
			BalanceSubsidiaria1 = Var_Resultado
		WHERE ClaveEntidad = Con_Participacion;

	END IF;

    CREATE TEMPORARY TABLE TMPREGULATORIOA SELECT * FROM TMPREGULATORIOA1219;
    CREATE TEMPORARY TABLE TMPREGULATORIOB SELECT * FROM TMPREGULATORIOA1219;
	...
	IF Par_NumRep = Rep_Excel THEN
		...
		FROM TMPREGULATORIOA1219 ORDER BY ClaveEntidad;

	END IF;

	IF Par_NumRep = Rep_Csv THEN

        (SELECT CONCAT(ClaveEntidad,';',NumReporte,';',CveTipoSaldo,';',Entero_Cero,';',BalanceSubsidiaria1) AS Valor
        FROM TMPREGULATORIOA1219)
        UNION
        (SELECT CONCAT(ClaveEntidad,';',NumReporte,';',CveTipoSaldo135,';',Entero_Cero,';',BalanceSubsidiaria1) AS Valor
        FROM TMPREGULATORIOA)
        UNION
		(SELECT CONCAT(ClaveEntidad,';',NumReporte,';',CveTipoSaldo136,';',Entero_Cero,';',BalanceSubsidiaria1) AS Valor
        FROM TMPREGULATORIOB);
