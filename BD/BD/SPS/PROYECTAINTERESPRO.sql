...
-- FNCALMONTOLIQ
DELIMITER ;
DROP PROCEDURE IF EXISTS `PROYECTAINTERESPRO`;
DELIMITER $$

CREATE PROCEDURE `PROYECTAINTERESPRO`(
/** ==================================================================================
...
ManejoErrores:BEGIN
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET	Par_ErrMen	:= CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
							'Disculpe las molestias que esto le ocaciona. Ref: SP-PROYECTAINTERESPRO');
		END;
	...
	SET Var_CapitalPorPAgar := IFNULL(Var_CapitalPorPAgar, Entero_Cero);
		...
		IF(Var_CapitalMora > Var_ConCero) THEN
			...
			WHILE(Var_AmoDevMoraIni < Var_IniAmortizaID) DO
				ProyectaMora:BEGIN
					...
					SET Var_SaldoProvAmo 		:= Entero_Cero;
					SET Var_InteresPactado		:= Entero_Cero;
					SET Var_CapitalMora			:= Entero_Cero;
					SET Var_SaldoDevMoraInt		:= Entero_Cero;
					/* FIN RESETEO DE VARIABLES*/
					...
					SET Var_SaldoProvAmo	:= IFNULL(Var_SaldoProvAmo, Entero_Cero);
					SET Var_InteresPactado 	:= IFNULL(Var_InteresPactado, Entero_Cero);
					SET Var_CapitalMora 	:= IFNULL(Var_CapitalMora, Entero_Cero);

					SET Var_SaldoDevMoraInt := ROUND(Var_FactorMora*Var_CapitalMora/(Var_DiasAnio*Var_ConCien),2)*Var_DiasProyecta;
					SET Var_SaldoDevMoraInt	:= IFNULL(Var_SaldoDevMoraInt, Entero_Cero);

					IF(Var_IVAIntMor = Var_ConSI) THEN
						SET	Var_SaldoDevIVAMoraInt := ROUND(Var_SaldoDevMoraInt*Var_IVA,2);
					END IF;

					SET Var_SaldoDevIVAMoraInt	:= IFNULL(Var_SaldoDevIVAMoraInt, Entero_Cero);
					SET Var_SaldoProvMoraInt 	:= Var_SaldoProvMoraInt + Var_SaldoDevMoraInt;
					SET Var_SaldoProvMoraInt	:= IFNULL(Var_SaldoProvMoraInt, Entero_Cero);
					SET Var_SaldoProvIVAMoraInt := Var_SaldoProvIVAMoraInt + Var_SaldoDevIVAMoraInt;
					SET Var_SaldoProvIVAMoraInt := IFNULL(Var_SaldoProvIVAMoraInt, Entero_Cero);
	...
	WHILE (Var_IniAmortizaID <= Var_UltAmortizaID AND Var_IniAmortizaID <> Var_ConCero) DO
	 ProyectInteres:BEGIN
		...
		IF (Var_IniAmortizaID = Var_UltAmortizaID) THEN
			...
			SET Var_SaldoDevInteres := IFNULL(Var_SaldoDevInteres, Entero_Cero);
	...
	SET Var_CapitalPorPAgar 		:= IFNULL(Var_CapitalPorPAgar, Entero_Cero);
	SET Var_SaldoProvInteres 		:= IFNULL(Var_SaldoProvInteres, Entero_Cero);
	SET Var_SaldoProvIVAInteres 	:= IFNULL(Var_SaldoProvIVAInteres, Entero_Cero);
	SET Var_SaldoProvMoraInt 		:= IFNULL(Var_SaldoProvMoraInt, Entero_Cero);
	SET Var_SaldoProvIVAMoraInt 	:= IFNULL(Var_SaldoProvIVAMoraInt, Entero_Cero);
