...
-- TIPOSPLANAHORROMOD
DELIMITER ;
DROP PROCEDURE IF EXISTS `TIPOSPLANAHORROMOD`;
DELIMITER $$

CREATE PROCEDURE `TIPOSPLANAHORROMOD`(
-- ==================================================
-- STORE PARA ACTUALIZAR LOS TIPOS DE PLAN DE AHORRO
-- ==================================================
	...
	ManejoErrores:BEGIN
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen	:= CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
							'Disculpe las molestias que esto le ocasiona. Ref: SP-TIPOSPLANAHORROMOD');
			SET Var_Control	:='sqlException';
		END;

		IF(IFNULL(Par_PlanID,Entero_Cero)=Entero_Cero)THEN
			SET Par_NumErr	:= 01;
			SET Par_ErrMen	:= 'El Id del Plan esta vacio.';
		...
		IF(IFNULL(Par_Serie,Cadena_Vacia)=Cadena_Vacia)THEN
			SET Par_NumErr	:= 08;
			SET Par_ErrMen	:= 'El numero de folios por Plan de Ahorro esta vacio.';
		...
		UPDATE TIPOSPLANAHORRO SET
			Nombre 				=	Par_Nombre,
			FechaInicio 		= 	Par_FechaInicio,
			FechaVencimiento 	=	Par_FechaVencimiento,
			FechaLiberacion 	= 	Par_FechaLiberacion,
			DepositoBase 		= 	Par_DepositoBase,

			MaxDep 				= 	Par_MaxDep,
			Prefijo 			=	Par_Prefijo,
			Serie 				=	Par_Serie,
			leyendaBloqueo 		=	Par_LeyendaBloqueo,
			LeyendaTicket 		=	Par_LeyendaTicket,

			DiasDesbloqueo		=	Par_DiasDesbloqueo,
		...
		DELETE FROM TIPOSCUEPLANAHORRO WHERE PlanAhoID = Par_PlanID;
        ...
        WHILE(Var_TipoCuentaID<>Par_TiposCuentas)DO
            ...
            INSERT INTO TIPOSCUEPLANAHORRO VALUES(
				Par_PlanID , 		Var_TipoCuentaID, 	Aud_EmpresaID, 	Aud_Usuario, 		Aud_FechaActual,
				Aud_DireccionIP,	Aud_ProgramaID,		Aud_Sucursal,	Aud_NumTransaccion);

		END WHILE;

		SET Par_NumErr	:= Entero_Cero;
		SET Par_ErrMen	:= CONCAT("Plan de Ahorro Modificado Exitosamente: ",CONVERT(Par_PlanID,CHAR));
		SET Var_Control	:= 'planID';
