...
-- CREDITOREESTMOD
DELIMITER ;
DROP PROCEDURE IF EXISTS `CREDITOREESTMOD`;DELIMITER $$

CREATE PROCEDURE `CREDITOREESTMOD`(
# ==================================================================================================================
# ------- SP QUE SE EJECUTA DESDE LA PANTALLA DE REESTRUCTURA DE CREDITO PARA MODIFICAR UN CREDITO RENOVACION ------
# ==================================================================================================================
	...
	ManejoErrores: BEGIN
		...
		BEGIN
				...
				SET Par_ErrMen = CONCAT('El SAFI ha tenido un problema al concretar la operacion. Disculpe las molestias que  ',
					' esto le ocasiona. Ref: SP-CREDITOREESTMOD');
			END;
	   ...
	   IF(IFNULL(Par_DestinoCreID, Entero_Cero) =  Entero_Cero) THEN
		...
		IF NOT EXISTS (SELECT CreditoID FROM CREDITOS WHERE CreditoID = Par_Relacionado) THEN
			SET Par_NumErr	:= 10;
			SET Par_ErrMen	:= 'El Credito a Reestructurar No Existe.';
			SET Var_Control	:= 'destinoCreID';
			LEAVE ManejoErrores;
		END IF;
		IF NOT EXISTS (SELECT CreditoDestinoID FROM REESTRUCCREDITO WHERE CreditoDestinoID= Par_CreditoID) THEN
			SET Par_NumErr	:= 11;
			SET Par_ErrMen	:= 'El Indicado No es Reestructura.';
			SET Var_Control	:= 'destinoCreID';
			LEAVE ManejoErrores;
		END IF;
		...
		CALL CREDITOSMOD (
			...
			TipoCredito, 			Par_Relacionado,    Par_SolicCredID,    Par_MontoCredito,   Par_MonedaID,
			Par_FechaInicio, 		Par_FechaVencim,    Par_FactorMora,     Par_CalcInterID,    Par_TasaBase,
			...
			Par_FecInhabil, 		Par_CalIrregul,     Par_DiaPagoInt,     Par_DiaPagoCap,     Par_DiaMesInt,
			Par_DiaMesCap, 			Par_AjFUlVenAm,     Par_AjuFecExiVe,    Par_InsFondeoID,    Par_LineaFondeo,
			Par_FecTraspVen, 		Par_FechTermina,    Par_NumTransacSim,  Par_TipoFondeo,     Par_MonComApe,
			Par_IVAComApe, 			Par_ValorCAT,       Par_Plazo,          Par_TipoDisper,     Cadena_Vacia,
			Par_TipoCalInt, 		Par_DestinoCreID,   Par_NumAmortInt,    Par_MontoCuota,   	Decimal_Cero,
			Par_ClasiDestinCred, 	Var_TipoPrepago,   	Par_FechaInicio,	Var_ForCobroSegVida,Var_DescSeguro,
			Var_MontoSegOri,		Cadena_Vacia,		Cadena_Vacia,		Cadena_Vacia,		Cadena_Vacia,
		...
		SET  Par_ErrMen  := CONCAT('Credito Reestructura Modificado Exitosamente: ', CONVERT(Par_CreditoID, CHAR), '.');
		SET Var_Control	:= 'creditoID';
