...
-- CUENTASCONTABLESALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `CUENTASCONTABLESALT`;

DELIMITER $$
CREATE PROCEDURE `CUENTASCONTABLESALT`(
...
TerminaStore: BEGIN
	...
	ManejoErrores:BEGIN
		...
		INSERT INTO CUENTASCONTABLES (
			EmpresaID,				CuentaCompleta,			CuentaMayor,			Descripcion,		DescriCorta,
			Naturaleza,				Grupo,					TipoCuenta,				MonedaID,			Restringida,
			CodigoAgrupador,		Nivel,					FechaCreacionCta,
			Usuario,				FechaActual,			DireccionIP,			ProgramaID,			Sucursal,
			NumTransaccion)
		VALUES (
			Aud_EmpresaID,			Par_CuentaCompleta, 	Par_CuentaMayor,		Par_Descripcion,	Par_DescriCorta,
			Par_Naturaleza, 		Par_Grupo, 				Par_TipoCuenta,			Par_MonedaID,		Par_Restringida,
			Par_CodigoAgrupador,	Par_Nivel,				Par_FechaCreacionCta,
			Aud_Usuario,			Aud_FechaActual,		Aud_DireccionIP,		Aud_ProgramaID,		Aud_Sucursal,
			Aud_NumTransaccion);

		SET Par_NumErr 	:= Entero_Cero;
		SET Par_ErrMen 	:= CONCAT('Cuenta Contable Agregada Exitosamente: ',CONVERT(Par_CuentaCompleta, CHAR));
		SET Var_Control	:= 'cuentaCompleta';
