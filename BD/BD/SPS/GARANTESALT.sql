...
-- GARANTESALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `GARANTESALT`;DELIMITER $$

CREATE PROCEDURE `GARANTESALT`(
  # ========== SP PARA ALTA DE GARANTES =============================================
	...
	INOUT   Par_GaranteID   INT(11),        	-- Garante que genero el error
	...
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			SET Par_ErrMen   := CONCAT('El SAFI ha tenido un problema al concretar la operacion. Disculpe las molestias que ',
				'esto le ocasiona. Ref: SP-GARANTESALT');
		...
		IF (Par_CURP != Cadena_Vacia)THEN
			SET Valida_CURP:=(SELECT  CURP FROM GARANTES WHERE CURP = Par_CURP AND  TipoPersona != Per_Moral AND Par_TipoPersona != Per_Moral);
		...
		IF(Par_TipoPersona <> Per_Moral) THEN
            ...
            ELSE
                SET Valida_RFC:=(SELECT  RFCOficial FROM GARANTES WHERE RFCOficial = Par_RFC);
		...
		IF(Par_TipoPersona = Per_Moral) THEN
			...
			IF (Par_RFCpm = Cadena_Vacia)THEN
				SET Valida_RFC:=Cadena_Vacia;
			ELSE
				SET Valida_RFC:=(SELECT  RFCOficial FROM GARANTES WHERE RFCOficial = Par_RFCpm);
        ...
        SET NumeroGarante 	:= (SELECT IFNULL(MAX(GaranteID),Entero_Cero) + 1 FROM GARANTES);
		SET Par_GaranteID 	:= LPAD(NumeroGarante, 10, 0);
		SET Aud_FechaActual := NOW();


		INSERT INTO GARANTES (
		    GaranteID,              TipoPersona,            Titulo,                 PrimerNombre,           SegundoNombre,
            ...
            FechaRegPub,			EstadoIDReg,			MunicipioRegPub,		EmpresaID,				Usuario,
            FechaActual,		    DireccionIP,		    ProgramaID,			    Sucursal,				NumTransaccion

		)
		VALUES (
		    NumeroGarante,          Par_TipoPersona,        Par_Titulo,             Par_PrimerNombre,       Par_SegundoNombre,
            Par_TercerNombre,       Par_ApellidoPaterno,    Par_ApellidoMaterno,    Par_FechaNacimiento,    Par_Nacionalidad,
            Par_LugarNacimiento,    Par_EstadoID,           Par_PaisResidencia,     Par_Sexo,               Par_CURP,
            Par_RegistroHacienda,   Par_RFC,                Par_FechaConstitucion,  Par_EstadoCivil,        Par_TelefonoCel,
            Par_Telefono,           Par_ExtTelefonoPart,    Par_Correo,             Par_Fax,                Par_Observaciones,
            Par_RazonSocial,        Par_RFCpm,              Var_RFCOficial,         Par_PaisConstitucionID, Par_CorreoAlterPM,
            Par_TipoSocID,          Par_GrupoEmp,           Par_FEA,                Par_PaisFEA,			NombreComplet,
			Par_TipoIdentID,		Par_NumIndentif,		Par_FecExIden,			Par_FecVenIden,			Par_EstadoIDCli,
            Par_MunicipioID,		Par_LocalidadID,		Par_ColoniaID,			Par_Calle,				Par_NumeroCasa,
            Par_NumInterior,		Par_CP,					Par_Lote,				Par_Manzana,   			DirecCompleta,
			Par_Esc_Tipo,			Par_EscriPub,			Par_LibroEscr,			Par_VolumenEsc,			Par_FechaEsc,
            Par_EstadoIDEsc,		Par_MunicipioEsc,		Par_Notaria,			Par_NomApoder,			Par_RFC_Apoder,
            Par_RegistroPub,		Par_FolioRegPub,		Par_VolRegPub,			Par_LibroRegPub,		Par_AuxiRegPub,
			Par_FechaRegPub,		Par_EstadoIDReg,		Par_MunicipioRegPub,	Par_EmpresaID,          Aud_Usuario,
			Aud_FechaActual,		Aud_DireccionIP,		Aud_ProgramaID,			Aud_Sucursal,			Aud_NumTransaccion
		);

		SET Par_NumErr  	:= 0;
		SET Par_ErrMen  	:= CONCAT("Garante Agregado Exitosamente: ", CONVERT(NumeroGarante, CHAR));
		SET Var_Control 	:= 'numero';
		SET Var_Consecutivo := LPAD(NumeroGarante, 10, 0);
