...
-- ---------------------------------------------------------------------------------
-- CREDCAPITALNETOFIRAPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `CREDCAPITALNETOFIRAPRO`;DELIMITER $$

CREATE PROCEDURE `CREDCAPITALNETOFIRAPRO`(
	/*SP para el reporte de fira de Capital Neto*/
	...
	ManejoErrores: BEGIN
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									'Disculpe las molestias que esto le ocasiona. Ref: SP-CREDCAPITALNETOFIRAPRO');
		...
		SELECT
			...
			Par_FechaCorte,
			@cont:=@cont+1,
			CONCAT_WS(",",CAT.GrupoID,CAT.Grupo, CONCAT('"',CAT.Concepto,'"'),0),
			Aud_EmpresaID,
			Aud_Usuario,
			Aud_FechaActual,
			Aud_DireccionIP,
