...
-- POLIZACEDESPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `POLIZACEDESPRO`;DELIMITER $$

CREATE PROCEDURE `POLIZACEDESPRO`(
# ===========================================================
# ----- SP PARA REGISTRAR DETALLE DE POLIZA DE LOS CEDES-----
# ===========================================================
	INOUT Par_Poliza	BIGINT(20),
	Par_Fecha			DATE,
	Par_Cliente			INT(11),
	Par_ConceptoOpera	INT(11),
	Par_CedeID			BIGINT(11),
	...
	SET	Procedimiento				:= 'POLIZACEDESPRO';			-- Nombre del Procedimiento Contable
	SET TipoInstrumentoID			:= 28;							-- Tipo Instrumento CEDES corresponde con TIPOINSTRUMENTOS
	SET	Var_Cuenta					:= '0000000000000000000000000';	-- Cuenta Contable Vacia
	SET Var_Instrumento 			:= CONVERT(Par_CedeID, CHAR);
	...
	ManejoErrores:BEGIN
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION
			BEGIN
				...
				SET Par_ErrMen = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									    'Disculpe las molestias que esto le ocasiona. Ref: SP-POLIZACEDESPRO');
		...
		SELECT	Nomenclatura, Cuenta, NomenclaturaCR  INTO Var_Nomenclatura, Var_CuentaMayor, Var_NomenclaturaCR
			FROM  CUENTASMAYORCEDE Ctm
			WHERE Ctm.ConceptoCedeID	= Par_ConceptoOpera;
		...
		ELSE
			...
			SELECT	tip.TipoCedeID, ced.Plazo INTO Var_TipoCedeID , Var_Plazo
				FROM	CEDES		ced,
						TIPOSCEDES	tip
				WHERE	ced.CedeID 		= Par_CedeID
				AND 	tip.TipoCedeID	= ced.TipoCedeID;
			...
			IF LOCATE(For_TipProduc, Var_Cuenta) > Entero_Cero THEN
				SELECT	SubCuenta INTO Var_SubCuentaTP
					...
					AND		ConceptoCedeID	= Par_ConceptoOpera;
			...
			IF LOCATE(For_Moneda, Var_Cuenta) > Entero_Cero THEN
				SELECT	SubCuenta INTO Var_SubCuentaTM
					FROM  	SUBCTAMONEDACEDE Sub
					WHERE	Sub.MonedaID		= Par_MonedaID
					AND		Sub.ConceptoCedeID	= Par_ConceptoOpera;
			...
			IF LOCATE(For_TipPlazo, Var_Cuenta) > Entero_Cero THEN
				SELECT	Sub.PlazoInferior, Sub.PlazoSuperior, Sub.SubCuenta
						INTO Var_PlazoInferior, Var_PlazoSuperior, Var_SubCuentaTD
							FROM  	SUBCTAPLAZOCEDE Sub
							WHERE	Sub.ConceptoCedeID	= Par_ConceptoOpera
