...
-- OBLSOLIDMOD
DELIMITER ;
DROP PROCEDURE IF EXISTS `OBLSOLIDMOD`;

DELIMITER $$
CREATE PROCEDURE `OBLSOLIDMOD`(
	-- SP QUE Modificar Obligados Solidarios
	Par_OblSolidID			BIGINT(20),				-- ID del obligado solidario
	...
	Par_EstadoCivil			CHAR(2),				/* Estado civil del obligado solidario S.-soltero, CS.-casado b. separados, CM.-casado b.
...
TerminaStore: BEGIN
	...
	DECLARE	SalidaSI			CHAR(1);			-- Si genera salida
	DECLARE	SalidaNO			CHAR(1);			-- No genera salida
	DECLARE FechaSis			DATE;				-- Fecha del sistema
    DECLARE Tipo_Agregar		CHAR(1);			-- Tipo de operacion agregar
    DECLARE Tipo_Modificar		CHAR(1);			-- Tipo de operacion modificar
    DECLARE PaisMexico			INT(11);			-- Identificador de pais mexico
	DECLARE EsOblSolid			CHAR(3);			-- Valor obligado solidario
	DECLARE Cons_Si				CHAR(1);			-- Constante si

	-- Declaracion de Variables
	DECLARE	Per_Fisica			CHAR(1);			-- Valor personal fisica
	DECLARE	Per_ActEmp			CHAR(1);			-- Valor Actual
	DECLARE	Per_Moral			CHAR(1);			-- Tipo personal moral
	DECLARE Var_NombreComp		VARCHAR(200);		-- Nombre completo
	DECLARE Var_ValidaRFC		CHAR(13);			-- Validador RFC
	DECLARE Var_LocalidadID 	INT(11);			-- Identificador de la localidad
	DECLARE Var_ColoniaID   	INT(11);			-- Identificador de colonia
	DECLARE	Var_NombEstado		VARCHAR(100);		-- Nombre del estado
	DECLARE	Var_NombMunicipio	VARCHAR(150);		-- Nombre del municipio
	DECLARE	Var_DirecCompl		VARCHAR(500);		-- Direccion completa
	DECLARE Var_DirecComplCte	VARCHAR(500);		-- Direccion completa del cliente
	DECLARE Var_OblSolidID		INT(11);			-- Identificador obligado solidario
	DECLARE Var_ClienteID		INT(11);			-- Identificador del cliente
	DECLARE Var_Edad			INT(11);			-- Edad
	DECLARE Var_Control			VARCHAR(50);		-- Control de validaciones
	...
	SET EsOblSolid 				:= 'OBS';			-- Valor obligdo solidario
...
ManejoErrores:BEGIN
	...
	ELSE
		...
		IF(Var_ValidaRFC = Par_RFC)THEN
			...
			SET Var_OblSolidID = (SELECT OblSolidID FROM OBLIGADOSSOLIDARIOS WHERE RFC = Par_RFC);

            IF(IFNULL(Var_OblSolidID,Entero_Cero)!=Par_OblSolidID)THEN
				SET Par_NumErr := 015;
    ...
    IF(Par_TipoPersona = Per_Fisica OR Par_TipoPersona = Per_ActEmp) THEN
		...
		SELECT COUNT(OblSolidID)	INTO Var_NumRegistros
		FROM ESCPUBOBLSOLID
		WHERE OblSolidID = Par_OblSOlidID;

		IF(Var_NumRegistros > Entero_Cero) THEN
			DELETE FROM ESCPUBOBLSOLID
			WHERE OblSolidID = OblSolidID;
		END IF;
	END IF;

	/*SECCION PLD: Deteccion de operaciones inusuales*/
	CALL PLDDETECCIONPRO(
		Par_OblSolidID,			Par_PrimerNom,			Par_SegundoNom,			Par_TercerNom,			Par_ApellidoPat,
		...
		EsOblSolid,				Cons_Si,				Cons_Si,				Cons_Si,				SalidaNO,
	...
	IF(Par_NumErr!=Entero_Cero)THEN
		...
		SET Var_Control			:= 'oblSolidID';
		LEAVE ManejoErrores;
	END IF;
	/*FIN SECCION PLD: Deteccion de operaciones inusuales*/

	UPDATE OBLIGADOSSOLIDARIOS SET
