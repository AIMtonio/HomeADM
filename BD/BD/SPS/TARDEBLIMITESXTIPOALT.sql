...
-- ---------------------------------------------------------------------------------
-- TARDEBLIMITESXTIPOALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `TARDEBLIMITESXTIPOALT`;DELIMITER $$

CREATE PROCEDURE `TARDEBLIMITESXTIPOALT`(
    Par_TipoTarjetaDebID            int(11),
    Par_DisposicionesDia            int(11),
...
Set Pro_Actualiza	:= 1;
    ...
    if exists (SELECT TipoTarjetaDebID
        ...
        if(ifnull(Par_MontoMaxDia, Cadena_Vacia) = Cadena_Vacia) then
            if (Par_Salida = Salida_SI) then
                select '003' as NumErr,
                    'Especifique el Monto Max. al Di­a' as ErrMen,
        ...
        if(ifnull(Par_MontoMaxCompraDia, Cadena_Vacia) = Cadena_Vacia) then
            if (Par_Salida = Salida_SI) then
                select '005' as NumErr,
                    'Especifique la Monto Max. de Compra al Di­a' as ErrMen,
                    'montoComDia' as control,
                    Entero_Cero as consecutivo;
            else
                set	Par_NumErr := 5;
                set	Par_ErrMen :='Especifique la Monto Max. de Compra al Di­a';
            end if;
            LEAVE TerminaStore;
        end if;
        ...
        INSERT INTO `TARDEBLIMITESXTIPO`    VALUES(
            Par_TipoTarjetaDebID,   Par_DisposicionesDia,       Par_NumConsultaMes,			Par_MontoMaxDia,
			Par_MontoMaxMes,    	Par_MontoMaxCompraDia,  	Par_MontoMaxComprasMensual, Par_BloqueoATM,
			Par_BloqueoPOS,   		Par_BloqueoCashback,    	Par_OperacionesMOTO,    	Aud_EmpresaID,
        ...
        if(Par_Salida =  Salida_SI) then
			SELECT
				'000' as CodigoRespuesta,
				concat('Limites por Tipo de Tarjeta Agregados Exitosamente: ', Par_TipoTarjetaDebID) as MensajeRespuesta,
                'tipoTarjetaDebID' as LimiteRegistrado,
                Par_TipoTarjetaDebID as NumeroTransaccion;
        else
            set Par_NumErr	:= 0;
            set Par_ErrMen	:= concat('Limites por Tipo de Tarjeta Agregados Exitosamente: ', Par_TipoTarjetaDebID);
        end if;
        LEAVE TerminaStore;
    end if;
