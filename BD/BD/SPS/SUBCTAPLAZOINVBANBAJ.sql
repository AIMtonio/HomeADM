...
-- ---------------------------------------------------------------------------------
-- SUBCTAPLAZOINVBANBAJ
DELIMITER ;
DROP PROCEDURE IF EXISTS `SUBCTAPLAZOINVBANBAJ`;DELIMITER $$

CREATE PROCEDURE `SUBCTAPLAZOINVBANBAJ`(
	...
	ManejoErrores: BEGIN
		...
		BEGIN
			...
			SET Par_ErrMen = concat('Estimado Usuario(a), Ha ocurrido una falla en el sistema, ' ,
									 ...
									 'esto le ocasiona. Ref: SP-SUBCTAPLAZOINVBANBAJ');
		...
		IF NOT EXISTS( SELECT ConceptoInvBanID FROM SUBCTAPLAZOINVBAN WHERE ConceptoInvBanID=Par_ConceptoInvBanID) THEN
				Set Par_NumErr	:= '006';
