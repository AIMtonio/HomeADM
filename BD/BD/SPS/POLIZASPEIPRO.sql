...
-- POLIZASPEIPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `POLIZASPEIPRO`;
DELIMITER $$

CREATE PROCEDURE `POLIZASPEIPRO`(
    Par_Poliza          BIGINT,
    Par_Empresa         INT,
    Par_Fecha           DATE,
...
TerminaStore: BEGIN
	...
	SET Salida_NO       	:= 'N';
	SET Procedimiento		:= 'POLIZASPEIPRO';
	...
	ManejoErrores:BEGIN
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION
			BEGIN
				...
				SET Par_ErrMen	= CONCAT('El SAFI ha tenido un problema al concretar la operación. ',
									'Disculpe las molestias que esto le ocasiona. Ref: SP-POLIZASPEIPRO');
				SET Var_Control = 'sqlException';
			END;


		SELECT	CtaSPei,	Clabe	INTO	Var_Cuenta,		Var_ClabeInst
			FROM PARAMETROSSPEI
		...
		CALL DETALLEPOLIZASALT(
			...
			Var_ClabeInst,		Par_Moneda,			Par_Cargos,			Par_Abonos,			Par_Descripcion,
		...
		SET	Par_NumErr := 000;
		SET	Par_ErrMen := 'Poliza SPEI Aplicada.';
		SET Var_Control := 'Poliza' ;
		SET Var_Consecutivo := Par_Poliza;
