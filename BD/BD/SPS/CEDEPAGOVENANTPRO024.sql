...
-- ---------------------------------------------------------------------------------
-- CEDEPAGOVENANTPRO024
DELIMITER ;
DROP PROCEDURE IF EXISTS `CEDEPAGOVENANTPRO024`;DELIMITER $$

CREATE PROCEDURE `CEDEPAGOVENANTPRO024`(
# =========================================================================
...
# =========================================================================
	...
	DECLARE PAGOCEDECUR CURSOR FOR
		SELECT	Ced.CedeID,					MAX(Ced.CuentaAhoID),    	MAX(Ced.TipoCedeID),	MAX(Ced.MonedaID),   		MAX(Ced.Monto),
				MAX(Amo.Interes),			MAX(Amo.ISRCal), 			Ced.ClienteID,			MAX(Ced.SaldoProvision),	MAX(cte.SucursalOrigen),
	  ...
	  DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			SET Par_ErrMen  := CONCAT(  'El SAFI ha tenido un problema al concretar la operacion. Disculpe las molestias que ',
										'esto le ocasiona. Ref: SP-CEDEPAGOVENANTPRO024');
		END;
			...
			BEGIN
				...
				LOOP
					...
					BEGIN
						...
						SET Es_Migrada := (SELECT IFNULL(COUNT(CedeIDSAFI),Entero_Cero) FROM EQU_CEDES WHERE CedeIDSAFI= Var_CedeID);

						-- Si es migrada la CEDE, toma el interés a retener migrado. Si no toma el calculado desde SAFI.
						IF(Es_Migrada > Entero_Cero) THEN
							SET Var_InteresRetener :=(SELECT InteresRetener FROM AMORTIZACEDES
										WHERE CedeID= Var_CedeID AND AmortizacionID=Var_AmortizacionID);
						ELSE
