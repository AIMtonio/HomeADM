DELIMITER ;
DROP PROCEDURE IF EXISTS ISOTRXREVERSOCONSULTAPRO;
DELIMITER $$

CREATE PROCEDURE ISOTRXREVERSOCONSULTAPRO(
	-- Descripcion: Proceso operativo que realiza el reverso de operacion de consultas
...
TerminaStore: BEGIN
	...
	DECLARE Var_TipoOperacion		INT(11);		-- Variable para almacenar el tipo de operacion
	...
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
								'Disculpe las molestias que esto le ocasiona. Ref: SP-ISOTRXREVERSOCONSULTAPRO');
		...
		-- Se evalua que exista la operacion para aplicar su reverso.
		SELECT	MontoSurcharge,		TipoOperacionID
		INTO	Var_MontoComision,	Var_TipoOperacion
		FROM TARDEBBITACORAMOVS
		WHERE TarjetaDebID = Par_NumeroTarjeta
			AND CodigoAprobacion = Par_OriCodigoAutorizacion
			AND FechaHrOpe = Var_FechaHrOpe
		...
		SET Var_TipoOperacion	:= IFNULL(Var_TipoOperacion,Entero_Cero);

		-- Cuando hay comision se realiza el reverso de este
		IF(Var_MontoComision > Decimal_Cero)THEN

			-- El reverso solo aplica para la operacion de consulta
			IF(Var_TipoOperacion <> Tran_Consulta)THEN
				SET Par_NumErr	:= 1117;
				SET Par_ErrMen	:= 'La Operacion de reversa, solo aplica para transaccion de consulta.';
				SET Var_Control	:= 'numeroTarjeta';
				LEAVE ManejoErrores;
			END IF;
			...
			CALL ISOTRXTARJETADEBVAL(
					...
					Var_MontoComision,	Entero_Cero,			Par_GiroComercial,
			...
			CALL ISOTRXCONTAPRO(
					...
					Var_MontoComision,		Var_Descripcion,			Par_CodigoAutorizacion,		Mov_CompraDebito,		Var_MonedaID,
