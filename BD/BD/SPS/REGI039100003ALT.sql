...
-- REGI039100003ALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `REGI039100003ALT`;
DELIMITER $$

CREATE PROCEDURE `REGI039100003ALT`(
/************************************************************************
--------- SP QUE AGREGA UN REGISTRO PARA EL REGULATORIO I0391 SOFIPO ----
************************************************************************/
    Par_Anio            INT,			-- Ano del reporte
    Par_Mes             INT, 			-- Mes del reporte
	Par_Entidad			VARCHAR(150), 	-- Clave de la Entidad
    Par_TipoValorID		VARCHAR(100), 	-- Clave tipo de Valor
    ...
    Par_TipoInversion	VARCHAR(5), 	-- Id del tipo de inversion
...
SELECT ClaveEntidad INTO Var_Entidad FROM PARAMETROSSIS WHERE EmpresaID = Aud_Empresa;
 ...
 ManejoErrores: BEGIN
		DECLARE EXIT HANDLER FOR SQLEXCEPTION
			BEGIN
				...
				SET Par_ErrMen	= CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									'Disculpe las molestias que esto le ocasiona. Ref: SP-REGI039100003ALT');
	...
	IF (IFNULL(Par_Anio,Entero_Cero)=Entero_Cero)THEN
		SET Par_NumErr  := 1;
        SET Par_ErrMen  := 'El a&ntilde;o esta vacio';
	...
	END IF;
     ...
     IF (IFNULL(Par_TipoValorID,Cadena_Vacia)=Cadena_Vacia)THEN
		SET Par_NumErr  := 19;
        SET Par_ErrMen  := 'El Tipo de Valor esta Vacio';
        SET Var_Control	:= CONCAT('tipoValor',Var_Consecutivo);
        LEAVE ManejoErrores;
	END IF;

    IF NOT EXISTS (SELECT TipoValorID FROM CATTIPOVALOR WHERE ClaveValor = Par_TipoValorID) THEN
		SET Par_NumErr  := 21;
        SET Par_ErrMen  := 'El Tipo de Valor es Invalido';
        SET Var_Control	:= CONCAT('tipoValor',Var_Consecutivo);
        LEAVE ManejoErrores;
    END IF;
    ...
    IF (IFNULL(Par_TipoInversion,Cadena_Vacia)=Cadena_Vacia)THEN
		SET Par_NumErr  := 21;
        SET Par_ErrMen  := 'El Tipo de Inversion esta Vacio';
        SET Var_Control	:= CONCAT('tipoInversion',Var_Consecutivo);
        LEAVE ManejoErrores;
	END IF;
    ...
    VALUES(
		...
		Var_Periodo,		Var_Consecutivo,	Par_TipoValorID,			Par_TipoInversion,		Par_TipoInstitucion
    );
