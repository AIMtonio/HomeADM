...
-- ---------------------------------------------------------------------------------
-- ISOTRXPREAUTORIZACIONPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `ISOTRXPREAUTORIZACIONPRO`;

DELIMITER $$
CREATE PROCEDURE `ISOTRXPREAUTORIZACIONPRO`(
    -- Descripcion: Proceso operativo que realiza la afectacion cuando la transacion sea de tipo preautorizacion
...
TerminaStore: BEGIN
    ...
    DECLARE Var_Descripcion			VARCHAR(150);  -- Descripicion
    DECLARE Var_FechaSistema		DATE;          -- Fecha del sistema
    DECLARE Var_MonedaID			INT(1);        -- Indica el tipo de moneda

    DECLARE Var_UsuarioID			INT(11);       -- Id del usuario logger
    DECLARE Var_AutorizadoID		INT(11);       -- id de autorizacion
    ...
    DECLARE Cons_BloqueoID			INT(11);       -- COnstante del bloqueoID 0

    DECLARE Cons_NatMovimiento		CHAR(1);       -- Natualeza del tipo bloqueo B
...
ManejoErrores: BEGIN
	...
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									'Disculpe las molestias que esto le ocasiona. Ref: SP-ISOTRXPREAUTORIZACIONPRO');
		END;
    ...
    -- Se realiza la llamada al procedimiento de validacion de tarjetas
    ...
    -- CONCATENACION DE LA DESCRIPCION DE LA OPERACION PARA LAS OPERACIONES CONTABLES
	...
	SET Par_NumErr := 000;
	SET Par_ErrMen := 'Proceso de Pre-Autorizacion Exitoso.';
