...
-- OBLSOLIDALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `OBLSOLIDALT`;

DELIMITER $$
CREATE PROCEDURE `OBLSOLIDALT`(

	-- SP DE ALTA DE OBLIGADOS SOLIDARIOS
	Par_PrimerNom				VARCHAR(50),		-- Primer nombre del OBLIGADO SOLIDARIO
	Par_SegundoNom				VARCHAR(50),		-- Segundo nombre del OBLIGADO SOLIDARIO
	Par_TercerNom				VARCHAR(50),		-- Tercer nombre del OBLIGADO SOLIDARIO
	...
	Par_EstadoCivil				CHAR(2),			-- Estado civil del OBLIGADO SOLIDARIO S.-soltero, CS.-casado b. separados, CM.-casado b. mancomunados, CC.-casado b. man. con capitulacion, V.-viudo, D.-divorciado,SE.-separado, U.-union libre */
	...
	Par_Nacion					CHAR(1),			-- Nacionalidad del OBLIGADO SOLIDARIO N.- Nacional E.- Extranjero
	Par_LugarNacimiento			INT(11),			-- Pais de Nacimiento
	Par_OcupacionID				INT(11),			-- Identificador de la ocupacion
	Par_Puesto					VARCHAR(100),		-- Puesto que ocupa el OBLIGADO SOLIDARIO
	Par_DomicilioTrabajo		VARCHAR(500),		-- Lugar de trabajo del OBLIGADO SOLIDARIO
	Par_TelefonoTrabajo			VARCHAR(13),		-- Telefono de trabajo del OBLIGADO SOLIDARIO
	Par_ExtTelTrabajo			VARCHAR(4),			-- Extension del telefono de trabajo del OBLIGADO SOLIDARIO
...
TerminaStore: BEGIN
	...
	DECLARE Var_OblSolidID		INT(11);				-- Numero consecutivo para el nuevo registro del Obligado solidario
	DECLARE Var_NombreComp		VARCHAR(200);		-- Nombre completo del Obligado solidario
	DECLARE Var_LocalidadID  	INT(11);				-- Numero de la localidad
	DECLARE Var_ColoniaID     	INT(11);				-- Numero de la colonia
	DECLARE Var_ValidaRFC		CHAR(13);			-- RFC del Obligado solidario
	DECLARE	Var_NombEstado		VARCHAR(100);		-- Nombre del estado
	DECLARE	Var_NombMunicipio	VARCHAR(150);		-- Nombre del municipio
	DECLARE	Var_DirecCompl		VARCHAR(500);		-- Domicilio completo del Obligado solidario
	DECLARE Var_Edad			INT(11);				-- Edad calculada del Obligado solidario
	...
	DECLARE Var_Consecutivo		INT(11);				-- Numero consecutivo (nuevo registro del Obligado solidario)
	DECLARE Var_OcupacionID		INT(11);			-- Variable para obtener la ocupacion de la tabla OCUPACIONES
	DECLARE	Var_SoloNombres		VARCHAR(500);		-- Solo Nombres del obligado solidario.
	DECLARE	Var_SoloApellidos	VARCHAR(500);		-- Solo Apellidos del obligado solidario.
	DECLARE	Var_RazonSocialPLD	VARCHAR(200);		-- Razon Social del obligado solidario.
	...
	DECLARE FechaSis			DATE;				-- Declaracion de Constante fecha si
	DECLARE	SalidaSI			CHAR(1);			-- Declaracion de Constante salida si
	DECLARE	SalidaNO			CHAR(1);			-- Declaracion de Constante salida no
    ...
    DECLARE Tipo_Agregar		CHAR(1);			-- Declaracion de Constante tipo agregar
    DECLARE Tipo_Modificar		CHAR(1);			-- Declaracion de Constante tipo modificar
    DECLARE PaisMexico			INT(11);			-- Declaracion de Constante pais Mexico
    DECLARE EsNA				CHAR(3);			-- Declaracion de Constante es NA
    DECLARE Cons_Si				CHAR(1);			-- Declaracion de Constante constante si
	...
	SET Var_OblSolidID			:= Entero_Cero;
	...
	BEGIN
		...
		SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									'Disculpe las molestias que esto le ocasiona. Ref: SP-OBLSOLIDALT');
	END;
	...
	ELSE
		...
		IF(Var_Edad < 18)THEN
			SET Par_NumErr := 013;
			SET Par_ErrMen := CONCAT('El Obligado Solidario Debe de Ser Mayor de Edad.');
			SET Var_Control := 'fechaNac';
			LEAVE ManejoErrores;
		END IF;
	...
	IF(Par_RFC = Cadena_Vacia)THEN
		SET Var_ValidaRFC:=Cadena_Vacia;
	ELSE
		SET Var_ValidaRFC:=(SELECT  RFC FROM OBLIGADOSSOLIDARIOS WHERE RFC = Par_RFC);
		IF (Var_ValidaRFC = Par_RFC)THEN
			SET Par_NumErr := 014;
			SET Par_ErrMen := CONCAT('Existe Otro Obligado Solidario con el Mismo RFC.');
			SET Var_Control := 'RFC';
			LEAVE ManejoErrores;
		END IF;
		...
		IF(Var_ValidaRFC = Par_RFC)THEN
			...
			SET Par_ErrMen := CONCAT('El RFC ',Par_RFC,
				...
				', favor de utilizar la pantalla de Asignacion de Obligado Solidario.');
			SET Var_Control := 'RFC';
			LEAVE ManejoErrores;
		END IF;
       ...
       IF(Var_ValidaRFC = Par_RFC)THEN
			...
			SET Par_ErrMen := CONCAT('El RFC ',Par_RFC,
				...
				', favor de utilizar la pantalla de Asignacion Obligado Solidario.');
			SET Var_Control := 'RFC';
			LEAVE ManejoErrores;
		END IF;
	END IF;
	...
	IF(IFNULL(Par_Sexo, Cadena_Vacia)) = Cadena_Vacia  THEN
		SET Par_NumErr := 020;
		SET Par_ErrMen := CONCAT('El sexo del Obligado Solidario no esta indicado.');
	...
	IF(IFNULL(Par_EstadoCivil, Cadena_Vacia)) = Cadena_Vacia  THEN
		SET Par_NumErr := 021;
		SET Par_ErrMen := CONCAT('El estado civil del Obligado Solidario no esta indicado.');
	...
	IF(IFNULL(Par_LocalidadID, Entero_Cero)) = Entero_Cero THEN
		SET Par_NumErr := 022;
		SET Par_ErrMen := CONCAT('La localidad del Obligado Solidario no esta indicado.');
	...
	IF(IFNULL(Par_ColoniaID, Entero_Cero)) = Entero_Cero THEN
		SET Par_NumErr := 023;
		SET Par_ErrMen := CONCAT('La colonia del Obligado Solidario no esta indicada.');
		SET Var_Control := 'coloniaID';
		LEAVE ManejoErrores;
	END IF;
	...
	IF((IFNULL(Par_OcupacionID, Entero_Cero)) = Entero_Cero AND Par_TipoPersona <> Per_Moral) THEN
		SET Par_NumErr	:= 28;
	...
	IF (Var_OcupacionID = Entero_Cero AND Par_TipoPersona <> Per_Moral) THEN
		SET Par_NumErr	:= 29;
	...
	SET Var_OblSolidID:= (SELECT IFNULL(MAX(OblSolidID),Entero_Cero) + 1
	...
	INSERT INTO OBLIGADOSSOLIDARIOS(
		OblSolidID,				TipoPersona,			RazonSocial,			PrimerNombre,		SegundoNombre,
		...
		DomicilioTrabajo,		TelefonoTrabajo,		ExtTelTrabajo,			SoloNombres,		SoloApellidos,
		RazonSocialPLD,			EmpresaID,				Usuario,				FechaActual,		DireccionIP,
		ProgramaID,				Sucursal,				NumTransaccion)
	VALUES(
		Var_OblSolidID,			Par_TipoPersona,		Par_RazonSocial,		Par_PrimerNom,		Par_SegundoNom,
		...
		Par_DomicilioTrabajo,	Par_TelefonoTrabajo,	Par_ExtTelTrabajo,		Var_SoloNombres,	Var_SoloApellidos,
		Var_RazonSocialPLD,		Aud_EmpresaID,			Aud_Usuario,			Aud_FechaActual,	Aud_DireccionIP,
		Aud_ProgramaID,			Aud_Sucursal,			Aud_NumTransaccion);

    IF(Par_TipoPersona = Per_Moral) THEN
		CALL ESCPUBOBLSOLIDALT(
								Var_OblSolidID,		Par_Esc_Tipo,		Par_NomApoder,		Par_RFC_Apoder,		Par_EscriPub,
								...
								Par_LocalRegPub,	SalidaNO, 			Par_NumErr,			Par_ErrMen,         Tipo_Agregar,
		...
		IF(Par_NumErr	!=	Entero_Cero)THEN
			SET Par_NumErr			:= 30;
			SET Par_ErrMen			:= Par_ErrMen;
			SET Var_Control			:= 'REGISTRO PUBLICO';
			LEAVE ManejoErrores;
		END IF;
	...
	SET Par_NumErr := 000;
	SET Par_ErrMen := CONCAT('Obligado Solidario Agregado Exitosamente: ',Var_OblSolidID);
	SET Var_Control := 'oblSolidarioID';
    SET Var_Consecutivo	:= Var_OblSolidID;
