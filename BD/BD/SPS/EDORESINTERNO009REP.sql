...
-- EDORESINTERNO009REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `EDORESINTERNO009REP`;
DELIMITER $$
CREATE PROCEDURE `EDORESINTERNO009REP`(
	-- SP para generar el Estado de Resultados AyE
...
TerminaStore: BEGIN
	...
	DECLARE Con_NO 			CHAR(1);			-- Constante es calculado NO
			...
			IF(Var_NumeroRegistros > Entero_Cero) THEN
				...
				WHILE( Var_Contador <= Var_NumeroRegistros) DO
					...
					IF(Var_CuentaContable != Cadena_Vacia) THEN
                        ...
                        NumeroTransaccion, CuentaContable, Grupo, SaldoInicialDeu, SaldoInicialAcre,
                        Cargos, Abonos, SaldoDeudor,SaldoAcreedor, DescripcionCuenta,
                        CuentaMayor, CentroCosto) VALUES(
	...
	IF IFNULL(Aud_NumTransaccion, Entero_Cero) > Entero_Cero THEN
		...
		OPEN  cur_Balance;
			...
			LOOP
				...
				SET Var_CreateTable := CONCAT(Var_CreateTable, CASE WHEN  @numPos > 0  THEN " ," ELSE " " END, Var_Columna, " DECIMAL(18,2)");


				SET Var_InsertTable := CONCAT(Var_InsertTable, CASE WHEN  @numPos > 0  THEN " ," ELSE " " END, Var_Columna);

				SET Var_InsertValores   := CONCAT(Var_InsertValores,  CASE WHEN  @numPos > 0  THEN " ," ELSE " " END, CAST(Var_Monto AS CHAR));
