...
-- ---------------------------------------------------------------------------------
-- BALANCEINTERNO008REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `BALANCEINTERNO008REP`;
DELIMITER $$


CREATE PROCEDURE `BALANCEINTERNO008REP`(
...
TerminaStore: BEGIN
  ...
  DECLARE For_TotCartCredi  VARCHAR(500);
  DECLARE Des_TotCartCredi  VARCHAR(200);
  DECLARE For_TotCartNeto   VARCHAR(500);
  DECLARE Des_TotCartNeto   VARCHAR(200);
  DECLARE For_CaptaTradi    VARCHAR(500);
  DECLARE Des_CaptaTradi    VARCHAR(200);
  DECLARE For_PresBanca     VARCHAR(500);
  DECLARE Des_PresBanca     VARCHAR(200);
  DECLARE For_TotPasCapital VARCHAR(500);
  DECLARE Des_TotPasCapital   VARCHAR(200);
  DECLARE For_CapSocial   VARCHAR(500);
  DECLARE Des_CapSocial     VARCHAR(200);
  DECLARE For_TotCapitalConta VARCHAR(500);
  DECLARE Des_TotCapitalConta VARCHAR(200);
  ...
  DECLARE Con_TotCartCred   INT;
  DECLARE Con_TotCartNeto   INT;
  DECLARE Con_CaptaTradi    INT;
  DECLARE Con_PresBanca   INT;
  DECLARE Con_TotPasCapital INT;
  DECLARE Con_CapSocial     INT;
  DECLARE Con_TotCapitalConta INT;
  ...
  SET  Ubi_Histor     := 'H';
  SET  Tif_Balance    := 1;
  SET  Con_ResulNeto    := 45;
  SET  Con_TotCartCred  := 48;
  SET  Con_TotCartNeto  := 49;
  SET  Con_CaptaTradi   := 50;
  SET  Con_PresBanca    := 23;
  SET  Con_TotPasCapital  := 76;
  SET  Con_CapSocial    := 36;
  SET  Con_TotCapitalConta:= 75;
  SET  NumCliente     := 8;
  ...
  SELECT CuentaContable, Desplegado INTO For_TotCartCredi, Des_TotCartCredi
   FROM CONCEPESTADOSFIN
     WHERE EstadoFinanID = Tif_Balance
        AND ConceptoFinanID = Con_TotCartCred
        AND NumClien = NumCliente;

  SET  For_TotCartCredi := IFNULL(For_TotCartCredi, Cadena_Vacia);
  SET  Des_TotCartCredi := IFNULL(Des_TotCartCredi, Cadena_Vacia);

  SELECT CuentaContable, Desplegado INTO For_TotCartNeto, Des_TotCartNeto
   FROM CONCEPESTADOSFIN
    WHERE EstadoFinanID = Tif_Balance
      AND ConceptoFinanID = Con_TotCartNeto
      AND NumClien = NumCliente;

  SET  For_TotCartNeto := IFNULL(For_TotCartNeto, Cadena_Vacia);
  SET  Des_TotCartNeto := IFNULL(Des_TotCartNeto, Cadena_Vacia);


  SELECT CuentaContable, Desplegado INTO For_CaptaTradi, Des_CaptaTradi
   FROM CONCEPESTADOSFIN
    WHERE EstadoFinanID = Tif_Balance
      AND ConceptoFinanID = Con_CaptaTradi
      AND NumClien = NumCliente;

  SET  For_CaptaTradi := IFNULL(For_CaptaTradi, Cadena_Vacia);
  SET  Des_CaptaTradi := IFNULL(Des_CaptaTradi, Cadena_Vacia);

  SELECT CuentaContable, Desplegado INTO For_PresBanca, Des_PresBanca
   FROM CONCEPESTADOSFIN
    WHERE EstadoFinanID = Tif_Balance
      AND ConceptoFinanID = Con_PresBanca
      AND NumClien = NumCliente;

  SET  For_PresBanca := IFNULL(For_PresBanca, Cadena_Vacia);
  SET  Des_PresBanca := IFNULL(Des_PresBanca, Cadena_Vacia);



  SELECT CuentaContable, Desplegado INTO For_TotPasCapital, Des_TotPasCapital
   FROM CONCEPESTADOSFIN
    WHERE EstadoFinanID = Tif_Balance
      AND ConceptoFinanID = Con_TotPasCapital
      AND NumClien = NumCliente;

  SET  For_TotPasCapital := IFNULL(For_TotPasCapital, Cadena_Vacia);
  SET  Des_TotPasCapital := IFNULL(Des_TotPasCapital, Cadena_Vacia);
  ...
  SELECT CuentaContable, Desplegado INTO For_TotCapitalConta, Des_TotCapitalConta
   FROM CONCEPESTADOSFIN
    WHERE EstadoFinanID = Tif_Balance
      AND ConceptoFinanID = Con_TotCapitalConta
      AND NumClien = NumCliente;

  SET  For_TotCapitalConta := IFNULL(For_TotCapitalConta, Cadena_Vacia);
  SET  Des_TotCapitalConta := IFNULL(Des_TotCapitalConta, Cadena_Vacia);
  ...
  IF (Par_TipoConsulta = Por_Fecha) THEN
    ...
    IF (Var_FechaSaldos = Fecha_Vacia) THEN
     ...
     IF(For_TotCartCredi != Cadena_Vacia) THEN
      CALL `EVALFORMULACONTAPRO`(
        For_TotCartCredi,   Ubi_Actual,   Por_Fecha,      Par_Fecha,      Var_UltEjercicioCie,
        Var_UltPeriodoCie,  Par_Fecha,    Par_AcumuladoCta, Par_CCInicial,    Par_CCFinal,
        Par_EmpresaID,    Aud_Usuario,  Aud_FechaActual,  Aud_DireccionIP,  Aud_ProgramaID,
        Aud_Sucursal,     Aud_NumTransaccion);
       ...
       INSERT INTO TMPBALANZACONTA	(
				...
				CuentaMayor,					CentroCosto)
		VALUES(
				Aud_NumTransaccion, "TotCarteCredito", Cadena_Vacia,   Entero_Cero, Entero_Cero,
				Entero_Cero,     Entero_Cero, Par_AcumuladoCta, Entero_Cero, Des_TotCartCredi,
						Cadena_Vacia, Cadena_Vacia);

     END IF;

    SET  Par_AcumuladoCta:=Entero_Cero;
    IF(For_TotCartNeto != Cadena_Vacia) THEN
     CALL `EVALFORMULACONTAPRO`(
        For_TotCartNeto, Ubi_Actual,    Por_Fecha, Par_Fecha,   Var_UltEjercicioCie,
        Var_UltPeriodoCie, Par_Fecha,   Par_AcumuladoCta, Par_CCInicial,    Par_CCFinal,
        Par_EmpresaID,    Aud_Usuario,  Aud_FechaActual, Aud_DireccionIP, Aud_ProgramaID,
        Aud_Sucursal,   Aud_NumTransaccion);
      ...
      INSERT INTO TMPBALANZACONTA	(
				...
				CuentaMayor,					CentroCosto)
		VALUES(
				Aud_NumTransaccion, "TotCarteCreditoNeto",  Cadena_Vacia,     Entero_Cero, Entero_Cero,
				Entero_Cero,    Entero_Cero,      Par_AcumuladoCta,   Entero_Cero, Des_TotCartNeto,
				Cadena_Vacia,     Cadena_Vacia);
    ...
    SET  Par_AcumuladoCta:=Entero_Cero;
      IF(For_CaptaTradi != Cadena_Vacia) THEN
      CALL `EVALFORMULACONTAPRO`(
        For_CaptaTradi,   Ubi_Actual,   Por_Fecha,      Par_Fecha,  Var_UltEjercicioCie,
        Var_UltPeriodoCie,  Par_Fecha,    Par_AcumuladoCta, Par_CCInicial,    Par_CCFinal,
        Par_EmpresaID,    Aud_Usuario,  Aud_FechaActual,  Aud_DireccionIP,  Aud_ProgramaID,
        Aud_Sucursal,     Aud_NumTransaccion);
      ...
      INSERT INTO TMPBALANZACONTA	(
				...
				CuentaMayor,					CentroCosto)
		VALUES(
				Aud_NumTransaccion, "TotalCaptacionTra", Cadena_Vacia,    Entero_Cero, Entero_Cero,
				Entero_Cero,    Entero_Cero,     Par_AcumuladoCta,  Entero_Cero, Des_CaptaTradi,
				Cadena_Vacia,     Cadena_Vacia);
     END IF;

    SET  Par_AcumuladoCta:=Entero_Cero;
      IF(For_PresBanca != Cadena_Vacia) THEN
      CALL `EVALFORMULACONTAPRO`(
        For_PresBanca,    Ubi_Actual,   Por_Fecha,      Par_Fecha,      Var_UltEjercicioCie,
        Var_UltPeriodoCie,  Par_Fecha,    Par_AcumuladoCta, Par_CCInicial,    Par_CCFinal,
        Par_EmpresaID,    Aud_Usuario,  Aud_FechaActual,  Aud_DireccionIP,  Aud_ProgramaID,
        Aud_Sucursal,     Aud_NumTransaccion);
      ...
      INSERT INTO TMPBALANZACONTA	(
				...
				CuentaMayor,					CentroCosto)
		VALUES(
				Aud_NumTransaccion, "PasPrestamosBanca",  Cadena_Vacia,     Entero_Cero, Entero_Cero,
				Entero_Cero,    Entero_Cero,      Par_AcumuladoCta,   Entero_Cero, Des_PresBanca,
				Cadena_Vacia,     Cadena_Vacia);

     END IF;
      SET  Par_AcumuladoCta:=Entero_Cero;
      IF(For_TotPasCapital != Cadena_Vacia) THEN
      CALL `EVALFORMULACONTAPRO`(
        For_TotPasCapital,  Ubi_Actual,   Por_Fecha,      Par_Fecha,      Var_UltEjercicioCie,
        Var_UltPeriodoCie,  Par_Fecha,    Par_AcumuladoCta, Par_CCInicial,    Par_CCFinal,
        Par_EmpresaID,    Aud_Usuario,  Aud_FechaActual,  Aud_DireccionIP,  Aud_ProgramaID,
        Aud_Sucursal,     Aud_NumTransaccion);
      ...
      INSERT INTO TMPBALANZACONTA	(
				...
				CuentaMayor,					CentroCosto)
		VALUES(
				Aud_NumTransaccion, "TotPasivCapitConta",   Cadena_Vacia,     Entero_Cero, Entero_Cero,
				Entero_Cero,    Entero_Cero,      Par_AcumuladoCta,   Entero_Cero, Des_TotPasCapital,
				Cadena_Vacia,     Cadena_Vacia);
     ...
     SET  Par_AcumuladoCta:=Entero_Cero;
      ...
      INSERT INTO TMPBALANZACONTA	(
		...
		VALUES(
				...
				Entero_Cero,    Entero_Cero,     Par_AcumuladoCta,
				Entero_Cero, Des_CapSocial,
				Cadena_Vacia,     Cadena_Vacia);
     END IF;

      SET  Par_AcumuladoCta:=Entero_Cero;
        IF(For_TotCapitalConta != Cadena_Vacia) THEN
        CALL `EVALFORMULACONTAPRO`(
          For_TotCapitalConta,    Ubi_Actual,   Por_Fecha,      Par_Fecha,    Var_UltEjercicioCie,
          Var_UltPeriodoCie,  Par_Fecha,    Par_AcumuladoCta, Par_CCInicial,    Par_CCFinal,
          Par_EmpresaID,    Aud_Usuario,  Aud_FechaActual,  Aud_DireccionIP,  Aud_ProgramaID,
          Aud_Sucursal,     Aud_NumTransaccion);
        ...
        INSERT INTO TMPBALANZACONTA	(
				...
				CuentaMayor,					CentroCosto)
		VALUES(
				Aud_NumTransaccion, "TotCapitalConta",  Cadena_Vacia,     Entero_Cero, Entero_Cero,
				Entero_Cero,    Entero_Cero,     Par_AcumuladoCta,  Entero_Cero, Des_TotCapitalConta,
				Cadena_Vacia,     Cadena_Vacia);
       END IF;
    ELSE
  ...
  IF(For_TotCartCredi != Cadena_Vacia) THEN
    CALL `EVALFORMULACONTAPRO`(
      For_TotCartCredi,   Var_Ubicacion,    Por_Fecha,      Par_Fecha,      Var_UltEjercicioCie,
      Var_UltPeriodoCie,  Var_FecIniPer,    Par_AcumuladoCta, Par_CCInicial,    Par_CCFinal,
      Par_EmpresaID,    Aud_Usuario,    Aud_FechaActual,  Aud_DireccionIP,  Aud_ProgramaID,
      Aud_Sucursal,     Aud_NumTransaccion);
  ...
  IF(For_TotCartNeto != Cadena_Vacia) THEN
    CALL `EVALFORMULACONTAPRO`(
      For_TotCartNeto,Var_Ubicacion,    Por_Fecha, Par_Fecha,   Var_UltEjercicioCie,
      Var_UltPeriodoCie, Var_FecIniPer,   Par_AcumuladoCta, Par_CCInicial,    Par_CCFinal,
      Par_EmpresaID,    Aud_Usuario,  Aud_FechaActual, Aud_DireccionIP, Aud_ProgramaID,
      Aud_Sucursal,   Aud_NumTransaccion);
  ...
  IF(For_CaptaTradi != Cadena_Vacia) THEN
    CALL `EVALFORMULACONTAPRO`(
      For_CaptaTradi,   Var_Ubicacion,    Por_Fecha,      Par_Fecha,      Var_UltEjercicioCie,
      Var_UltPeriodoCie,  Var_FecIniPer,    Par_AcumuladoCta, Par_CCInicial,    Par_CCFinal,
      Par_EmpresaID,    Aud_Usuario,    Aud_FechaActual,  Aud_DireccionIP,  Aud_ProgramaID,
      Aud_Sucursal,     Aud_NumTransaccion);
  ...
  IF(For_PresBanca != Cadena_Vacia) THEN
    CALL `EVALFORMULACONTAPRO`(
    For_PresBanca,    Var_Ubicacion,    Por_Fecha,      Par_Fecha,      Var_UltEjercicioCie,
    Var_UltPeriodoCie,  Var_FecIniPer,    Par_AcumuladoCta, Par_CCInicial,    Par_CCFinal,
    Par_EmpresaID,    Aud_Usuario,    Aud_FechaActual,  Aud_DireccionIP,  Aud_ProgramaID,
    Aud_Sucursal,     Aud_NumTransaccion);
  ...
  IF(For_TotPasCapital != Cadena_Vacia) THEN
    CALL `EVALFORMULACONTAPRO`(
    For_TotPasCapital,  Var_Ubicacion,    Por_Fecha,      Par_Fecha,      Var_UltEjercicioCie,
    Var_UltPeriodoCie,  Var_FecIniPer,    Par_AcumuladoCta, Par_CCInicial,    Par_CCFinal,
    Par_EmpresaID,    Aud_Usuario,    Aud_FechaActual,  Aud_DireccionIP,  Aud_ProgramaID,
    Aud_Sucursal,     Aud_NumTransaccion);
    ...
    IF(For_TotCapitalConta != Cadena_Vacia) THEN
      CALL `EVALFORMULACONTAPRO`(
        For_TotCapitalConta,  Var_Ubicacion,    Por_Fecha,      Par_Fecha,      Var_UltEjercicioCie,
        Var_UltPeriodoCie,  Var_FecIniPer,    Par_AcumuladoCta, Par_CCInicial,    Par_CCFinal,
        Par_EmpresaID,    Aud_Usuario,    Aud_FechaActual,  Aud_DireccionIP,  Aud_ProgramaID,
        Aud_Sucursal,     Aud_NumTransaccion);
