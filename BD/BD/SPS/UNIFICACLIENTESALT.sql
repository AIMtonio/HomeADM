-- UNIFICACLIENTESALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `UNIFICACLIENTESALT`;

DELIMITER $$

CREATE PROCEDURE `UNIFICACLIENTESALT`(
	-- === SP para realizar alta de clientes unificados =====
	...
	Par_DirDescripcion			VARCHAR(500),	-- DESCRIPCION de la Direccion
	...
	Par_Clasificacion			CHAR(1),		-- Clasificacion del Cliente
	Par_MotivoApertura			CHAR(1),		-- Motivo de apertura de cuenta
	...
	Par_Nacionalidad			VARCHAR(4),		-- Nacionalidad del Cliente
	...
	INOUT	Par_ClienteID		INT(11),		-- ClienteID en caso de Unificar
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION
			BEGIN
				...
				SET Par_ErrMen = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									'Disculpe las molestias que esto le ocasiona. Ref: SP-UNIFICACLIENTESALT');
			END;
			...
			IF NOT EXISTS(SELECT EstadoID FROM ESTADOSREPUB
				...
				SET Par_NumErr	:= 072;
				SET Par_ErrMen  := 'El El Valor Indicado para la Estado No Existe.';
				LEAVE ManejoErrores;
			END IF;
			...
			IF IFNULL(Par_ClienteID, Entero_Cero) != Entero_Cero THEN
				IF EXISTS(SELECT ClienteID
					...
					SET Par_NumErr := 096;
					SET Par_ErrMen := 'El Numero de Cliente ya Existe.';
					LEAVE ManejoErrores;
				ELSE
			...
			-- LLamada a CLIENTESALT para registrar un Cliente.
			CALL CLIENTESALT(
				...
				Par_OcupacionID,			Par_Puesto,					Par_LugardeTrabajo,			Par_AntiguedadTra,		Par_DomicilioTrabajo,
				Par_TelTrabajo,				Par_Clasificacion,			Par_MotivoApertura,  		Par_PagaISR,			Par_PagaIVA,
				Par_PagaIDE,				Par_NiveldeRiesgo,			Par_SectorGeneral,			Par_ActividadBancoMX,	Par_ActINEGI,
				Par_SecEconomic,			Par_ActFR,					Par_ActFOMUR,				Par_PromotorInicial,	Par_PromotorActual,
				Entero_Cero,				Par_EsMenorEdad,			Par_CorpRelacionado,		Par_RegistroHacienda,	Entero_Cero,
				Entero_Cero,				Par_Observaciones,			Par_NoEmpleado,				Par_TipoEmpleado,		Par_ExtTelefonoPart,
			...
			CALL DIRECCLIENTEALT(
				 ...
				 Par_ColoniaID,    	Var_NomColonia,				Par_Calle,				Par_Numero,    			Cadena_Vacia,
				 Par_Piso,     		Par_PrimECalle,				Par_SegECalle,			Par_CP,	 				Par_DirDescripcion,
				 Par_Latitud,		Par_Longitud,				Par_Oficial,			Par_Fiscal,		 		Par_EmpresaID,
				 Cadena_Vacia,     	Cadena_Vacia,				Cadena_Vacia,           SalidaNO,				Var_PaisIDDom,
				 Var_AniosRes,		Var_NumErr,					Var_MenErr,        		Aud_Usuario,            Aud_FechaActual,
