...
-- ASIGCREFONANTPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `ASIGCREFONANTPRO`;DELIMITER $$

CREATE PROCEDURE `ASIGCREFONANTPRO`(
...
(SELECT Cf.FormaSeleccion,Cr.CreditoID,Cl.NombreCompleto,Cr.FechaInicio,Cr.FechaVencimiento,Cr.MontoCredito,(Cr.SalCapVigente+Cr.SalCapAtrasado+Cr.SalCapVencido+Cr.SalCapVenNoExi) AS SaldoCapital,
       CASE Cl.TipoPersona WHEN 'F' THEN 'FISICA'
            WHEN 'M' THEN 'MORAL'
            WHEN 'A' THEN 'FISICA ACT. EMP'
            END	AS TIPO, Pr.Descripcion,Dir.DireccionCompleta,Cr.DiasAtraso
...
IF(Par_TipoConsulta = Principal) THEN
    ...
    CALL TMPCREDFONASIGALT(Par_LineaFondeoID,Par_FechaAsig,Var_FecInCred,Var_Sexo,Var_DesCre,Var_ActBmx,Var_Clasificacion,Foranea);
...
GROUP BY TmpCre.CreditoID,		TmpCre.FormaSeleccion,	TmpCre.NombreCliente,	TmpCre.FechaInicio,		TmpCre.FechaVencim,
		 ...
		 Cl.ClienteID,			Cl.ActividadBancoMx,	Act.Descripcion,		Cl.Sexo, 				Cl.EstadoCivil,
         Cr.DestinoCreID,		Dest.Descripcion,		TmpCre.DiasMora
