...
-- ---------------------------------------------------------------------------------
-- REGULATORIOD0842003MOD
DELIMITER ;
DROP PROCEDURE IF EXISTS `REGULATORIOD0842003MOD`;DELIMITER $$

CREATE PROCEDURE `REGULATORIOD0842003MOD`(
/*====================================================================================
  ...
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
   BEGIN
      ...
      SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al
                    concretar la operacion.Disculpe las molestias que ', 'esto le ocasiona. Ref: SP-REGULATORIOD0842003MOD');
      SET Var_Control := 'SQLEXCEPTION';
    END;

    SELECT Consecutivo INTO Var_Consecutivo
		FROM REGULATORIOD0842003
			WHERE Consecutivo = Par_IdentificadorID
				AND Anio = Par_Anio
                AND	Mes  = Par_Mes;

     IF (Var_Consecutivo != Par_IdentificadorID)THEN
      SET Par_NumErr  := 01;
      SET Par_ErrMen  := 'El Registro Seleccionado No Existe.';
    ...
    UPDATE REGULATORIOD0842003 SET
	NumeroIden			=	Par_NumeroIden	,
    TipoPrestamista		=	Par_TipoPrestamista,
    PaisEntidadExt		=	Par_PaisEntidadExt,
    NumeroCuenta		=	Par_NumeroCuenta,
    NumeroContrato		=   Par_NumeroContrato,
    ClasificaConta		=	Par_ClasificaConta,
    FechaContra			=   Par_FechaContra,
    FechaVencim			=   Par_FechaVencim,
    PlazoVencimiento	=	Par_PlazoVencin,
	PeriodoPago			=	Par_PeriodoPago,
    MontoInicial		= 	Par_MontoInicial,
    MontoInicialMNX		=	Par_MontoInicialMNX,
    TipoTasa			=	Par_TipoTasa,
    ValorTasa			=	Par_ValorTasa,
    ValorTasaInt		=	Par_ValorTasaInt,
    TasaIntReferencia	=	Par_TasaIntRef,
    DifTasaReferencia	=	Par_DifTasaRefere,
    OperaDifTasaRefe	=	Par_OperaDifTasaRef,
    FrecRevTasa			=	Par_FrecRevTasa,
    TipoMoneda			= 	Par_TipoMoneda,
    PorcentajeComision	=	Par_PorcentajeCom,
    ImporteComision		=	Par_ImporteComision,
    PeriodoComision		=	Par_PeriodoComision,
    TipoDispCredito		=	Par_TipoDispCredito,
    DestinoCredito		=	Par_DestinoCredito,
    ClasificaCortLarg	=	Par_ClasificaCortLarg,
    SaldoIniPeriodo		=	Par_SaldoIniPeriodo,
    PagosPeriodo		=	Par_PagosPeriodo,
    ComPagadasPeriodo	=	Par_ComPagadasPeriodo,
    InteresPagado		=	Par_InteresPagado,
    InteresDevengados	=	Par_InteresDevengado,
    SaldoCierre			=	Par_SaldoCierre,
    PorcentajeLinRev	=	Par_PorcentajeLinRev,
    FechaUltPago		=	Par_FechaUltPago,
    PagoAnticipado		=	Par_PagoAnticipado,
    MontoUltimoPago		=	Par_MontoUltimoPago,
    FechaPagoSig		=	Par_FechaPagoSig,
    MontoPagImediato	=	Par_MonImediato,
    TipoGarantia		=	Par_TipoGarantia,
    MontoGarantia		=	Par_MontoGarantia,
    FechaValuaGaran		=	Par_FechaValuaGaran,
    ...
    SET Par_ErrMen  := CONCAT('Registro Modificado Correctamente: ',Par_IdentificadorID);
    SET Var_Control := 'identificadorID';
    SET Var_Consecutivo	:= Par_IdentificadorID;
