...
-- ---------------------------------------------------------------------------------
-- PARAMCREDITPAYMENMOD
DELIMITER ;
DROP PROCEDURE IF EXISTS PARAMCREDITPAYMENMOD;
DELIMITER $$

CREATE PROCEDURE PARAMCREDITPAYMENMOD(
	-- SP PARA MODIFICAR LOS PARAMETROS DE COBRO DE WS CREDITO PAYMENT
	...
	SET Par_ParamCreditPaymentID	:= IFNULL(Par_ParamCreditPaymentID,Entero_Cero);
	...
	ManejoErrores:BEGIN
		...
		BEGIN
			...
			SET Par_ErrMen = CONCAT('El SAFI ha tenido un problema al concretar la operacion. Disculpe las molestias que ',
							'esto le ocasiona. Ref: SP-PARAMCREDITPAYMENMOD');
			SET Var_Control = 'SQLEXCEPTION';
			SET Var_Consecutivo	:= Entero_Cero;
		END;

		IF(Par_ParamCreditPaymentID = Entero_Cero) THEN
			SET Par_NumErr		:= 1;
			SET Par_ErrMen		:= 'El Numero de Parametro Credito Payment Encuentra Vacio.';
			SET Var_Control		:= 'paramCreditPaymentID';
			LEAVE ManejoErrores;
		END IF;

		IF NOT EXISTS(SELECT ParamCreditPaymentID FROM PARAMCREDITPAYMENT WHERE ParamCreditPaymentID = Par_ParamCreditPaymentID) THEN
			SET Par_NumErr		:= 2;
			SET Par_ErrMen		:= 'El Numero de Parametro Credito Payment No existe.';
			SET Var_Control		:= 'paramCreditPaymentID';
			LEAVE ManejoErrores;
		END IF;
		...
		UPDATE PARAMCREDITPAYMENT
			SET	ProducCreditoID			=	Par_ProducCreditoID,
		...
		WHERE ParamCreditPaymentID = Par_ParamCreditPaymentID;
