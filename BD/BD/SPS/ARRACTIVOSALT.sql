...
-- ARRACTIVOSALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `ARRACTIVOSALT`;DELIMITER $$

CREATE PROCEDURE `ARRACTIVOSALT`(
# =====================================================================================
# -- STORED PROCEDURE PARA DAR DE ALTA LOS ACTIVOS.
# =====================================================================================
	...
	INOUT Par_ConsecutivoAc INT(11),            -- Parametro de salida que corresponde al consecutivo del registro agregado.
	...
	DECLARE Var_FolioActivo			BIGINT;	        -- Variable que tiene el folio generado para el activo
	...
	DECLARE Par_AseguradoNo			CHAR(1);		-- Indica que el activo no esta asegurado.
	...
	SET Par_AseguradoNo			:= 'N';      		-- No esta asegurado el activo N = no
	...
	ManejoErrores:BEGIN
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen  = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									 'Disculpe las molestias que esto le ocasiona. REF: SP-ARRACTIVOSALT');
			SET Var_Control = 'SQLEXCEPTION';
		END;

		SET Par_ConsecutivoAc	:= Entero_Cero;
		...
		-- se genera el folio
		CALL FOLIOSAPLICAACT('ARRACTIVOS', Var_FolioActivo);

		-- insert en la tabla: ARRACTIVOS
		INSERT INTO ARRACTIVOS (ActivoID,	            Descripcion,            TipoActivo,             SubtipoActivoID,        Modelo,
								MarcaID,                NumeroSerie,            NumeroFactura,          ValorFactura,           CostosAdicionales,
								FechaAdquisicion,       VidaUtil,               PorcentDepreFis,        PorcentDepreAjus,       PlazoMaximo,
								PorcentResidMax,        Estatus,                EstadoID,               MunicipioID,            LocalidadID,
								ColoniaID,              Calle,                  NumeroCasa,             NumeroInterior,         Piso,
								PrimerEntrecalle,       SegundaEntreCalle,      CP,                     Latitud,                Longitud,
								Lote,                   Manzana,                DescripcionDom,         AseguradoraID,          EstaAsegurado,
								NumPolizaSeguro,        FechaAdquiSeguro,       InicioCoberSeguro,      FinCoberSeguro,         SumaAseguradora,
								ValorDeduciSeguro,      Observaciones,          DireccionCompleta,      EmpresaID,              Usuario,
								FechaActual,            DireccionIP,            ProgramaID,             Sucursal,               NumTransaccion)

						VALUES (Var_FolioActivo,        Par_Descripcion,        Par_TipoActivo,         Par_SubtipoActivoID,    Par_Modelo,
								Par_MarcaID,            Par_NumeroSerie,        Par_NumeroFactura,      Par_ValorFactura,       Par_CostosAdicionales,
								Par_FechaAdquisicion,   Par_VidaUtil,           Par_PorcentDepreFis,    Par_PorcentDepreAjus,   Par_PlazoMaximo,
								Par_PorcentResidMax,    Par_Estatus,            Par_EstadoID,           Par_MunicipioID,        Par_LocalidadID,
								Par_ColoniaID,          Par_Calle,              Par_NumeroCasa,         Par_NumeroInterior,     Par_Piso,
								Par_PrimerEntrecalle,   Par_SegundaEntreCalle,  Par_CP,                 Par_Latitud,            Par_Longitud,
								Par_Lote,               Par_Manzana,            Par_DescripcionDom,     Par_AseguradoraID,      Par_EstaAsegurado,
								Par_NumPolizaSeguro,    Par_FechaAdquiSeguro,   Par_InicioCoberSeguro,  Par_FinCoberSeguro,     Par_SumaAseguradora,
								Par_ValorDeduciSeguro,  Par_Observaciones,      Var_DirecCompleta,      Par_EmpresaID,          Aud_Usuario,
								Aud_FechaActual,        Aud_DireccionIP,        Aud_ProgramaID,         Aud_Sucursal,           Aud_NumTransaccion);

        SET Par_NumErr          := 000;
        SET Par_ErrMen          := CONCAT('Activo agregado exitosamente: ',Var_FolioActivo);
        SET Var_Control         := 'tipoActivo';
        SET Par_ConsecutivoAc   := Var_FolioActivo;
	END ManejoErrores;
	...
	IF (Par_Salida = Salida_Si) THEN
		SELECT  Par_NumErr AS NumErr,
				...
				Par_ConsecutivoAc AS consecutivo;
	END IF;
