...
CREATE PROCEDURE `TIPOTARJETADEBITOALT`(
  ...
  Par_NumeroTipoTarjeta   INT(11),    -- llave primaria de la tabla
    ...
    Par_TipoMaquilador    INT(11),        -- Tipo de maquilador 1-ISS, 2-TGS
...
DECLARE MaquilTGS   INT(11);      -- Maquilador TGS
...
ManejoErrores:BEGIN
    ...
    IF(IFNULL(Par_TipoMaquilador,Entero_Cero)) = Entero_Cero THEN
      SET Par_NumErr  := 1;
      SET Par_ErrMen  := 'Especifique el tipo de maquilador';
  ...
  IF(Par_TipoMaquilador = MaquilTGS)THEN
    ...
    IF(Par_TipoCore NOT IN (1,2,3)) THEN
        SET Par_NumErr  := 21;
        SET Par_ErrMen  := 'Especifique un tipo de core valido 1-Externo, 2-SAFI Externo o 3-SAFI Interno';
  ...
  IF (Par_TipoTran = TranTipoTarjeta) THEN
    ...
    INSERT INTO TIPOTARJETADEB (
            ...
            NumTransaccion)
    VALUES(
            Par_NumeroTipoTarjeta,    Par_Descripcion,    Par_CompraPOS,        Par_Estatus,        Par_IdentificaSocio,
      ...
      SET Par_NumErr  := 000;
      SET Par_ErrMen  := CONCAT('Tipo de Tarjeta de Debito Agregado Exitosamente: ', Par_NumeroTipoTarjeta);
      SET varControl  := 'tipoTarjetaDebID' ;

      IF(Par_TipoTarjeta = TipoCred) THEN
        SET Par_ErrMen  := CONCAT('Tipo de Tarjeta de Credito Agregado Exitosamente: ', Par_NumeroTipoTarjeta);
      END IF;
...
END ManejoErrores;
  ...
  IF(Par_Salida = SalidaSI) THEN
    SELECT  Par_NumErr AS NumErr,
        ...
        Par_NumeroTipoTarjeta AS consecutivo;
  END IF;
