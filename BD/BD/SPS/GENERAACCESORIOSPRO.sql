...
-- GENERAACCESORIOSPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `GENERAACCESORIOSPRO`;
DELIMITER $$

CREATE PROCEDURE `GENERAACCESORIOSPRO`(
# ======================================================================================================
...
ManejoErrores:BEGIN
    ...
    WHILE(Var_Contador <= Var_NumRegistros) DO
		...
		IF(Var_CobraAccesorios = SiCobraAccesorio AND Var_MontoAccesorio > Decimal_Cero)THEN
            ...
            SET Ref_GenAccesorios := CONCAT('OTRAS COMISIONES ' , Var_AbrevAccesorio);
			...
			CALL CONTACCESORIOSCREDPRO (
					...
					Cadena_Vacia, 			Cadena_Vacia,				Par_Salida,				Par_NumErr,				Par_ErrMen,
		...
		IF(Var_MontoIva > Decimal_Cero) THEN
         ...
         SET Ref_GenIVAAccesorios := CONCAT('IVA OTRAS COMISIONES ' , Var_AbrevAccesorio);
        ...
        SET Var_CreditoStr = CONCAT(CONVERT(Var_CreditoID, CHAR)) ;
			...
			IF Error_Key = 1 THEN
				...
				START TRANSACTION;
					CALL EXCEPCIONBATCHALT(
						Pro_CobAccesorios,	Par_Fecha,			Var_CreditoStr,		Des_ErrorGral,		Par_EmpresaID,
						Aud_Usuario,		Aud_FechaActual,	Aud_DireccionIP,	Aud_ProgramaID,		Aud_Sucursal,
						Aud_NumTransaccion);
				COMMIT;
			END IF;
			IF Error_Key = 2 THEN
				...
				START TRANSACTION;
					CALL EXCEPCIONBATCHALT(
						Pro_CobAccesorios,	Par_Fecha,			Var_CreditoStr,		Des_ErrorLlavDup,	Par_EmpresaID,
						Aud_Usuario,		Aud_FechaActual,	Aud_DireccionIP,	Aud_ProgramaID,		Aud_Sucursal,
						Aud_NumTransaccion);
				COMMIT;
			END IF;
			IF Error_Key = 3 THEN
				...
				START TRANSACTION;
					CALL EXCEPCIONBATCHALT(
						Pro_CobAccesorios,	Par_Fecha,			Var_CreditoStr,		Des_ErrorCallSP,	Par_EmpresaID,
						Aud_Usuario,		Aud_FechaActual,	Aud_DireccionIP,	Aud_ProgramaID,		Aud_Sucursal,
						Aud_NumTransaccion);
				COMMIT;
			END IF;
			IF Error_Key = 4 THEN
				...
				START TRANSACTION;
					CALL EXCEPCIONBATCHALT(
						Pro_CobAccesorios,	Par_Fecha,			Var_CreditoStr,		Des_ErrorValNulos,	Par_EmpresaID,
						Aud_Usuario,		Aud_FechaActual,	Aud_DireccionIP,	Aud_ProgramaID,		Aud_Sucursal,
						Aud_NumTransaccion);
				COMMIT;
			END IF;
            IF Error_Key = 99 THEN
				...
				START TRANSACTION;
					CALL EXCEPCIONBATCHALT(
						Pro_CobAccesorios,	Par_Fecha,			Var_CreditoStr,		UPPER(Par_ErrMen),	Par_EmpresaID,
						Aud_Usuario,		Aud_FechaActual,	Aud_DireccionIP,	Aud_ProgramaID,		Aud_Sucursal,
						Aud_NumTransaccion);
				COMMIT;
			END IF;
