...
-- CINTAINTFMENSUALSUSP000PRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `CINTAINTFMENSUALSUSP000PRO`;
DELIMITER $$

CREATE PROCEDURE `CINTAINTFMENSUALSUSP000PRO`(
...
TerminaStore: BEGIN
  ...
  -- se agrego esta actualización por que no puede tener un mop diferente de 1
  UPDATE TMPDATOSCTESCREDPAG Dat
    SET Dat.DiasAtraso = Entero_Cero
  ...
  WHERE ApellidoPaterno = 'X' or ApellidoPaterno = '';
  ...
  SET Var_FechaCorteStr :=  DATE_FORMAT(Var_FechaCorte,FormatoFecha) ;
    ...
    Cinta = CONCAT_WS(Separador,
      ...
      IFNULL(Dat.RFC,Cadena_Vacia),         IFNULL(Dat.Prefijo,Cadena_Vacia),       IFNULL(Dat.Nacionalidad,Cadena_Vacia),      IFNULL(Dat.EstadoCivil,Soltero_BC),
            IFNULL(Dat.Sexo,Cadena_Vacia),          Dat.FechaDefuncion,   Dat.IndicadorDefuncion,       IFNULL(Dat.CalleNumero,Cadena_Vacia),     IFNULL(Dat.DirComplemento,Cadena_Vacia),
