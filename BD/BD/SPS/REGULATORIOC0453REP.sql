...
-- REGULATORIOC0453REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `REGULATORIOC0453REP`;DELIMITER $$

CREATE PROCEDURE `REGULATORIOC0453REP`(
	/* Genera la llamada al regulatorio C0453 para Sofipo */
...
-- Variables
	...
	SET NumReporte			:= 'C0453';					-- Clave del reporte
	SET Entero_Cero			:= 0;
	SET Cadena_Vacia 		:= '';
    ...
    SET Var_Fecha := CONCAT(Par_Anio,'-',CASE WHEN Par_Mes < 10 THEN Entero_Cero ELSE Cadena_Vacia END,Par_Mes,'-01');
	...
	SET Var_Llamada := CONCAT(' CALL ',Var_NombreReporte,
		'("',Var_Fecha,'",',Par_NumRep,',',Par_EmpresaID,',',Aud_Usuario,',',"'",Aud_FechaActual,"'",',',"'",Aud_DireccionIP,"'",',',"'",Aud_ProgramaID,"'",',',Aud_Sucursal,',',Aud_NumTransaccion,');');
