...
-- TD_BLOQUEOSALT
DELIMITER ;
DROP PROCEDURE IF EXISTS `TD_BLOQUEOSALT`;

DELIMITER $$
CREATE PROCEDURE `TD_BLOQUEOSALT`(
	-- SP para el bloqueo automatico de saldo de una tarjeta de debito
	...
	Par_TarjetaDebID		BIGINT(12),		-- Indica el numero de la tarjeta de debito
...
TerminaStore: BEGIN
	...
	DECLARE Var_Estatus				CHAR(1);		-- Estatus de Linea de la cuenta de ahorro
	...
	DECLARE Var_CtaTarDebID			BIGINT(12);		-- Cuenta de Ahorro
	...
	ManejoErrores:BEGIN
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen	= CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									'Disculpe las molestias que esto le ocasiona. Ref: SP-TD_BLOQUEOSALT');
		...
		IF( Par_TarjetaDebID = Cadena_Vacia )THEN
			SET Par_NumErr	:= 1404;
			SET Par_ErrMen	:= 'El Numero de Debito esta vacio.';
			SET Var_Control	:= 'tarjetaDebID';
			LEAVE ManejoErrores;
		END IF;

		IF( CHAR_LENGTH(Par_TarjetaDebID) != LongitudTarjeta ) THEN
			SET Par_NumErr	:= 1404;
		...
		SELECT CuentaAhoID, 		TarjetaDebID
		INTO 	Var_CtaTarDebID,	Var_TarjetaDebID
		FROM TARJETADEBITO 
		...
		IF( Var_TarjetaDebID = Cadena_Vacia ) THEN
			SET Par_NumErr	:= 1408;
			SET Par_ErrMen	:= CONCAT('El Numero de Tarjeta de Debito no Existe: ',Par_TarjetaDebID);
			SET Var_Control	:= 'tarjetaDebitoID';
			LEAVE ManejoErrores;
		END IF;
		...
		IF( Var_EstatusTarjeta <> Con_TarjetaActiva ) THEN
			SET Par_NumErr	:= 1409;
			SET Par_ErrMen	:= CONCAT('La Tarjeta de Debito: ',Par_TarjetaDebID, ' No esta Activa');
			SET Var_Control	:= 'tarjetaDebitoID';
			LEAVE ManejoErrores;
		END IF;

		IF( Var_CtaTarDebID = Entero_Cero ) THEN
			SET Par_NumErr	:= 1210;
			SET Par_ErrMen	:= CONCAT('La Tarjeta de Debito: ',Par_TarjetaDebID,' no tiene la cuenta de ahorro.');
			SET Var_Control	:= 'tarjetaDebitoID';
			LEAVE ManejoErrores;
		END IF;

		SELECT Estatus,		SaldoDispon,			SaldoBloq
		INTO  Var_Estatus,	Var_MontoDisponible,	Var_MontoBloqueado
		FROM CUENTASAHO
			WHERE CuentaAhoID = Var_CtaTarDebID;


		IF( Var_Estatus != Est_Activa ) THEN
			SET Par_NumErr	:= 1409;
			SET Par_ErrMen	:= 'La Cuenta de la Tarjeta de Debito no esta activa.';
			SET Var_Control	:= 'tarjetaDebitoID';
			LEAVE ManejoErrores;
		END IF;
		...
		IF( Par_NatMovimiento = Nat_Bloqueo ) THEN
			...
			IF( Par_MontoBloqueo > Var_MontoDisponible) THEN
				SET Par_NumErr	:= 1313;
				SET Par_ErrMen	:= CONCAT("Saldo a bloquear es mayor al disponible. Tarjeta de Debito: ", Par_TarjetaDebID, ".");
				SET Var_Control	:= 'BloqueoID';
				LEAVE ManejoErrores;
			END IF;

			-- BLOUEO DE SALDO
			CALL CUENTAAHOTARDEBACT(
				Var_CtaTarDebID,	Par_FechaMovimiento,	Par_MontoBloqueo,	Act_BloqueoSaldo,
		...
		IF( Par_NatMovimiento = Nat_Desbloqueo ) THEN
			...
			IF( Var_MontoBloqueo > Var_MontoBloqueado ) THEN
				SET Par_NumErr	:= 1317;
				SET Par_ErrMen	:= CONCAT("Saldo a desbloquear es mayor al bloqueado. Tarjeta de Debito ", Par_TarjetaDebID, ".");
				SET Var_Control	:= 'montoBloqueo';
				LEAVE ManejoErrores;
			END IF;
			...
			-- DESBLOQUEO DE SALDO
			CALL CUENTAAHOTARDEBACT(
				Var_CtaTarDebID,	Par_FechaMovimiento,	Par_MontoBloqueo,	Act_DesbloqueoSaldo,
				Salida_NO,			Par_NumErr,				Par_ErrMen,			Aud_EmpresaID,		Aud_Usuario,
				Aud_FechaActual,	Aud_DireccionIP,		Aud_ProgramaID,		Aud_Sucursal,		Aud_NumTransaccion);
		...
		SET Var_NumBloqueoID 	:= (SELECT IFNULL(MAX(BloqueoID),Entero_Cero)+1 FROM TD_BLOQUEOS FOR UPDATE);
		...
		IF( Par_BloqueoID > Entero_Cero AND
			...
			VALUES (
				Var_NumBloqueoID,		Par_TarjetaDebID,	Par_NatMovimiento,		Var_FechaMovimiento,	Var_MontoBloqueo,
		...
		IF(Par_NatMovimiento = Nat_Bloqueo)THEN
			...
			INSERT INTO TD_BLOQUEOS (
				...
				Sucursal,				NumTransaccion)
			VALUES (
				Var_NumBloqueoID,		Par_TarjetaDebID,		Par_NatMovimiento,		Var_FechaMovimiento,	Par_MontoBloqueo,
				Par_FechaDesbloqueo,	Par_TiposBloqID,		UPPER(Par_Descripcion),	Par_Referencia, 		Entero_Cero,
				Par_TerminalID,			Par_NombreTerminal,		Par_LocacionTerminal,	Par_NumAutorizacionAct,	Par_NumAutorizacionAnt,
				Aud_EmpresaID,			Aud_Usuario,			Aud_FechaActual,		Aud_DireccionIP,		Aud_ProgramaID,
				Aud_Sucursal,			Aud_NumTransaccion);
