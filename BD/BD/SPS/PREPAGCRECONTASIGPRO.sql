...
-- ---------------------------------------------------------------------------------
-- PREPAGCRECONTASIGPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `PREPAGCRECONTASIGPRO`;
DELIMITER $$

CREATE PROCEDURE `PREPAGCRECONTASIGPRO`(
/* SP DE PROCESO CONTABLE PARA EL PREPAGO CUOTAS SIGUIENTES INMEDIATAS */
	Par_CreditoID     	BIGINT(12),
	Par_CuentaContable	VARCHAR(25),
    Par_MontoPagar		DECIMAL(16,2),
    Par_MonedaID		INT(11),
    Par_EmpresaID		INT(11),

	Par_PagarIVA		CHAR(1),
INOUT Var_MontoIVAInt	DECIMAL(16,2),
    Par_Salida			CHAR(1),
    Par_AltaEncPoliza	CHAR(1),
    Par_AltaDetPoliza	CHAR(1),

	OUT Par_MontoPago	DECIMAL(16,2),
...
DECLARE Var_CreTasa         DECIMAL(16,4);
DECLARE Var_MonedaID        INT;
DECLARE Var_SubClasifID     INT;

DECLARE Var_CreditoID       INT;           			-- Variables para los Cursores
DECLARE Var_AmortizacionID  INT;
DECLARE Var_SalCapitales	DECIMAL(16,2);
DECLARE Var_SaldoCapVigente DECIMAL(16,2);
DECLARE Var_SaldoCapVenNExi DECIMAL(16,2);
DECLARE Var_FechaInicio     DATE;
DECLARE Var_FechaVencim     DATE;
DECLARE Var_FechaExigible   DATE;
DECLARE Var_AmoEstatus      CHAR(1);
DECLARE Var_SaldoInteresPro	DECIMAL(16,4);
DECLARE	Var_SaldoIntNoConta	DECIMAL(16,4);
DECLARE Var_ProvisionAcum   DECIMAL(16,4);

DECLARE Var_IVASucurs   	DECIMAL(16, 4);
DECLARE Var_ValIVAIntOr 	DECIMAL(16,2);
DECLARE Var_TotDeuda    	DECIMAL(16,2);
DECLARE Var_NumAtrasos  	INT;
DECLARE Var_EsHabil     	CHAR(1);
DECLARE Var_GrupoCtaID      INT;
DECLARE Var_CreditoStr		VARCHAR(20);
DECLARE Var_SaldoPago       DECIMAL(16,2);
DECLARE Var_CantidPagar     DECIMAL(16,2);
DECLARE Var_SaldoCapita     DECIMAL(16,2);
DECLARE Var_CalInteresID    INT;
DECLARE Var_Dias            INT;
DECLARE Var_TotCapital      DECIMAL(16,2);
DECLARE Var_Interes         DECIMAL(16,4);
...
ManejoErrores: BEGIN
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			SET Par_ErrMen := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
								'Disculpe las molestias que esto le ocasiona. Ref: SP-PREPAGCRECONTASIGPRO');
		END;
	...
	SET	Var_MontoIVAInt	:= IFNULL(Var_MontoIVAInt, Decimal_Cero);
