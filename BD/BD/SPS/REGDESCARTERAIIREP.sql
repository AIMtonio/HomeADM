...
-- REGDESCARTERAIIREP
DELIMITER ;
DROP PROCEDURE IF EXISTS `REGDESCARTERAIIREP`;
DELIMITER $$


CREATE PROCEDURE `REGDESCARTERAIIREP`(
# ============================================================================================================
...
TerminaStore: BEGIN
	...
	IF(Par_NumReporte = Rep_Excel) THEN
		SELECT	Var_Periodo,	Var_ClaveEntidad,	For_0451,
				MunicipioClave, EstadoClave,	CONVERT(Tem.ClienteID, CHAR) AS ClienteID,
				TipoPersona,	Denominacion,	ApellidoPat,	ApellidoMat,	RFC,
				CURP,			Genero,			CONVERT(Tem.CreditoID, CHAR) AS CreditoID,
				...
				TipoAmorti,	ROUND(IFNULL(Tem.MontoCredito,Entero_Cero),Par_NumDecimales) AS MontoCredito, Tem.PeriodicidadCap,
                Tem.PeriodicidadInt,
				...
				NumDiasAtraso,	Tem.TipoCredito,	SituacContable,
				...
				F.Formula
			FROM TMPREGR04C0451 Tem INNER JOIN
				CREDITOS AS CR ON Tem.CreditoID=CR.CreditoID LEFT JOIN
	...
	ELSE
		IF(Par_NumReporte = Rep_Csv) THEN
			...
			SELECT	CONCAT(
                    ...
                    CONVERT(IFNULL(Tem.ClienteID,Cadena_Vacia), CHAR),';',
                    ...
                    CONVERT(IFNULL(Tem.CreditoID,Cadena_Vacia), CHAR),';',
                    ...
                    ROUND(IFNULL(Tem.MontoCredito,Entero_Cero),Par_NumDecimales),';',
                    Tem.PeriodicidadCap,';',
					IFNULL(Tem.PeriodicidadInt,Entero_Cero),';',
                    F.Formula,';',
					...
					IFNULL(Tem.TipoCredito,Cadena_Vacia),';',
