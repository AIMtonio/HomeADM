...
-- ---------------------------------------------------------------------------------
-- CONDICIONESVENCIMAPORTMOD
DELIMITER ;
DROP PROCEDURE IF EXISTS `CONDICIONESVENCIMAPORTMOD`;
DELIMITER $$

CREATE PROCEDURE `CONDICIONESVENCIMAPORTMOD`(
# ====================================================================================
# ------ SP PARA MODIFICAR LAS CONDICIONES DE VENCIMIENTO DE UNA APORTACION---------
# ====================================================================================
	...
	Par_Condiciones 				VARCHAR(700),		-- Indica las condiciones de la Aportacion
...
TerminaStore: BEGIN
 ...
 ManejoErrores: BEGIN
		DECLARE EXIT HANDLER FOR SQLEXCEPTION
			BEGIN
				...
				SET Par_ErrMen :=	CONCAT('El SAFI ha tenido un problema al concretar la operacion. Disculpe las molestias que ',
									'esto le ocasiona. Ref: SP-CONDICIONESVENCIMAPORTMOD');
	 ...
	 IF(Par_ReinversionAutomatica = Constante_SI) THEN
		...
		IF(IFNULL(Par_MontoGlobal, Cadena_Vacia) = Cadena_Vacia) THEN
			SET Par_NumErr	:=	7;
			SET Par_ErrMen	:=	'El Monto Global esta vacio';
			SET Var_Control	:=	'montoGlobalNuevaAport';
			LEAVE ManejoErrores;
		END IF;
		...
		IF(IFNULL(Par_TasaBruta, Decimal_Cero) = Decimal_Cero) THEN
			SET Par_NumErr	:=	13;
		...
		IF(IFNULL(Par_TasaNeta, Decimal_Cero) = Decimal_Cero) THEN
			SET Par_NumErr	:=	15;
		...
		IF(IFNULL(Par_GatNominal, Decimal_Cero) = Decimal_Cero) THEN
			SET Par_NumErr	:=	17;
		...
		UPDATE CONDICIONESVENCIMAPORT SET
			ReinversionAutomatica 	= Par_ReinversionAutomatica,
			TipoReinversion 		= Par_TipoReinversion,
			OpcionAportID 			= Par_OpcionAport,
			Cantidad 				= Par_Cantidad,
			Monto 					= Par_Monto,

			MontoRenovacion 		= Par_MontoRenovacion,
			MontoGlobal 			= Par_MontoGlobal,
			TipoPago 				= Par_TipoPago,
			DiaPago 				= Par_DiaPago,
			Plazo 					= Var_Plazo,

			PlazoOriginal 			= Var_PlazoOriginal,
			FechaInicio 			= Par_FechaInicio,
			FechaVencimiento 		= Par_FechaVencimiento,
			TasaBruta 				= Par_TasaBruta,
			TasaISR 				= Par_TasaISR,

			TasaNeta 				= Par_TasaNeta,
			CapitalizaInteres 		= Par_CapitalizaInteres,
			GatNominal 				= Par_GatNominal,
			InteresGenerado 		= Par_InteresGenerado,
			ISRRetener 				= Par_ISRRetener,

			InteresRecibir 			= Par_InteresRecibir,
			TotalRecibir 			= Par_TotalRecibir,
			Notas 					= Par_Notas,
			Especificaciones 		= Par_Especificaciones,
			Estatus 				= Estatus_PorAutorizar,

			Reinversion 			= Par_Reinversion,
			GatReal 				= Par_GatReal,
			ConsolidarSaldos		= Par_ConsolidarSaldos,
			Condiciones             = Par_Condiciones,
