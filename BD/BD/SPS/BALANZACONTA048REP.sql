...
-- BALANZACONTA015REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `BALANZACONTA048REP`;

DELIMITER $$
CREATE PROCEDURE `BALANZACONTA048REP`(
	-- Store Procedure: Para generar la Balanza Contable del Cliente: NATGAS
...
TerminaStore: BEGIN
	...
	SELECT	Aud_NumTransaccion, Cue.CuentaCompleta,
			...
			CASE WHEN Cue.Grupo = Tip_Encabezado AND Var_NumSegConta = 1 AND
					  Cue.CuentaCompleta NOT IN ('649000000000', '240120000000', '505090000000','505010000000') THEN
	...
	UPDATE TMPBALCUENTACONTABLE SET
		CuentaMayor = '64900'
	WHERE CuentaCompleta = '649000000000'
	...
	FROM TMPBALANZACENCOS
	WHERE NumeroTransaccion = Aud_NumTransaccion
	GROUP BY NumeroTransaccion,CuentaContable;
	...
	IF (Par_TipoBalanza = Tipo_CenCostos) THEN
		...
		GROUP BY NumeroTransaccion,CuentaContable, CentroCosto;
