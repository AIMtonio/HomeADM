...
-- ---------------------------------------------------------------------------------
-- EDOFLUJOEFECTIVO024REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `EDOFLUJOEFECTIVO024REP`;
DELIMITER $$


CREATE PROCEDURE `EDOFLUJOEFECTIVO024REP`(
	-- Genera el reporte Contable de Flujo de Efectivo
	-- Modulo Contabilidad
...
TerminaStore: BEGIN
	...
	SELECT IFNULL(ClaveEntidad, Cadena_Vacia),	JefeContabilidad,	GerenteGeneral
	INTO   Var_ClaveEntidad,					Var_ContaGeneral,	Var_DirectGeneral
	FROM PARAMETROSSIS
	...
	IF IFNULL(Var_FechaCorte,Fecha_Vacia) != Fecha_Vacia THEN
		...
		SET Var_FechaIniEjerAct := DATE_ADD(Var_FechaCorte, INTERVAL 1 DAY);

		IF Var_FechaCorte = Par_FechaActual THEN
	...
	SET Var_ConsultaEdoFin := CONCAT(Var_ConsultaEdoFin,' "',Var_ContaGeneral,'" AS Var_ContaGeneral, "',Var_DirectGeneral,'" AS Var_DirectGeneral, 0.00 AS Var_DecimalCero," ',
