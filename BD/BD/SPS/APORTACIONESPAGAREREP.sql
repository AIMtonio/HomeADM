...
-- ---------------------------------------------------------------------------------
-- APORTACIONESPAGAREREP
DELIMITER ;
DROP PROCEDURE IF EXISTS `APORTACIONESPAGAREREP`;DELIMITER $$

CREATE PROCEDURE `APORTACIONESPAGAREREP`(
# ===================================================
# ------ SP PARA GENERAR PAGARE DE APORTACIONES------
# ===================================================
	...
	DECLARE Var_MontoAport		VARCHAR(100);
	DECLARE TipoAport_RECA		VARCHAR(100);
	...
	SET Auto_Aport            	:=  4;               -- Para Autorizar el Pagare de la Aport
	...
	CREATE TEMPORARY TABLE IF NOT EXISTS TMPPAGAREAPORTA (
	   ID INT AUTO_INCREMENT PRIMARY KEY,
	   NombreCompleto VARCHAR(200) DEFAULT NULL
	) ENGINE=InnoDB DEFAULT CHARSET=latin1;

	TRUNCATE TMPPAGAREAPORTA;
	...
	-- OBTENIENDO DATOS DE LA Aportación
	SELECT
	...
	INTO
		...
		Var_TasaFija,         Var_Reinversion,	  Var_ClienteID,	       Var_MontoAport,
		TipoAport_RECA,		  Var_DepositaPMRS
		FROM APORTACIONES AS CD
			LEFT OUTER JOIN TIPOSAPORTACIONES  AS TC  ON TC.TipoAportacionID = CD.TipoAportacionID,
			  CLIENTES AS CLI
			LEFT OUTER JOIN DIRECCLIENTE AS DIR ON DIR.ClienteID = CLI.ClienteID AND DIR.Oficial = Var_Si
		WHERE CD.ClienteID = CLI.ClienteID
			 AND CD.AportacionID = Par_AportacionID;
	...
	-- AUTORIZANDO EL PAGARE DE LA Aportación

	CALL APORTACIONESACT(
		Par_AportacionID, 	Entero_Cero,		Entero_Cero,		Auto_Aport,			SalidaNO,
		Par_NumErr,			Par_ErrMen,			Par_EmpresaID,      Aud_Usuario,		Aud_FechaActual,
		Aud_DireccionIP,	Aud_ProgramaID,		Aud_Sucursal, 	    Aud_NumTransaccion);

	SELECT
		Depositante1,		Depositante2,       Depositante3,    		  Depositante4,      Depositante5,
		Var_LugarExp,		Var_DomicilioCte,   FORMAT(Var_MontoAport,2) AS Var_MontoAport,   	 Var_PlazoLetras,   Var_TasaFijaLetras,
		Var_Reinversion,	CONVPORCANT(ROUND(Var_MontoAport, 2), '$P', '2', '') AS CapitalLetra,
