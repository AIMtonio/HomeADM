...
-- ---------------------------------------------------------------------------------
-- CALCOTRASCOMISIONESPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `CALCOTRASCOMISIONESPRO`;
DELIMITER $$


CREATE PROCEDURE `CALCOTRASCOMISIONESPRO`(
# =====================================================================================
	...
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
		BEGIN
		   ...
		   SET Par_ErrMen  := CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
					  'Disculpe las molestias que esto le ocasiona. Ref: SP-CALCOTRASCOMISIONESPRO');
		...
		IF (Var_ClienteEsp = Var_ClienteATE) THEN
			...
			IF (Var_Credito <> Entero_Cero) THEN 
				SELECT FrecuenciaCap INTO Var_PagoCuota FROM CREDITOS WHERE CreditoID = Var_Credito;
        ...
        WHILE (Contador <= Var_NumCuotas) DO
			...
			IF(Var_Credito <> Entero_Cero) THEN
				...
				UPDATE DETALLEACCESORIOS DET SET
					...
					DET.MontoIntCuota  		= 	IF (DET.CobraIVA = SalidaNO, Entero_Cero, (DET.MontoInteres - (ROUND(DET.MontoInteres/Var_NumCuotas,2) * (Var_NumCuotas - 1)))),
