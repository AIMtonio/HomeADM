...
-- ---------------------------------------------------------------------------------
-- CINTAINTFMENSUAL024PRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `CINTAINTFMENSUAL024PRO`;
DELIMITER $$

CREATE PROCEDURE `CINTAINTFMENSUAL024PRO`(
...
TerminaStore: BEGIN
  ...
  INDEX TMPDATOSCTESCREDPAG3 (CreditoIDSAFI));
	...
	CASE IFNULL(REPLACE(MAX(Cli.LugardeTrabajo),Entero_Cero,Cadena_Vacia),Cadena_Vacia)
		WHEN Cadena_Vacia THEN
			LEFT(IF(ifnull(REPLACE(MAX(Cli.LugardeTrabajo),Entero_Cero,Cadena_Vacia),Cadena_Vacia)=Cadena_Vacia,'TRABAJADOR INDEPENDIENTE',MAX(Cli.LugardeTrabajo)),40)
		WHEN 'ASALARIADO' THEN
			'NEGOCIO PROPIO'
		ELSE
			LEFT(MAX(Cli.LugardeTrabajo),40) END AS Empresa,
    ...
    MAX(Pro.DiasPasoAtraso) AS DiasPasoAtraso,  Entero_Cero AS  SaldoTotal,                       Cadena_Vacia AS MOP,                        MAX(Pro.TipoContratoBCID) AS TipoContratoBCID,
	...
	WHERE Cre.Estatus IN (EstatusVigente, EstatusVencido, EstatusCastigado, EstatusPagado, EstatusVendido)
		AND Cre.FechaInicio <= Var_FechaFin
		AND (	Cre.FechTerminacion >= Var_FechaInicial
  ...
  -- -------------------------------------------------
    ...
    UPDATE TMPDATOSCTESCREDPAG Det
		INNER JOIN CREDITOS Cre 		ON Det.CreditoIDSAFI	= Cre.CreditoID
        SET Det.TipoCuenta = TipoCtaPagosFijos
	WHERE Cre.LineaCreditoID = Entero_Cero
		AND Det.TipoCuenta = TipoCtaRevolvente;
  /* ---------------------------------------------------------------------------------
  ...
  SELECT  CreditoID,EstatusCastigado,
      (SaldoCapital + SaldoInteres + SaldoMoratorio + SaldoAccesorios) AS SaldoAtrasoVencido,
      (SaldoCapital + SaldoInteres + SaldoMoratorio + SaldoAccesorios) AS ExigibleSinPeriodo,
      Fecha_Vacia AS PrimerIncu,
      (SaldoCapital + SaldoInteres + SaldoMoratorio + SaldoAccesorios)  AS SaldoTotal,
  ...
  INSERT INTO TMPMONTOCUOTACREDITO
   ...
   -- calculo de mop 01 para creditos condonados en el mes
	UPDATE TMPDATOSCTESCREDPAG T
	INNER JOIN CREQUITAS C ON T.CreditoIDSAFI = C.CreditoID
	INNER JOIN CREDITOS CR ON T.CreditoIDSAFI = CR.CreditoID
	SET T.MOP = (CASE WHEN T.Vencido = 0
    THEN  Mop01 ELSE Mop97 END)
    WHERE C.FechaRegistro BETWEEN Var_FechaInicial AND Var_FechaFin
	AND CR.Estatus = EstatusVencido;
  ...
  INSERT INTO TMPFECHACIERREBC
	...
	WHERE Estatus IN(EstatusVigente,EstatusVencido,EstatusAtrasado)
	AND Vencido BETWEEN 1 AND 49;
