...
-- BALANZACONTA009REP
DELIMITER ;
DROP PROCEDURE IF EXISTS `BALANZACONTA009REP`;

DELIMITER $$
CREATE PROCEDURE `BALANZACONTA009REP`(
	-- Store Procedure: Para generar la Balanza Contable del Cliente: ACCIÓN Y EVOLUCIÓN S.A. DE C.V. S.F.P.
...
TerminaStore: BEGIN
	...
	SELECT Aud_NumTransaccion, Cue.CuentaCompleta,
		   Cen.CentroCostoID,
		   CASE WHEN Cue.Grupo = Tip_Encabezado AND Var_NumSegConta = 1 THEN
	...
	IF( Par_TipoConsulta = Por_Periodo ) THEN
		...
		IF(Par_Ejercicio >= Entero_Uno AND Par_Periodo = Entero_Cero) THEN  -- T_15030 by hussein chan (se agrega >= para que tome todos los ejercicios)
