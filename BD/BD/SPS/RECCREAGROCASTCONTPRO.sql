...
-- ---------------------------------------------------------------------------------
-- RECCREAGROCASTCONTPRO
DELIMITER ;
DROP PROCEDURE IF EXISTS `RECCREAGROCASTCONTPRO`;DELIMITER $$

CREATE PROCEDURE `RECCREAGROCASTCONTPRO`(
/* SP PARA EL PROCESO CONTABLE DE LA RECUPERACION DE CARTERA CASTIGADA*/
	...
	DECLARE Var_TotalCastigoCont	DECIMAL(14,2);
	DECLARE SumaMonRecuperado		DECIMAL(14,2);

	DECLARE Var_SaldoCapitalCont	DECIMAL(14,2);
	DECLARE Var_SaldoInteresCont	DECIMAL(14,2);
	DECLARE Var_SaldoMoratorioCont	DECIMAL(14,2);
	DECLARE Var_SaldoAccesoriosCont	DECIMAL(14,2);
	...
	SET Con_OrdCastigo      := 102;          -- Concepto Contable Cartera: Cta de Orden de Castigo
	SET Con_CorCastigo      := 103;          -- Concepto Contable Cartera: Correlativa de Orden de Castigo
	SET	Con_OrdCasInt 		:= 104;			-- Cta. Orden Interes Cartera Castigada
	SET	Con_CorCasInt 		:= 105;			-- Corr. Cta. Orden Interes Cartera Castigada
	SET	Con_OrdCasMora 		:= 106;			-- Cta. Orden Moratorio Cartera Castigada
	SET	Con_CorCasMora 		:= 107; 		-- Corr. Cta. Orden Moratorio Cartera Castigada
	SET	Con_OrdCasComi 		:= 108; 		-- Cta. Orden Comisiones Cartera Castigada
	SET	Con_CorCasComi 		:= 109; 		-- Corr. Cta. Orden Comisiones Cartera Castigada
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION
			BEGIN
				...
				SET Par_ErrMen  = CONCAT('El SAFI ha tenido un problema al concretar la operacion. ',
									'Disculpe las molestias que esto le ocasiona. Ref: SP-RECCREAGROCASTCONTPRO');
	...
	SET Var_TotalCastigoCont := ROUND((Var_TotalCastigoCont * (1 + Var_TasaIVA)), 2 );

	-- DATOS GENERALES DEL CREDITO CASTIGADO
	SELECT   TotalCastigo, 			SaldoCapital, 		  SaldoInteres,		    SaldoMoratorio,			 SaldoAccesorios
		INTO  Var_TotalCastigoCont, Var_SaldoCapitalCont, Var_SaldoInteresCont,	Var_SaldoMoratorioCont,  Var_SaldoAccesoriosCont
		FROM CRECASTIGOSCONT
		WHERE CreditoID = Par_CreditoID;

	-- Inicializaciones
	SET Par_SaldoRecupera   := IFNULL(Par_SaldoRecupera, Entero_Cero);
	SET Var_TotalCastigoCont	:= IFNULL(Var_TotalCastigoCont, Entero_Cero );
	SET Var_SaldoCapitalCont 	:= IFNULL(Var_SaldoCapitalCont, Entero_Cero );
	SET Var_SaldoInteresCont 	:= IFNULL(Var_SaldoInteresCont, Entero_Cero );
	SET Var_SaldoMoratorioCont 	:= IFNULL(Var_SaldoMoratorioCont, Entero_Cero );
	SET Var_SaldoAccesoriosCont := IFNULL(Var_SaldoAccesoriosCont, Entero_Cero );
	...
	IF(Var_SaldoAccesoriosCont > Entero_Cero AND Par_SaldoRecupera > Entero_Cero) THEN

		IF(Par_SaldoRecupera >= ROUND(Var_SaldoAccesoriosCont * (1+Var_TasaIVA), 2)) THEN
			SET Var_MontoAplicar 	:= Var_SaldoAccesoriosCont;
			SET	Var_SaldoAccesoriosCont := Entero_Cero;
			SET Var_IVAConcepto		 = ROUND(Var_MontoAplicar * Var_TasaIVA, 2);
		ELSE
			SET Var_MontoAplicar 	:= ROUND(Par_SaldoRecupera / (1 + Var_TasaIVA), 2);
			SET	Var_SaldoAccesoriosCont := Var_SaldoAccesoriosCont - Var_MontoAplicar;
	...
	IF(Var_SaldoMoratorioCont > Entero_Cero AND Par_SaldoRecupera > Entero_Cero) THEN



		IF(Par_SaldoRecupera >= ROUND(Var_SaldoMoratorioCont * (1+Var_TasaIVA), 2)) THEN
			SET Var_MontoAplicar := Var_SaldoMoratorioCont;
			SET	Var_SaldoMoratorioCont := Entero_Cero;
			SET Var_IVAConcepto = ROUND(Var_MontoAplicar * Var_TasaIVA, 2);
		ELSE
			SET Var_MontoAplicar := ROUND(Par_SaldoRecupera / (1 + Var_TasaIVA), 2);
			SET Var_IVAConcepto := Par_SaldoRecupera - Var_MontoAplicar;
			SET	Var_SaldoMoratorioCont := Var_SaldoMoratorioCont - Var_MontoAplicar;
		END IF;
			SET Var_TotalIVA := Var_TotalIVA + Var_IVAConcepto;
	...
	IF(Var_SaldoInteresCont > Entero_Cero AND Par_SaldoRecupera > Entero_Cero) THEN

		IF(Par_SaldoRecupera >= ROUND(Var_SaldoInteresCont * (1+Var_TasaIVA), 2)) THEN
			SET Var_MontoAplicar := Var_SaldoInteresCont;
			SET	Var_SaldoInteresCont := Entero_Cero;
			SET Var_IVAConcepto = ROUND(Var_MontoAplicar * Var_TasaIVA, 2);
		ELSE
			SET Var_MontoAplicar := ROUND(Par_SaldoRecupera / (1 + Var_TasaIVA), 2);
			SET Var_IVAConcepto := Par_SaldoRecupera - Var_MontoAplicar;
			SET	Var_SaldoInteresCont := Var_SaldoInteresCont - Var_MontoAplicar;
		END IF;
			SET Var_TotalIVA := Var_TotalIVA + Var_IVAConcepto;
	...
	SET	Var_IVAConcepto := Entero_Cero;
	-- Recuperamos Capital
	IF(Var_SaldoCapitalCont > Entero_Cero AND Par_SaldoRecupera > Entero_Cero) THEN

		IF(Par_SaldoRecupera >= ROUND(Var_SaldoCapitalCont * (1+Var_TasaIVA), 2)) THEN
			SET Var_MontoAplicar := Var_SaldoCapitalCont;
			SET	Var_SaldoCapitalCont := Entero_Cero;
			SET Var_IVAConcepto = ROUND(Var_MontoAplicar * Var_TasaIVA, 2);
		ELSE
			SET Var_MontoAplicar := ROUND(Par_SaldoRecupera / (1 + Var_TasaIVA), 2);
			SET Var_IVAConcepto := Par_SaldoRecupera - Var_MontoAplicar;
			SET	Var_SaldoCapitalCont := Var_SaldoCapitalCont - Var_MontoAplicar;
		END IF;
			SET Var_TotalIVA := Var_TotalIVA + Var_IVAConcepto;
	...
	INSERT  INTO CRECASTIGOSRECCONT	(
	...
	UPDATE CRECASTIGOSCONT SET
		MonRecuperado    =  MonRecuperado + SumaMonRecuperado,
		SaldoCapital	 = 	Var_SaldoCapitalCont,
		SaldoInteres	 = 	Var_SaldoInteresCont,
		SaldoMoratorio	 = 	Var_SaldoMoratorioCont,
		SaldoAccesorios	 =	Var_SaldoAccesoriosCont
	WHERE CreditoID = Par_CreditoID;
