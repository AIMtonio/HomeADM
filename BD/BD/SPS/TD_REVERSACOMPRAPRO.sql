DELIMITER ;
DROP PROCEDURE IF EXISTS `TD_REVERSACOMPRAPRO`;
DELIMITER $$

CREATE PROCEDURE `TD_REVERSACOMPRAPRO`(
	-- Store Procedure de Reversa Compra Pos Para tarjetas de Debito por medio de Web Service
...
TerminaStore: BEGIN
	...
	DECLARE AltaPolDeb_NO			CHAR(1);			-- Constante Alta Poliza Debito NO
	DECLARE AltaPolDeb_SI			CHAR(1);			-- Constante Alta Poliza Debito SI
	...
	ManejoErrores:BEGIN
		...
		DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN
			...
			SET Par_ErrMen	= CONCAT('El SAFI ha tenido un problema al concretar la operacion. Disculpe las molestias que ',
					 				'esto le ocasiona. Ref: SP-TD_REVERSACOMPRAPRO');
		...
		CALL TD_CONTACTAAHORROPRO (
			...
			ConConta_TarDeb,		Var_PolizaID,				AltaPolDeb_SI,				AltaMov_SI,			Mov_CapOrdinario,
